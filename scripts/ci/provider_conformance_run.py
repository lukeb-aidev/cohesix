...
def main() -> int:
    ...
    if args.case and args.case[0] in {"m28c1-mlx-live", "m28c1-vmlx-live"}:
        ...
        try:
            ...
            parser.error(str(exc))
    if args.case and args.case[0] == "m28d-mcp-live":
        if not args.reference_config or args.validate_only:
            parser.error("M28d MCP live case requires --reference-config and real evidence")
        from provider_m28d_live import run_live
        ...
        try:
            ...
            parser.error(str(exc))
    if args.case and args.case[0] == "m28e-a2a-live":
        if not args.reference_config or args.validate_only:
            parser.error("M28e A2A live case requires --reference-config and real evidence")
        from provider_m28e_live import run_live
