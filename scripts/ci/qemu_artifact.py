...
RELEASE_CONFIG_INVENTORY = "configs/generated/implementation_surface_inventory.json"
...
def release_config_paths(inventory: Mapping[str, Any]) -> list[str]:
    """Validate the compiler's exact, flat generated release configuration set."""

    release = inventory.get("release", {})
    paths = release.get("generated_configs") if isinstance(release, dict) else None
    if (
        ...
        or any(
            ...
            or Path(path).parent.as_posix() != "configs/generated"
            or Path(path).name in {".", ".."}
            or Path(path).as_posix() != path
            or re.fullmatch(r"[A-Za-z0-9_.-]+", Path(path).name) is None
            for path in paths
        )
        or len(set(paths)) != len(paths)
        or RELEASE_CONFIG_INVENTORY not in paths
    ):
        raise EvidenceError("compiler release generated_configs inventory is invalid")
    return sorted(paths)


def retained_release_config_paths(artifact_root: Path) -> list[str]:
    """Require the retained directory to match its compiler-owned inventory."""

    root = artifact_root / "release-configs"
    if root.is_symlink():
        raise EvidenceError("release configuration directory must not be a symlink")
    inventory = read_json(safe_relative_file(root, RELEASE_CONFIG_INVENTORY))
    paths = release_config_paths(inventory)
    actual = set()
    for path in root.rglob("*"):
        if path.is_symlink():
            raise EvidenceError(f"release configuration must not be a symlink: {path}")
        if not path.is_dir():
            actual.add(path.relative_to(root).as_posix())
    if actual != set(paths):
        raise EvidenceError("retained release configurations differ from compiler inventory")
    return [f"release-configs/{path}" for path in paths]


def command_stage_release_configs(args: argparse.Namespace) -> int:
    """Retain generated bytes in the build before another profile regenerates them."""

    generated = args.generated_dir.resolve(strict=True)
    inventory = read_json(generated / Path(RELEASE_CONFIG_INVENTORY).name)
    paths = release_config_paths(inventory)
    sources = [generated / Path(path).name for path in paths]
    for source in sources:
        if source.is_symlink() or not source.is_file():
            raise EvidenceError(f"release configuration is not a regular file: {source}")
    artifact_root = args.artifact_dir.resolve(strict=True)
    destination = artifact_root
    for part in ("release-configs", "configs", "generated"):
        destination /= part
        if destination.is_symlink():
            raise EvidenceError(f"release configuration directory is a symlink: {destination}")
        destination.mkdir(exist_ok=True)
    for source in sources:
        atomic_write_bytes(destination / source.name, source.read_bytes())
    retained_release_config_paths(artifact_root)
    return 0
...
def command_record(args: argparse.Namespace) -> int:
    ...
    # Older artifacts remain usable for their original diagnostic evidence.
    # Release assembly separately requires this complete retained payload.
    if (artifact_root / "release-configs").exists():
        relative_files.extend(retained_release_config_paths(artifact_root))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    configs_parser = subparsers.add_parser(
        "stage-release-configs",
        help="retain the compiler-selected generated release configuration bytes",
    )
    configs_parser.add_argument("--generated-dir", type=Path, required=True)
    configs_parser.add_argument("--artifact-dir", type=Path, required=True)
    configs_parser.set_defaults(function=command_stage_release_configs)
