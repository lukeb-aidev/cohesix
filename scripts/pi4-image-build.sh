...
allowed_exact = {
    ...
    "configs/generated/cuda_recipe.json",
...
verify_boot_cmd_handoff() {
    ...
    grep -Fq 'echo "[cohesix] loaded ${coh_image} and ${coh_runtime_cpio_file}; bootm with ${coh_dtb_file}"; setenv stdout serial,vidconsole; setenv stderr serial,vidconsole; bootm ${coh_addr} ${coh_runtime_cpio_addr} ${coh_dtb_addr}; setenv stdout serial,vidconsole; setenv stderr serial,vidconsole; echo "[cohesix] returned from image"' "$path" || fail "boot.cmd does not retain boot video through the Cohesix handoff"
...
build_pi4_image() {
    ...
    log "Building root-task (${ROOT_TASK_FEATURES}, Pi root opt-level=3)"
