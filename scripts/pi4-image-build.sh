...
verify_boot_cmd_handoff() {
    ...
    grep -q 'askenv coh_wifi_psk_new "Wi-Fi PSK (blank for open network): " 64' "$path" || fail "boot.cmd does not collect replacement Wi-Fi PSKs in the protected USB-only prompt"
    ...
    grep -q 'Replace Wi-Fi credentials' "$path" || fail "boot.cmd does not permit replacement of existing Wi-Fi credentials"
    grep -q 'invalid coh_show_logo value' "$path" || fail "boot.cmd does not reject invalid saved logo policy"
    grep -Fq 'echo "[cohesix] wifi-ssid=<set>"' "$path" || fail "boot.cmd does not redact Wi-Fi SSIDs from menu and serial summaries"
...
setenv coh_normalize_choice 'if test -z "${coh_choice}"; then setenv coh_choice 1; elif test "${coh_choice}" = " 0"; then setenv coh_choice 0; elif test "${coh_choice}" = "  0"; then setenv coh_choice 0; elif test "${coh_choice}" = " 1"; then setenv coh_choice 1; elif test "${coh_choice}" = "  1"; then setenv coh_choice 1; elif test "${coh_choice}" = " 2"; then setenv coh_choice 2; elif test "${coh_choice}" = "  2"; then setenv coh_choice 2; elif test "${coh_choice}" = " 3"; then setenv coh_choice 3; elif test "${coh_choice}" = "  3"; then setenv coh_choice 3; elif test "${coh_choice}" = " 4"; then setenv coh_choice 4; elif test "${coh_choice}" = "  4"; then setenv coh_choice 4; elif test "${coh_choice}" = " 5"; then setenv coh_choice 5; elif test "${coh_choice}" = "  5"; then setenv coh_choice 5; fi'
...
setenv coh_prompt_dhcp 'run coh_prepare_input; cls; echo "[cohesix] Guided network setup"; echo "[cohesix] Select address acquisition mode"; echo "  1. DHCP ON (automatic address)"; echo "  2. DHCP OFF (static IPv4)"; echo "  3. Discard changes and return to boot options"; echo "  0. Exit to U-Boot prompt"; run coh_read_choice; if test "${coh_choice}" = "1"; then setenv coh_net_mode dhcp; setenv coh_static_ip ""; setenv coh_static_prefix_len ""; setenv coh_static_gateway ""; setenv coh_menu_page interface; elif test "${coh_choice}" = "2"; then setenv coh_net_mode static; setenv coh_menu_page interface; elif test "${coh_choice}" = "3"; then run coh_load_saved_policy; setenv coh_menu_page root; elif test "${coh_choice}" = "0"; then setenv coh_menu_running 0; else echo "[cohesix] invalid selection"; fi'
setenv coh_prompt_interface 'run coh_prepare_input; cls; echo "[cohesix] Guided network setup"; echo "[cohesix] Select active interface"; echo "  1. Wired Ethernet (GENET)"; echo "  2. Wi-Fi (CYW43455)"; echo "  3. Back to address selection"; echo "  0. Exit to U-Boot prompt"; run coh_read_choice; if test "${coh_choice}" = "1"; then setenv coh_net_interface wired; setenv coh_wifi_ssid ""; setenv coh_wifi_psk ""; if test "${coh_net_mode}" = "static"; then setenv coh_menu_page static; else setenv coh_menu_page confirm; fi; elif test "${coh_choice}" = "2"; then setenv coh_net_interface wifi; setenv coh_menu_page wifi; elif test "${coh_choice}" = "3"; then setenv coh_menu_page dhcp; elif test "${coh_choice}" = "0"; then setenv coh_menu_running 0; else echo "[cohesix] invalid selection"; fi'
...
setenv coh_capture_wifi_credentials 'run coh_prepare_input; if test "${coh_usb_input_ready}" = "1"; then echo "[cohesix] USB-only Wi-Fi credential entry; serial echo disabled"; setenv coh_wifi_ssid_new; setenv coh_wifi_psk_new; run coh_begin_wifi_secret_input; askenv coh_wifi_ssid_new "Wi-Fi SSID (required): " 32; if test -z "${coh_wifi_ssid_new}"; then run coh_end_wifi_secret_input; setenv coh_wifi_ssid_new; setenv coh_wifi_psk_new; echo "[cohesix] Wi-Fi SSID is required; existing credentials were not changed"; setenv coh_menu_page wifi; else askenv coh_wifi_psk_new "Wi-Fi PSK (blank for open network): " 64; setenv coh_wifi_input_valid 1; if test -n "${coh_wifi_psk_new}"; then if test "${coh_wifi_psk_new}" =~ "${coh_psk_min_regex}"; then setenv coh_wifi_input_valid 1; else setenv coh_wifi_input_valid 0; fi; fi; run coh_end_wifi_secret_input; if test "${coh_wifi_input_valid}" = "1"; then setenv coh_wifi_ssid "${coh_wifi_ssid_new}"; setenv coh_wifi_psk "${coh_wifi_psk_new}"; echo "[cohesix] Wi-Fi credentials captured via USB local console"; run coh_finish_wifi_setup; else echo "[cohesix] Wi-Fi PSK must be blank for an open network or at least 8 characters; existing credentials were not changed"; setenv coh_menu_page wifi; fi; setenv coh_wifi_ssid_new; setenv coh_wifi_psk_new; setenv coh_wifi_input_valid; fi; else echo "[cohesix] Serial Wi-Fi password entry is disabled because U-Boot echoes input"; echo "[cohesix] Stage coh_wifi_ssid and coh_wifi_psk in ${coh_policy_file} on the boot partition, then reboot"; if test -n "${coh_wifi_ssid}"; then setenv coh_menu_page wifi; else setenv coh_menu_page interface; fi; fi'
setenv coh_wifi_setup 'run coh_prepare_input; cls; echo "[cohesix] Configure Wi-Fi credentials"; if test -n "${coh_wifi_ssid}"; then echo "[cohesix] Current Wi-Fi credentials are available"; echo "  1. Keep current Wi-Fi credentials"; echo "  2. Replace Wi-Fi credentials"; echo "  3. Back to interface selection"; echo "  0. Exit to U-Boot prompt"; run coh_read_choice; if test "${coh_choice}" = "1"; then run coh_finish_wifi_setup; elif test "${coh_choice}" = "2"; then run coh_capture_wifi_credentials; elif test "${coh_choice}" = "3"; then setenv coh_menu_page interface; elif test "${coh_choice}" = "0"; then setenv coh_menu_running 0; else echo "[cohesix] invalid selection"; fi; else run coh_capture_wifi_credentials; fi'
setenv coh_static_setup 'run coh_prepare_input; cls; echo "[cohesix] Configure static IPv4 for ${coh_net_interface}"; echo "  1. Enter or replace static IPv4 settings"; echo "  2. Back to interface selection"; echo "  0. Exit to U-Boot prompt"; run coh_read_choice; if test "${coh_choice}" = "1"; then askenv coh_static_ip "Static IPv4 address (required): " 15; if test -z "${coh_static_ip}"; then echo "[cohesix] Static IPv4 address is required"; elif test "${coh_static_ip}" =~ "${coh_ipv4_text_regex}"; then askenv coh_static_prefix_len "Prefix length (required, 1-32): " 2; if test -z "${coh_static_prefix_len}"; then echo "[cohesix] Prefix length is required"; elif test "${coh_static_prefix_len}" =~ "${coh_prefix_text_regex}"; then if itest ${coh_static_prefix_len} < 1; then echo "[cohesix] Prefix length must be 1-32"; elif itest ${coh_static_prefix_len} > 32; then echo "[cohesix] Prefix length must be 1-32"; else askenv coh_static_gateway "Gateway IPv4 (optional): " 15; if test -z "${coh_static_gateway}"; then setenv coh_menu_page confirm; elif test "${coh_static_gateway}" =~ "${coh_ipv4_text_regex}"; then setenv coh_menu_page confirm; else echo "[cohesix] Gateway must use dotted-decimal IPv4 syntax"; fi; fi; else echo "[cohesix] Prefix length must contain decimal digits"; fi; else echo "[cohesix] Static address must use dotted-decimal IPv4 syntax"; fi; elif test "${coh_choice}" = "2"; then setenv coh_menu_page interface; elif test "${coh_choice}" = "0"; then setenv coh_menu_running 0; else echo "[cohesix] invalid selection"; fi'
setenv coh_confirm_prompt 'run coh_prepare_input; cls; echo "[cohesix] Review network settings"; run coh_emit_policy_summary; echo "  1. Boot with these settings"; echo "  2. Save settings and reboot"; echo "  3. Edit settings"; echo "  4. Discard changes and return"; echo "  0. Exit to U-Boot prompt"; run coh_read_choice; if test "${coh_choice}" = "1"; then run coh_boot_sequence; setenv coh_menu_page confirm; elif test "${coh_choice}" = "2"; then run coh_persist_policy; if test "${coh_policy_persisted}" = "1"; then echo "[cohesix] saved settings verified; rebooting"; reset; echo "[cohesix] ERROR: reset returned; settings remain saved"; else echo "[cohesix] save failed; review settings and retry"; fi; setenv coh_menu_page confirm; elif test "${coh_choice}" = "3"; then setenv coh_menu_page dhcp; elif test "${coh_choice}" = "4"; then run coh_load_saved_policy; setenv coh_menu_page root; elif test "${coh_choice}" = "0"; then setenv coh_menu_running 0; else echo "[cohesix] invalid selection"; fi'
setenv coh_prompt_root 'run coh_show_logo_splash; run coh_prepare_input; run coh_detect_saved_config; cls; echo "[cohesix] Cohesix boot options"; if test "${coh_has_saved_config}" = "1"; then echo "[cohesix] Saved network settings detected"; run coh_emit_policy_summary; echo "  1. Continue with existing config"; else echo "[cohesix] No saved network settings; manifest defaults remain active"; echo "  1. Boot with manifest defaults"; fi; echo "  2. Configure networking"; echo "  3. Toggle HDMI logo"; echo "  4. Clear saved settings and use manifest defaults"; echo "  5. Save current settings and reboot"; echo "  0. Exit to U-Boot prompt"; run coh_read_choice; if test "${coh_choice}" = "1"; then run coh_boot_sequence; setenv coh_menu_page root; elif test "${coh_choice}" = "2"; then setenv coh_menu_page dhcp; elif test "${coh_choice}" = "3"; then run coh_toggle_logo; elif test "${coh_choice}" = "4"; then run coh_clear_saved_policy; setenv coh_show_logo 1; run coh_persist_policy; if test "${coh_policy_persisted}" = "1"; then echo "[cohesix] saved settings cleared; manifest defaults restored"; else echo "[cohesix] ERROR: could not clear saved settings; reloading policy from SD"; run coh_load_saved_policy; fi; setenv coh_menu_page root; elif test "${coh_choice}" = "5"; then run coh_persist_policy; if test "${coh_policy_persisted}" = "1"; then echo "[cohesix] saved settings verified; rebooting"; reset; echo "[cohesix] ERROR: reset returned; settings remain saved"; else echo "[cohesix] save failed; remaining at boot options"; fi; setenv coh_menu_page root; elif test "${coh_choice}" = "0"; then setenv coh_menu_running 0; else echo "[cohesix] invalid selection"; fi'
setenv coh_menu_loop 'while test "${coh_menu_running}" = "1"; do if test "${coh_menu_page}" = "root"; then run coh_prompt_root; elif test "${coh_menu_page}" = "dhcp"; then run coh_prompt_dhcp; elif test "${coh_menu_page}" = "interface"; then run coh_prompt_interface; elif test "${coh_menu_page}" = "wifi"; then run coh_wifi_setup; elif test "${coh_menu_page}" = "static"; then run coh_static_setup; elif test "${coh_menu_page}" = "confirm"; then run coh_confirm_prompt; else echo "[cohesix] WARNING: invalid menu page ${coh_menu_page}; returning to boot options"; setenv coh_menu_page root; fi; done'
