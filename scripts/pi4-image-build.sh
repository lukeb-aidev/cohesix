...
verify_boot_cmd_handoff() {
    ...
    grep -Fq 'echo "[cohesix] loaded ${coh_image} and ${coh_runtime_cpio_file}; bootm with ${coh_dtb_file}"; cls; setenv stdout serial; setenv stderr serial; bootm ${coh_addr} ${coh_runtime_cpio_addr} ${coh_dtb_addr}; setenv stdout serial,vidconsole; setenv stderr serial,vidconsole; echo "[cohesix] returned from image"' "$path" || fail "boot.cmd does not clear the final U-Boot frame and silence vidconsole across the Cohesix handoff"
