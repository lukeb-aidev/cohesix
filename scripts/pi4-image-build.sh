...
setenv coh_prepare_input 'if test "${coh_usb_input_ready}" != "1"; then if usb reset; then setenv coh_usb_input_ready 1; else setenv coh_usb_input_ready 0; echo "[cohesix] WARNING: usb reset failed; leaving serial input active"; fi; fi; if test "${coh_usb_input_ready}" = "1"; then setenv stdin serial,usbkbd; else setenv stdin serial; fi; setenv stdout serial,vidconsole; setenv stderr serial,vidconsole'
