...
verify_boot_cmd_handoff() {
    ...
    grep -q 'USB keyboard input disabled by serial-only menu image' "$path" || fail "boot.cmd must identify serial-only menu images when USB input is disabled"
    ...
    grep -q 'setenv coh_wifi_serial_recovery' "$path" || fail "boot.cmd is missing serial Wi-Fi saved-policy recovery"
    grep -q 'U-Boot policy missing: ${coh_policy_file} has no saved Wi-Fi credentials' "$path" || fail "boot.cmd does not identify missing U-Boot Wi-Fi policy"
    grep -q 'This image was staged in serial-only menu mode; USB Wi-Fi entry is disabled' "$path" || fail "boot.cmd must identify serial-only Wi-Fi recovery cause"
    grep -q 'do not type PSK on serial' "$path" || fail "boot.cmd does not warn against serial PSK entry"
    grep -q 'run coh_wifi_serial_recovery' "$path" || fail "boot.cmd does not route serial Wi-Fi setup to saved-policy recovery"
}
...
setenv coh_prepare_input 'run coh_bootstrap_usb_session; if test "${coh_usb_input_ready}" = "1"; then echo "[cohesix] USB keyboard input active"; setenv stdin usbkbd,serial; else if test "${coh_menu_input}" = "serial"; then echo "[cohesix] USB keyboard input disabled by serial-only menu image"; else echo "[cohesix] USB keyboard input unavailable; serial only"; fi; setenv stdin serial; fi; setenv stdout serial,vidconsole; setenv stderr serial,vidconsole'
...
setenv coh_wifi_serial_recovery 'run coh_prepare_input; cls; echo "[cohesix] U-Boot policy missing: ${coh_policy_file} has no saved Wi-Fi credentials"; if test "${coh_menu_input}" = "serial"; then echo "[cohesix] This image was staged in serial-only menu mode; USB Wi-Fi entry is disabled"; fi; echo "[cohesix] Serial Wi-Fi password entry is disabled because U-Boot echoes input"; echo "[cohesix] Stage coh_wifi_ssid and coh_wifi_psk in ${coh_policy_file} on the boot partition, or reboot with USB menu input"; echo "[cohesix] Recovery is file-based or local-USB only; do not type PSK on serial"; echo "  1. Back to interface selection"; echo "  2. Use wired Ethernet setup"; echo "  3. Boot with manifest defaults"; echo "  0. Exit to U-Boot prompt for file-based policy recovery"; run coh_read_choice; if test "${coh_choice}" = "1"; then run coh_prompt_interface; elif test "${coh_choice}" = "2"; then setenv coh_net_interface wired; setenv coh_wifi_ssid ""; setenv coh_wifi_psk ""; run coh_after_interface; elif test "${coh_choice}" = "3"; then run coh_reset_policy; run coh_boot_sequence; elif test "${coh_choice}" = "0"; then echo "[cohesix] U-Boot prompt recovery only; do not type Wi-Fi PSKs on serial"; exit; else echo "[cohesix] invalid selection"; run coh_wifi_serial_recovery; fi'
setenv coh_wifi_setup 'run coh_prepare_input; cls; echo "[cohesix] Configure Wi-Fi credentials"; if test -n "${coh_wifi_ssid}"; then echo "[cohesix] Wi-Fi credentials loaded from ${coh_policy_file}"; if test "${coh_net_mode}" = "static"; then run coh_static_setup; else run coh_confirm_prompt; fi; else if test "${coh_usb_input_ready}" = "1"; then echo "[cohesix] USB-only Wi-Fi credential entry; serial echo disabled"; run coh_begin_wifi_secret_input; askenv coh_wifi_ssid "Wi-Fi SSID (required): " 32; if test -z "${coh_wifi_ssid}"; then run coh_end_wifi_secret_input; echo "[cohesix] Wi-Fi SSID is required"; run coh_prompt_interface; else askenv coh_wifi_psk "Wi-Fi PSK (blank for open network): " 64; run coh_end_wifi_secret_input; echo "[cohesix] Wi-Fi credentials captured via USB local console"; if test "${coh_net_mode}" = "static"; then run coh_static_setup; else run coh_confirm_prompt; fi; fi; else run coh_wifi_serial_recovery; fi; fi'
