...
setenv coh_finalize_xhci_handoff 'if env exists coh_xhci_handoff_ready && test "${coh_xhci_handoff_ready}" = "1"; then setenv coh_xhci_handoff_ready_post 1; else setenv coh_xhci_handoff_ready_post 0; fi; if env exists coh_xhci_irq_quiesced && test "${coh_xhci_irq_quiesced}" = "1"; then setenv coh_xhci_irq_quiesced_post 1; else setenv coh_xhci_irq_quiesced_post 0; fi; if test "${coh_xhci_handoff_ready_post}" = "1" && test "${coh_xhci_irq_quiesced_post}" = "1"; then setenv coh_xhci_handoff_ready 1; setenv coh_xhci_irq_quiesced 1; setenv coh_xhci_handoff_source post-stop-paired; echo "[cohesix] xhci handoff tokens exported after usb stop"; elif test "${coh_xhci_handoff_ready_saved}" = "1" && test "${coh_xhci_irq_quiesced_saved}" = "1"; then setenv coh_xhci_handoff_ready 1; setenv coh_xhci_irq_quiesced 1; setenv coh_xhci_handoff_source saved-pre-stop-pair; echo "[cohesix] xhci handoff tokens restored from pre-stop controller state"; else setenv coh_xhci_handoff_ready; setenv coh_xhci_irq_quiesced; if test "${coh_xhci_handoff_ready_post}" = "1" || test "${coh_xhci_irq_quiesced_post}" = "1" || test "${coh_xhci_handoff_ready_saved}" = "1" || test "${coh_xhci_irq_quiesced_saved}" = "1"; then setenv coh_xhci_handoff_source partial; else setenv coh_xhci_handoff_source absent; fi; fi; echo "[cohesix] xhci-handoff-ready post-stop=${coh_xhci_handoff_ready_post}"; echo "[cohesix] xhci-irq-quiesced post-stop=${coh_xhci_irq_quiesced_post}"; if env exists coh_xhci_handoff_ready; then echo "[cohesix] xhci-handoff-ready=${coh_xhci_handoff_ready} final"; else echo "[cohesix] xhci-handoff-ready=absent final"; fi; if env exists coh_xhci_irq_quiesced; then echo "[cohesix] xhci-irq-quiesced=${coh_xhci_irq_quiesced} final"; else echo "[cohesix] xhci-irq-quiesced=absent final"; fi; if env exists coh_xhci_handoff_ready && env exists coh_xhci_irq_quiesced; then echo "[cohesix] xhci-handoff-contract state=paired action=mirror source=${coh_xhci_handoff_source}"; elif test "${coh_xhci_handoff_source}" = "partial"; then echo "[cohesix] xhci-handoff-contract state=partial action=reject source=${coh_xhci_handoff_source}"; else echo "[cohesix] xhci-handoff-contract state=absent action=reject source=${coh_xhci_handoff_source}"; fi'
setenv coh_quiesce_usb 'setenv stdin serial; setenv coh_xhci_handoff_source absent; setenv coh_xhci_handoff_ready_post 0; setenv coh_xhci_irq_quiesced_post 0; run coh_capture_xhci_handoff_snapshot; if usb stop; then echo "[cohesix] USB host quiesced before handoff"; run coh_finalize_xhci_handoff; else setenv coh_xhci_handoff_ready; setenv coh_xhci_irq_quiesced; setenv coh_xhci_handoff_ready_post 0; setenv coh_xhci_irq_quiesced_post 0; setenv coh_xhci_handoff_source usb-stop-failed; echo "[cohesix] WARNING: usb stop failed before handoff"; echo "[cohesix] xhci-handoff-contract state=absent action=reject source=usb-stop-failed"; fi'
