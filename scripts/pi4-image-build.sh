...
verify_boot_cmd_usb_cold_boot() {
    ...
    grep -q 'xHCI stop seed exported as diagnostic only' "$path" || fail "boot.cmd does not keep xHCI handoff export disabled"
    grep -q 'xHCI default stop seed diagnostic only' "$path" || fail "boot.cmd does not export a default xHCI seed when no U-Boot USB session is active"
...
setenv coh_quiesce_usb 'setenv stdin serial; run coh_clear_xhci_handoff_live; if test "${coh_usb_input_ready}" = "1"; then if usb stop; then run coh_clear_xhci_handoff_live; run coh_export_xhci_stop_seed; echo "[cohesix] USB host stopped; xHCI stop seed exported as diagnostic only"; else run coh_clear_xhci_handoff_live; echo "[cohesix] WARNING: usb stop failed before Cohesix boot"; fi; else run coh_export_xhci_stop_seed; echo "[cohesix] USB host session was not active; xHCI default stop seed diagnostic only"; fi'
