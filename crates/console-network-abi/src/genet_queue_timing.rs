...
/// Producer-owned completed Signal observation, disjoint from packet authority.
pub const GENET_QUEUE_SIGNAL_OFFSET: usize = 2112;
/// Six words; publish the packet sequence in the final word after Signal returns.
pub const GENET_QUEUE_SIGNAL_BYTES: usize = 48;
/// Sequence-last commit offset in a completed Signal observation.
pub const GENET_QUEUE_SIGNAL_COMMIT_OFFSET: usize = 40;
...
pub const GENET_TX_QUEUE_TIMING_OFFSET: usize = 896;
/// Exact diagnostic record size; the last word is its sequence-last commit.
pub const GENET_QUEUE_TIMING_BYTES: usize = 256;
...
const _: () =
    assert!(GENET_QUEUE_STAMP_OFFSET + GENET_QUEUE_STAMP_BYTES <= GENET_QUEUE_SIGNAL_OFFSET);
const _: () = assert!(GENET_QUEUE_SIGNAL_OFFSET + GENET_QUEUE_SIGNAL_BYTES <= SHARED_PAGE_BYTES);
const _: () = assert!(
...
/// Last completed blocking receive in the sole consumer, with no CPU-time claim.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct GenetWaitObservation {
    /// CNTVCT immediately before the existing receive boundary.
    pub entered: u64,
    /// CNTVCT immediately after that boundary returns.
    pub returned: u64,
    /// Unmodified badge returned by the receive.
    pub badge: u64,
    /// 1=Recv, 2=Wait, 3=ReplyRecv; zero means unavailable.
    pub kind: u64,
}

impl GenetWaitObservation {
    /// No completed receive has been observed in this lifetime.
...
/// Exact completed producer Signal, written only after the syscall returns.
#[must_use]
pub fn genet_queue_signal(
    generation: u64,
    sequence: u64,
    entered: u64,
    returned: u64,
) -> [u8; GENET_QUEUE_SIGNAL_BYTES] {
    let mut bytes = [0; GENET_QUEUE_SIGNAL_BYTES];
    for (index, value) in [SIGNAL_ID, generation, sequence, entered, returned, sequence]
        .into_iter()
        .enumerate()
    {
        bytes[index * 8..index * 8 + 8].copy_from_slice(&value.to_le_bytes());
...
pub struct GenetWakeContext {
    ...
    pub signal_returned: u64,
}

impl GenetWakeContext {
    ...
    /// Admit only a stable exact-packet signal and chronological receive.
    #[must_use]
    pub fn from_observation(
        wait: GenetWaitObservation,
        signal: &[u8; GENET_QUEUE_SIGNAL_BYTES],
        generation: u64,
        sequence: u64,
        copied: u64,
    ) -> Self {
        let mut context = Self::EMPTY;
        if wait.valid_at(copied) {
            context.wait = wait;
        }
        if generation != 0
            && sequence != 0
            && word(signal, 0) == SIGNAL_ID
            && word(signal, 1) == generation
            && word(signal, 2) == sequence
            && word(signal, 5) == sequence
            && word(signal, 3) != 0
            && word(signal, 3) <= word(signal, 4)
            && word(signal, 4) <= copied
        {
            context.signal_entered = word(signal, 3);
            context.signal_returned = word(signal, 4);
        }
        context
    }
...
pub struct GenetQueueTiming {
    /// Receive and exact producer notification observations for the peak only.
    pub wake: GenetWakeContext,
...
impl GenetQueueTiming {
    ...
    pub fn observe(
        ...
        if self.samples == 0 || elapsed > self.copied_ticks - self.produced_ticks {
            self.wake = if wake.valid_at(copied)
                && (wake.signal_entered == 0 || wake.signal_entered >= produced)
            {
                wake
            } else {
    ...
    /// Serialize the exact fixed record; callers publish word31 last.
    #[must_use]
    pub fn encode(self) -> [u8; GENET_QUEUE_TIMING_BYTES] {
        let words = [
            ...
            self.wake.wait.entered,
            self.wake.wait.returned,
            self.wake.wait.badge,
            self.wake.wait.kind,
    ...
    pub fn decode(bytes: &[u8; GENET_QUEUE_TIMING_BYTES], generation: u64) -> Option<Self> {
        ...
        let wake = GenetWakeContext {
            wait: GenetWaitObservation {
                entered: word(bytes, 11),
                returned: word(bytes, 12),
                badge: word(bytes, 13),
                kind: word(bytes, 14),
            },
...
mod tests {
    ...
    #[test]
    fn completed_signal_and_wait_bind_exact_packet_without_transport_authority() {
        // Independent v1 completed-Signal wire oracle: six little-endian words.
        let words: [u64; 6] = [0x0030_0001_434e_4753, 7, 9, 110, 115, 9];
        let mut signal = [0; 48];
        for (index, word) in words.into_iter().enumerate() {
            signal[index * 8..index * 8 + 8].copy_from_slice(&word.to_le_bytes());
        }
        assert_eq!(genet_queue_signal(7, 9, 110, 115), signal);
        assert_eq!(
            (
                GENET_QUEUE_SIGNAL_OFFSET,
                GENET_QUEUE_SIGNAL_BYTES,
                GENET_QUEUE_SIGNAL_COMMIT_OFFSET
            ),
            (2112, 48, 40)
        );
        let wait = GenetWaitObservation {
            entered: 90,
            returned: 135,
            badge: 0x100,
            kind: 2,
        };
        let context = GenetWakeContext::from_observation(wait, &signal, 7, 9, 140);
        assert_eq!(
            context,
            GenetWakeContext {
                wait,
        ...
        assert_eq!(&encoded[88..96], &90u64.to_le_bytes());
        assert_eq!(&encoded[96..104], &135u64.to_le_bytes());
        assert_eq!(&encoded[120..128], &110u64.to_le_bytes());
        assert_eq!(&encoded[128..136], &115u64.to_le_bytes());
        assert_eq!(&encoded[248..256], &1u64.to_le_bytes());
        ...
        assert_eq!(
            timing.wake, context,
            "a faster packet cannot replace the peak's receive"
        );
        for (index, value) in [(0, 0), (1, 8), (2, 10), (3, 0), (3, 116), (4, 141), (5, 0)] {
            let mut invalid = signal;
            invalid[index * 8..index * 8 + 8].copy_from_slice(&(value as u64).to_le_bytes());
            let missing = GenetWakeContext::from_observation(wait, &invalid, 7, 9, 140);
            assert_eq!(missing.wait, wait);
            assert_eq!((missing.signal_entered, missing.signal_returned), (0, 0));
        }
        let invalid_wait = GenetWaitObservation {
            returned: 141,
            ..wait
        };
        let partial = GenetWakeContext::from_observation(invalid_wait, &signal, 7, 9, 140);
        assert_eq!(partial.wait, GenetWaitObservation::EMPTY);
        assert_eq!(partial.signal_returned, 115);
        let invalid_context = GenetWakeContext {
            wait: invalid_wait,
            ..context
        };
        let mut accepted = GenetQueueTiming::EMPTY;
        assert!(accepted.observe(&stamp(), 7, 9, 140, &frame(), invalid_context));
        assert_eq!(accepted.samples, 1);
        assert_eq!(accepted.wake, GenetWakeContext::EMPTY);
        let before_publication = GenetWakeContext {
            signal_entered: 90,
            signal_returned: 95,
            ..context
        };
        let mut accepted = GenetQueueTiming::EMPTY;
        assert!(accepted.observe(&stamp(), 7, 9, 140, &frame(), before_publication));
        assert_eq!(accepted.wake, GenetWakeContext::EMPTY);
    }
    ...
    fn queue_timing_wire_fixture_and_reserved_regions() {
        ...
        for index in [0, 8, 16, 24, 40, 56, 88, 96, 104, 112, 120, 128, 136, 248] {
    ...
    fn queue_timing_invalid_observations_cannot_mutate_state() {
        ...
        for (g, s, t) in [(0, 9, 140), (8, 9, 140), (7, 10, 140), (7, 9, 99)] {
            assert!(!timing.observe(&stamp(), g, s, t, &frame(), GenetWakeContext::EMPTY));
        ...
        for (index, value) in [
            ...
            invalid[index] = value;
            assert!(!timing.observe(&stamp(), 7, 9, 140, &invalid, GenetWakeContext::EMPTY));
            assert_eq!(timing, before);
        }
        assert!(!timing.observe(&[0; 32], 7, 9, 140, &frame(), GenetWakeContext::EMPTY));
        assert!(!timing.observe(
            &genet_queue_stamp(7, 9, 0),
            7,
            9,
            140,
            &frame(),
