    ...
    fn handle_command(&mut self, command: ConsoleCommand) -> Vec<String> {
        ...
        match command {
            ...
            ConsoleCommand::Log => {
                ...
                for line in tail_lines("/log/queen.log") {
                    lines.push((*line).to_owned());
                }
                lines.push("END".to_owned());
                lines
...
fn transcripts_root() -> PathBuf {
    target_root().join("cohsh-transcripts")
}
...
fn translate_command(line: &str) -> String {
    ...
    if keyword.eq_ignore_ascii_case("log") && parts.next().is_none() {
        return "tail /log/queen.log".to_owned();
    }
    line.to_owned()
}
...
fn run_tcp_transcript(script_path: &Path, harness: ConsoleHarness) -> Vec<String> {
    let (port, sent_lines, handle) = spawn_tcp_server(harness);
...
fn write_transcript(
    ...
    let path = dir.join(format!("{transport}.txt"));
...
fn diff_transcripts(left: &Path, right: &Path) -> Result<(), String> {
    let output = ProcessCommand::new("diff")
        .args(["-u", left.to_str().unwrap(), right.to_str().unwrap()])
        .output()
...
fn transcripts_match_across_transports() {
    let boot_script_path = repo_root().join(BOOT_SCRIPT);
    ...
    let transcripts_dir = transcripts_root();
    ...
    for scenario in scenarios {
        let mut serial_harness = ConsoleHarness::new(build_validator(), POOL_TOTAL_SESSIONS);
        let mut core_harness = ConsoleHarness::new(build_validator(), POOL_TOTAL_SESSIONS);
        ...
        let tcp_lines = run_tcp_transcript(&scenario.script_path, tcp_harness);

        let serial_norm = normalize_lines(&serial_lines);
        let core_norm = normalize_lines(&core_lines);
        let tcp_norm = tcp_lines;

        let serial_path =
            write_transcript(&transcripts_dir, scenario.name, "serial", &serial_norm);
        let core_path = write_transcript(&transcripts_dir, scenario.name, "core", &core_norm);
        let tcp_path = write_transcript(&transcripts_dir, scenario.name, "tcp", &tcp_norm);

        diff_transcripts(&serial_path, &core_path)
            .unwrap_or_else(|diff| panic!("serial vs core drift:\n{diff}"));
        diff_transcripts(&serial_path, &tcp_path)
