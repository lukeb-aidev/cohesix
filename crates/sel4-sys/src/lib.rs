...
mod imp {
    ...
    /// Invocation label for `CNodeDelete` on non-MCS kernels without SMP or hardware debug extensions.
    ///
    /// The value matches the code generated by libsel4's invocation header generator
    /// (`libsel4/tools/invocation_header_gen.py`) for the configuration Cohesix targets.
    const C_NODE_DELETE_LABEL: seL4_Word = 18;
    /// Invocation label for `CNodeMove` as generated by libsel4 for AArch64 kernels.
    const C_NODE_MOVE_LABEL: seL4_Word = 4;
    /// Invocation label for `CNodeCopy` mirroring libsel4's generated bindings.
    const C_NODE_COPY_LABEL: seL4_Word = 5;
    ...
    pub unsafe fn seL4_CNode_Delete(
        ...
        let msg = seL4_MessageInfo::new(C_NODE_DELETE_LABEL, 0, 0, 2);
        let mut mr0 = index;
        let mut mr1 = depth & 0xff;
        let mut mr2 = 0usize;
        let mut mr3 = 0usize;

        let info = seL4_CallWithMRs(root, msg, &mut mr0, &mut mr1, &mut mr2, &mut mr3);
    ...
    pub unsafe fn seL4_CNode_Move(
        ...
        let msg = seL4_MessageInfo::new(C_NODE_MOVE_LABEL, 0, 1, 4);
    ...
    pub unsafe fn seL4_CNode_Copy(
        ...
        let msg = seL4_MessageInfo::new(C_NODE_COPY_LABEL, 0, 1, 5);
        let mut mr0 = dest_index;
        let mut mr1 = dest_depth & 0xff;
        let mut mr2 = src_index;
        let mut mr3 = src_depth & 0xff;
