...
path (`--mock --list`) and an optional NVML backend (enabled by default on Linux
builds; use `--no-default-features` to omit NVML). Output from the CLI feeds directly into NineDoor via
