...
// Purpose: Enter the isolated heartbeat Worker through the sealed Worker ABI.
// Author: Lukas Bower
...
/// Retained image-admission identity for the heartbeat Worker executable.
...
static COHESIX_WORKER_METADATA: WorkerImageMetadata =
    WorkerImageMetadata::for_role(WorkerRole::Heartbeat);
...
pub extern "C" fn _start(runtime_init_address: usize) -> ! {
    ...
    worker_heart::target_runtime::run(WorkerRole::Heartbeat, runtime_init_address)
}
