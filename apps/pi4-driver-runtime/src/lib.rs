...
const XHCI_DMA_HID_OUTPUT_BUFFER_OFFSET: usize = 0x27000;
...
const USB_KEYBOARD_INTERRUPT_QUEUE_DEPTH: usize = 32;
...
const CYW43_RX_GLOM_SUBFRAME_CAP: usize = 16;
const CYW43_RX_QUEUE_CAP: usize = 16;
...
const SDIO_R5_OUT_OF_RANGE: u32 = 0x0100;
...
impl Cyw43RuntimeState {
    ...
    fn reset_firmware_upload_state(&mut self) {
        ...
        self.glom_subframe_lens = [0; CYW43_RX_GLOM_SUBFRAME_CAP];
...
fn mark_engine_initialized(hot_path: u32) -> Option<u16> {
    ...
    let detail = runtime_engine_init(hot_path, descriptor)?;
...
fn runtime_engine_init(hot_path: u32, descriptor: DriverRuntimeInitDescriptor) -> Option<u16> {
    match hot_path {
        ...
        HOT_PATH_GENET_NIC => {
            GENET_RUNTIME_STATE.with_mut(|state| genet_runtime_init(state).then_some(FAULT_NONE))
        }
        HOT_PATH_CYW43_WIFI => CYW43_RUNTIME_STATE
            .with_mut(|state| cyw43_runtime_init(state, descriptor).then_some(FAULT_NONE)),
        HOT_PATH_SDIO_HOST => {
            reset_sdio_register_shadows();
            SDIO_RUNTIME_STATE.with_mut(|state| sdio_runtime_init(state).then_some(FAULT_NONE))
        }
        HOT_PATH_PCIE_ROOT => {
            PCIE_RUNTIME_STATE.with_mut(|state| pcie_runtime_init(state).then_some(FAULT_NONE))
        }
        _ => Some(FAULT_NONE),
    }
}
...
fn service_engine_init(command: DriverTaskCommandRecord) -> Option<DriverTaskCompletionRecord> {
    ...
    if let Some(detail) = mark_engine_initialized(command.arg0) {
...
fn cyw43_runtime_init(
    ...
    state.initialized = state.bus_link_ready
        ...
        .is_some();
    if state.initialized && cfg!(not(target_os = "none")) {
...
fn service_sdio_descriptor_command(command: DriverTaskCommandRecord) -> DriverTaskCompletionRecord {
    ...
    let shared_payload_bytes = runtime_shared_buffer_bytes(RUNTIME_DESCRIPTOR.load());
...
fn cyw43_stream_next_bytes(
    ...
    let next_bytes = current_bytes.checked_add(u32::from(payload_len))?;
    (next_bytes <= expected_total_len).then_some(next_bytes)
}
...
fn service_cyw43_descriptor_command(
    ...
    let result = CYW43_RUNTIME_STATE.with_mut(|state| match desc.op {
        ...
        DRIVER_RUNTIME_CYW43_OP_RX_POLL => {
            frame_ready = cyw43_runtime_poll_rx(state, CYW43_RX_FRAME_FLAG_MASK_DATA);
            frame_ready.map_or(0, |frame| u32::from(frame.len))
        }
        DRIVER_RUNTIME_CYW43_OP_CONTROL_POLL => {
            frame_ready = cyw43_runtime_poll_rx(state, CYW43_RX_FRAME_FLAG_MASK_CONTROL_EVENT);
            frame_ready.map_or(0, |frame| u32::from(frame.len))
        }
        _ => 0,
    });
...
fn usb_uses_pcie_bus_link() -> bool {
    RUNTIME_INIT_HOT_PATH.load(Ordering::Acquire) == HOT_PATH_USB_KEYBOARD
        && RUNTIME_DESCRIPTOR
            .load()
            .has_pointer_free_bus_link(HOT_PATH_PCIE_ROOT, DRIVER_RUNTIME_BUS_LINK_CHANNEL_USB_PCIE)
}
...
const USB_PCIE_BUS_LINK_ATTEMPTS: usize = 16_384;

#[cfg(target_os = "none")]
fn usb_pcie_bus_link_posted_write_flush(stage: u16, offset: usize, _value: u32) -> bool {
    if !usb_uses_pcie_bus_link() || offset > u32::MAX as usize {
        return false;
    }
    ...
    let mut command = DriverTaskCommandRecord {
        ...
        arg1: ROLE_PCIE,
        aux0: (u32::from(DRIVER_RUNTIME_PCIE_OP_POSTED_WRITE_FLUSH) << 16) | u32::from(stage),
        aux1: offset as u32,
        budget: DriverTaskBudgetGrant {
            max_ops: 1,
            max_frames: 1,
    ...
    command.frame.flags = DRIVER_RUNTIME_COMMAND_FLAG_ONE_WAY;
    let completion_reset = DriverTaskCompletionRecord::fault(0, FAULT_REJECTED_COMMAND);
    // SAFETY: Root maps the PCIe owner command ring into USB at this fixed
    // address and mints the PCIe endpoint cap into the fixed child slot before
    // the USB runtime can issue xHCI doorbells. The call is send-only and the
    // completion is observed through the pointer-free owner ring.
    unsafe {
        ...
        sel4_sys::seL4_NBSend(DRIVER_TASK_CHILD_PCIE_BUS_ENDPOINT_SLOT, info);
        for _ in 0..USB_PCIE_BUS_LINK_ATTEMPTS {
            sel4_sys::seL4_Yield();
            ...
            if completion.sequence == command.sequence {
                return completion.code != COMPLETION_FAULT;
            }
        }
        false
    }
...
#[cfg(not(target_os = "none"))]
fn usb_pcie_bus_link_posted_write_flush(_stage: u16, _offset: usize, _value: u32) -> bool {
    false
}
...
fn cyw43_backplane_write_chunk_shape(
    ...
    if firmware_released {
        ...
        let chunk_len = remaining.min(CYW43_FIRMWARE_BYTE_MODE_CHUNK_BYTES);
...
const fn cyw43_backplane_write_prefers_byte_mode_at_window_edge(
    ...
    !firmware_released
        ...
        && window_remaining <= CYW43_FIRMWARE_STAGE_BYTES
}

const fn cyw43_linked_pre_release_upload_forces_byte_mode(
    firmware_released: bool,
    owner_link: bool,
) -> bool {
    let _ = firmware_released;
    let _ = owner_link;
    false
}
...
fn cyw43_prepare_firmware_upload_retry(state: &mut Cyw43RuntimeState) -> bool {
    ...
    state.backplane_window_valid = false;
    while usize::from(state.firmware_retry_clock_index) < CYW43_SDIO_FIRMWARE_RETRY_CLOCKS_HZ.len()
    {
        ...
        let flags = if retry_index == 0 {
            let _ = cyw43_sdio_cmd52_write(0, SDIO_CCCR_IF, 0);
            0
        } else {
            let _ = cyw43_sdio_cmd52_write(0, SDIO_CCCR_IF, SDIO_BUS_WIDTH_4BIT);
            DriverRuntimeSdioCommandDescriptor::FLAG_HOST_BUS_WIDTH_4BIT
                | DriverRuntimeSdioCommandDescriptor::FLAG_HOST_HIGH_SPEED
        };
...
fn cyw43_flush_firmware_stage(state: &mut Cyw43RuntimeState) -> bool {
    ...
    if cyw43_backplane_write_ring_with_mode(
        state,
    ...
    while done < len {
        ...
        loop {
            ...
            if !cyw43_backplane_set_window(state, chunk_addr) {
                if attempts < CYW43_SDIO_FIRMWARE_RETRY_CLOCKS_HZ.len()
                    && cyw43_prepare_firmware_upload_retry(state)
                {
            ...
            let byte_mode_fallback = force_byte_mode
                || state.firmware_byte_mode_fallback
                || edge_byte_mode
                || cyw43_linked_pre_release_upload_forces_byte_mode(
                    state.firmware_released,
...
fn usb_runtime_poll_keyboard(state: &mut UsbRuntimeState) -> usize {
    ...
    let Some((event, report_slot)) = xhci_next_keyboard_transfer_event(state, descriptor) else {
        return 0;
    };
    ...
    let report_vaddr = dma_range.vaddr as usize + xhci_keyboard_report_buffer_offset(report_slot);
    let mut report = [0u8; USB_BOOT_REPORT_BYTES];
    for (index, byte) in report.iter_mut().enumerate() {
        *byte = read_dma_byte(report_vaddr + index);
    }
    let bytes = usb_keyboard_report_bytes_to_frame(state, report);
    let _ = xhci_arm_keyboard_interrupt_queue(state, descriptor);
    bytes
}
...
fn usb_runtime_retry_keyboard_enumeration(state: &mut UsbRuntimeState) -> Option<u16> {
        ...
        && state.enumeration_cold_reinit_attempts < USB_ENUMERATION_COLD_REINIT_LIMIT
    {
        ...
        if let Some(detail) = usb_runtime_init_hw(descriptor, state) {
            state.initialized = true;
...
fn xhci_flush_posted_write(state: &UsbRuntimeState, stage: u16, offset: usize, value: u32) -> bool {
    let owner_link_required = usb_uses_pcie_bus_link();
    if usb_pcie_bus_link_posted_write_flush(stage, offset, value) {
        return true;
    }
    let _ = usb_read32(state.cap_length as usize + XHCI_USBSTS);
    xhci_posted_write_flush_acceptance_result(owner_link_required, false)
}

const fn xhci_posted_write_flush_acceptance_result(
    owner_link_required: bool,
    owner_link_flushed: bool,
) -> bool {
    owner_link_flushed || !owner_link_required
}
...
fn xhci_address_device(
    ...
    zero_dma_range(
        dma_range.vaddr as usize + device.ep0_ring_offset,
        XHCI_DMA_EP0_RING_STRIDE,
    );
...
fn xhci_control_transfer(
    ...
    let complete = xhci_wait_transfer_completion(state, descriptor, device.slot, 1);
    if complete && data_in {
        dma_load_barrier();
    }
    complete
...
fn usb_set_hid_leds(
    ...
    let mut device = UsbEnumerationDevice {
...
fn usb_prepare_hub_port(
    ...
    usb_spin_wait(USB_HUB_PORT_POWER_SETTLE_SPINS);
    let mut last = None;
...
fn usb_probe_hub_child_with_speed_fallback(
    ...
    for candidate in USB_HUB_CHILD_SPEED_CANDIDATES {
        if candidate == primary_speed {
            continue;
        }
        let _ = usb_prepare_hub_port(state, descriptor, hub, hub_info.interface, port);
...
fn usb_probe_device_for_keyboard(
    ...
    if usb_parse_keyboard_endpoint(state, config_vaddr, config_len) {
        ...
        let configured =
            endpoint_ready && usb_set_configuration(state, descriptor, &mut device, config_value);
        let protocol_ready =
            configured && usb_hid_attach_control_ready(state, descriptor, &mut device);
        let ready = configured && endpoint_ready && protocol_ready;
        state.ep0_enqueue = device.ep0_enqueue;
        state.ep0_cycle = device.ep0_cycle;
        if !ready {
...
fn usb_keyboard_enumerate(
    ...
    if !state.command_path_proven && !xhci_prime_command_path_for_port_sampling(state, descriptor) {
...
fn xhci_keyboard_event_trb_index(
    ...
    let delta = event.parameter.checked_sub(ring_bus)?;
...
fn usb_runtime_init_hw(
    ...
    if !xhci_flush_posted_write(state, XHCI_FLUSH_STAGE_INIT, int_base + XHCI_ERSTSZ, 1) {
        return None;
    }
...
pub fn runtime_main(task_key: usize) -> ! {
    ...
    loop {
        ...
        if command_expects_reply(command) {
...
mod tests {
    ...
    fn stage_sdpcm_rx_header(
        ...
        write_runtime_payload_byte(offset + 6, 0);
    ...
    fn cyw43_descriptor_rejects_direct_sdio_mmio_even_with_bus_link() {
        ...
        assert!(!descriptor_resources_ready(descriptor, HOT_PATH_CYW43_WIFI));
        let mut state = Cyw43RuntimeState::new();
        assert!(!cyw43_runtime_init(&mut state, descriptor));
    }
    ...
    #[test]
    fn cyw43_backplane_upload_falls_back_to_byte_mode_before_firmware_release() {
        assert_eq!(
    ...
    fn cyw43_firmware_upload_uses_byte_mode_at_backplane_window_edge() {
        ...
        assert!(CYW43_FIRMWARE_BYTE_MODE_CHUNK_BYTES < SDIO_CMD53_BYTE_MODE_MAX);
    ...
    #[test]
    fn cyw43_linked_pre_release_owner_upload_keeps_block_mode_first() {
        assert!(!cyw43_linked_pre_release_upload_forces_byte_mode(
            false, true
        ));
        assert!(!cyw43_linked_pre_release_upload_forces_byte_mode(
            true, true
    ...
    fn sdio_cmd53_r5_errors_match_old_hal_status_mask() {
        ...
        assert_eq!(
            sdio_cmd53_r5_error(SDIO_CMD53, firmware_write, 0x0800),
            Some(0x0800)
        );
        assert_eq!(
    ...
    #[test]
    fn cyw43_firmware_stage_coalesces_to_declared_owner_link_window() {
        let _guard = test_guard();
        reset_runtime_for_test();
        let mut state = Cyw43RuntimeState::new();
        state.initialized = true;
        ...
        for chunk in 0..8usize {
            ...
            let expected_stage = if chunk == 7 { 0 } else { (chunk + 1) * 1024 };
            assert_eq!(usize::from(state.firmware_stage_len), expected_stage);
        }

        assert_eq!(state.firmware_stage_len, 0);
        for chunk in 0..8usize {
            assert_eq!(
                read_runtime_payload_byte(CYW43_FIRMWARE_STAGE_OFFSET + chunk * 1024),
                chunk as u8
            );
        }
    }
    ...
    fn cyw43_firmware_stage_retry_flushes_preserved_window_once() {
        ...
        state.firmware_stage[CYW43_FIRMWARE_STAGE_BYTES - 1024] = 0x77;
        ...
        assert!(cyw43_firmware_stage_covers(
            &state,
            CYW43_RAM_BASE_4345 + (CYW43_FIRMWARE_STAGE_BYTES - 1024) as u32,
            1024
        ));
        assert!(cyw43_stage_firmware_chunk(
            &mut state,
        ...
        assert_eq!(
            read_runtime_payload_byte(
                CYW43_FIRMWARE_STAGE_OFFSET + CYW43_FIRMWARE_STAGE_BYTES - 1024
            ),
            0x77
        );
    }
    ...
    #[test]
    fn cyw43_descriptor_firmware_chunks_coalesce_before_owner_transfer() {
        let _guard = test_guard();
        reset_runtime_for_test();
        init_cyw43_engine_for_test();
        CYW43_RUNTIME_STATE.with_mut(|state| {
        ...
        for chunk in 0..2usize {
            ...
            for index in 0..1024usize {
                write_ring_byte(usize::from(payload_offset) + index, fill);
            }
            stage_cyw43_descriptor(DriverRuntimeCyw43CommandDescriptor {
                op: DRIVER_RUNTIME_CYW43_OP_FIRMWARE_CHUNK,
                flags: 0,
                target_addr: CYW43_RAM_BASE_4345 + (chunk * 1024) as u32,
                payload_offset,
                payload_len: 1024,
                total_len: 609_309,
                arg0: 0,
            ...
            assert_eq!(
                ...
                DriverTaskCompletionRecord::progress(300 + chunk as u32, 1024)
            );
            CYW43_RUNTIME_STATE.with_mut(|state| {
                assert_eq!(usize::from(state.firmware_stage_len), (chunk + 1) * 1024);
                assert_eq!(state.firmware_stage_addr, CYW43_RAM_BASE_4345);
            });
        }
    }
    ...
    fn cyw43_firmware_stage_flushes_final_short_chunk() {
        ...
        assert_eq!(state.firmware_stage_len, 512);
        ...
        assert_eq!(
            read_runtime_payload_byte(CYW43_FIRMWARE_STAGE_OFFSET + 512),
            0x5a
        );
        assert_eq!(
            read_runtime_payload_byte(CYW43_FIRMWARE_STAGE_OFFSET + 767),
            0x5a
        );
    }

    #[test]
    fn cyw43_firmware_upload_retry_steps_down_clock_and_promotes_after_upload() {
        let mut state = Cyw43RuntimeState::new();
        assert!(!state.firmware_byte_mode_fallback);
        ...
        assert!(cyw43_prepare_firmware_upload_retry(&mut state));
        assert_eq!(state.firmware_retry_clock_index, 2);
        assert!(cyw43_prepare_firmware_upload_retry(&mut state));
        assert_eq!(state.firmware_retry_clock_index, 3);
        assert!(cyw43_prepare_firmware_upload_retry(&mut state));
        assert_eq!(state.firmware_retry_clock_index, 4);
        assert!(!cyw43_prepare_firmware_upload_retry(&mut state));
    ...
    fn cyw43_runtime_stream_completion_requires_declared_total_lengths() {
        assert_eq!(cyw43_stream_next_bytes(0, 0, 4096, 1024), Some(1024));
        assert_eq!(cyw43_stream_next_bytes(1024, 4096, 4096, 3072), Some(4096));
        assert_eq!(cyw43_stream_next_bytes(1024, 4096, 2048, 512), None);
        assert_eq!(cyw43_stream_next_bytes(4090, 4096, 4096, 16), None);
        assert_eq!(cyw43_stream_next_bytes(0, 0, 0, 0), None);
    }
    ...
    fn cyw43_release_requires_nvram_tail_after_complete_streams() {
        ...
        state.firmware_uploaded = true;
        state.nvram_uploaded = true;
        assert!(!cyw43_release_firmware(&mut state, 0));
    ...
    #[test]
    fn usb_keyboard_report_to_frame_drops_arrow_keys() {
        let _guard = test_guard();
        reset_runtime_for_test();
        let mut state = UsbRuntimeState::new();
        let arrows = [
    ...
    fn sdio_descriptor_cmd53_accepts_8192_byte_shared_payload_window() {
        ...
        assert_eq!(
            service_command(0, init),
            DriverTaskCompletionRecord::progress(75, 1)
        );
        let desc_offset = DRIVER_TASK_RING_FRAME_OFFSET;
        write_sdio_descriptor_for_test(
            desc_offset,
    ...
    #[test]
    fn usb_runtime_cold_reinit_policy_targets_deep_enumeration_failures() {
    ...
    fn usb_keyboard_decodes_report_buffer_from_transfer_event_trb() {
        ...
        let event = XhciTrb {
            parameter: kbd_ring + XHCI_TRB_BYTES as u64,
    ...
    fn usb_hid_leds_use_dedicated_output_report_dma() {
        ...
        assert!(
            XHCI_DMA_REPORT_BUFFER_OFFSET
                + USB_KEYBOARD_INTERRUPT_QUEUE_DEPTH * XHCI_BOOT_REPORT_BYTES
                <= XHCI_DMA_HID_OUTPUT_BUFFER_OFFSET
        );
    ...
    #[test]
    fn usb_owner_link_flush_is_required_for_acceptance_when_declared() {
        assert!(xhci_posted_write_flush_acceptance_result(false, false));
        assert!(xhci_posted_write_flush_acceptance_result(true, true));
        assert!(!xhci_posted_write_flush_acceptance_result(true, false));
    }
    ...
    #[test]
    fn usb_cold_reinit_returns_before_resuming_enumeration_scan() {
        let _guard = test_guard();
        reset_runtime_for_test();
        init_runtime_for_test(HOT_PATH_USB_KEYBOARD, ROLE_USB);
        let init = DriverTaskCommandRecord {
