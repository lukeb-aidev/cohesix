...
const RPI4_VL805_XHCI_CAP_LENGTH: u8 = 0x20;
const RPI4_VL805_XHCI_HCI_VERSION: u16 = 0x0100;
const RPI4_VL805_XHCI_HCS1: u32 = 0x0500_0420;
const RPI4_VL805_XHCI_HCS2: u32 = 0xFC00_0031;
const RPI4_VL805_XHCI_HCCPARAMS1: u32 = 0x0028_41EB;
const RPI4_VL805_XHCI_DBOFF: u32 = 0x0000_0100;
const RPI4_VL805_XHCI_RTSOFF: u32 = 0x0000_0200;
...
// aperture as VL805 BAR0. Treat that as a layout fact, not bootloader authority:
// the default path may preseed that high BAR for Cohesix-owned cold start, but
// it still rebuilds xHCI rings locally and keeps xHCI event progression
// poll-driven.
...
const XHCI_PRE_CONTROLLER_VTIMER_SHADOW_ENABLED: bool = false;
const TRUSTED_XHCI_IRQ_BINDING_LIMIT: usize = TRUSTED_XHCI_PCIE_SINK_IRQS.len();
...
const fn xhci_runtime_init_strategy_prompt_safe(strategy: XhciRuntimeInitStrategy) -> bool {
    match strategy.firmware_handoff {
        ...
        | XhciFirmwareHandoff::PreserveControllerState => true,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct UsbRuntimeProofStatus {
...
#[inline]
const fn usb_probe_preflight_next_step_for_source(
    mmio: usize,
    preferred_handoff: XhciFirmwareHandoff,
    strategy: XhciRuntimeInitStrategy,
) -> &'static str {
    let _ = preferred_handoff;
    if xhci_linux_capture_full_reset_mailbox_reset_required_for_strategy(mmio, strategy) {
        "mailbox-reset-notify"
    } else if xhci_controller_skips_initial_live_operational_reads(mmio, strategy) {
        "skip-pre-reset"
    } else {
        usb_probe_preflight_next_step(strategy)
    }
}
...
#[inline]
const fn usb_probe_preflight_followup_step_for_attempt(
    mmio: usize,
    preferred_handoff: XhciFirmwareHandoff,
    strategy: XhciRuntimeInitStrategy,
    strategy_idx: usize,
...
#[inline]
const fn usb_probe_preflight_expected_diag_stage_for_source(
    mmio: usize,
    strategy: XhciRuntimeInitStrategy,
) -> u16 {
    if xhci_controller_skips_initial_live_operational_reads(mmio, strategy) {
        ...
        usb_probe_preflight_expected_diag_stage(strategy)
    }
}
...
fn usb_ownership_fresh_ownership_label(
    ...
    if cfg_replay_ready && command_replay_ready {
        ...
        "skipped"
    } else if strategy == effective_strategy
...
fn usb_probe_preflight_status(
    ...
    let mailbox_reset_completed =
        runtime_vl805_mailbox_reset_completed(VL805_RUNTIME_RESET_STATE.load(Ordering::Acquire));
    let effective_strategy = xhci_runtime_init_strategy_after_mailbox_reset_with_ownership(
        effective_mmio,
        strategy,
        mailbox_reset_completed,
    ...
    let summary = UsbProbePathwaySummary::new(
        ...
        strategy_idx + 1,
        strategy_count,
        xhci_runtime_init_strategy_policy_label(effective_strategy),
        xhci_runtime_init_strategy_origin_label(effective_strategy),
        xhci_firmware_handoff_mode_label(effective_strategy.firmware_handoff),
        xhci_runtime_init_strategy_seed_label(effective_strategy),
        xhci_runtime_init_strategy_halt_guard_label(effective_strategy),
        false,
        XHCI_PCIE_DMA_WINDOW_ENABLED,
        xhci_polling_only_runtime(effective_mmio, effective_strategy.firmware_handoff),
    );
    ...
    Some(UsbProbePreflightStatus {
        ...
        seed: summary.seed,
        halt_guard: xhci_runtime_init_strategy_halt_guard_label_for_source(
            effective_mmio,
            effective_strategy,
        ),
        constructor: xhci_runtime_init_strategy_constructor_label_for_source(
            effective_mmio,
            effective_strategy,
        ),
        pre_reset: xhci_runtime_init_strategy_pre_reset_label_for_source(
            effective_mmio,
            effective_strategy,
        ),
        legacy: xhci_runtime_init_strategy_legacy_label_for_source(
            effective_mmio,
            effective_strategy,
        ),
        run: xhci_runtime_init_strategy_run_label(effective_strategy),
        publish: xhci_runtime_init_strategy_publish_label(effective_strategy),
        post_ready_irq: xhci_runtime_init_strategy_post_ready_irq_label(effective_strategy),
...
impl Pi4LocalSeat {
    ...
    pub fn new(hal: &mut KernelHal<'_>, hints: Pi4LocalSeatHints) -> Result<Self, Pi4SeatError> {
        ...
        let mut contract_line = heapless::String::<512>::new();
        let _ = core::fmt::Write::write_fmt(
            &mut contract_line,
            format_args!(
                "[local-seat] xhci boot contract raw_hint=0x{raw:016x}/{raw_present} mmio=0x{mmio:016x}/{mmio_present} pci_cmd=0x{pci_cmd:04x}/{pci_present} handoff={handoff} irq_quiesced={irq_quiesced} reset_auth={reset_auth} cap_snapshot={cap_snapshot} stop_snapshot={stop_snapshot} irq_policy={irq_policy} poll_only=1",
                raw = hints.xhci_mmio_hint.unwrap_or(0),
                raw_present = if hints.xhci_mmio_hint.is_some() { 1 } else { 0 },
                mmio = xhci_mmio_hint.unwrap_or(0),
                mmio_present = if xhci_mmio_hint.is_some() { 1 } else { 0 },
                pci_cmd = hints.xhci_pci_cmd.unwrap_or(0),
                pci_present = if hints.xhci_pci_cmd.is_some() { 1 } else { 0 },
                handoff = if hints.xhci_handoff_ready { 1 } else { 0 },
                irq_quiesced = if hints.xhci_irq_quiesced { 1 } else { 0 },
                reset_auth = if hints.xhci_bootloader_reset_authorized {
                    1
                } else {
                    0
                },
                cap_snapshot = if hints.xhci_capability_snapshot.is_some() {
                    1
                } else {
                    0
                },
                stop_snapshot = if hints.xhci_stop_state_snapshot.is_some() {
                    1
                } else {
                    0
                },
        ...
        if let Some(snapshot) = hints.xhci_capability_snapshot {
            let mut line = heapless::String::<256>::new();
            let _ = core::fmt::Write::write_fmt(
                &mut line,
                format_args!(
                    "[local-seat] xhci boot cap-snapshot caplen=0x{caplen:02x} hciver=0x{hciver:04x} hcs1=0x{hcs1:08x} hcs2=0x{hcs2:08x} hcc1=0x{hcc1:08x} dboff=0x{dboff:08x} rtsoff=0x{rtsoff:08x}",
                    caplen = snapshot.cap_length,
                    hciver = snapshot.hci_version,
                    hcs1 = snapshot.hcs1,
                    hcs2 = snapshot.hcs2,
                    hcc1 = snapshot.hccparams1,
                    dboff = snapshot.db_offset,
                    rtsoff = snapshot.rts_offset,
                ),
            );
            boot_log::force_uart_line(line.as_str());
        }
        if let Some(snapshot) = hints.xhci_stop_state_snapshot {
            let mut line = heapless::String::<192>::new();
            let _ = core::fmt::Write::write_fmt(
                &mut line,
                format_args!(
                    "[local-seat] xhci boot stop-snapshot usbcmd=0x{usbcmd:08x}/{usbcmd_p} usbsts=0x{usbsts:08x}/{usbsts_p} iman0=0x{iman0:08x}/{iman0_p}",
                    usbcmd = snapshot.usbcmd.unwrap_or(0),
                    usbcmd_p = if snapshot.usbcmd.is_some() { 1 } else { 0 },
                    usbsts = snapshot.usbsts.unwrap_or(0),
                    usbsts_p = if snapshot.usbsts.is_some() { 1 } else { 0 },
                    iman0 = snapshot.iman0.unwrap_or(0),
                    iman0_p = if snapshot.iman0.is_some() { 1 } else { 0 },
                ),
            );
            boot_log::force_uart_line(line.as_str());
        }
...
#[inline]
const fn vl805_command_bus_master_ready(command: u16) -> bool {
    (command & (PCI_COMMAND_MEMORY_SPACE | PCI_COMMAND_BUS_MASTER))
        == (PCI_COMMAND_MEMORY_SPACE | PCI_COMMAND_BUS_MASTER)
}

#[inline]
const fn vl805_live_cfg_bus_master_ready_from_sources(live_command: Option<u16>) -> bool {
    match live_command {
        Some(command) => vl805_command_bus_master_ready(command),
        None => false,
    }
}
...
#[inline]
fn xhci_preseed_allows_high_bar_candidate(
    xhci_mmio_hint: Option<usize>,
    vl805_hint: Option<usize>,
    hal_high_bar_source: bool,
) -> bool {
    xhci_mmio_hint == Some(RPI4_XHCI_MMIO_HIGH_CANDIDATE)
        || vl805_hint == Some(RPI4_XHCI_MMIO_HIGH_CANDIDATE)
        || hal_high_bar_source
        || xhci_cohesix_owned_linux_capture_preseed_source()
}

#[inline]
fn xhci_cohesix_owned_linux_capture_preseed_source() -> bool {
    linux_captured_vl805_cfg_matches_high_bar()
}
...
#[inline]
fn xhci_preseed_runtime_trusted(
    mmio: usize,
    firmware_hint: Option<usize>,
    vl805_hint: Option<usize>,
    xhci_pci_cmd: Option<u16>,
...
#[inline]
const fn xhci_linux_capture_full_reset_mailbox_reset_required_for_strategy(
    mmio: usize,
    strategy: XhciRuntimeInitStrategy,
) -> bool {
...
#[inline]
const fn xhci_runtime_init_strategy_after_mailbox_reset(
    mmio: usize,
    strategy: XhciRuntimeInitStrategy,
    mailbox_reset_completed: bool,
...
fn xhci_linux_capture_full_reset_mailbox_reset_required(
    ...
    _trusted_pinned_mmio: Option<usize>,
    _firmware_hint: Option<usize>,
    _verified_vl805_hint: Option<usize>,
    _vl805_pci_mmio: Option<usize>,
    _pci_cfg_ready: bool,
) -> bool {
    // The Linux capture proves the VL805 high-BAR layout, not Cohesix ownership
...
const fn xhci_firmware_handoff_mode_label(firmware_handoff: XhciFirmwareHandoff) -> &'static str {
    match firmware_handoff {
        ...
        XhciFirmwareHandoff::PlatformResetComplete => "platform-reset-complete",
    }
}
...
const fn linux_captured_vl805_pci_cfg_snapshot() -> Vl805PciCfgSnapshot {
    Vl805PciCfgSnapshot {
        vendor_device: RPI4_VL805_LINUX_VENDOR_DEVICE,
        class_revision: RPI4_VL805_LINUX_CLASS_REVISION,
        command: RPI4_VL805_LINUX_COMMAND,
        bar0: RPI4_VL805_LINUX_BAR0,
        bar1: RPI4_VL805_LINUX_BAR1,
        bar_mmio: Some(RPI4_XHCI_MMIO_HIGH_CANDIDATE),
    }
}

fn linux_captured_vl805_cfg_matches_high_bar() -> bool {
    let snapshot = linux_captured_vl805_pci_cfg_snapshot();
    let vendor_id = (snapshot.vendor_device & 0xffff) as u16;
    let device_id = ((snapshot.vendor_device >> 16) & 0xffff) as u16;
    let class_code = (snapshot.class_revision >> 8) & 0x00ff_ffff;
...
fn xhci_cohesix_owned_linux_capture_source(
    ...
    // This is a bounded layout witness for Cohesix-owned cold start, not a
    // bootloader ownership token. It is only accepted when the high BAR was
    // already pinned locally and no firmware/PCI source is active. IRQ sink
    // readiness is a separate controller-run contract; it must not suppress the
    // static high-BAR layout source.
    xhci_cohesix_owned_linux_capture_source_evidence(
        mmio,
        pinned_xhci_state,
        trusted_pinned_mmio,
        firmware_hint,
...
fn xhci_linux_capture_static_layout_blocked(
    pinned_xhci_state: Option<(usize, bool)>,
    trusted_pinned_mmio: Option<usize>,
    firmware_hint: Option<usize>,
    verified_vl805_hint: Option<usize>,
...
fn xhci_cohesix_owned_linux_capture_source_evidence(
    mmio: usize,
    pinned_xhci_state: Option<(usize, bool)>,
    trusted_pinned_mmio: Option<usize>,
    firmware_hint: Option<usize>,
...
fn xhci_runtime_mmio_candidate_allowed(
    ...
    _firmware_hint: Option<usize>,
    _verified_vl805_hint: Option<usize>,
    cohesix_linux_capture_source: bool,
    _legacy_mirror_allowed: bool,
) -> bool {
    if mmio == RPI4_XHCI_MMIO_HIGH_CANDIDATE {
        trusted_pinned_mmio == Some(mmio) || cohesix_linux_capture_source
...
fn xhci_runtime_mmio_has_accessible_window(
    ...
    if mmio == RPI4_XHCI_MMIO_HIGH_CANDIDATE {
        return trusted_pinned_mmio == Some(mmio)
            || (cohesix_linux_capture_source && has_pinned_window);
    }
    has_device_coverage
        || trusted_pinned_mmio == Some(mmio)
...
#[inline]
fn clear_usb_runtime_proof_status() {
    USB_RUNTIME_KEYBOARD_READY.store(false, Ordering::Release);
    USB_RUNTIME_FIRST_REPORT.store(false, Ordering::Release);
    USB_RUNTIME_FIRST_BYTE.store(false, Ordering::Release);
}

#[inline]
fn mark_usb_runtime_keyboard_ready() {
    USB_RUNTIME_KEYBOARD_READY.store(true, Ordering::Release);
}

#[inline]
fn mark_usb_runtime_first_report() {
    USB_RUNTIME_FIRST_REPORT.store(true, Ordering::Release);
}

#[inline]
fn mark_usb_runtime_first_byte() {
...
pub(crate) fn latest_usb_runtime_proof_status() -> UsbRuntimeProofStatus {
    ...
    let first_byte = USB_RUNTIME_FIRST_BYTE.load(Ordering::Acquire);
    let (proof_gate, next_step, blocker) = if first_byte {
        (10, "none", "none")
    } else if first_report {
        (9, "keyboard-first-byte", "keyboard-first-byte")
    } else if keyboard_ready {
        (8, "hid-first-report", "hid-first-report")
    } else {
        (0, "keyboard-ready", "keyboard-not-ready")
    };
    UsbRuntimeProofStatus {
        keyboard_ready,
        first_report,
        first_byte,
        proof_gate,
...
pub(crate) fn latest_usb_probe_route_status() -> Option<UsbProbeRouteStatus> {
    ...
    Some(UsbProbeRouteStatus {
        ...
        msi_irq_bound: summary.msi_irq_bound,
...
const fn xhci_diag_stage_exact_issue_label(stage: u16) -> Option<&'static str> {
    match stage {
        ...
        0x0267 => Some("uboot-post-run-imod-zero-write-wedged"),
        0x0268 => Some("uboot-post-run-iman-zero-write-wedged"),
...
const fn xhci_diag_stage_value_labels(
    ...
    match stage {
        ...
        0x0202 => Some(("usbcmd", "unused", "skipped")),
        0x0203 => Some(("usbsts", "clear_mask", "deferred")),
        0x0204 => Some(("mmio", "handoff", "seed_flags")),
        0x0205 => Some(("usbcmd", "masked_usbcmd", "write_needed")),
        0x0209 => Some(("usbcmd_off", "usbcmd", "skipped")),
        ...
        0x03ef => Some(("reg_off", "addr", "policy")),
        ...
        0x0403 => Some(("port", "delay_spins", "profile")),
...
fn xhci_diag_stage_label(stage: u16) -> Option<&'static str> {
    match stage {
        ...
        0x0202 => Some("pre-reset-usbcmd-mask-result"),
        0x0203 => Some("pre-reset-usbsts-clear-deferred"),
        ...
        0x0267 => Some("uboot-post-run-imod-zero-write"),
        0x0268 => Some("uboot-post-run-iman-zero-write"),
        ...
        0x0403 => Some("usb-stale-uboot-root-port-reset-begin"),
...
fn xhci_command_probe_verb_label(
    ...
    let _ = event_candidate_mask;
    if preserved_leading_port_events {
        "enable-slot-after-preserved-psc"
    } else {
        "enable-slot"
    }
}
...
#[inline]
fn xhci_stale_bootloader_root_reset_needed(
    strategy: XhciRuntimeInitStrategy,
    command_probe: &str,
) -> bool {
    matches!(
...
fn xhci_probe_enable_slot_fresh_recovery_after_timeout(
    ctrl: &XhciCtrl<SeatDma>,
    bus: &str,
    event_candidate_mask: u32,
    ...
    let recovery_event_generation = "uboot-timeout-polling-fresh-recovery";
    let mut line = heapless::String::<512>::new();
    let _ = core::fmt::Write::write_fmt(
        &mut line,
        format_args!(
            "[local-seat] xhci root-port command-probe result={timeout_result} recovery_event_generation={recovery_event_generation} bus={bus} action=recover-polling-event-generation detail=cmd-event-ring-timeout irq27_role=timer-only pcie_irqs=179,175,180 event_candidate_mask=0x{event_candidate_mask:04x} event_generation={event_generation}"
        ),
    );
    boot_log::force_uart_line(line.as_str());
    let recovery_result = ctrl.probe_enable_slot_fresh_polling_recovery_prompt_safe();
    match recovery_result {
        Ok(slot_id) => {
            ...
            let _ = core::fmt::Write::write_fmt(
                &mut line,
                format_args!(
                    "[local-seat] xhci root-port command-probe result={result} bus={bus} slot={slot_id} cleanup={cleanup} cleanup_generation=uboot-poll-only action=unlock-port-sampling reason=fresh-uboot-polling-recovery-after-uboot-timeout recovery_source={timeout_result} event_candidate_mask=0x{event_candidate_mask:04x} event_generation={recovery_event_generation} uboot_event_generation={event_generation}"
                ),
            );
            boot_log::force_uart_line(line.as_str());
            ...
            let _ = core::fmt::Write::write_fmt(
                &mut summary,
                format_args!(
                    "[local-seat] usb proof_summary gate=4 blocker=none controller=ready command={result} event=command-completion cleanup_generation=uboot-poll-only recovery_source={timeout_result} event_generation={recovery_event_generation} uboot_event_generation={event_generation} irq27_role=timer-only pcie_irqs=179,175,180"
                ),
            );
        ...
        Err(err @ (UsbError::EnableSlotTimeout | UsbError::Timeout)) => {
            ...
            let linux_event_generation =
                "linux-captured-command-event-generation-after-uboot-timeout";
            let linux_cleanup_generation = "linux-captured-command-event-generation";
            let mut line = heapless::String::<640>::new();
            let _ = core::fmt::Write::write_fmt(
                &mut line,
                format_args!(
                    "[local-seat] xhci root-port command-probe result={result} bus={bus} action=probe-linux-captured-event-generation detail=cmd-event-ring-timeout irq27_role=timer-only pcie_irqs=179,175,180 event_candidate_mask=0x{event_candidate_mask:04x} event_generation={recovery_event_generation} uboot_event_generation={event_generation} fallback_event_generation={linux_event_generation}"
                ),
            );
            boot_log::force_uart_line(line.as_str());
            match ctrl.probe_enable_slot_linux_captured_event_generation_prompt_safe() {
                Ok(slot_id) => {
                    let cleanup_result =
                        ctrl.disable_slot_linux_captured_event_generation_prompt_safe(slot_id);
                    let cleanup = usb_disable_slot_cleanup_label(cleanup_result);
                    let result = if cleanup == "disable-slot-ok" {
                        "enable-slot-linux-captured-ok"
                    } else {
                    ...
                    let _ = core::fmt::Write::write_fmt(
                        &mut line,
                        format_args!(
                            "[local-seat] xhci root-port command-probe result={result} bus={bus} slot={slot_id} cleanup={cleanup} cleanup_generation={linux_cleanup_generation} action=unlock-port-sampling reason=linux-captured-command-event-generation-after-uboot-timeout recovery_source=enable-slot-recovery-timeout event_candidate_mask=0x{event_candidate_mask:04x} event_generation={linux_event_generation} uboot_event_generation={event_generation} uboot_recovery_event_generation={recovery_event_generation}"
                        ),
                    );
                    boot_log::force_uart_line(line.as_str());
                    let mut summary = heapless::String::<640>::new();
                    let _ = core::fmt::Write::write_fmt(
                        &mut summary,
                        format_args!(
                            "[local-seat] usb proof_summary gate=4 blocker=none controller=ready command={result} event=command-completion cleanup_generation={linux_cleanup_generation} recovery_source=enable-slot-recovery-timeout event_generation={linux_event_generation} uboot_event_generation={event_generation} uboot_recovery_event_generation={recovery_event_generation} irq27_role=timer-only pcie_irqs=179,175,180"
                        ),
                    );
                    boot_log::force_uart_line(summary.as_str());
                    result
                ...
                Err(UsbError::EnableSlotTimeout | UsbError::Timeout) => {
                    let result = "enable-slot-linux-captured-timeout";
                    let mut line = heapless::String::<640>::new();
                    let _ = core::fmt::Write::write_fmt(
                        &mut line,
                        format_args!(
                            "[local-seat] xhci root-port command-probe result={result} bus={bus} action=return-to-shell detail=cmd-event-ring-timeout irq27_role=timer-only pcie_irqs=179,175,180 event_candidate_mask=0x{event_candidate_mask:04x} event_generation={linux_event_generation} uboot_event_generation={event_generation} uboot_recovery_event_generation={recovery_event_generation} recovery_source=enable-slot-recovery-timeout"
                        ),
                    );
                    boot_log::force_uart_line(line.as_str());
                    let mut summary = heapless::String::<640>::new();
                ...
                Err(err) => {
                    let result = match &err {
                        UsbError::CmdFail(_) => "enable-slot-linux-captured-command-failed",
                        UsbError::PostedWriteFlushFailed => {
                            "enable-slot-linux-captured-posted-flush-failed"
                        }
                        _ => "enable-slot-linux-captured-error",
                    };
                    let mut line = heapless::String::<640>::new();
                    let _ = core::fmt::Write::write_fmt(
                        &mut line,
                        format_args!(
                            "[local-seat] xhci root-port command-probe result={result} bus={bus} action=return-to-shell detail={err:?} event_candidate_mask=0x{event_candidate_mask:04x} event_generation={linux_event_generation} uboot_event_generation={event_generation} uboot_recovery_event_generation={recovery_event_generation} recovery_source=enable-slot-recovery-timeout"
                        ),
                    );
                    boot_log::force_uart_line(line.as_str());
                    result
...
fn xhci_probe_command_ring_after_event_drain(
    ...
    let _ = core::fmt::Write::write_fmt(
        &mut line,
        format_args!(
            "[local-seat] xhci root-port command-probe begin event_candidate_mask=0x{event_candidate_mask:04x} verb={verb} bus={bus} event_generation={event_generation} pci_intx_masked=yes irq27_role=timer-only reason=uboot-command-contract command_publish=uboot-trb16"
        ),
    );
    boot_log::force_uart_line(line.as_str());
    ...
    match enable_slot_result {
        ...
        Err(UsbError::EnableSlotTimeout | UsbError::Timeout) if preserved_leading_port_events => {
            xhci_probe_enable_slot_fresh_recovery_after_timeout(
                ctrl,
                bus,
                event_candidate_mask,
                event_generation,
...
#[inline]
const fn xhci_cap_probe_uses_linux_capture(mmio_base: usize) -> bool {
    mmio_base == RPI4_XHCI_MMIO_HIGH_CANDIDATE
}
...
fn probe_xhci_capability_window(
    ...
    if xhci_cap_probe_uses_linux_capture(mmio_base) {
        let probe = pi4_vl805_linux_capture_capability_probe(mmio_base).ok_or("linux-capture")?;
        let mut line = heapless::String::<256>::new();
        let _ = core::fmt::Write::write_fmt(
            &mut line,
            format_args!(
...
#[inline]
const fn xhci_controller_allows_port_register_access(
    mmio: usize,
    strategy: XhciRuntimeInitStrategy,
) -> bool {
...
#[inline]
const fn xhci_controller_skips_constructor_live_scrub(
    mmio: usize,
    strategy: XhciRuntimeInitStrategy,
) -> bool {
    xhci_controller_skips_initial_live_operational_reads(mmio, strategy)
}

#[inline]
const fn xhci_controller_skips_initial_live_operational_reads(
    mmio: usize,
    strategy: XhciRuntimeInitStrategy,
) -> bool {
...
#[inline]
const fn xhci_runtime_init_strategy_constructor_label_for_source(
    mmio: usize,
    strategy: XhciRuntimeInitStrategy,
) -> &'static str {
    if xhci_controller_skips_constructor_live_scrub(mmio, strategy) {
        "capture-quiesce"
    } else {
...
fn xhci_controller_params_from_probe(
    ...
    let strategy = XhciRuntimeInitStrategy::new(firmware_handoff, runtime_seed_snapshot.is_some());
    XhciControllerParams {
...
const fn xhci_runtime_init_strategy_policy_label(
    ...
    match (strategy.firmware_handoff, strategy.seed_stop_state) {
        ...
        (XhciFirmwareHandoff::PlatformResetComplete, _) => "platform-reset-complete",
    }
}
...
const fn xhci_runtime_init_strategy_origin_label(
    ...
    match (strategy.firmware_handoff, strategy.seed_stop_state) {
        ...
        (XhciFirmwareHandoff::PlatformResetComplete, true) => "stop-state-platform-reset-complete",
        (XhciFirmwareHandoff::PlatformResetComplete, false) => "mailbox-reset-complete",
    }
}
...
#[inline]
const fn xhci_runtime_init_strategy_halt_guard_label_for_source(
    mmio: usize,
    strategy: XhciRuntimeInitStrategy,
) -> &'static str {
    if mmio == RPI4_XHCI_MMIO_HIGH_CANDIDATE
...
#[inline]
const fn xhci_root_port_detect_passes(strategy: XhciRuntimeInitStrategy) -> usize {
    if matches!(
        strategy.firmware_handoff,
        XhciFirmwareHandoff::PlatformResetComplete
    ) && !strategy.seed_stop_state
    {
        return XHCI_PLATFORM_RESET_PORT_DETECT_PASSES;
    }
...
fn xhci_fresh_runtime_ownership_ready(pci_cfg_ready: bool) -> bool {
    ...
    xhci_fresh_runtime_ownership_ready_from_sources(
        pci_cfg_ready,
        vl805_cfg_command().is_some_and(vl805_command_ownership_ready),
        vl805_capture_cfg_witness_ready(),
    )
}
...
#[inline]
const fn xhci_runtime_init_strategy_pre_reset_label_for_source(
    mmio: usize,
    strategy: XhciRuntimeInitStrategy,
) -> &'static str {
    if matches!(
...
#[inline]
const fn xhci_runtime_init_strategy_legacy_label_for_source(
    mmio: usize,
    strategy: XhciRuntimeInitStrategy,
) -> &'static str {
    if xhci_controller_skips_initial_live_operational_reads(mmio, strategy) {
...
fn pi4_vl805_linux_capture_capability_probe(mmio_base: usize) -> Option<XhciCapProbe> {
    if mmio_base != RPI4_XHCI_MMIO_HIGH_CANDIDATE {
        return None;
    }
    cache_xhci_capability_probe_from_snapshot(LocalSeatXhciCapabilitySnapshot {
        cap_length: RPI4_VL805_XHCI_CAP_LENGTH,
        hci_version: RPI4_VL805_XHCI_HCI_VERSION,
        hcs1: RPI4_VL805_XHCI_HCS1,
        hcs2: RPI4_VL805_XHCI_HCS2,
        hccparams1: RPI4_VL805_XHCI_HCCPARAMS1,
        db_offset: RPI4_VL805_XHCI_DBOFF,
        rts_offset: RPI4_VL805_XHCI_RTSOFF,
    })
}
...
fn prime_pinned_xhci_window(
    ...
    let vl805_hint = current_vl805_xhci_mmio_hint();
    let hal_high_bar_source = hal
        .device_coverage(RPI4_XHCI_MMIO_HIGH_CANDIDATE, crate::sel4::PAGE_BITS)
        .is_some();
    ...
    if xhci_preseed_allows_high_bar_candidate(xhci_mmio_hint, vl805_hint, hal_high_bar_source) {
        if hal_high_bar_source
            && xhci_mmio_hint != Some(RPI4_XHCI_MMIO_HIGH_CANDIDATE)
            && vl805_hint != Some(RPI4_XHCI_MMIO_HIGH_CANDIDATE)
        {
            boot_log::force_uart_line(
                "[local-seat] xhci preseed source=hal-device-coverage reason=sel4-device-untyped-high-bar",
            );
        }
        push_candidate(RPI4_XHCI_MMIO_HIGH_CANDIDATE);
    } else {
        boot_log::force_uart_line(
            "[local-seat] xhci preseed deferred reason=no-trusted-high-bar-source",
        );
    }
    ...
    for &mmio in &candidates[..candidate_count] {
        ...
        for &length in &preseed_lengths {
            ...
            let runtime_trusted = xhci_preseed_runtime_trusted(
                validated_mmio,
                xhci_mmio_hint,
                vl805_hint,
                xhci_pci_cmd,
            ...
            if pin_xhci_mmio_window(hal, validated_mmio, pin_length, runtime_trusted).is_ok() {
                if runtime_trusted {
                    remember_vl805_xhci_mmio_hint(validated_mmio);
                } else if validated_mmio == RPI4_XHCI_MMIO_HIGH_CANDIDATE {
                    boot_log::force_uart_line(
                        "[local-seat] xhci preseed high-bar mapped-untrusted reason=no-vl805-pci-or-handoff-owner",
                    );
                }
                let mut line = heapless::String::<176>::new();
                let _ = core::fmt::Write::write_fmt(
                    &mut line,
                    format_args!(
                        "[local-seat] xhci mmio preseeded mmio=0x{mmio:016x} bytes=0x{bytes:05x} trusted={trusted}",
                        mmio = validated_mmio,
                        bytes = pin_length,
                        trusted = runtime_trusted as u8,
                    ),
                );
                boot_log::force_uart_line(line.as_str());
                return;
...
fn prepare_vl805_pci_bcm2711(
    ...
    if !ensure_vl805_config_replay_irq_sinks(hal) {
        boot_log::force_uart_line(
            "[local-seat] vl805 bcm2711-pcie ext-cfg skipped reason=pcie-sinks-unavailable-before-select",
        );
        return None;
    }
    ...
    let pcie_device_control_ready = proof.pcie_device_control_ready();
    ...
    if !pcie_device_control_ready {
        let mut line = heapless::String::<224>::new();
        let _ = core::fmt::Write::write_fmt(
            &mut line,
            format_args!(
                "[local-seat] vl805 bcm2711-pcie reject devctl=0x{devctl:04x} reason=pcie-devctl-not-ready",
                devctl = pcie_devctl_after.unwrap_or(0),
            ),
        );
        boot_log::force_uart_line(line.as_str());
        return None;
    ...
    let _ = core::fmt::Write::write_fmt(
        &mut line,
        format_args!(
            "[local-seat] vl805 bcm2711-pcie selected cfg=hal-ext status=0x{status:08x} vid:did={vendor_id:04x}:{device_id:04x} class=0x{class_code:06x} cmd=0x{command_before:04x}->0x{command_after:04x} devctl=0x{devctl:04x} interrupts=quiesced bar=0x{bar0:08x}/0x{bar1:08x} mmio=0x{mmio:016x}",
            devctl = pcie_devctl_after.unwrap_or(0),
        ),
    );
    boot_log::force_uart_line(line.as_str());
        ...
        match pin_xhci_mmio_window(hal, mmio, XHCI_MMIO_PRESEED_BYTES_FALLBACK, true) {
            Ok(()) => {
                ...
                let _ = core::fmt::Write::write_fmt(
                    &mut line,
                    format_args!(
                        "[local-seat] vl805 bcm2711-pcie xhci-pin mmio=0x{mmio:016x} bytes=0x{bytes:05x} trusted=1",
                        bytes = XHCI_MMIO_PRESEED_BYTES_FALLBACK,
                    ),
                );
                boot_log::force_uart_line(line.as_str());
            ...
            Err(_) => {
                ...
                let _ = core::fmt::Write::write_fmt(
                    &mut line,
                    format_args!(
                        "[local-seat] vl805 bcm2711-pcie xhci-pin skipped mmio=0x{mmio:016x} reason=map-failed"
                    ),
                );
                boot_log::force_uart_line(line.as_str());
                return None;
...
#[derive(Clone, Copy)]
struct XhciIrqGuard {
    bindings: [Option<XhciIrqBinding>; TRUSTED_XHCI_IRQ_BINDING_LIMIT],
}
...
struct XhciIrqBinding {
    irq: u32,
    shadow: bool,
    hal_binding: Option<KernelIrqBinding>,
}
...
impl XhciIrqGuard {
    ...
    fn covers_irq(&self, irq: u32) -> bool {
        self.bindings.iter().flatten().any(|binding| {
            binding.irq == irq && binding.hal_binding.map_or(true, |raw| raw.irq().0 == irq)
        })
    }
    ...
    fn install_binding(
        ...
        let hal_binding = match hal.bind_irq_notification(Irq(irq), IrqTrigger::Level) {
            Ok(binding) => binding,
            Err(err) => {
                let mut line = heapless::String::<240>::new();
                let _ = core::fmt::Write::write_fmt(
                    &mut line,
                    format_args!(
                        "[local-seat] xhci irq {}bind failed irq={} mmio=0x{mmio:016x} err={err} action=defer",
        ...
        let _ = core::fmt::Write::write_fmt(
            &mut line,
            format_args!(
                "[local-seat] xhci irq {}bind irq={} mmio=0x{mmio:016x} trigger=level handler={} notification={} badge=0x{:x}",
                if shadow { "shadow " } else { "sink " },
                irq,
                hal_binding.handler_slot_for_diagnostics(),
                hal_binding.notification_slot_for_diagnostics(),
                hal_binding.badge(),
            ),
        );
        boot_log::force_uart_line(line.as_str());
        Ok(Some(XhciIrqBinding {
            irq,
            shadow,
            hal_binding: Some(hal_binding),
        }))
    }

    fn release_partial_bindings(
        hal: &mut KernelHal<'_>,
        bindings: &mut [Option<XhciIrqBinding>; TRUSTED_XHCI_IRQ_BINDING_LIMIT],
    ) {
        for binding in bindings.iter_mut().filter_map(Option::take) {
            if let Some(raw) = binding.hal_binding {
                let _ = hal.release_irq_notification(raw);
            }
        }
    }
    ...
    fn install(
        ...
        let mut bindings = [None; TRUSTED_XHCI_IRQ_BINDING_LIMIT];
        match sink_mode {
            XhciIrqSinkMode::TrustedPcieSinks => {
                for (index, irq) in TRUSTED_XHCI_PCIE_SINK_IRQS.iter().copied().enumerate() {
                    match Self::install_binding(hal, mmio, irq, false) {
                        ...
                        Err(err) => {
                            Self::release_partial_bindings(hal, &mut bindings);
                            return Err(err);
                        }
                    }
                }
                if xhci_pre_controller_vtimer_shadow_needed(mmio, phase) {
                    let mut line = heapless::String::<240>::new();
                    let _ = core::fmt::Write::write_fmt(
                        &mut line,
                        format_args!(
                            "[local-seat] xhci irq shadow disabled irq={} mmio=0x{mmio:016x} action=pcie-sinks-authoritative",
...
#[inline]
fn cached_xhci_irq_guard() -> Option<XhciIrqGuard> {
    *RETAINED_XHCI_IRQ_GUARD.lock()
}

#[inline]
fn remember_xhci_irq_guard(guard: XhciIrqGuard) {
    *RETAINED_XHCI_IRQ_GUARD.lock() = Some(guard);
}
...
const fn xhci_irq_policy_reason(firmware_handoff: XhciFirmwareHandoff) -> &'static str {
    if !xhci_irq_service_ack_loop_ready() {
        "hal-irq-ack-loop-missing"
    } else if !xhci_vl805_irq_delivery_ready() {
        "vl805-pcie-irq-sink-missing"
    } else {
        match firmware_handoff {
...
const fn xhci_irq_service_ack_loop_ready() -> bool {
    XHCI_HAL_IRQ_ACK_LOOP_AVAILABLE
}

#[inline]
const fn xhci_vl805_irq_delivery_ready() -> bool {
    XHCI_VL805_PCIE_IRQ_SINK_ENABLED && TRUSTED_XHCI_PCIE_SINKS_ENABLED
}

#[inline]
const fn xhci_pre_controller_vtimer_shadow_needed(mmio: usize, phase: XhciIrqInstallPhase) -> bool {
    XHCI_PRE_CONTROLLER_VTIMER_SHADOW_ENABLED
        && mmio == RPI4_XHCI_MMIO_HIGH_CANDIDATE
        && matches!(phase, XhciIrqInstallPhase::PreControllerReady)
}
        ...
        && xhci_vl805_irq_delivery_ready()
    {
        match phase {
            ...
            XhciIrqInstallPhase::ControllerReady => {
                ...
                // controller-ready, enumeration is poll-driven and no extra
                // IRQ binding is required.
                XhciIrqSinkMode::Disabled
            }
...
fn xhci_quiesce_pcie_irq_sources_before_reset(
    ...
    if host_mmio_quiesce {
        ...
        boot_log::force_uart_line(line.as_str());
        let mut complete_line = heapless::String::<256>::new();
        let _ = core::fmt::Write::write_fmt(
            &mut complete_line,
            format_args!(
...
impl UsbKeyboard {
    fn new(
        ...
        if !XHCI_PORT_ACCESS_HOOK_INSTALLED.swap(true, Ordering::AcqRel) {
            set_xhci_port_access_hooks(
                Some(pi4_pcie::vl805_xhci_port_read32),
        ...
        let (vl805_pci_mmio, pci_cfg_ready) = if runtime_cfg_replay_enabled {
            ...
            if !pci_cfg_ready {
                ...
                let _ = core::fmt::Write::write_fmt(
                    &mut line,
                    format_args!(
                        "[local-seat] vl805 live cfg unavailable; xhci probe forced no-live-bus-master cmd=0x{command:04x}"
                    ),
                );
                boot_log::force_uart_line(line.as_str());
            }
        ...
        let mut fresh_runtime_ownership_ready = xhci_fresh_runtime_ownership_ready(pci_cfg_ready);
        ...
        let runtime_vl805_reset_strategy = xhci_runtime_vl805_mailbox_reset_required(
            ...
            xhci_handoff_ready,
            xhci_irq_quiesced,
            runtime_vl805_reset_pci_replay_ready,
        );
        if trusted_fw_handoff_high_bar
            && !runtime_vl805_reset_pci_replay_ready
            && !VL805_RUNTIME_RESET_REPLAY_GATED_LOGGED.swap(true, Ordering::AcqRel)
        {
            let mut line = heapless::String::<256>::new();
            let _ = core::fmt::Write::write_fmt(
                &mut line,
        ...
        let trusted_cohesix_vl805_high_bar = verified_vl805_hint
            == Some(RPI4_XHCI_MMIO_HIGH_CANDIDATE)
            || vl805_pci_mmio == Some(RPI4_XHCI_MMIO_HIGH_CANDIDATE);
        let live_cohesix_vl805_high_bar = xhci_cohesix_owned_bcm2711_ext_cfg_source(
            RPI4_XHCI_MMIO_HIGH_CANDIDATE,
            vl805_pci_mmio,
            pci_cfg_ready,
        );
        let effective_trusted_pinned_xhci_mmio = trusted_pinned_xhci_mmio.filter(|&mmio| {
            !matches!(mmio, RPI4_XHCI_MMIO_HIGH_CANDIDATE)
                ...
                || trusted_cohesix_vl805_high_bar
        });
        ...
        {
            let mut line = heapless::String::<224>::new();
            if live_cohesix_vl805_high_bar {
                let _ = core::fmt::Write::write_fmt(
                    &mut line,
                    format_args!(
                ...
                let _ = core::fmt::Write::write_fmt(
                    &mut line,
                    format_args!(
                        "[local-seat] xhci runtime layout source=linux-capture-witness mmio=0x{mmio:016x} cmd_shadow=0x{cmd:04x} pci_cfg_ready=no irq=poll-only",
                        mmio = RPI4_XHCI_MMIO_HIGH_CANDIDATE,
                        cmd = current_vl805_cfg_command_shadow().unwrap_or(0),
                    ),
                );
            }
            boot_log::force_uart_line(line.as_str());
        }
        let cohesix_linux_capture_mmio = if xhci_cohesix_owned_linux_capture_source(
            RPI4_XHCI_MMIO_HIGH_CANDIDATE,
            pinned_xhci_state,
            ...
            boot_log::force_uart_line(
                "[local-seat] xhci runtime layout source=cohesix-linux-capture-poll-only mmio=0x0000000600000000 cfg=static irq=masked ownership=cohesix-cold-start",
            );
            Some(RPI4_XHCI_MMIO_HIGH_CANDIDATE)
        } else {
            None
        ...
        let mut consider_candidate = |mmio: usize| -> bool {
            ...
            let cohesix_linux_capture_source = xhci_cohesix_owned_linux_capture_source(
                mmio,
                pinned_xhci_state,
                effective_trusted_pinned_xhci_mmio,
                xhci_mmio_hint,
        ...
        if let Some(mmio) = cohesix_linux_capture_mmio {
            consider_candidate(mmio);
        }
        ...
        if candidate_count == 0 {
            ...
            if xhci_linux_capture_static_layout_blocked(
                pinned_xhci_state,
                ...
                boot_log::force_uart_line(
                    "[local-seat] xhci runtime blocked action=reject-static-linux-capture reason=no-live-owner-or-safe-pci-cfg",
                );
            }
            if xhci_mmio_hint == Some(RPI4_XHCI_MMIO_HIGH_CANDIDATE)
                && xhci_firmware_handoff_safe(xhci_pci_cmd)
        ...
        let mut retained_xhci_irq_guard = cached_xhci_irq_guard();
        for &mmio_base in &candidates[..candidate_count] {
            let (
                ...
                {
                    authorize_bootloader_vl805_reset(mmio_base, "pre-cap-snapshot");
                ...
                if trusted_linux_capture_probe.is_some() {
                    let mut line = heapless::String::<256>::new();
                    let _ = core::fmt::Write::write_fmt(
                        &mut line,
                        format_args!(
                ...
                let trusted_cap_probe = trusted_handoff_probe.or(trusted_linux_capture_probe);
                ...
                let raw_probe = if let Some(raw_probe) = trusted_cap_probe {
                    let mut line = heapless::String::<352>::new();
                    let _ = core::fmt::Write::write_fmt(
                        &mut line,
                        format_args!(
                            "[local-seat] xhci cap snapshot source={source} mmio=0x{mmio:016x} caplen=0x{caplen:02x} hciver=0x{hciver:04x} hcs1=0x{hcs1:08x} hcs2=0x{hcs2:08x} hcc1=0x{hccparams1:08x} dboff=0x{dboff:08x} rtsoff=0x{rtsoff:08x} slots={} ports={} scratch={} span=0x{span:05x}",
                            raw_probe.max_slots,
                            raw_probe.max_ports,
                            raw_probe.max_scratchpad,
                            source = trusted_cap_probe_source,
                            mmio = mmio_base,
                ...
                .enumerate()
            {
                    ...
                    )
                {
                    ...
                    let _ = core::fmt::Write::write_fmt(
                        &mut line,
                        format_args!(
                            "[local-seat] xhci prompt-safe skip attempt={}/{} policy={} origin={} handoff={} seed={} reason=source-not-prompt-safe",
                            strategy_idx + 1,
                            init_strategy_count,
                ...
                let strategy_runtime_vl805_reset_requested =
                    xhci_linux_capture_full_reset_mailbox_reset_required(
                        effective_mmio,
                        strategy,
                        pinned_xhci_state,
                        effective_trusted_pinned_xhci_mmio,
                ...
                if strategy_runtime_vl805_reset_requested {
                    if !runtime_vl805_mailbox_reset_completed(
                        VL805_RUNTIME_RESET_STATE.load(Ordering::Acquire),
                    ) {
                        let mut line = heapless::String::<320>::new();
                        let _ = core::fmt::Write::write_fmt(
                        ...
                        boot_log::force_uart_line(line.as_str());
                        if let Err(err) = ensure_runtime_vl805_mailbox_reset(hal) {
                            let mut skip = heapless::String::<288>::new();
                            let _ = core::fmt::Write::write_fmt(
                                &mut skip,
                                format_args!(
                    ...
                    let reset_state = VL805_RUNTIME_RESET_STATE.load(Ordering::Acquire);
                    if !runtime_vl805_mailbox_reset_completed(reset_state) {
                        let mut skip = heapless::String::<288>::new();
                        let _ = core::fmt::Write::write_fmt(
                            &mut skip,
                            format_args!(
                    ...
                    let _ = core::fmt::Write::write_fmt(
                        &mut promoted,
                        format_args!(
                            "[local-seat] xhci probe params promoted attempt={}/{} policy={} origin={} mode={} halt_guard={} ctor={} pre={} legacy={} run={} publish={} post_ready={} seed_flags=0x{promoted_seed_flags:02x} reason=mailbox-reset-complete",
                            strategy_idx + 1,
                            init_strategy_count,
                            xhci_runtime_init_strategy_policy_label(effective_strategy),
                            xhci_runtime_init_strategy_origin_label(effective_strategy),
                            ...
                            xhci_runtime_init_strategy_run_label(effective_strategy),
                            xhci_runtime_init_strategy_publish_label(effective_strategy),
                            xhci_runtime_init_strategy_post_ready_irq_label(effective_strategy),
                        ),
                    );
                ...
                let mut xhci_irq_guard = if retained_xhci_irq_guard.as_ref().is_some_and(|guard| {
                    ...
                    let guard = retained_xhci_irq_guard;
                    if let Some(existing_guard) = guard.as_ref() {
                        let mut line = heapless::String::<288>::new();
                        let _ = core::fmt::Write::write_fmt(
                            &mut line,
                            format_args!(
                    ...
                    match XhciIrqGuard::install(
                        ...
                        Ok(guard) => {
                            if let Some(installed_guard) = guard {
                                retained_xhci_irq_guard = Some(installed_guard);
                                remember_xhci_irq_guard(installed_guard);
                            }
                            guard
                        }
                        Err(reason) => {
                ...
                for &prefer_high in dma_probe_order {
                    ...
                    for (bus_mode_idx, &pcie_dma_window) in dma_bus_modes.iter().enumerate() {
                        ...
                        pathway_summary.msi_irq_bound = msi_irq_bound;
                        ...
                        let ctrl = match XhciCtrl::new_with_params(
                            ...
                            Err(err) => {
                                ...
                                boot_log::force_uart_line(line.as_str());
                                let mailbox_suppression_edge =
                                    matches!(strategy.firmware_handoff, XhciFirmwareHandoff::None)
                                        && (runtime_vl805_reset_requested
                                            || strategy_runtime_vl805_reset_requested);
                                log_xhci_probe_failure_edge(
                                    mailbox_suppression_edge,
                                    strategy,
                                    diag_before,
                                    diag_after,
                                );
                        ...
                        if xhci_irq_sink_needed(
                            ...
                            match XhciIrqGuard::install(
                                ...
                                Ok(Some(guard)) => {
                                    retained_xhci_irq_guard = Some(guard);
                                    remember_xhci_irq_guard(guard);
                                    xhci_irq_guard = Some(guard);
                                }
                                Ok(None) => {}
                                Err(reason) => {
                                    let mut line = heapless::String::<224>::new();
                        ...
                        if bootloader_port_reads_toxic {
                            ...
                            if xhci_runtime_init_strategy_skips_command_probe_after_toxic_portsc(
                                ...
                                command_probe = "deferred-platform-reset-portsc-toxic";
                                let mut line = heapless::String::<224>::new();
                                let _ = core::fmt::Write::write_fmt(
                                    &mut line,
                                    format_args!(
                                        "[local-seat] xhci root-port command-probe skipped reason=platform-reset-portsc-toxic probe={command_probe}"
                                    ),
                                );
                                boot_log::force_uart_line(line.as_str());
                            } else {
                        ...
                        if !bootloader_port_reads_toxic {
                            ...
                            let detect_passes = xhci_root_port_detect_passes(strategy);
                            {
                                ...
                                let _ = core::fmt::Write::write_fmt(
                                    &mut line,
                                    format_args!(
                                        "[local-seat] xhci root-port sample begin ports={} passes={} timer_irq={} timer_role=kernel-vtimer",
                                        max_ports, detect_passes, PI4_GENERIC_VTIMER_IRQ,
                                    ),
                                );
                                boot_log::force_uart_line(line.as_str());
                            }
                            for pass in 0..detect_passes {
                        ...
                        let final_wait_ms = xhci_root_port_final_wait_ms(strategy);
                        if connected_mask == 0
                            && max_ports != 0
                            && !bootloader_port_reads_toxic
                            && final_wait_ms != 0
                        ...
                        let _ = core::fmt::Write::write_fmt(
                            &mut line,
                            format_args!(
                                "[local-seat] xhci controller-ready mmio=0x{mmio:016x} dma={} bus={} ports={} ctx={} connected_mask=0x{mask:04x} event_candidate_mask=0x{event_mask:04x} command_probe={} detect_passes={} slow_recheck={}",
                                if prefer_high { "high" } else { "low" },
                                if pcie_dma_window {
                                    "pcie-window"
                                } else {
                                ...
                                mmio = effective_mmio,
                                mask = connected_mask,
                                event_mask = event_candidate_mask,
                            ),
                        );
                        boot_log::force_uart_line(line.as_str());
                        ...
                        let stale_bootloader_root_reset =
                            xhci_stale_bootloader_root_reset_needed(strategy, command_probe);
                        if stale_bootloader_root_reset {
                            let mut line = heapless::String::<224>::new();
                            let _ = core::fmt::Write::write_fmt(
                                &mut line,
                                format_args!(
                                    "[local-seat] usb root-port reset profile=stale-uboot-device action=extra-post-command-proof-reset command_probe={command_probe} event_candidate_mask=0x{event_candidate_mask:04x}"
                                ),
                            );
                            boot_log::force_uart_line(line.as_str());
                        }
                        ...
                        for port in 0..max_ports {
                            ...
                            } else if stale_bootloader_root_reset {
                                UsbDevice::new_with_stale_bootloader_root_reset(
                                    ctrl.clone(),
                                    port as u8,
                                )
                            } else {
                            ...
                            let mut device = match root_device {
                                Ok(device) => device,
                                Err(err) => {
                                    let diag_after = read_latest_xhci_diag_snapshot();
                                    usb_probe_pathway_record(
...
fn pcie_dma_bus_addr(phys: usize) -> Option<usize> {
    ...
    RPI4_PCIE_DMA_BUS_ALIAS_BASE.checked_add(phys)
}
...
mod driver_coverage_tests {
    ...
    fn driver_coverage_pi4_local_seat_usb_vl805_dma_contracts() {
        ...
        assert!(xhci_stale_bootloader_root_reset_needed(
            XhciRuntimeInitStrategy::new(XhciFirmwareHandoff::PlatformResetComplete, false),
            "enable-slot-ok"
        ));
        assert!(!xhci_stale_bootloader_root_reset_needed(
        ...
        assert_eq!(xhci_command_probe_verb_label(0, false), "enable-slot");
        assert_eq!(
            xhci_command_probe_event_generation_label(0, false),
            "uboot-poll-empty"
        );
        assert_eq!(xhci_command_probe_verb_label(0x001f, false), "enable-slot");
        assert_eq!(
...
mod tests {
    ...
    #[test]
    fn xhci_runtime_init_strategies_keep_live_default_without_stop_seed() {
        let (strategies, count) = xhci_runtime_init_strategies(
            XhciFirmwareHandoff::None,
            super::VL805_RUNTIME_RESET_STATE_UNATTEMPTED,
            None,
    ...
    #[test]
    fn mailbox_acked_without_config_replay_uses_platform_reset_published_gate() {
        let full_reset = XhciRuntimeInitStrategy::new(XhciFirmwareHandoff::None, false);
        let strategy = xhci_runtime_init_strategy_after_mailbox_reset_with_ownership(
            RPI4_XHCI_MMIO_HIGH_CANDIDATE,
            full_reset,
    ...
    fn usb_probe_route_labels_distinguish_primary_and_fallback_paths() {
        ...
        let live_primary = UsbProbePathwaySummary::new(
            1,
            1,
            3,
            "full-reset-start",
        ...
        let mailbox_reset = UsbProbePathwaySummary::new(
            1,
            1,
            1,
            "platform-reset-complete",
        ...
        assert_eq!(
            super::usb_probe_route_label(mailbox_reset),
            "trusted-high-bar-mailbox-reset"
        );
    }
    ...
    #[test]
    fn usb_runtime_proof_status_tracks_report_and_console_byte_gates() {
        super::clear_usb_runtime_proof_status();
        let empty = super::latest_usb_runtime_proof_status();
        ...
        assert_eq!(report.proof_gate, 9);
        assert_eq!(report.next_step, "keyboard-first-byte");
        assert_eq!(report.blocker, "keyboard-first-byte");

        super::mark_usb_runtime_first_byte();
        let byte = super::latest_usb_runtime_proof_status();
    ...
    #[test]
    fn usb_probe_preflight_status_reports_mailbox_notify_before_linux_capture_full_reset() {
        let status = super::usb_probe_preflight_status(
            Some(RPI4_XHCI_MMIO_HIGH_CANDIDATE),
            None,
            false,
        ...
        .expect("Linux-captured high BAR full-reset path should be classified");
        assert_eq!(status.route, "trusted-high-bar-primary");
        assert_eq!(status.strategy_idx, 1);
        assert_eq!(status.strategy_count, 1);
        assert_eq!(status.policy, "full-reset-start");
    ...
    fn usb_probe_next_step_advances_event_candidates_to_safe_command_frontier() {
        ...
        let no_event_linux_captured_ready = UsbProbePathwaySummary {
            command_probe: "enable-slot-linux-captured-ok",
            ..no_event_command_ready
        };
        assert_eq!(
            super::usb_probe_next_step(no_event_linux_captured_ready),
            "safe-port-state"
        );
    ...
    #[test]
    fn xhci_linux_capture_high_bar_runtime_default_skips_constructor_live_scrub() {
        let probe = super::pi4_vl805_linux_capture_capability_probe(RPI4_XHCI_MMIO_HIGH_CANDIDATE)
            .expect("Pi4 high BAR should use Linux-captured caps");
        let strategy = XhciRuntimeInitStrategy::new(XhciFirmwareHandoff::None, false);
        let params = xhci_controller_params_from_probe_with_strategy(
        ...
        assert!(xhci_controller_skips_initial_live_operational_reads(
            RPI4_XHCI_MMIO_HIGH_CANDIDATE,
            strategy,
        ));
        assert_eq!(
    ...
    fn linux_capture_shadow_does_not_make_live_pci_cfg_ready() {
        ...
        assert!(!super::vl805_live_cfg_bus_master_ready_from_sources(None));
        assert!(super::vl805_command_bus_master_ready(capture_cmd));
        assert!(super::vl805_live_cfg_bus_master_ready_from_sources(Some(
            capture_cmd
        )));
        assert!(!super::xhci_fresh_runtime_ownership_ready_from_sources(
            false, false, true,
    ...
    #[test]
    fn xhci_constructor_live_scrub_override_stays_high_bar_and_unseeded_only() {
        assert!(!xhci_controller_skips_constructor_live_scrub(
            RPI4_XHCI_MMIO_PRIMARY_CANDIDATE,
            XhciRuntimeInitStrategy::new(XhciFirmwareHandoff::None, false),
        ));
    ...
    #[test]
    fn pi4_linux_capture_capability_probe_uses_known_good_vl805_caps() {
        let probe = pi4_vl805_linux_capture_capability_probe(RPI4_XHCI_MMIO_HIGH_CANDIDATE)
            .expect("Pi4 high BAR should use the Linux-captured VL805 cap snapshot");
        assert_eq!(probe.cap_length, RPI4_VL805_XHCI_CAP_LENGTH);
        assert_eq!(probe.hci_version, RPI4_VL805_XHCI_HCI_VERSION);
        assert_eq!(probe.hcs1, RPI4_VL805_XHCI_HCS1);
        assert_eq!(probe.hcs2, RPI4_VL805_XHCI_HCS2);
        assert_eq!(probe.hccparams1, RPI4_VL805_XHCI_HCCPARAMS1);
        assert_eq!(probe.db_offset, RPI4_VL805_XHCI_DBOFF);
        assert_eq!(probe.rts_offset, RPI4_VL805_XHCI_RTSOFF);
        assert_eq!(probe.max_slots, 32);
        assert_eq!(probe.max_ports, 4);
        assert_eq!(probe.max_scratchpad, 31);
        assert_eq!(probe.mmio_size, 0x1000);
        assert!(
            pi4_vl805_linux_capture_capability_probe(RPI4_XHCI_MMIO_PRIMARY_CANDIDATE).is_none()
        );
    }

    #[test]
    fn pi4_high_bar_cap_probe_uses_capture_not_live_read() {
        assert!(xhci_cap_probe_uses_linux_capture(
            RPI4_XHCI_MMIO_HIGH_CANDIDATE
        ));
        assert!(!xhci_cap_probe_uses_linux_capture(
            RPI4_XHCI_MMIO_PRIMARY_CANDIDATE
    ...
    fn xhci_diag_stage_labels_cover_runtime_write_fault_markers() {
        ...
        assert_eq!(
            xhci_diag_stage_label(0x0226),
            Some("reset-pre-usbcmd-source")
        );
        assert_eq!(
        ...
        assert_eq!(
            xhci_diag_stage_label(0x0267),
            Some("uboot-post-run-imod-zero-write")
        );
        assert_eq!(
            xhci_diag_stage_label(0x0268),
            Some("uboot-post-run-iman-zero-write")
        );
        ...
        assert_eq!(
            xhci_diag_stage_label(0x03ef),
            Some("dma-address-translate-failed")
        );
        assert_eq!(
        ...
        assert_eq!(
            xhci_diag_stage_label(0x0400),
            Some("usb-root-port-reset-retry")
        );
        assert_eq!(
            xhci_diag_stage_label(0x0403),
            Some("usb-stale-uboot-root-port-reset-begin")
        );
        assert_eq!(
            xhci_diag_stage_label(0x0405),
            Some("usb-stale-uboot-root-port-reset-failed")
        );
        assert_eq!(
    ...
    fn xhci_diag_exact_issue_labels_call_out_pre_run_usb_stalls() {
        ...
        assert_eq!(
            ...
            Some("uboot-post-run-imod-zero-write-wedged")
        );
        assert_eq!(
            super::xhci_diag_stage_exact_issue_label(0x0268),
            Some("uboot-post-run-iman-zero-write-wedged")
        );
        assert_eq!(
    ...
    fn xhci_diag_stage_after_run_distinguishes_post_run_and_pre_run_edges() {
        ...
        assert!(super::xhci_diag_stage_after_run(0x0400));
        assert!(super::xhci_diag_stage_after_run(0x0405));
        assert!(super::xhci_diag_stage_after_run(0x0406));
    ...
    fn xhci_diag_value_labels_name_pre_run_ring_snapshot_fields() {
        ...
        assert_eq!(
            ...
            Some(("stop_usbcmd", "live_state_skipped", "policy"))
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(("settle_spins", "poll_safe", "run_usbcmd"))
        );
        assert_eq!(
        ...
        assert_eq!(
            xhci_diag_stage_value_labels(0x0400),
            Some(("port", "retry_state", "delay_spins"))
        );
        assert_eq!(
            xhci_diag_stage_value_labels(0x0405),
            Some(("port", "error", "profile"))
        );
        assert_eq!(
            xhci_diag_stage_value_labels(0x0406),
            Some(("psc_count", "psc_mask", "event_syncs"))
        );
    ...
    fn fresh_platform_reset_root_port_sample_stays_gated_to_command_proof() {
        ...
        let stop_seed =
            XhciRuntimeInitStrategy::new(XhciFirmwareHandoff::PlatformResetComplete, true);
        let cold = XhciRuntimeInitStrategy::new(XhciFirmwareHandoff::ColdStartFromSnapshot, false);
        ...
        assert_eq!(
            super::xhci_root_port_final_wait_ms(stop_seed),
            super::XHCI_PORT_DETECT_FINAL_WAIT_MS
        );
        assert_eq!(
            super::xhci_root_port_detect_passes(cold),
            super::XHCI_PORT_DETECT_PASSES
        );
    }
    ...
    #[test]
    fn xhci_irq_sink_mode_enables_bounded_pcie_sinks_before_primary_writes() {
        assert_eq!(
            xhci_irq_sink_mode(
                RPI4_XHCI_MMIO_HIGH_CANDIDATE,
                super::xhci_preferred_trusted_handoff_mode(
    ...
    #[test]
    fn xhci_irq_policy_allows_pi4_pcie_irq_sinks_before_primary_writes() {
        assert!(XHCI_HAL_IRQ_ACK_LOOP_AVAILABLE);
        assert!(xhci_irq_service_ack_loop_ready());
        assert!(XHCI_VL805_PCIE_IRQ_SINK_ENABLED);
        assert!(xhci_vl805_irq_delivery_ready());
        assert!(!XHCI_PRE_CONTROLLER_VTIMER_SHADOW_ENABLED);
        assert!(!xhci_pre_controller_vtimer_shadow_needed(
            RPI4_XHCI_MMIO_HIGH_CANDIDATE,
            XhciIrqInstallPhase::PreControllerReady,
        ));
        assert!(!xhci_pre_controller_vtimer_shadow_needed(
    ...
    #[test]
    fn disabled_trusted_xhci_irq_guard_does_not_gate_polling_path() {
    ...
    #[test]
    fn primary_pcie_irq_is_required_for_untrusted_or_unseeded_high_bar_writes() {
        assert!(xhci_runtime_init_strategy_requires_primary_pcie_irq(
            RPI4_XHCI_MMIO_HIGH_CANDIDATE,
            XhciRuntimeInitStrategy::new(XhciFirmwareHandoff::ColdStartFromSnapshot, false),
        ));
    ...
    fn vl805_bcm2711_ext_cfg_runtime_touch_uses_hal_owned_proof() {
        ...
        let command_before = PCI_COMMAND_MEMORY_SPACE;
        let masked_command = vl805_poll_only_intx_mask_command(command_before);
        assert_eq!(
            masked_command,
    ...
    #[test]
    fn xhci_high_candidate_requires_trusted_runtime_pin() {
    ...
    #[test]
    fn xhci_high_candidate_accepts_linux_capture_as_bounded_layout_source() {
        assert!(super::linux_captured_vl805_cfg_matches_high_bar());
        assert!(super::xhci_cohesix_owned_linux_capture_source_evidence(
            RPI4_XHCI_MMIO_HIGH_CANDIDATE,
            Some((RPI4_XHCI_MMIO_HIGH_CANDIDATE, false)),
            None,
            None,
            None,
            None,
        ...
        let runtime_source = super::xhci_cohesix_owned_linux_capture_source(
            RPI4_XHCI_MMIO_HIGH_CANDIDATE,
            Some((RPI4_XHCI_MMIO_HIGH_CANDIDATE, false)),
            None,
            None,
        ...
        assert!(!super::xhci_linux_capture_static_layout_blocked(
            Some((RPI4_XHCI_MMIO_HIGH_CANDIDATE, false)),
            None,
            None,
            None,
    ...
    #[test]
    fn xhci_linux_capture_evidence_requires_pin_and_no_other_source() {
        assert!(!super::xhci_cohesix_owned_linux_capture_source_evidence(
            RPI4_XHCI_MMIO_HIGH_CANDIDATE,
            None,
            None,
    ...
    #[test]
    fn xhci_high_bar_preseed_snapshot_is_not_runtime_trust() {
        assert!(!xhci_preseed_runtime_trusted(
            RPI4_XHCI_MMIO_HIGH_CANDIDATE,
            None,
            None,
            None,
        ...
        assert!(!xhci_preseed_runtime_trusted(
            ...
            Some(RPI4_XHCI_MMIO_HIGH_CANDIDATE),
            None,
            Some(PCI_COMMAND_MEMORY_SPACE | PCI_COMMAND_BUS_MASTER),
            false,
            false,
        ));
        assert!(xhci_preseed_runtime_trusted(
        ...
        assert!(xhci_preseed_runtime_trusted(
            ...
            Some(RPI4_XHCI_MMIO_HIGH_CANDIDATE),
            None,
            Some(PCI_COMMAND_MEMORY_SPACE | PCI_COMMAND_BUS_MASTER | PCI_COMMAND_INTERRUPT_DISABLE),
            true,
            true,
        ));
    }
    ...
    #[test]
    fn xhci_high_bar_preseed_accepts_capture_layout_source() {
        assert!(super::xhci_cohesix_owned_linux_capture_preseed_source());
        assert!(xhci_preseed_allows_high_bar_candidate(None, None, false));
        assert!(xhci_preseed_allows_high_bar_candidate(
            Some(RPI4_XHCI_MMIO_HIGH_CANDIDATE),
            None,
            false
        ));
        ...
        assert!(xhci_preseed_allows_high_bar_candidate(None, None, true));
    }
    ...
    #[test]
    fn linux_capture_full_reset_requires_runtime_mailbox_notify() {
        let full_reset = XhciRuntimeInitStrategy::new(XhciFirmwareHandoff::None, false);
        let stop_seed = XhciRuntimeInitStrategy::new(XhciFirmwareHandoff::None, true);

        assert!(
            super::xhci_linux_capture_full_reset_mailbox_reset_required_for_strategy(
                RPI4_XHCI_MMIO_HIGH_CANDIDATE,
                full_reset,
            )
        );
    ...
    #[test]
    fn vl805_runtime_mailbox_reset_requires_safe_pci_replay() {
        assert!(super::xhci_runtime_vl805_mailbox_reset_pci_replay_ready(
            true, true
        ));
        assert!(!super::xhci_runtime_vl805_mailbox_reset_pci_replay_ready(
            false, true
