...
impl IsolatedSelfTestState {
    ...
    pub(crate) fn observe(
        ...
        let tx_ok = response_drained
            && (observation.direct_virtio || observation.tx_complete > self.start_tx_complete);
...
mod tests {
    ...
    #[test]
    fn direct_virtio_uses_exact_child_response_drain_as_tx_boundary() {
        let mut state = IsolatedSelfTestState::new(true);
        assert!(state.start(1, 0, 3, 31, 0, 0, Some(7)));
        let mut complete = observation(2);
        complete.direct_virtio = true;
