...
fn resolve_cnode_depth(init_cnode_bits: u8) -> (u8, sys::seL4_Word) {
    ...
    // The initial thread's CNode is configured with guard bits so that canonical
    // capability pointers (the raw `seL4_CPtr` values) select the correct slots.
    // However, seL4's CNode invocations still expect the *actual* radix width of
    // the CNode for the depth arguments, not the architectural word size. Passing
    // the canonical depth causes the kernel to overrun the radix and reject valid
    // slots (manifesting as `Invalid source slot`). Use the bootinfo-declared CNode
    // size bits for both the integer and word representations so that capability
    // lookups align with the guard configuration on every architecture.
