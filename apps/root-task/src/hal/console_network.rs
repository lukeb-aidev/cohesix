...
impl ConsoleNetworkRuntime {
    ...
    pub(crate) fn direct_genet_queue_timing(
        ...
        [GENET_RX_QUEUE_TIMING_OFFSET, GENET_TX_QUEUE_TIMING_OFFSET].map(|offset| {
            let bytes =
                sample_direct_genet_observation::<256, 32>(self.direct_genet_root_ptrs[0], offset)?;
...
mod tests {
    ...
    fn direct_genet_queue_reader_requires_stable_commit_and_live_generation() {
        ...
        for offset in [640, 896] {
            page.0[offset..offset + 256].copy_from_slice(&timing.encode());
            let bytes = sample_direct_genet_observation::<256, 32>(ptr, offset)
            ...
            page.0[offset + 248..offset + 256].fill(0);
            assert_eq!(
                sample_direct_genet_observation::<256, 32>(ptr, offset),
                None
            );
        }
        assert_eq!(sample_direct_genet_observation::<256, 32>(ptr, 4096), None);
        assert_eq!(sample_direct_genet_observation::<256, 32>(ptr, 641), None);
    }
    ...
    fn direct_genet_runtime_diagnostic_reader_rejects_uncommitted_and_stale_records() {
        ...
        let commit_ptr = root_ptr
            ...
            + console_network_abi::DIRECT_GENET_RUNTIME_DIAGNOSTIC_COMMIT_OFFSET;
