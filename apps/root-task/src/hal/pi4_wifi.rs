...
fn wait_for_firmware_ready_can_assume_mailbox_ready(
    ...
    let fragile_mailbox_err = (is_sdhci_io_path_error(err) || is_sdhci_int_timeout(err))
        || is_sdio_cmd52_access_error(err)
        || is_sdhci_fragile_read_error(err);
    allow_function2_ready_bypass && fragile_mailbox_err && attempt > 0
}
...
const fn required_ht_clock_bounded_no_ht_shortcut_loops() -> usize {
    CYW43_HT_CLOCK_SOFT_WAIT_LOOPS * 4
}
...
const fn control_plane_function2_interrupts_armed(ienx: Option<u8>) -> bool {
    matches!(ienx, Some(bits) if (bits & SDIO_CCCR_IEN_FUNC2) != 0)
}
...
const fn control_plane_snapshot_exact_blocker_label(
    ...
    } else if matches!(ienx, Some(bits) if (bits & SDIO_CCCR_IEN_FUNC2) == 0) {
        "f2-interrupt-gated"
...
fn control_plane_snapshot_exact_error_label(blocker: &'static str) -> &'static str {
    match blocker {
        "f2-disabled" => "cyw43-function2-disabled",
        "f2-ready-hidden-from-cccr" => "cyw43-function2-ready-hidden-from-cccr",
        "f2-enable-latched-not-ready" => "cyw43-function2-enable-latched-not-ready",
        "f2-ready-unreadable" => "cyw43-function2-ready-unreadable",
        "linux-configured-interrupts-deferred" => "cyw43-control-plane-linux-interrupts-deferred",
        "f2-interrupt-gated" => "cyw43-function2-interrupt-gated",
        "f2-interrupt-unreadable" => "cyw43-function2-interrupt-unreadable",
        "sdio-core-hints-unreadable" => "cyw43-control-plane-sideband-unreadable",
        "interrupt-programming-drift" => "cyw43-control-plane-interrupt-programming-drift",
        "reply-frame-missing" => "cyw43-control-plane-reply-frame-missing",
        "partial-hint-visibility" => "cyw43-control-plane-partial-hint-visibility",
        _ => "cyw43-control-plane-state-visible-no-reply",
    }
}
...
fn control_plane_snapshot_exact_error_label_with_sdhci_diag(
    ...
    match blocker {
        ...
        "f2-enable-latched-not-ready" if sdhci_read_diag == "f2-reply-read-command-crc" => {
            "cyw43-function2-enable-latched-not-ready-command-crc"
        }
        "f2-enable-latched-not-ready" if sdhci_read_diag == "f2-reply-read-command-end-bit" => {
            "cyw43-function2-enable-latched-not-ready-command-end-bit"
        }
        "f2-enable-latched-not-ready" if sdhci_read_diag == "f2-reply-read-command-index" => {
            "cyw43-function2-enable-latched-not-ready-command-index"
        }
        "f2-enable-latched-not-ready" if sdhci_read_diag == "f2-reply-read-command-error" => {
        ...
        "f2-enable-latched-not-ready" if sdhci_read_diag == "f2-reply-read-data-end-bit" => {
        ...
        "sdio-core-hints-unreadable" if sdhci_read_diag == "f1-reply-read-command-crc" => {
            "cyw43-control-plane-sideband-command-crc"
        }
        "sdio-core-hints-unreadable" if sdhci_read_diag == "f1-reply-read-command-end-bit" => {
            "cyw43-control-plane-sideband-command-end-bit"
        }
        "sdio-core-hints-unreadable" if sdhci_read_diag == "f1-reply-read-command-index" => {
            "cyw43-control-plane-sideband-command-index"
        }
        "sdio-core-hints-unreadable" if sdhci_read_diag == "f1-reply-read-command-error" => {
...
const CYW43_CONTROL_PLANE_STARTUP_LINK_PROMOTED_DIAGNOSTIC_PREFIX_PROBE_LENS: [usize; 1] =
    [CYW43_CONTROL_PLANE_LINUX_FIRSTREAD_LEN];
// Once the low-touch startup-link lane proves the first fixed-address Function 2
// read is stalling, escalating to larger diagnostic prefixes only repeats the
// same failure more slowly. The minimal SDPCM prefix is enough to validate a
// reply header and drive the remainder read on success.
const CYW43_CONTROL_PLANE_LOW_TOUCH_DIAGNOSTIC_PREFIX_PROBE_LENS: [usize; 1] =
    [CYW43_SDPCM_PREFIX_MIN_LEN];
...
const fn sdio_function_ready_uses_force_reenable_budget(
    ...
    // Linux brcmfmac's rxfail path aborts/terminates/drains and sends SMB_NAK,
    // but it does not clear Function 2 on the follow-on receive path. Keep
    // the hard clear+re-enable policy only for strict reply recovery.
    matches!(step.function, SdioFunction::Function2)
        && matches!(
            budget,
            SdioFunctionReadyBudget::ControlPlaneReplyStrictRecovery
...
#[inline]
const fn sdio_function_ready_should_force_reenable(
    step: SdioFunctionEnableStep,
    budget: SdioFunctionReadyBudget,
    enabled: u8,
    ready: u8,
) -> bool {
    sdio_function_ready_uses_force_reenable_budget(step, budget)
        && (enabled & step.enable_bit) == step.enable_bit
        && (ready & step.ready_bit) == 0
}

#[inline]
const fn sdio_function_ready_force_reenable_settle_loops(
    step: SdioFunctionEnableStep,
    budget: SdioFunctionReadyBudget,
) -> usize {
    if sdio_function_ready_uses_force_reenable_budget(step, budget) {
...
enum SdioFunctionReadyBudget {
    Strict,
    ControlPlaneReplyStrictRecovery,
...
const fn sdio_function_ready_budget_name(budget: SdioFunctionReadyBudget) -> &'static str {
    match budget {
        ...
        SdioFunctionReadyBudget::ControlPlaneReplyStrictRecovery => "reply-strict-recovery",
...
impl SdioHost {
    ...
    fn keep_control_plane_on_startup_link_after_first_write(&mut self, stage: &'static str) {
        self.experimental_control_plane_startup_link_stabilized = true;
        self.experimental_control_plane_reply_rearm_mode = control_plane_reply_rearm_none();
        self.experimental_control_plane_reply_rearm_attempts = 0;
        self.experimental_control_plane_reply_rearm_empty_polls = 0;
        self.experimental_control_plane_promoted_probe_pending = false;
        self.clear_control_plane_speculative_reply_frame();
        emit_breadcrumb(format_args!(
            "[pi4-wifi] firmware stage={stage} action=stabilize-startup-link current={}Hz width={} reply_mode=startup-link-passive write_chunk_limit={} reply_chunk_limit={} no_ht={}",
            self.current_clock_hz,
            sdio_bus_width_name(self.desired_bus_width),
    ...
    fn begin_control_plane_startup_link_rescue(
        ...
        let next_cycle = clamp_startup_link_rescue_cycle(
    ...
    fn fail_passive_startup_link_wait(&mut self) -> Result<usize, HalError> {
        ...
        self.experimental_control_plane_reply_rearm_empty_polls = 0;
        self.experimental_control_plane_promoted_probe_pending = promoted_probe_pending;
        match self.try_capture_control_plane_reply_speculative_frame() {
            Ok(frame_len) => {
                if frame_len != 0 {
                    self.clear_control_plane_sideband_unreadable_recovery_cycles();
                    if control_plane_reply_progress_clears_startup_link_rescue(false, true) {
                    ...
                    let ready = self
                        ...
                        .ok();
                    emit_breadcrumb(format_args!(
                        "[pi4-wifi] firmware stage=control-plane-reply action=passive-startup-link-timeout-diag ioex=0x{ioex:02x}/{} iorx=0x{ready:02x}/{} current_clock={}Hz width={} chunk_limit={} no_ht={}",
                        yn(ioex.is_some()),
                        yn(ready.is_some()),
                        self.current_clock_hz,
                        sdio_bus_width_name(self.desired_bus_width),
                        ...
                        self.experimental_no_ht_transport,
                        ioex = ioex.unwrap_or(0),
                        ready = ready.unwrap_or(0),
                    ));
                    let timeout_exact_error = cached_wifi_debug_snapshot()
                        .map(|snapshot| snapshot.control_plane_exact_error)
                        .filter(|exact_error| !exact_error.is_empty());
                    if passive_startup_link_timeout_skips_strict_recovery(
                        self.experimental_control_plane_startup_link_rescue_cycles,
                        ioex,
                        ready,
                        timeout_exact_error,
                    ) {
                        let exact_error = timeout_exact_error
                            .unwrap_or("cyw43-control-plane-startup-link-rescue-budget-exhausted");
                        emit_breadcrumb(format_args!(
                    ...
                    if control_plane_function2_enable_latched_not_ready(ioex, ready)
                        && self.try_strict_control_plane_reply_recovery(
                            "passive-startup-link-timeout",
                            reply_rearm_mode,
                            usize::from(reply_rearm_attempts),
                            ready.unwrap_or(0),
                        )?
                    {
                        emit_breadcrumb(format_args!(
                            "[pi4-wifi] firmware stage=control-plane-reply action=passive-startup-link-timeout-strict-recovered current_clock={}Hz width={} chunk_limit={} no_ht={}",
                            self.current_clock_hz,
                            sdio_bus_width_name(self.desired_bus_width),
                            self.control_plane_chunk_limit(),
                            self.experimental_no_ht_transport,
                        ...
                        self.resume_control_plane_reply_probe_on_startup_link(
                            "control-plane-passive-startup-link-timeout-strict-recovered",
                        );
                    ...
                    emit_breadcrumb(format_args!(
                        ...
                        self.control_plane_chunk_limit(),
                        self.experimental_no_ht_transport,
                        yn(final_ioex.is_some()),
                        yn(final_ready.is_some()),
                        ioex = final_ioex.unwrap_or(0),
                        ready = final_ready.unwrap_or(0),
                    ));
                    self.log_function2_ready_snapshot("passive-startup-link-timeout-no-reply");
                    self.log_control_plane_finish_snapshot(
                        "control-plane-passive-startup-link-timeout-no-reply",
                    );
                    self.experimental_control_plane_reply_rearm_mode =
            ...
            Err(err) => {
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage=control-plane-reply action=passive-startup-link-timeout-probe-fail current_clock={}Hz width={} chunk_limit={} no_ht={} err={err}",
                    self.current_clock_hz,
                    sdio_bus_width_name(self.desired_bus_width),
                    self.control_plane_chunk_limit(),
                    self.experimental_no_ht_transport,
    ...
    fn try_capture_passive_startup_link_reply(&mut self, poll: u8) -> Result<usize, HalError> {
        let previous_mode = self.experimental_control_plane_reply_rearm_mode;
        let previous_attempts = self.experimental_control_plane_reply_rearm_attempts;
        let previous_promoted_probe_pending =
            self.experimental_control_plane_promoted_probe_pending;
        emit_breadcrumb(format_args!(
            "[pi4-wifi] firmware stage=control-plane-reply action=passive-startup-link-probe poll={} current_clock={}Hz width={} chunk_limit={} no_ht={}",
            poll,
            self.current_clock_hz,
            sdio_bus_width_name(self.desired_bus_width),
            self.control_plane_chunk_limit(),
        ...
        self.experimental_control_plane_reply_rearm_mode = control_plane_reply_rearm_startup_link();
        self.experimental_control_plane_reply_rearm_attempts = 0;
        self.experimental_control_plane_promoted_probe_pending = false;
        let probe_result = self.try_capture_control_plane_reply_speculative_frame();
        match probe_result {
            Ok(0) => {
                self.experimental_control_plane_reply_rearm_mode = previous_mode;
                self.experimental_control_plane_reply_rearm_attempts = previous_attempts;
                self.experimental_control_plane_promoted_probe_pending =
                    previous_promoted_probe_pending;
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage=control-plane-reply action=passive-startup-link-probe-empty poll={} current_clock={}Hz width={} chunk_limit={} no_ht={}",
                    poll,
                    self.current_clock_hz,
                    sdio_bus_width_name(self.desired_bus_width),
                    self.control_plane_chunk_limit(),
            ...
            Ok(frame_len) => {
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage=control-plane-reply action=passive-startup-link-probe-ready poll={} frame_len={} current_clock={}Hz width={} chunk_limit={} no_ht={}",
                    poll,
                    frame_len,
                    self.current_clock_hz,
                    sdio_bus_width_name(self.desired_bus_width),
            ...
            Err(err) => {
                ...
                self.experimental_control_plane_promoted_probe_pending =
                    previous_promoted_probe_pending;
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage=control-plane-reply action=passive-startup-link-probe-fail poll={} current_clock={}Hz width={} chunk_limit={} no_ht={} err={err}",
                    poll,
                    self.current_clock_hz,
                    sdio_bus_width_name(self.desired_bus_width),
                    self.control_plane_chunk_limit(),
    ...
    fn rearm_firmware_channel_after_first_control_write(&mut self) -> Result<(), HalError> {
        ...
        self.log_control_plane_recovery_transition("control-plane-post-write-rearm-begin");
                ...
                .rearm_firmware_channel_once(SdioFunctionReadyBudget::ControlPlaneReplyProbe, true)
            {
                Ok(()) => {
                    ...
                    self.log_control_plane_recovery_transition(
                        "control-plane-post-write-rearm-ready",
                    );
                    return Ok(());
                }
                Err(err) if firmware_phase_can_retry(&err, attempt) => {
    ...
    fn resume_control_plane_after_post_write_rearm_timeout(
        &mut self,
        err: &HalError,
    ) -> Result<(), HalError> {
        ...
        let preserved_exact_error =
            cached_snapshot.map(|snapshot| snapshot.control_plane_exact_error);
        let repeat_no_progress_prefers_direct_cutover =
        ...
        if control_plane_startup_link_repeat_no_progress_should_cutover_to_bounded_no_ht(
            ...
            if repeat_no_progress_prefers_direct_cutover {
                let exact_error = cached_snapshot
                    .map(|snapshot| snapshot.control_plane_exact_error)
                    .unwrap_or("none");
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage=control-plane-post-write-rearm action=repeat-no-progress-direct-cutover cycle={}/{} err={err} current={}Hz width={} exact_error={exact_error} mode=bounded-no-ht-direct-reply",
                    cutover_cycle,
                    CYW43_CONTROL_PLANE_STARTUP_LINK_RESCUE_LIMIT,
                    self.current_clock_hz,
                    sdio_bus_width_name(self.desired_bus_width),
                ...
                self.cutover_control_plane_reply_probe_to_bounded_no_ht_without_startup_link_rearm(
                    ...
                    "repeat-no-progress-direct-first-reply",
                )?;
                return Ok(());
            }
            emit_breadcrumb(format_args!(
        ...
        if startup_link_resume_prefers_direct_reply_probe_cutover(
            ...
            let exact_error = preserved_exact_error.unwrap_or("none");
            emit_breadcrumb(format_args!(
                "[pi4-wifi] firmware stage=control-plane-post-write-rearm action=resume-policy mode=direct-reply-probe current={}Hz width={} no_ht={} exact_error={exact_error} reason=state-visible-linux-f2-first-reply-blocker",
                self.current_clock_hz,
                sdio_bus_width_name(self.desired_bus_width),
                self.experimental_no_ht_transport,
            ));
            self.cutover_control_plane_reply_probe_to_bounded_no_ht_without_startup_link_rearm(
                "control-plane-post-write-rearm-resume",
                "post-write-timeout-state-visible-first-reply",
            )?;
            emit_breadcrumb(format_args!(
                "[pi4-wifi] firmware stage=control-plane-post-write-rearm action=resume-ready previous={}Hz/{} current={}Hz width={} mode=bounded-no-ht-direct-reply",
                previous_clock_hz,
                sdio_bus_width_name(previous_bus_width),
                self.current_clock_hz,
                sdio_bus_width_name(self.desired_bus_width),
            ...
            self.log_control_plane_recovery_transition(
                "control-plane-post-write-rearm-resume-ready",
            );
            return Ok(());
        }
        emit_breadcrumb(format_args!(
        ...
        {
            Ok(()) => {
                let preserved_exact_error = cached_snapshot
                    .map(|snapshot| snapshot.control_plane_exact_error)
                    .filter(|exact_error| !exact_error.is_empty());
                if experimental_control_plane_post_write_timeout_prefers_passive_startup_link(
                    self.experimental_no_ht_transport,
                    self.current_clock_hz,
                    preserved_exact_error,
                ) {
                    emit_breadcrumb(format_args!(
                        "[pi4-wifi] firmware stage=control-plane-post-write-rearm action=resume-policy mode=passive-startup-link current={}Hz width={} no_ht={} reason=first-write-committed",
                        self.current_clock_hz,
                        sdio_bus_width_name(self.desired_bus_width),
                        self.experimental_no_ht_transport,
                    ));
                    self.keep_control_plane_on_startup_link_after_first_write(
                        "control-plane-post-write-rearm-resume",
                    );
                    emit_breadcrumb(format_args!(
                        "[pi4-wifi] firmware stage=control-plane-post-write-rearm action=resume-ready previous={}Hz/{} current={}Hz width={} mode=passive-startup-link",
                        previous_clock_hz,
                        sdio_bus_width_name(previous_bus_width),
                        self.current_clock_hz,
                        sdio_bus_width_name(self.desired_bus_width),
                ...
                if experimental_control_plane_post_write_timeout_prefers_direct_reply_probe_cutover(
                    self.experimental_no_ht_transport,
                    self.current_clock_hz,
                    cached_snapshot.as_ref(),
                ) {
                    let exact_error = cached_snapshot
                        ...
                        .unwrap_or("none");
                    emit_breadcrumb(format_args!(
                        "[pi4-wifi] firmware stage=control-plane-post-write-rearm action=resume-policy mode=direct-reply-probe current={}Hz width={} no_ht={} exact_error={exact_error} reason=preserved-linux-f2-first-reply-blocker",
                        self.current_clock_hz,
                        sdio_bus_width_name(self.desired_bus_width),
                        self.experimental_no_ht_transport,
                    ));
                    self.cutover_control_plane_reply_probe_to_bounded_no_ht_without_startup_link_rearm(
                        "control-plane-post-write-rearm-resume",
                        "post-write-timeout-direct-first-reply",
                    )?;
                    emit_breadcrumb(format_args!(
                        "[pi4-wifi] firmware stage=control-plane-post-write-rearm action=resume-ready previous={}Hz/{} current={}Hz width={} mode=bounded-no-ht-direct-reply",
                        previous_clock_hz,
                ...
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage=control-plane-post-write-rearm action=resume-policy mode=startup-link-rearm current={}Hz width={} no_ht={}",
                    self.current_clock_hz,
                    sdio_bus_width_name(self.desired_bus_width),
                    self.experimental_no_ht_transport,
                ));
            ...
            Err(resume_err) => {
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage=control-plane-post-write-rearm action=resume-live-fail err={resume_err} fallback=destructive-reattach current={}Hz width={} no_ht={}",
                    self.current_clock_hz,
                    sdio_bus_width_name(self.desired_bus_width),
                    self.experimental_no_ht_transport,
                ));
                self.reattach_startup_link_control_plane_transport(
                    "control-plane-post-write-rearm-resume",
                )?;
            }
        }
        ...
        self.log_control_plane_recovery_transition("control-plane-post-write-rearm-resume-ready");
        Ok(())
    }
    ...
    fn rearm_firmware_channel_after_transport_promotion(
        ...
        self.log_control_plane_recovery_transition("control-plane-promote-rearm-begin");
        ...
        loop {
            match self.rearm_firmware_channel_once(
                ...
                Ok(()) => {
                    ...
                    self.log_control_plane_recovery_transition("control-plane-promote-rearm-ready");
                    return Ok(());
                }
                Err(err) if firmware_phase_can_retry(&err, attempt) => {
                    emit_breadcrumb(format_args!(
    ...
    fn try_strict_control_plane_reply_recovery(
        &mut self,
        trigger: &'static str,
        reply_rearm_mode: u8,
        attempt: usize,
        ...
        emit_breadcrumb(format_args!(
            "[pi4-wifi] firmware stage=control-plane-reply action=strict-recover-begin trigger={trigger} mode={} attempt={} iorx=0x{ready:02x} current_clock={}Hz width={} reply_chunk_limit={} no_ht={}",
            control_plane_reply_rearm_mode_name(reply_rearm_mode),
            attempt,
            self.current_clock_hz,
            sdio_bus_width_name(self.desired_bus_width),
        ...
        self.log_control_plane_recovery_transition("control-plane-reply-strict-recover-begin");
        self.recover_command_path_and_refresh_transport("control-plane-reply-strict-recover")?;
        match self.rearm_firmware_channel_once(
            SdioFunctionReadyBudget::ControlPlaneReplyStrictRecovery,
            false,
        ) {
            Ok(()) => {
                ...
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage=control-plane-reply action=strict-recover-ready trigger={trigger} mode={} attempt={} iorx=0x{ready_after:02x} current_clock={}Hz width={} reply_chunk_limit={} no_ht={}",
                    control_plane_reply_rearm_mode_name(reply_rearm_mode),
                    attempt,
                    self.current_clock_hz,
                    sdio_bus_width_name(self.desired_bus_width),
                ...
                if self.control_plane_uses_low_touch_pure_f2_diagnostics() {
                    ...
                    emit_breadcrumb(format_args!(
                        "[pi4-wifi] firmware stage=control-plane-reply action=strict-recover-state-pure-f2 trigger={trigger} mode={} attempt={} ioex=0x{ioex:02x}/{} iorx=0x{ready_after:02x} ienx=0x{ienx:02x}/{} current_clock={}Hz width={} reply_chunk_limit={} no_ht={} sideband=skipped reason=pure-f2",
                        control_plane_reply_rearm_mode_name(reply_rearm_mode),
                        attempt,
                        yn(ioex_after.is_some()),
                        yn(ienx_after.is_some()),
                        self.current_clock_hz,
                        sdio_bus_width_name(self.desired_bus_width),
                        self.control_plane_reply_chunk_limit(),
                        self.experimental_no_ht_transport,
                        ioex = ioex_after.unwrap_or(0),
                        ienx = ienx_after.unwrap_or(0),
                    ));
                    self.log_control_plane_recovery_transition(
                    ...
                    if !matches!(
                        ioex_after,
                        Some(bits) if (bits & SDIO_FUNC_ENABLE_2) == SDIO_FUNC_ENABLE_2
                    ) {
                        emit_breadcrumb(format_args!(
                            "[pi4-wifi] firmware stage=control-plane-reply action=strict-recover-pure-f2-no-progress trigger={trigger} mode={} attempt={} reason=f2-disabled current_clock={}Hz width={} reply_chunk_limit={} no_ht={}",
                            control_plane_reply_rearm_mode_name(reply_rearm_mode),
                            attempt,
                            self.current_clock_hz,
                            sdio_bus_width_name(self.desired_bus_width),
                        ...
                        self.log_control_plane_finish_snapshot(
                            "control-plane-reply-strict-recover-no-progress",
                        );
                        self.experimental_control_plane_reply_rearm_mode =
                            control_plane_reply_rearm_none();
                        self.experimental_control_plane_reply_rearm_attempts = 0;
                        self.experimental_control_plane_reply_rearm_empty_polls = 0;
                        self.experimental_control_plane_promoted_probe_pending = false;
                        return Err(HalError::Unsupported("cyw43-function2-disabled"));
                    }
                    let sideband_err =
                        HalError::Unsupported("cyw43-control-plane-sideband-unreadable");
                    let next_cycle = self
                        .experimental_control_plane_sideband_unreadable_recovery_cycles
                        .saturating_add(1);
                    self.experimental_control_plane_sideband_unreadable_recovery_cycles =
                        next_cycle;
                    self.mark_control_plane_hint_reads_unstable(
                        "control-plane-reply-strict-recover-ready",
                        "linux-sideband-suppressed",
                        reply_rearm_mode,
                        bounded_attempt,
                        &sideband_err,
                    );
                    if next_cycle >= CYW43_CONTROL_PLANE_SIDEBAND_UNREADABLE_RECOVERY_LIMIT {
                        emit_breadcrumb(format_args!(
                            "[pi4-wifi] firmware stage=control-plane-reply action=pure-f2-startup-link-exhausted trigger={trigger} mode={} attempt={} cycle={}/{} current_clock={}Hz width={} reply_chunk_limit={} no_ht={} exact_error=cyw43-control-plane-pure-f2-startup-link-no-reply cause=sideband-suppressed-no-reply",
                            control_plane_reply_rearm_mode_name(reply_rearm_mode),
                            attempt,
                            next_cycle,
                    ...
                    emit_breadcrumb(format_args!(
                        "[pi4-wifi] firmware stage=control-plane-reply action=strict-recover-pure-f2-continue trigger={trigger} mode={} attempt={} cycle={}/{} current_clock={}Hz width={} reply_chunk_limit={} no_ht={} exact_error=cyw43-control-plane-sideband-unreadable continue=pure-f2-startup-link",
                        control_plane_reply_rearm_mode_name(reply_rearm_mode),
                        attempt,
                        next_cycle,
                        CYW43_CONTROL_PLANE_SIDEBAND_UNREADABLE_RECOVERY_LIMIT,
                    ...
                    self.resume_control_plane_after_post_write_rearm_timeout(&sideband_err)?;
                    return Ok(true);
                }
                let ienx_after = self
                    .io_direct_read(SdioFunction::Function0, SDIO_CCCR_IENX)
                ...
                emit_breadcrumb(format_args!(
                    ...
                    yn(ioex_after.is_some()),
                    yn(ienx_after.is_some()),
                    yn(corecontrol_after.is_some()),
                    control_plane_snapshot_u32_bit_state_label(corecontrol_after, CC_F2RDY),
                    yn(watermark_after.is_some()),
                    yn(devctl_after.is_some()),
                    yn(mesbusy_after.is_some()),
                    control_plane_function2_linux_state_label(
                        ioex_after,
                        Some(ready_after),
                        ienx_after,
                    ...
                    ioex = ioex_after.unwrap_or(0),
                    ienx = ienx_after.unwrap_or(0),
                    corecontrol = corecontrol_after.unwrap_or(0),
                    watermark = watermark_after.unwrap_or(0),
                    devctl = devctl_after.unwrap_or(0),
                    mesbusy = mesbusy_after.unwrap_or(0),
                ));
                self.log_control_plane_recovery_transition(
                    "control-plane-reply-strict-recover-ready",
                );
                ...
                if let Some(exact_error) = strict_control_plane_reply_recovery_error(
                    ...
                    if matches!(
                        ...
                        if next_cycle >= CYW43_CONTROL_PLANE_SIDEBAND_UNREADABLE_RECOVERY_LIMIT {
                            emit_breadcrumb(format_args!(
                                "[pi4-wifi] firmware stage=control-plane-reply action=pure-f2-startup-link-exhausted trigger={trigger} mode={} attempt={} cycle={}/{} current_clock={}Hz width={} reply_chunk_limit={} no_ht={} exact_error=cyw43-control-plane-pure-f2-startup-link-no-reply cause={exact_error}",
                                control_plane_reply_rearm_mode_name(reply_rearm_mode),
                                attempt,
                                next_cycle,
                                CYW43_CONTROL_PLANE_SIDEBAND_UNREADABLE_RECOVERY_LIMIT,
                    ...
                    emit_breadcrumb(format_args!(
                        "[pi4-wifi] firmware stage=control-plane-reply action=strict-recover-no-progress trigger={trigger} mode={} attempt={} reason=linux-f2-latched-configured-no-iorx exact_error={exact_error} current_clock={}Hz width={} reply_chunk_limit={} no_ht={}",
                        control_plane_reply_rearm_mode_name(reply_rearm_mode),
                        attempt,
                        self.current_clock_hz,
                        sdio_bus_width_name(self.desired_bus_width),
            ...
            Err(err) => {
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage=control-plane-reply action=strict-recover-fail trigger={trigger} mode={} attempt={} iorx=0x{ready:02x} current_clock={}Hz width={} reply_chunk_limit={} no_ht={} err={err}",
                    control_plane_reply_rearm_mode_name(reply_rearm_mode),
                    attempt,
                    self.current_clock_hz,
                    sdio_bus_width_name(self.desired_bus_width),
    ...
    fn maybe_rearm_control_plane_reply_on_zero_frame(&mut self) -> Result<(), HalError> {
        ...
        if !control_plane_zero_frame_needs_reply_rearm(reply_rearm_mode, function2_ready, attempt) {
            if reply_rearm_mode == control_plane_reply_rearm_startup_link_direct_probe() {
                ...
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage=control-plane-reply action=direct-reply-probe-terminal-fail mode={} attempt={} iorx=0x{ready:02x} current_clock={}Hz width={} chunk_limit={} no_ht={} exact_error={exact_error}",
                    control_plane_reply_rearm_mode_name(reply_rearm_mode),
                    attempt,
                    self.current_clock_hz,
                    sdio_bus_width_name(self.desired_bus_width),
                ...
                self.log_control_plane_finish_snapshot(
                    "control-plane-direct-reply-probe-terminal-fail",
                );
                self.experimental_control_plane_reply_rearm_mode = control_plane_reply_rearm_none();
                self.experimental_control_plane_reply_rearm_attempts = 0;
                self.experimental_control_plane_reply_rearm_empty_polls = 0;
    ...
    fn try_capture_control_plane_reply_full_block_probe(
        ...
        if low_touch_pure_f2_mode && !follow_on_linux_firstread_disables_prefix_only {
            emit_breadcrumb(format_args!(
                "[pi4-wifi] firmware stage=control-plane-reply action=diagnostic-full-block-skip trigger={trigger} mode={} attempt={} current_clock={}Hz width={} reply_chunk_limit={} no_ht={} diagnosis=bounded-pure-f2-prefix-only",
                control_plane_reply_rearm_mode_name(reply_rearm_mode),
                attempt,
                self.current_clock_hz,
                sdio_bus_width_name(self.desired_bus_width),
            ...
            self.log_transport_shadow("control-plane-reply-diagnostic-full-block-skip");
            return Ok(None);
        }

        emit_breadcrumb(format_args!(
            "[pi4-wifi] firmware stage=control-plane-reply action=diagnostic-full-block-read trigger={trigger} mode={} attempt={} len={} current_clock={}Hz width={} reply_chunk_limit={} no_ht={}",
            control_plane_reply_rearm_mode_name(reply_rearm_mode),
            attempt,
            CYW43_CONTROL_PLANE_SPECULATIVE_FRAME_CAPACITY,
            self.current_clock_hz,
        ...
        match self.read_function2_reply_single_chunk_diagnostic_with_linux_recovery(
            ...
            Ok(()) => {
                let Some(frame_len) = control_plane_reply_speculative_frame_len(frame) else {
                    emit_breadcrumb(format_args!(
                        "[pi4-wifi] firmware stage=control-plane-reply action=diagnostic-full-block-invalid trigger={trigger} mode={} attempt={} diagnosis=full-block-read-returned-invalid packet=0x{:02x}{:02x} len_inv=0x{:02x}{:02x} seq=0x{:02x} channel=0x{:02x} credit=0x{:02x} header_len=0x{:02x}",
                        control_plane_reply_rearm_mode_name(reply_rearm_mode),
                        attempt,
                        frame[1],
                        frame[0],
                    ...
                    self.log_transport_shadow("control-plane-reply-diagnostic-full-block-invalid");
                    return Ok(None);
                };
        ...
        let write_result = (|| {
            ...
            if defer_interrupts {
                ...
                self.arm_function2_interrupts_for_firmware_channel(
                    "slow-link-channel-rearm-interrupts",
                    allow_setup_write_bypass,
                )?;
            }
    ...
    fn log_control_plane_finish_snapshot(&mut self, stage: &'static str) -> &'static str {
        self.log_transport_shadow(stage);
        ...
        if !use_live_sdio_core_reads && !use_low_clock_probe {
            emit_breadcrumb(format_args!(
        ...
        let (
            ...
            let (
                ...
                corecontrol,
                corestatus,
            ) = self.read_control_plane_snapshot_u32s_with_low_clock_probe(stage);
            (
                hostintmask,
                function_int_mask,
                tohost_mailbox,
        ...
        } else if use_live_sdio_core_reads {
            (
                ...
                self.read_control_plane_snapshot_u32(stage, "corecontrol", SDIO_CORECONTROL),
                self.read_control_plane_snapshot_u32(stage, "corestatus", SDIO_CORESTATUS),
                "live",
            )
        } else {
            (
        ...
        emit_breadcrumb(format_args!(
            "[pi4-wifi] control-plane snapshot {stage} probe={probe_source} hostintmask=0x{hostintmask:08x}/{} fn_int_mask=0x{function_int_mask:08x}/{} corecontrol=0x{corecontrol:08x}/{} corestatus=0x{corestatus:08x}/{} cc_f2rdy={} tohost=0x{tohost:08x}/{} int_status=0x{int_status:08x}/{} rframe=0x{rframe_hi:02x}{rframe_lo:02x}/{} watermark=0x{watermark:02x}/{} devctl=0x{devctl:02x}/{} mesbusy=0x{mesbusy:02x}/{}",
            yn(hostintmask.is_some()),
            yn(function_int_mask.is_some()),
            ...
            function_int_mask = function_int_mask.unwrap_or(0),
        ...
        emit_breadcrumb(format_args!(
            ...
            self.last_data_wait_cmd.unwrap_or(0),
            self.last_data_wait_arg.unwrap_or(0),
            self.last_data_wait_present.unwrap_or(0),
            self.last_data_wait_int_status.unwrap_or(0),
            self.preserved_control_plane_data_wait_cmd.unwrap_or(0),
            self.preserved_control_plane_data_wait_arg.unwrap_or(0),
            self.preserved_control_plane_data_wait_present.unwrap_or(0),
            self.preserved_control_plane_data_wait_int_status.unwrap_or(0),
        ));
        let hint_visible = usize::from(hostintmask.is_some())
            + usize::from(function_int_mask.is_some())
            + usize::from(tohost_mailbox.is_some())
            + usize::from(int_status.is_some());
        let blocker = control_plane_snapshot_exact_blocker_label(
            ioex,
            iorx,
            ienx,
            corecontrol,
        ...
        emit_breadcrumb(format_args!(
            "[pi4-wifi] control-plane snapshot {stage} diag f2={}/{}/{} core_f2rdy={} hints={} hm={} fm={} rf={} blocker={} probe={} clk={}Hz mode={} try={}",
            control_plane_snapshot_bit_state_label(ioex, SDIO_FUNCTION_ENABLE_F2.enable_bit),
            control_plane_snapshot_bit_state_label(iorx, SDIO_FUNCTION_ENABLE_F2.enable_bit),
            control_plane_snapshot_bit_state_label(ienx, SDIO_CCCR_IEN_FUNC2),
            control_plane_snapshot_u32_bit_state_label(corecontrol, CC_F2RDY),
    ...
    fn read_control_plane_snapshot_u32s_with_low_clock_probe(
        &mut self,
        stage: &'static str,
    ) -> (
        Option<u32>,
        ...
        let restore_clock_hz = if self.current_clock_hz > CYW43_STARTUP_CLOCK_HZ {
            Some(self.current_clock_hz)
        } else {
            None
        };
        emit_breadcrumb(format_args!(
            "[pi4-wifi] control-plane snapshot {stage} action=low-clock-probe-begin current={}Hz target={}Hz width={} reply_mode={} reply_attempts={} chunk_limit={}",
            self.current_clock_hz,
            CYW43_STARTUP_CLOCK_HZ,
            sdio_bus_width_name(self.desired_bus_width),
            control_plane_reply_rearm_mode_name(self.experimental_control_plane_reply_rearm_mode),
        ...
        if let Some(previous_clock_hz) = restore_clock_hz {
            match self.set_clock_hz(CYW43_STARTUP_CLOCK_HZ) {
                Ok(current_clock_hz) => emit_breadcrumb(format_args!(
                    "[pi4-wifi] control-plane snapshot {stage} action=low-clock-probe-lowered previous={}Hz current={}Hz width={}",
                    previous_clock_hz,
                    current_clock_hz,
                    sdio_bus_width_name(self.desired_bus_width),
                )),
                Err(err) => {
                    emit_breadcrumb(format_args!(
                        "[pi4-wifi] control-plane snapshot {stage} action=low-clock-probe-lower-fail previous={}Hz current={}Hz width={} err={err}",
                        previous_clock_hz,
                        self.current_clock_hz,
                        sdio_bus_width_name(self.desired_bus_width),
                    ));
        ...
        if let Err(err) = self.recover_command_path_and_refresh_transport(stage) {
            emit_breadcrumb(format_args!(
                "[pi4-wifi] control-plane snapshot {stage} action=low-clock-probe-refresh-fail err={err}"
            ));
        }
        let hostintmask =
            self.read_control_plane_snapshot_u32(stage, "hostintmask", SDPCMD_REG_HOSTINTMASK);
        let function_int_mask = self.read_control_plane_snapshot_u32(
            stage,
            "function-int-mask",
            SDPCMD_REG_FUNCTIONINTMASK,
        );
        let corecontrol =
            self.read_control_plane_snapshot_u32(stage, "corecontrol", SDIO_CORECONTROL);
        let corestatus = self.read_control_plane_snapshot_u32(stage, "corestatus", SDIO_CORESTATUS);
        ...
        if let Some(previous_clock_hz) = restore_clock_hz {
            match self.set_clock_hz(previous_clock_hz) {
                Ok(current_clock_hz) => emit_breadcrumb(format_args!(
                    "[pi4-wifi] control-plane snapshot {stage} action=low-clock-probe-restored previous={}Hz current={}Hz width={}",
                    CYW43_STARTUP_CLOCK_HZ,
                    current_clock_hz,
                    sdio_bus_width_name(self.desired_bus_width),
                )),
                Err(err) => emit_breadcrumb(format_args!(
                    "[pi4-wifi] control-plane snapshot {stage} action=low-clock-probe-restore-fail target={}Hz current={}Hz width={} err={err}",
                    previous_clock_hz,
                    self.current_clock_hz,
                    sdio_bus_width_name(self.desired_bus_width),
                )),
    ...
    fn try_card_init(&mut self) -> Result<(), HalError> {
        ...
        emit_breadcrumb(format_args!(
            "[pi4-wifi] sdio card-init phase=preseed-block-sizes"
        ));
        self.configure_function_block_sizes_before_enable()?;
    ...
    fn configure_function_block_sizes_before_enable(&mut self) -> Result<(), HalError> {
        self.set_function_block_size(SdioFunction::Function1, SDIO_FUNCTION_ENABLE_F1.block_size)?;
        self.set_function_block_size(SdioFunction::Function2, SDIO_FUNCTION_ENABLE_F2.block_size)?;
        let function1_block_size = self.read_function_block_size(SdioFunction::Function1).ok();
        let function2_block_size = self.read_function_block_size(SdioFunction::Function2).ok();
        emit_breadcrumb(format_args!(
            "[pi4-wifi] sdio block-size preseed f1={} readback=0x{function1_block_size:04x}/{} f2={} readback=0x{function2_block_size:04x}/{}",
            SDIO_FUNCTION_ENABLE_F1.block_size,
            yn(function1_block_size.is_some()),
            SDIO_FUNCTION_ENABLE_F2.block_size,
            yn(function2_block_size.is_some()),
            function1_block_size = function1_block_size.unwrap_or(0),
            function2_block_size = function2_block_size.unwrap_or(0),
        ));
        Ok(())
    }
    ...
    fn io_extended_single_chunk_diagnostic(
        &mut self,
        function: SdioFunction,
        addr: u32,
        increment_addr: bool,
        ...
        let plan = sdio_transfer_plan(function, buffer.len(), write)?;
        let chunk_addr = sdio_transfer_addr(addr, 0, increment_addr)?;
        emit_breadcrumb(format_args!(
            "[pi4-wifi] sdio xfer diag stage={stage} fn={} op={} addr=0x{chunk_addr:05x} len={} block_mode={} block_size={} block_count={} cmd53_count={}",
            function.number(),
            if write { "write" } else { "read" },
            buffer.len(),
            if plan.block_mode { "yes" } else { "no" },
            plan.block_size,
            plan.block_count,
            plan.cmd53_count,
        ));
    ...
    fn recover_control_plane_function2_read_path(
        ...
        let (exact_error, sdhci_read_diag) = cached_wifi_debug_snapshot()
            .map(|snapshot| {
                let exact_error = (!snapshot.control_plane_exact_error.is_empty())
                    .then_some(snapshot.control_plane_exact_error);
                let sdhci_read_diag = (snapshot.control_plane_sdhci_read_diag != "none")
                    .then_some(snapshot.control_plane_sdhci_read_diag);
                (exact_error, sdhci_read_diag)
            })
            .unwrap_or((None, None));
        if control_plane_reply_skips_transport_refresh_after_read_recover(
            self.experimental_no_ht_transport,
            reply_rearm_mode,
            self.preferred_data_clock_hz,
            exact_error,
            ...
            emit_breadcrumb(format_args!(
                "[pi4-wifi] firmware stage={stage} action=function2-read-recover-skip-transport-refresh current_clock={}Hz width={} no_ht={} reason=promoted-startup-link-first-read-terminal",
                self.current_clock_hz,
                sdio_bus_width_name(self.desired_bus_width),
                self.experimental_no_ht_transport,
            ));
        ...
        if self.experimental_no_ht_transport {
            if control_plane_reply_waits_for_linux_rxskip_retransmit(
                self.experimental_no_ht_transport,
                reply_rearm_mode,
                self.experimental_control_plane_linux_rxskip_pending,
            ) {
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage={stage} action=function2-read-rearm-skip budget={} mode={} current_clock={}Hz width={} no_ht={} reason=linux-rxskip-retransmit-wait",
            ...
            match transfer(self) {
                    ...
                    if !recovered && control_plane_frame_transfer_error_needs_terminate(&err) =>
                {
                    ...
                    let (exact_error, sdhci_read_diag) = cached_wifi_debug_snapshot()
                        ...
                        .unwrap_or((None, None));
                    let sdhci_read_diag = control_plane_reply_retry_effective_sdhci_read_diag(
                        self.last_data_wait_cmd,
                        self.last_data_wait_arg,
                        self.last_data_wait_present,
                        self.last_data_wait_int_status,
                    ...
                    if control_plane_reply_waits_for_linux_rxskip_retransmit(
                        ...
                        emit_breadcrumb(format_args!(
                            "[pi4-wifi] firmware stage={stage} action=function2-read-retry-defer len={} current_clock={}Hz width={} no_ht={} reason=linux-rxskip-retransmit-wait",
                            len,
                            self.current_clock_hz,
                            sdio_bus_width_name(self.desired_bus_width),
                            self.experimental_no_ht_transport,
                        ...
                        return Err(HalError::Unsupported(
                            "cyw43-function2-reply-linux-rxskip-wait",
                        ));
                    }
                        ...
                        )
                    {
                        ...
                        if control_plane_reply_direct_blocker_defers_to_linux_firstread_probe(
                            ...
                            emit_breadcrumb(format_args!(
                                "[pi4-wifi] firmware stage={stage} action=function2-read-retry-linux-firstread-defer len={} retry_clock={}Hz current_clock={}Hz width={} no_ht={} reason=promoted-startup-link-direct-f2-blocker exact_error={exact_error}",
                                len,
                                self.preferred_data_clock_hz,
                                self.current_clock_hz,
                                sdio_bus_width_name(self.desired_bus_width),
                        ...
                        emit_breadcrumb(format_args!(
                            "[pi4-wifi] firmware stage={stage} action=function2-read-retry-fail-fast len={} retry_clock={}Hz current_clock={}Hz width={} no_ht={} reason=promoted-startup-link-first-read-terminal exact_error={exact_error}",
                            len,
                            self.preferred_data_clock_hz,
                            self.current_clock_hz,
                            sdio_bus_width_name(self.desired_bus_width),
    ...
    fn transfer_sdio_core_u32(
        ...
        // Match brcmfmac's sdio_readl/sdio_writel path for mailbox and
        // interrupt registers: one backplane-windowed 4-byte Function 1
        // transfer, not a raw current-window byte fallback ladder.
    ...
    fn read_sdio_core_u32_via_f1_incrementing_cmd53(
        &mut self,
        offset: u32,
    ) -> Result<u32, HalError> {
        let addr = sdio_core_reg_addr(offset);
        self.with_backplane_window_addr_trace(
            addr,
            sdio_core_transfer_function_addr(offset),
            true,
            |this, function_addr| this.read_f1_u32_incrementing_cmd53(function_addr),
        )
    }

    fn read_sdio_core_u32_via_f1_bytes(&mut self, offset: u32) -> Result<u32, HalError> {
        let addr = sdio_core_reg_addr(offset);
        self.with_backplane_window_addr_trace(
            addr,
            sdio_core_byte_function_addr(offset),
            true,
            |this, function_addr| this.read_f1_u32(function_addr),
        )
    }
    ...
    fn read_sdio_core_u32_with_f1_fallback(
        ...
        match self.read_sdio_core_u32(offset) {
            Ok(value) => Ok(value),
            Err(err) => {
                let backplane = sdio_core_reg_addr(offset);
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] control-plane snapshot {stage} reg={name} action=sdio-core-transfer-fallback backplane=0x{backplane:08x} xfer_fn=0x{xfer_fn:05x} byte_fn=0x{byte_fn:05x} err={err}",
                    xfer_fn = sdio_core_transfer_function_addr(offset),
                    byte_fn = sdio_core_byte_function_addr(offset),
                ));
                if sdio_core_read_retries_backplane_word_path(offset) {
                    emit_breadcrumb(format_args!(
                        "[pi4-wifi] control-plane snapshot {stage} reg={name} action=sdio-core-transfer-retry-rewindow backplane=0x{backplane:08x} xfer_fn=0x{xfer_fn:05x} err={err}",
                        xfer_fn = sdio_core_transfer_function_addr(offset),
                    ));
                    self.recover_command_path_and_refresh_transport(stage)?;
                    return match self.read_sdio_core_u32(offset) {
                        Ok(value) => Ok(value),
                        Err(retry_err) => {
                            emit_breadcrumb(format_args!(
                                "[pi4-wifi] control-plane snapshot {stage} reg={name} action=sdio-core-transfer-retry-fail backplane=0x{backplane:08x} xfer_fn=0x{xfer_fn:05x} err={retry_err}",
                                xfer_fn = sdio_core_transfer_function_addr(offset),
                            ));
                            Err(retry_err)
                        }
                ...
                match self.read_sdio_core_u32_via_f1_incrementing_cmd53(offset) {
                    Ok(value) => Ok(value),
                    Err(fallback_err) => {
                        emit_breadcrumb(format_args!(
                            "[pi4-wifi] control-plane snapshot {stage} reg={name} action=f1-byte-fallback backplane=0x{backplane:08x} byte_fn=0x{byte_fn:05x} err={fallback_err}",
                            byte_fn = sdio_core_byte_function_addr(offset),
                        ));
                        self.read_sdio_core_u32_via_f1_bytes(offset)
                    }
                }
            }
        }
    ...
    fn read_sdio_core_u32_for_firmware_ready(
        ...
        match self.read_sdio_core_u32(offset) {
            ...
            Err(err) => {
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage={stage} action=sdio-core-transfer-fallback addr=0x{offset:05x} backplane=0x{backplane:08x} xfer_fn=0x{xfer_fn:05x} byte_fn=0x{byte_fn:05x} err={err}",
                    backplane = sdio_core_reg_addr(offset),
                    xfer_fn = sdio_core_transfer_function_addr(offset),
                    byte_fn = sdio_core_byte_function_addr(offset),
                ));
                if wait_for_firmware_ready_short_circuits_after_primary_fragile_read(
                    allow_function2_ready_bypass,
                    attempt,
                    &err,
                ) {
                    let assumed_ready = HMB_DATA_DEVREADY
                        ...
                        | (SDPCM_PROT_VERSION << HMB_DATA_VERSION_SHIFT);
                    emit_breadcrumb(format_args!(
                        "[pi4-wifi] firmware stage={stage} action=experimental-assume-ready-primary addr=0x{offset:05x} value=0x{assumed_ready:08x} err={err}"
                    ));
                    self.recover_command_path_and_refresh_transport(stage)?;
                    return Ok(assumed_ready);
                }
            }
        }

        if !wait_for_firmware_ready_uses_experimental_mailbox_read(allow_function2_ready_bypass) {
            return self.read_sdio_core_u32_via_f1_bytes(offset);
        }

        emit_breadcrumb(format_args!(
            "[pi4-wifi] firmware stage={stage} action=experimental-cmd53-incrementing-read addr=0x{offset:05x} backplane=0x{backplane:08x} xfer_fn=0x{xfer_fn:05x}",
            backplane = sdio_core_reg_addr(offset),
            xfer_fn = sdio_core_transfer_function_addr(offset),
        ));
        match self.read_sdio_core_u32_via_f1_incrementing_cmd53(offset) {
            Ok(value) => {
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage={stage} action=experimental-cmd53-incrementing-read-ok addr=0x{offset:05x} backplane=0x{backplane:08x} xfer_fn=0x{xfer_fn:05x} value=0x{value:08x}",
                    backplane = sdio_core_reg_addr(offset),
                    xfer_fn = sdio_core_transfer_function_addr(offset),
                ));
                Ok(value)
    ...
    fn write_sdio_core_u32_via_f1_incrementing_cmd53(
        &mut self,
        offset: u32,
        value: u32,
    ) -> Result<(), HalError> {
        ...
        self.with_backplane_window_addr_trace(
            ...
            sdio_core_transfer_function_addr(offset),
            true,
            |this, function_addr| this.write_f1_u32_incrementing_cmd53(function_addr, value),
        )
    }

    fn write_sdio_core_u32_via_f1_bytes(
        &mut self,
        offset: u32,
        value: u32,
    ) -> Result<(), HalError> {
        ...
        self.with_backplane_window_addr_trace(
            ...
            sdio_core_byte_function_addr(offset),
            true,
            |this, function_addr| this.write_f1_u32(function_addr, value),
        )
    }
    ...
    fn write_sdio_core_u32_for_firmware_channel(
        ...
        match self.write_sdio_core_u32_for_firmware_channel_inner(
            stage,
            offset,
            value,
            allow_function2_ready_bypass,
            ...
            Ok(()) => Ok(()),
            Err(err)
                if firmware_channel_write_needs_low_clock_retry(
                    self.experimental_no_ht_transport,
                    self.current_clock_hz,
                    &err,
                ) =>
            {
                let promoted_clock_hz = self.current_clock_hz;
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage={stage} action=low-clock-retry-begin addr=0x{offset:05x} value=0x{value:08x} current={}Hz target={}Hz err={err}",
                    promoted_clock_hz,
                    CYW43_STARTUP_CLOCK_HZ,
                ));
    ...
    fn write_sdio_core_u32_for_firmware_channel_inner(
        &mut self,
        stage: &'static str,
        offset: u32,
        value: u32,
        ...
        match self.write_sdio_core_u32_via_f1_incrementing_cmd53(offset, value) {
            Ok(()) => {
                emit_breadcrumb(format_args!(
            ...
            Err(err) => {
                ...
                match self.write_sdio_core_u32_via_f1_bytes(offset, value) {
                    Ok(()) => {
                        emit_breadcrumb(format_args!(
    ...
    fn next_frame_len(&mut self) -> Result<usize, HalError> {
        ...
        if low_touch_pure_f2_mode && self.experimental_control_plane_reply_rearm_empty_polls == 0 {
            emit_breadcrumb(format_args!(
                "[pi4-wifi] firmware stage=control-plane-reply action=skip-upfront-f1-frame-len current_clock={}Hz width={} reply_mode={} no_ht={} reason=linux-firstread",
                self.current_clock_hz,
                sdio_bus_width_name(self.desired_bus_width),
                control_plane_reply_rearm_mode_name(self.experimental_control_plane_reply_rearm_mode),
                self.experimental_no_ht_transport,
        ...
        let frame_len = if low_touch_pure_f2_mode {
            0
        } else {
            self.read_frame_len_registers()?
        };
        if frame_len != 0 {
            self.clear_control_plane_sideband_unreadable_recovery_cycles();
            if control_plane_reply_progress_clears_startup_link_rescue(false, true) {
        ...
        if self.experimental_control_plane_startup_link_stabilized {
            ...
            if startup_link_passive_wait_prefers_direct_speculative_reply_probe(
                ...
                self.experimental_control_plane_reply_rearm_empty_polls = 0;
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage=control-plane-reply action=direct-speculative-reply-probe mode=startup-link-passive current_clock={}Hz width={} chunk_limit={} no_ht={} exact_error={exact_error} reason=preserved-linux-f2-first-reply-blocker",
                    self.current_clock_hz,
                    sdio_bus_width_name(self.desired_bus_width),
                    self.control_plane_chunk_limit(),
                    self.experimental_no_ht_transport,
                ...
                let speculative_frame_len =
                    self.try_capture_control_plane_reply_speculative_frame()?;
        ...
        if control_plane_reply_rearm_pending(self.experimental_control_plane_reply_rearm_mode) {
            if control_plane_reply_linux_rxskip_poll_waits_for_progress(
                self.experimental_no_ht_transport,
                self.experimental_control_plane_reply_rearm_mode,
                self.experimental_control_plane_linux_rxskip_pending,
                self.experimental_control_plane_reply_rearm_empty_polls,
                ...
                let next_empty_poll = self
                    ...
                    .saturating_add(1);
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage=control-plane-reply action=linux-rxskip-retransmit-poll mode={} poll={}/{} current_clock={}Hz width={} chunk_limit={} no_ht={}",
                    control_plane_reply_rearm_mode_name(
                        self.experimental_control_plane_reply_rearm_mode
                    ),
                    next_empty_poll,
            ...
            let cached_snapshot = cached_wifi_debug_snapshot();
            if startup_link_prefers_direct_speculative_reply_probe(
                self.experimental_no_ht_transport,
                self.experimental_control_plane_reply_rearm_mode,
                self.experimental_control_plane_linux_rxskip_pending,
                cached_snapshot.as_ref(),
            ) || startup_link_resume_prefers_direct_speculative_reply_probe(
                self.experimental_no_ht_transport,
                self.experimental_control_plane_reply_rearm_mode,
                self.experimental_control_plane_linux_rxskip_pending,
                cached_snapshot.as_ref(),
                ...
                self.experimental_control_plane_reply_rearm_empty_polls = 0;
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage=control-plane-reply action=direct-speculative-reply-probe mode={} current_clock={}Hz width={} chunk_limit={} no_ht={} exact_error={exact_error} reason=preserved-linux-f2-first-reply-blocker",
                    control_plane_reply_rearm_mode_name(
                        self.experimental_control_plane_reply_rearm_mode
                    ),
                    self.current_clock_hz,
                ...
                if startup_link_direct_speculative_probe_failure_skips_empty_poll_wait(
                    self.experimental_no_ht_transport,
                    self.experimental_control_plane_reply_rearm_mode,
                    self.experimental_control_plane_linux_rxskip_pending,
                    cached_snapshot.as_ref(),
                    ...
                    emit_breadcrumb(format_args!(
                        "[pi4-wifi] firmware stage=control-plane-reply action=skip-empty-frame-wait mode={} current_clock={}Hz width={} chunk_limit={} no_ht={} exact_error={exact_error} reason=preserved-linux-f2-first-reply-blocker",
                        control_plane_reply_rearm_mode_name(
                            self.experimental_control_plane_reply_rearm_mode
                        ),
                        self.current_clock_hz,
            ...
            if control_plane_reply_rearm_waits_for_empty_polls(
                ...
                let next_empty_poll = self
                    .experimental_control_plane_reply_rearm_empty_polls
                    .saturating_add(1);
                if control_plane_reply_rearm_should_log_empty_poll(
                    self.experimental_control_plane_reply_rearm_mode,
                    next_empty_poll,
                ) {
                    emit_breadcrumb(format_args!(
                        "[pi4-wifi] firmware stage=control-plane-reply action=empty-frame-wait mode={} poll={}/{} attempt={} current_clock={}Hz width={} chunk_limit={} no_ht={}",
                        control_plane_reply_rearm_mode_name(
                            self.experimental_control_plane_reply_rearm_mode
                        ),
                        next_empty_poll,
    ...
    fn read_frame_with_linux_request_shape(
        &mut self,
        frame_len: usize,
        out: &mut [u8],
    ) -> Result<(), HalError> {
    ...
    fn recover_failed_function2_frame_transfer(
        ...
        if function2_frame_recovery_requests_linux_nak_retransmit(
            write,
            policy,
            stage,
            self.experimental_no_ht_transport,
            ...
            match self.write_sdio_core_u32_for_firmware_channel(
                ...
                Ok(()) => {
                    self.experimental_control_plane_linux_rxskip_pending = true;
                    emit_breadcrumb(format_args!(
    ...
    fn read_frame(&mut self, out: &mut [u8]) -> Result<usize, HalError> {
        if self.control_plane_speculative_reply_frame_pending() {
            if self.experimental_control_plane_speculative_frame_len > out.len() {
                return Err(HalError::Unsupported("cyw43-frame-oversize"));
            }
            return Ok(self.take_control_plane_speculative_reply_frame(out));
        }
    ...
    fn write_frame(&mut self, frame: &mut [u8]) -> Result<(), HalError> {
        ...
        let finish_successful_write = |this: &mut Self| -> Result<(), HalError> {
            if experimental_control_plane_write_prefers_passive_startup_link_wait(
                this.experimental_no_ht_transport,
                first_control_plane_write_pending,
                this.current_clock_hz,
            ) {
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage=control-plane-write action=skip-post-write-rearm reason=first-write-startup-link len={} write_chunk_limit={} reply_chunk_limit={} current={}Hz width={} no_ht={}",
                    frame_len,
                ...
                this.keep_control_plane_on_startup_link_after_first_write(
                    "control-plane-write-startup-link-passive",
                );
                return Ok(());
            }
            if experimental_control_plane_write_needs_post_write_rearm(
                ...
                emit_breadcrumb(format_args!(
                        "[pi4-wifi] firmware stage=control-plane-write action=experimental-post-write-rearm len={} write_chunk_limit={} reply_chunk_limit={}",
                        frame_len,
                        this.control_plane_write_chunk_limit(),
                        this.control_plane_reply_chunk_limit(),
                    ));
                match this.rearm_firmware_channel_after_first_control_write() {
                        Ok(()) => {}
                        Err(err)
                            if experimental_control_plane_write_can_resume_after_post_write_rearm_timeout(
                                this.experimental_no_ht_transport,
                                first_control_plane_write_pending,
                                &err,
                            ) =>
                        {
                            this.resume_control_plane_after_post_write_rearm_timeout(&err)?;
                        }
                        Err(err) => return Err(err),
                    }
            }
        ...
        match self.write_frame_with_linux_request_shape(frame, "control-plane-write") {
                ...
                ) =>
            {
                let previous_clock_hz = self.current_clock_hz;
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage=control-plane-write action=fallback-startup-link len={} write_chunk_limit={} reply_chunk_limit={} current={}Hz width={} target={}Hz target_width={} err={err}",
                    frame_len,
                    self.control_plane_write_chunk_limit(),
                    self.control_plane_reply_chunk_limit(),
                    previous_clock_hz,
                ...
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage=control-plane-write action=fallback-startup-link-ready previous={}Hz current={}Hz width={} len={} write_chunk_limit={} reply_chunk_limit={}",
                    previous_clock_hz,
                    self.current_clock_hz,
                    sdio_bus_width_name(self.desired_bus_width),
                    frame_len,
                ...
                match self.write_frame_with_linux_request_shape(
                    frame,
            ...
            {
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage=control-plane-write action=experimental-assume-control-write len={} write_chunk_limit={} reply_chunk_limit={} err={err}",
                    frame.len(),
                    self.control_plane_write_chunk_limit(),
                    self.control_plane_reply_chunk_limit(),
                ));
    ...
    fn load_firmware(&mut self, bundle: WifiFirmwareBundle<'static>) -> Result<(), HalError> {
        ...
        if allow_function2_ready_bypass {
            ...
            let cached_snapshot = cached_wifi_debug_snapshot();
            emit_breadcrumb(format_args!(
                "[pi4-wifi] firmware stage=post-firmware-ready-function2-recheck action=strict-repoll f2_state={f2_state} ioex=0x{ioex:02x}/{} iorx=0x{iorx:02x}/{} ienx=0x{ienx:02x}/{} current_clock={}Hz width={} chunk_limit={} no_ht={}",
                yn(ioex.is_some()),
                yn(iorx.is_some()),
                yn(ienx.is_some()),
                self.current_clock_hz,
            ...
            if post_firmware_ready_function2_strict_repoll_prefers_cached_direct_reply_probe_cutover(
                self.experimental_no_ht_transport,
                ioex,
                iorx,
                ienx,
                ...
                let direct_cutover_exact_error = cached_snapshot
                    .as_ref()
                    .map(|snapshot| snapshot.control_plane_exact_error)
                    .unwrap_or("none");
                let direct_cutover_err = HalError::Unsupported(direct_cutover_exact_error);
                self.mark_control_plane_hint_reads_unstable(
                    "post-firmware-ready-function2-strict-repoll-direct-cutover",
                ...
                emit_breadcrumb(format_args!(
                    "[pi4-wifi] firmware stage=post-firmware-ready-function2-recheck action=strict-repoll-direct-cutover exact_error={direct_cutover_exact_error} f2_state={f2_state} current_clock={}Hz width={} chunk_limit={} no_ht={} next=direct-reply-probe source=cached-first-reply-blocker",
                    self.current_clock_hz,
                    sdio_bus_width_name(self.desired_bus_width),
                    self.control_plane_chunk_limit(),
                    self.experimental_no_ht_transport,
            ...
            match self.enable_function2(SdioFunctionReadyBudget::ControlPlaneReplyStrictRecovery) {
                ...
                Err(err) => {
                    ...
                    let cached_snapshot = cached_wifi_debug_snapshot();
                    let cached_direct_cutover = cached_snapshot
                        .as_ref()
                        .is_some_and(cached_first_reply_blocker_prefers_direct_reply_probe_cutover);
                    let direct_cutover_exact_error =
                        if control_plane_exact_error_is_first_reply_blocker(exact_error) {
                            exact_error
                        } else {
                            cached_snapshot
                                .as_ref()
                                .filter(|snapshot| {
                                    cached_first_reply_blocker_prefers_direct_reply_probe_cutover(
                                        snapshot,
                                    ) && control_plane_exact_error_is_first_reply_blocker(
                                        snapshot.control_plane_exact_error,
                                    )
                                ...
                                .map(|snapshot| snapshot.control_plane_exact_error)
                                .unwrap_or(exact_error)
                        };
                    let visible_linux_f2_state =
                        control_plane_function2_latched_linux_configured_without_iorx(
                            ioex, iorx, ienx, watermark, devctl, mesbusy,
                        );
                    if post_firmware_ready_function2_strict_repoll_prefers_direct_reply_probe_cutover(
                        self.experimental_no_ht_transport,
                        ioex,
                        iorx,
                        ienx,
                        ...
                        devctl,
                        mesbusy,
                        direct_cutover_exact_error,
                    ) || cached_direct_cutover
                    {
                        let direct_cutover_err = HalError::Unsupported(direct_cutover_exact_error);
                        self.mark_control_plane_hint_reads_unstable(
                            "post-firmware-ready-function2-strict-repoll-direct-cutover",
                        ...
                        emit_breadcrumb(format_args!(
                            "[pi4-wifi] firmware stage=post-firmware-ready-function2-recheck action=strict-repoll-direct-cutover err={err} exact_error={direct_cutover_exact_error} f2_state={f2_state} current_clock={}Hz width={} chunk_limit={} no_ht={} next=direct-reply-probe",
                            self.current_clock_hz,
                            sdio_bus_width_name(self.desired_bus_width),
                            self.control_plane_chunk_limit(),
                            self.experimental_no_ht_transport,
                    ...
                    emit_breadcrumb(format_args!(
                        "[pi4-wifi] firmware stage=post-firmware-ready-function2-recheck action=strict-repoll-fail-fast err={err} exact_error={direct_cutover_exact_error} f2_state={f2_state} retry=aborted"
                    ));
                    return Err(match direct_cutover_exact_error {
                        "none" => err,
                        _ => HalError::Unsupported(direct_cutover_exact_error),
                    });
                }
            }
        }
...
fn sdio_transfer_plan(
    ...
    // BCM2711 SDHCI edge where the command completes but the data phase never
    // becomes readable on Pi 4 function-2 startup-link replies.
    transfer_mode |= SDHCI_TRNS_BLK_CNT_EN;
    Ok(SdioTransferPlan {
        block_size,
        block_count: 1,
...
mod tests {
    ...
    use super::{
        ...
        firmware_channel_watermark, firmware_phase_can_retry, ht_clock_assist_shadow_is_complete,
        ht_clock_request_value, ht_clock_retry_can_cutover_to_bounded_no_ht_early,
        ...
        sdio_core_byte_function_addr, sdio_core_read_retries_backplane_word_path,
        ...
        CYW43_CONTROL_PLANE_SPECULATIVE_FRAME_CAPACITY, CYW43_FIRMWARE_BULK_CLOCK_HZ,
        CYW43_FIRMWARE_PROGRESS_INTERVAL, CYW43_RAM_BASE_4345, CYW43_SDIO_CORE_BASE,
        ...
        SDIO_CCCR_IEN_FUNC2, SDIO_FUNCTION_ENABLE_F2, SDIO_FUNCTION_ENABLE_SEQUENCE,
    ...
    #[test]
    fn startup_link_state_selects_low_touch_diagnostic_prefix_lens() {
        assert_eq!(
    ...
    #[test]
    fn function2_force_reenable_is_reserved_for_strict_reply_recovery() {
    ...
    #[test]
    fn control_plane_reply_hints_only_probe_when_frame_is_indicated() {
        assert!(control_plane_reply_int_status_has_frame_indication(
            I_HMB_FRAME_IND
        ));
        assert!(!control_plane_reply_int_status_has_frame_indication(0));
        ...
        assert!(control_plane_reply_mailbox_requires_ack(HMB_DATA_FC));
        ...
        assert!(firmware_channel_uses_linux_minimal_setup(true));
    ...
    #[test]
    fn startup_link_reply_keeps_low_touch_pure_f2_after_bulk_clock_promotion() {
        assert!(control_plane_reply_uses_low_touch_pure_f2_diagnostics(
            true,
            true,
            control_plane_reply_rearm_startup_link_resume(),
    ...
    #[test]
    fn promoted_startup_link_hintless_firstread_uses_pure_f2_prefix() {
        let startup_link_linux_firstread =
            control_plane_reply_uses_linux_firstread_on_startup_link_lane(
                true,
                control_plane_reply_rearm_startup_link(),
        ...
        assert!(!startup_link_linux_firstread);
        assert_eq!(
            control_plane_reply_first_read_len(
                CYW43_CONTROL_PLANE_SPECULATIVE_FRAME_CAPACITY,
                true,
        ...
        assert_eq!(
            control_plane_reply_first_read_mode_name(true, startup_link_linux_firstread),
            "pure-f2-prefix"
        );
    }

    #[test]
    fn promoted_startup_link_retry_fail_fast_promotes_direct_function2_reply_exact_error() {
        assert_eq!(
            control_plane_reply_fail_fast_exact_error_after_promoted_startup_link_retry(
                true,
                control_plane_reply_rearm_startup_link_resume(),
    ...
    #[test]
    fn promoted_startup_link_speculative_failure_promotes_direct_reply_exact_error() {
        assert_eq!(
            control_plane_reply_fail_fast_exact_error_after_speculative_read_failure(
                true,
                control_plane_reply_rearm_startup_link_resume(),
    ...
    #[test]
    fn wait_for_firmware_ready_assumes_mailbox_ready_only_after_recovery_attempt() {
    ...
    fn startup_link_cached_first_reply_blocker_prefers_direct_speculative_probe() {
        let cached = WifiDebugSnapshot {
            ...
            control_plane_startup_link_stable: false,
            control_plane_startup_profile_locked: true,
            control_plane_startup_profile_reason: "repeat-no-progress-direct-first-reply",
            control_plane_promoted_probe_pending: false,
            debug_snapshot_source: "cached",
            debug_snapshot_stage: "cyw43-init-control-plane-fail",
            control_plane_startup_link_rescue_cycles: 0,
    ...
    fn preserve_cached_first_reply_blocker_in_snapshot_keeps_enable_latch_f2_read_blocker_over_enable_latch_sideband_follow_on(
        ...
        let cached = WifiDebugSnapshot {
            ...
            control_plane_frame_recovery_count: Some(0),
            control_plane_bootstrap_phase: "startup-link-recovery",
            control_plane_reply_mode: "startup-link-direct",
            control_plane_reply_attempts: 0,
            control_plane_reply_empty_polls: 0,
            control_plane_no_ht_transport: true,
            control_plane_probe_pending: false,
            control_plane_startup_link_stable: false,
            control_plane_startup_profile_locked: true,
            control_plane_startup_profile_reason: "post-firmware-ready-function2-first-reply",
            control_plane_promoted_probe_pending: false,
            debug_snapshot_source: "cached",
            debug_snapshot_stage: "control-plane-reply-fallback-startup-link-empty",
            control_plane_startup_link_rescue_cycles: 0,
            control_plane_startup_link_rescue_limit: CYW43_CONTROL_PLANE_STARTUP_LINK_RESCUE_LIMIT,
            control_plane_passive_startup_link_empty_poll_limit:
                CYW43_CONTROL_PLANE_PASSIVE_STARTUP_LINK_EMPTY_POLL_LIMIT,
    ...
    #[test]
    fn bounded_no_ht_shortcut_wait_budget_is_shorter_than_full_stronger_retry_budget() {
        assert_eq!(
            required_ht_clock_bounded_no_ht_shortcut_loops(),
            super::CYW43_HT_CLOCK_SOFT_WAIT_LOOPS * 4
        );
        assert!(
