...
fn call_cache_op(
    ...
    info!(
        target: "hal-cache",
        "[cache] CACHE_OP enter op={} vspace=0x{vspace:04x} vaddr=0x{vaddr:016x}..0x{vend:016x} aligned=0x{astart:016x}..0x{aend:016x} len={} aligned_len={}",
        op,
        vspace = vspace,
        vaddr = vaddr,
        ...
        aend = aligned_end,
        len = len,
        aligned_len = aligned_end.saturating_sub(aligned_start),
    );
