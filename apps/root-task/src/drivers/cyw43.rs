...
const EAPOL_KEY_INFO_GROUP_M2: u16 =
    EAPOL_KEY_INFO_MIC | EAPOL_KEY_INFO_SECURE | EAPOL_KEY_VERSION_HMAC_SHA1_AES;
...
const WME_BSS_DISABLE_RSN_DEFAULT: u32 = 1;
...
const fn linux_wpa2_join_sets_rsn_capability_policy() -> bool {
    true
}
...
fn host_eapol_event_ap_mac_candidate(
    ...
    if host_eapol_verified_ap_mac_candidate(event.addr, station_mac) {
        return Some((event.addr, "event-addr-global"));
    }
    if host_eapol_verified_ap_mac_candidate(event.src_mac, station_mac) {
        return Some((event.src_mac, "ether-src-global"));
    }
    None
}
...
fn host_eapol_verified_ap_mac_candidate(mac: [u8; 6], station_mac: [u8; 6]) -> bool {
    host_eapol_ap_mac_candidate(mac, station_mac)
        && !mac_is_station_local_alias(mac, station_mac)
        && !mac_is_locally_administered(mac)
}

fn host_eapol_local_admin_ap_hint(
    event: Cyw43Event,
    station_mac: [u8; 6],
) -> Option<([u8; 6], &'static str)> {
    host_eapol_association_event_label(event)?;
    if host_eapol_ap_mac_candidate(event.addr, station_mac)
        && !mac_is_station_local_alias(event.addr, station_mac)
        && mac_is_locally_administered(event.addr)
    {
        return Some((event.addr, "event-addr-local-admin"));
    }
    if host_eapol_ap_mac_candidate(event.src_mac, station_mac)
        && !mac_is_station_local_alias(event.src_mac, station_mac)
        && mac_is_locally_administered(event.src_mac)
    {
        return Some((event.src_mac, "ether-src-local-admin"));
    }
    None
}
...
fn host_eapol_group_replay_counter_increases(current: &[u8], host_wpa: &HostWpaState) -> bool {
    let previous = if host_wpa.group_replay_counter_valid {
        &host_wpa.group_replay_counter
    } else {
        &host_wpa.m3_replay_counter
    };
    eapol_replay_counter_increases(current, previous)
}

#[inline]
fn host_eapol_rx_admission_rescue_effective(
    mode: HostEapolRxAdmissionMode,
    allmulti_ready: bool,
    promisc_ready: bool,
) -> bool {
    !matches!(mode, HostEapolRxAdmissionMode::CompatibilityM1Rescue)
        || (mode.allmulti() != 0 && allmulti_ready)
        || (mode.promisc() != 0 && promisc_ready)
}

#[inline]
fn cyw43_queued_glom_data_allowed(host_eapol_secure: bool, state: DeferredJoinState) -> bool {
    cyw43_data_rx_allowed(host_eapol_secure, state)
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
struct SdpcmTxProof {
    seq: u8,
    credit_observation: u32,
}

#[inline]
const fn sdpcm_credit_observation_covers_submitted_seq(seq_max: u8, submitted_seq: u8) -> bool {
    let next_seq = submitted_seq.wrapping_add(1);
    next_seq == seq_max || (seq_max.wrapping_sub(next_seq) & 0x80) == 0
}

#[inline]
fn host_eapol_tx_drain_proved(proof: SdpcmTxProof, credit_observations: u32, seq_max: u8) -> bool {
    credit_observations != proof.credit_observation
        && sdpcm_credit_observation_covers_submitted_seq(seq_max, proof.seq)
}
...
fn event_link_state_update(event: Cyw43Event, _deferred_join_pending: bool) -> Option<bool> {
    match event.event_type {
...
const fn host_eapol_start_due(polls: u16, sent: u8) -> bool {
    sent < HOST_EAPOL_START_MAX
        && polls >= HOST_EAPOL_START_DIAGNOSTIC_AFTER_POLLS
...
fn host_eapol_start_destination(
    ...
    if host_eapol_ap_source_is_unproved_local_admin(ap_source) {
        (PAE_GROUP_ADDR, "pae-group-local-admin-ap-hint")
    } else {
        (PAE_GROUP_ADDR, "pae-group-diagnostic")
    }
}
...
fn host_eapol_rx_admission_rescue_due(
    ...
    deferred_join_requires_host_eapol(deferred_join_state)
        ...
        && !m1_seen
        && !rescued_after_assoc
        && (post_assoc_polls >= HOST_EAPOL_RX_RESCUE_AFTER_POST_ASSOC_POLLS
            || eapol_start_sent >= HOST_EAPOL_RX_RESCUE_AFTER_STARTS)
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
enum HostEapolRxAdmissionMode {
    LinuxUnicastM1,
    CompatibilityM1Rescue,
}

impl HostEapolRxAdmissionMode {
    const fn path(self) -> &'static str {
        match self {
            Self::LinuxUnicastM1 => "linux-unicast-m1",
            Self::CompatibilityM1Rescue => "eapol-m1-rescue",
        }
    }

    const fn allmulti(self) -> u32 {
        match self {
            Self::LinuxUnicastM1 => 0,
            Self::CompatibilityM1Rescue => 1,
        }
    }

    const fn promisc(self) -> u32 {
        match self {
            Self::LinuxUnicastM1 => 0,
            Self::CompatibilityM1Rescue => 1,
        }
...
struct HostWpaState {
    ...
    rx_admission_rescue_after_assoc: bool,
}
...
impl HostWpaState {
    const fn empty() -> Self {
        Self {
            ...
            rx_admission_rescue_after_assoc: false,
        }
    }
...
fn host_eapol_frame_proof(packet: &[u8]) -> HostEapolFrameProof {
    ...
    let key_len = get_u16_be(body, 3).unwrap_or(0);
...
const fn host_eapol_m3_key_len_allowed(key_len: u16) -> bool {
    key_len == 0 || key_len == WPA_TK_LEN as u16
}
...
fn host_eapol_canonical_len(packet: &[u8]) -> Result<usize, DriverError> {
    let eapol = packet
        .get(ETH_HEADER_LEN..)
        .ok_or(DriverError::Protocol("eapol-short-ethernet"))?;
    if eapol.len() < EAPOL_HEADER_LEN {
        return Err(DriverError::Protocol("eapol-short-header"));
    }
    let body_len =
        usize::from(get_u16_be(eapol, 2).ok_or(DriverError::Protocol("eapol-body-len"))?);
    ...
    if canonical_len > packet.len() {
        return Err(DriverError::Protocol("eapol-body-truncated"));
    }
    Ok(canonical_len)
}
...
fn host_eapol_packet_dst_allowed(packet: &[u8], station_mac: [u8; 6]) -> bool {
    matches!(
        ethernet_dst(packet),
        Ok(dst) if dst == station_mac || mac_is_station_local_alias(dst, station_mac)
    )
}
...
fn eapol_key_data_contains_ie(key_data: &[u8], expected: &[u8]) -> bool {
    if expected.len() < 2 {
        return false;
    }
    let mut offset = 0usize;
    while offset + 2 <= key_data.len() {
        ...
        let element_end = match offset.checked_add(2 + element_len) {
            Some(value) => value,
            None => return false,
        };
        if element_end > key_data.len() {
        ...
        if &key_data[offset..element_end] == expected {
            return true;
        }
        offset = element_end;
    }
...
fn rsn_ie_is_wpa2_psk_ccmp_compatible(ie: &[u8]) -> bool {
    if ie.len() < 2 || ie[0] != RSN_IE_ID {
        return false;
    }
    let declared_len = usize::from(ie[1]);
    if declared_len + 2 != ie.len() {
        return false;
    }
...
impl Cyw43NetDevice {
    ...
    fn init_control_plane(
        ...
        control_step!("up", self.ioctl_set_u32(Ioctl::Up, 0, 0));
        control_step!("station-infra", self.ioctl_set_u32(Ioctl::SetInfra, 0, 1));
    ...
    fn configure_host_eapol_receive_admission(
        &mut self,
        mode: HostEapolRxAdmissionMode,
    ) -> Result<(), DriverError> {
        ...
        let allmulti = mode.allmulti();
        let promisc = mode.promisc();

        info!(
            "[cyw43] host-eapol rx-admission action=begin mcast={} allmulti={} promisc={} path={}",
        ...
        match self.set_iovar_u32("allmulti", allmulti) {
            Ok(()) => {
                allmulti_ready = true;
                info!(
                    "[cyw43] host-eapol rx-admission step=allmulti action=ready value={allmulti}"
                );
            }
        ...
        match self.ioctl_set_u32(Ioctl::SetPromisc, 0, promisc) {
            Ok(()) => {
                promisc_ready = true;
                info!("[cyw43] host-eapol rx-admission step=promisc action=ready value={promisc}");
            }
        ...
        if !host_eapol_rx_admission_rescue_effective(mode, allmulti_ready, promisc_ready) {
            warn!(
                "[cyw43] host-eapol rx-admission action=degraded path={} result=unsupported reason=no-effective-rescue-widening allmulti_ready={} promisc_ready={}",
                mode.path(),
                yes_no(allmulti_ready),
                yes_no(promisc_ready),
            );
            return Err(DriverError::Protocol(
                "host-eapol-rx-rescue-widening-unproved",
            ));
        }

        info!(
            "[cyw43] host-eapol rx-admission action=ready mcast_list_ready={} allmulti_ready={} promisc_ready={} allmulti={} promisc={} path={} data=blocked",
            yes_no(mcast_ready),
            yes_no(allmulti_ready),
            yes_no(promisc_ready),
            allmulti,
    ...
    fn ensure_host_eapol_rx_admission_rescue(&mut self, mode: &'static str, polls: u16) -> bool {
        ...
        self.promote_host_eapol_wait_stage();
        match self
            .configure_host_eapol_receive_admission(HostEapolRxAdmissionMode::CompatibilityM1Rescue)
        {
            Ok(()) => {
                self.host_wpa.rx_admission_rescue_after_assoc = true;
                info!(
                    "[cyw43] host-eapol rx-admission action=rescue mode={mode} polls={polls} assoc=yes start_sent={} ap_mac={} ap_source={} result=ready scope=eapol-only data=blocked next=eapol-start",
                    self.host_wpa.eapol_start_sent,
                    EthernetAddress(self.host_wpa.ap_mac),
                    self.host_wpa.ap_mac_source,
                );
            ...
            Err(err) => {
                warn!(
                    "[cyw43] host-eapol rx-admission action=rescue mode={mode} polls={polls} assoc=yes start_sent={} ap_mac={} ap_source={} result=error err={err} next=keep-waiting-m1",
                    self.host_wpa.eapol_start_sent,
                    EthernetAddress(self.host_wpa.ap_mac),
                    self.host_wpa.ap_mac_source,
                );
    ...
    fn restore_host_eapol_rx_admission_after_secure(&mut self) {
        match self.configure_host_eapol_receive_admission(HostEapolRxAdmissionMode::LinuxUnicastM1) {
            ...
            Err(err) => warn!(
                "[cyw43] host-eapol rx-admission action=restore-after-secure result=error err={err} data=allowed-after-keys"
            ),
        }
    }
    ...
    fn join(
        ...
        if !secure {
            ...
            if linux_wpa2_join_sets_rsn_capability_policy() {
            ...
            if matches!(completion_rule, JoinCompletionRule::HostEapolRequired) {
                self.configure_host_eapol_receive_admission(
                    HostEapolRxAdmissionMode::LinuxUnicastM1,
                )?;
            }
            self.program_join_request(ssid)?;
            if matches!(completion_rule, JoinCompletionRule::HostEapolRequired) {
        ...
        while total_polls < HOST_EAPOL_JOIN_SUBMIT_PROOF_POLLS {
            ...
            if !self.associated {
                ...
                if post_assoc_polls <= u16::MAX as usize {
                    ...
                    self.maybe_send_host_eapol_start(post_assoc_poll, "join-submit-proof");
                }
    ...
    fn apply_linux_wpa2_rsn_capability_policy(&mut self) -> Result<(), DriverError> {
        self.set_optional_iovar_u32("mfp", MFP_NONE)?;
        self.set_primary_bsscfg_u32("wme_bss_disable", WME_BSS_DISABLE_RSN_DEFAULT)?;
        info!(
            "[cyw43] join: rsn-cap-policy action=ready mfp={} wme_bss_disable={} source=linux-brcmfmac-key-mgmt",
            MFP_NONE,
            WME_BSS_DISABLE_RSN_DEFAULT,
        );
        Ok(())
    }
    ...
    fn maybe_send_host_eapol_start(&mut self, polls: u16, mode: &'static str) {
        ...
        if !self.ensure_host_eapol_rx_admission_rescue(mode, polls) || self.host_wpa.m1_seen {
            return;
        }
        ...
        match self.transmit(&frame[..frame_len]) {
            Ok(_proof) => {
                ...
                info!(
                    "[cyw43] host-eapol action=eapol-start mode={mode} polls={polls} dst={dst_label} dst_mac={} ap_mac={} ap_source={} sent={} result=diagnostic-passive-m1-wait",
                    EthernetAddress(dst),
                    EthernetAddress(self.host_wpa.ap_mac),
                    self.host_wpa.ap_mac_source,
                    self.host_wpa.eapol_start_sent,
    ...
    fn refresh_host_eapol_bssid(&mut self, reason: &'static str) {
        ...
        match self.ioctl_raw(IoctlType::Get, Ioctl::GetBssid, 0, &request) {
            Ok(response_len) if response_len >= ETHER_ADDR_LEN => {
                ...
                if host_eapol_verified_ap_mac_candidate(bssid, self.mac.0) {
                    let previous = self.host_wpa.ap_mac;
                    self.host_wpa.ap_mac = bssid;
                    self.host_wpa.ap_mac_source = "get-bssid";
                    if previous != bssid && self.host_wpa.eapol_start_sent > 0 {
                        self.host_wpa.eapol_start_sent = 0;
                        self.host_wpa.rx_admission_refreshed_after_assoc = false;
                        self.host_wpa.rx_admission_rescue_after_assoc = false;
                    }
                    info!(
                        "[cyw43] host-eapol bssid refresh action=ready reason={reason} bssid={} previous={} source=get-bssid next=keep-pae-group-start-ap-hint retry_count={}",
                        EthernetAddress(bssid),
                        EthernetAddress(previous),
                        self.host_wpa.eapol_start_sent,
                    );
                } else {
                    let detail = if host_eapol_ap_mac_candidate(bssid, self.mac.0)
                        && !mac_is_station_local_alias(bssid, self.mac.0)
                        && mac_is_locally_administered(bssid)
                    {
                        "local-admin-unproved"
                    } else {
                        "not-ap-candidate"
                    };
                    info!(
                        "[cyw43] host-eapol bssid refresh action=skip reason={reason} bssid={} source=wlc-get-bssid detail={detail} next=wait-m1-or-verified-bssid",
                        EthernetAddress(bssid),
                    );
                }
            }
    ...
    fn handle_host_eapol_frame(
        ...
        if proof.key_version != EAPOL_KEY_VERSION_HMAC_SHA1_AES {
            return Err(DriverError::Protocol(if proof.key_version == 3 {
                "host-eapol-pmf-required-unsupported"
            } else {
                "host-eapol-key-version-unsupported"
            }));
        }
        match proof.message.as_bytes() {
            b"m1" => self.handle_host_eapol_m1(packet, proof),
            b"m3" => self.handle_host_eapol_m3(packet, proof),
            b"group-key" => self.handle_host_eapol_group_key(packet, proof),
    ...
    fn handle_host_eapol_m1(
        ...
        let _tx_proof = self.transmit(&frame[..frame_len])?;
    ...
    fn handle_host_eapol_m3(
        ...
        if !host_eapol_m3_key_len_allowed(proof.key_len) {
            return Err(DriverError::Protocol("host-eapol-m3-key-len"));
        }
        let body =
        ...
        let previous_m3_replay = if self.host_wpa.m4_sent {
            &self.host_wpa.m3_replay_counter
        } else {
            &self.host_wpa.m1_replay_counter
        };
        if !eapol_replay_counter_increases(replay_counter, previous_m3_replay) {
        ...
        self.host_wpa
            .m3_replay_counter
            .copy_from_slice(replay_counter);
        self.host_wpa.group_replay_counter_valid = false;
        let key_data =
        ...
        if !eapol_key_data_contains_compatible_rsn_ie(&unwrapped[..unwrapped_len]) {
            return Err(DriverError::Protocol("host-eapol-m3-rsn-ie"));
        }
        let gtk = match find_gtk_kde(&unwrapped[..unwrapped_len]) {
            Ok(gtk) => Some(gtk),
            Err(DriverError::Protocol("eapol-gtk-kde-missing")) => None,
            Err(err) => return Err(err),
        };
        ...
        let tx_proof = self.transmit(&frame[..frame_len])?;
        self.host_wpa.m4_sent = true;
        self.wait_for_host_eapol_tx_drain("m4-before-wsec", tx_proof)?;
        ...
        let Some(gtk) = gtk else {
            ...
            return Ok("send-m4-install-ptk-wait-group-key");
        };
    ...
    fn handle_host_eapol_group_key(
        &mut self,
        packet: &[u8],
        proof: HostEapolFrameProof,
    ) -> Result<&'static str, DriverError> {
        if !self.host_wpa.m4_sent || !self.host_wpa.ptk_installed {
            return Err(DriverError::Protocol("host-eapol-group-before-ptk"));
        }
        if proof.pairwise || !proof.mic || !proof.secure || !proof.encrypted_key_data {
            return Err(DriverError::Protocol("host-eapol-group-shape"));
        }
        let body =
            host_eapol_key_body(packet).ok_or(DriverError::Protocol("host-eapol-group-body"))?;
        let replay_counter = &body
            [EAPOL_KEY_BODY_REPLAY_OFFSET..EAPOL_KEY_BODY_REPLAY_OFFSET + WPA_REPLAY_COUNTER_LEN];
        if !host_eapol_group_replay_counter_increases(replay_counter, &self.host_wpa) {
            return Err(DriverError::Protocol("host-eapol-group-replay"));
        }
        if !verify_eapol_key_mic(packet, &self.host_wpa.ptk[..WPA_KCK_LEN])? {
            return Err(DriverError::Protocol("host-eapol-group-mic"));
        }

        let key_data =
            host_eapol_key_data(body).ok_or(DriverError::Protocol("host-eapol-group-key-data"))?;
        let mut unwrapped = [0u8; HOST_EAPOL_KEY_DATA_MAX_LEN];
        let unwrapped_len = aes128_key_unwrap(
            &self.host_wpa.ptk[WPA_KCK_LEN..WPA_KCK_LEN + WPA_KEK_LEN],
            key_data,
        ...
        let tx_proof = self.transmit(&frame[..frame_len])?;
        self.wait_for_host_eapol_tx_drain("group-m2-before-secure", tx_proof)?;
        self.host_wpa
            .group_replay_counter
            .copy_from_slice(replay_counter);
        ...
        info!(
            "[cyw43] join complete mode=host-eapol secure=yes completion_rule=host-eapol-required m1=yes m2=yes m3=yes m4=yes wsec_key=ptk+gtk key_order=m4-before-wsec+group carrier=yes"
        );
        Ok("send-group-m2-install-gtk-complete")
    }

    fn wait_for_host_eapol_tx_drain(
        &mut self,
        stage: &'static str,
        proof: SdpcmTxProof,
    ) -> Result<(), DriverError> {
        ...
        Err(DriverError::Protocol("host-eapol-tx-drain-timeout"))
    }

    fn reassert_wsec_aes_after_gtk(&mut self) -> Result<(), DriverError> {
        self.set_primary_bsscfg_u32("wsec", WSEC_AES)?;
        info!(
            "[cyw43] host-eapol action=reassert-wsec-after-gtk value=0x{WSEC_AES:08x} result=ok source=linux-brcmfmac-default-key"
        );
        Ok(())
    }
    ...
    fn service_deferred_join(&mut self) {
        ...
        for _ in 0..frame_budget {
            ...
            if host_eapol_required && self.host_eapol_secure_complete() {
                self.deferred_join_state = DeferredJoinState::Disabled;
                return;
            }
        }
    ...
    fn maybe_seed_host_eapol_ap_from_event(&mut self, event: Cyw43Event) {
        ...
        let Some((ap_mac, source)) = host_eapol_event_ap_mac_candidate(event, self.mac.0) else {
            if let (Some(label), Some((hint, hint_source))) = (
                host_eapol_association_event_label(event),
                host_eapol_local_admin_ap_hint(event, self.mac.0),
            ) {
                info!(
                    "[cyw43] host-eapol ap-mac seed action=ignore source={hint_source} event={label} mac={} event_addr={} event_src={} reason=local-admin-unproved next=wait-m1-or-verified-bssid",
                    EthernetAddress(hint),
                    EthernetAddress(event.addr),
                    EthernetAddress(event.src_mac),
                );
            }
...
mod tests {
    use super::{
        ...
        host_eapol_local_admin_ap_hint, host_eapol_m3_key_len_allowed,
        ...
        host_eapol_rx_admission_rescue_due, host_eapol_rx_admission_rescue_effective,
        host_eapol_should_handle_frame, host_eapol_start_destination, host_eapol_start_due,
        host_eapol_start_poll_due, host_eapol_tx_drain_proved,
    ...
    fn broadcom_event_parser_preserves_ap_mac_candidates() {
        ...
        let ap_addr = [0xb8, 0x82, 0x33, 0xe8, 0x9f, 0x41];
        ...
        assert_eq!(
            ...
            Some((ap_addr, "event-addr-global"))
        );
    }
    ...
    fn host_eapol_ap_mac_candidate_rejects_p2p_station_alias_source() {
        ...
        assert_eq!(
            host_eapol_local_admin_ap_hint(event, sta),
        ...
        assert_eq!(host_eapol_local_admin_ap_hint(alias_only, sta), None);
    }
    ...
    #[test]
    fn host_eapol_ap_mac_candidate_rejects_local_event_addr_until_verified() {
        let sta = [0x88, 0xa2, 0x9e, 0x66, 0x59, 0x10];
    ...
    #[test]
    fn host_eapol_rejects_latest_local_admin_assoc_alias_shape() {
        let sta = [0x88, 0xa2, 0x9e, 0x66, 0x59, 0x10];
        let p2p_alias = [0x8a, 0xa2, 0x9e, 0x66, 0x59, 0x10];
        let local_hint = [0x32, 0x22, 0x91, 0x16, 0x44, 0x59];
        let event = Cyw43Event {
            flags: EVENT_FLAG_LINK,
            event_type: EVENT_LINK,
            status: STATUS_SUCCESS,
        ...
        assert_eq!(
            host_eapol_local_admin_ap_hint(event, sta),
            Some((local_hint, "event-addr-local-admin"))
        );
    }
    ...
    #[test]
    fn host_eapol_group_replay_advances_after_first_group_key() {
        let mut state = HostWpaState::empty();
        state.m3_replay_counter = [0, 0, 0, 0, 0, 0, 0, 7];

        assert!(host_eapol_group_replay_counter_increases(
            &[0, 0, 0, 0, 0, 0, 0, 8],
            &state
        ));
        assert!(!host_eapol_group_replay_counter_increases(
        ...
        state.group_replay_counter = [0, 0, 0, 0, 0, 0, 0, 8];
        state.group_replay_counter_valid = true;
        assert!(!host_eapol_group_replay_counter_increases(
            &[0, 0, 0, 0, 0, 0, 0, 8],
            &state
        ));
    ...
    #[test]
    fn host_eapol_m3_replay_anchor_moves_after_m4() {
        let mut state = HostWpaState::empty();
        state.m1_replay_counter = [0, 0, 0, 0, 0, 0, 0, 7];
        state.m3_replay_counter = [0, 0, 0, 0, 0, 0, 0, 8];

        let first_m3_anchor = if state.m4_sent {
            &state.m3_replay_counter
        } else {
            &state.m1_replay_counter
        };
        assert!(eapol_replay_counter_increases(
            &[0, 0, 0, 0, 0, 0, 0, 8],
            first_m3_anchor
        ...
        state.m4_sent = true;
        let duplicate_m3_anchor = if state.m4_sent {
            &state.m3_replay_counter
        } else {
            &state.m1_replay_counter
        };
    ...
    #[test]
    fn host_eapol_rescue_requires_effective_widening() {
        assert!(host_eapol_rx_admission_rescue_effective(
            HostEapolRxAdmissionMode::LinuxUnicastM1,
            false,
            false
        ));
        assert!(!host_eapol_rx_admission_rescue_effective(
            HostEapolRxAdmissionMode::CompatibilityM1Rescue,
            false,
            false
        ));
        assert!(host_eapol_rx_admission_rescue_effective(
    ...
    #[test]
    fn queued_glom_data_stays_blocked_until_host_eapol_secure() {
        ...
        assert!(!cyw43_queued_glom_data_allowed(false, pending));
        assert!(cyw43_queued_glom_data_allowed(true, pending));
        assert!(cyw43_queued_glom_data_allowed(
            false,
    ...
    #[test]
    fn host_eapol_tx_drain_requires_new_credit_observation() {
        let proof = SdpcmTxProof {
            seq: 7,
            credit_observation: 3,
        };

        assert!(!host_eapol_tx_drain_proved(proof, 3, 9));
        assert!(!host_eapol_tx_drain_proved(proof, 4, 7));
        assert!(host_eapol_tx_drain_proved(proof, 4, 8));
        assert!(host_eapol_tx_drain_proved(proof, 4, 9));
    }
    ...
    fn host_eapol_deferred_polling_gets_extended_budget() {
        ...
        assert!(!host_eapol_start_due(1_024, 0));
        assert!(host_eapol_start_due(
            HOST_EAPOL_START_DIAGNOSTIC_AFTER_POLLS,
            0
        ));
        assert!(host_eapol_start_due(
            HOST_EAPOL_START_DIAGNOSTIC_AFTER_POLLS,
    ...
    #[test]
    fn host_eapol_start_keeps_diagnostic_probe_on_pae_group() {
        let ap = [0xf0, 0x72, 0xea, 0x4c, 0xc7, 0xa5];
        ...
        assert_eq!(
            host_eapol_start_destination(ap, "get-bssid", 1),
            (PAE_GROUP_ADDR, "pae-group-diagnostic")
        );
        assert_eq!(
            host_eapol_start_destination(ap, "get-bssid", 2),
            (PAE_GROUP_ADDR, "pae-group-diagnostic")
        );
        assert_eq!(
        ...
        assert_eq!(
            host_eapol_start_destination(local_admin_ap, "get-bssid-local-admin", 2),
            (PAE_GROUP_ADDR, "pae-group-local-admin-ap-hint")
        );
        assert_eq!(
    ...
    #[test]
    fn host_eapol_rx_rescue_is_bounded_to_post_assoc_no_m1() {
        let pending = DeferredJoinState::Pending {
            completion: JoinCompletionState::default(),
            polls: 0,
            secure: true,
        ...
        assert_eq!(HOST_EAPOL_RX_RESCUE_AFTER_STARTS, 2);
        assert_eq!(HOST_EAPOL_RX_RESCUE_AFTER_POST_ASSOC_POLLS, 4_096);
        assert_eq!(HOST_EAPOL_START_DIAGNOSTIC_AFTER_POLLS, 8_192);
        assert!(host_eapol_rx_admission_rescue_due(
            pending,
            true,
            false,
            false,
        ...
        assert!(host_eapol_rx_admission_rescue_due(
            ...
            false,
            0,
            HOST_EAPOL_RX_RESCUE_AFTER_STARTS,
        ));
        assert!(!host_eapol_rx_admission_rescue_due(
            pending,
            true,
            false,
            false,
            HOST_EAPOL_RX_RESCUE_AFTER_POST_ASSOC_POLLS - 1,
            HOST_EAPOL_RX_RESCUE_AFTER_STARTS - 1,
        ));
        assert!(!host_eapol_rx_admission_rescue_due(
            pending,
            false,
    ...
    #[test]
    fn host_eapol_rx_admission_modes_are_eapol_only_filters() {
        assert_eq!(HostEapolRxAdmissionMode::LinuxUnicastM1.allmulti(), 0);
        assert_eq!(HostEapolRxAdmissionMode::LinuxUnicastM1.promisc(), 0);
        assert_eq!(
            HostEapolRxAdmissionMode::LinuxUnicastM1.path(),
            "linux-unicast-m1"
        );
        assert_eq!(
            HostEapolRxAdmissionMode::CompatibilityM1Rescue.allmulti(),
            1
        );
        assert_eq!(HostEapolRxAdmissionMode::CompatibilityM1Rescue.promisc(), 1);
        assert_eq!(
            HostEapolRxAdmissionMode::CompatibilityM1Rescue.path(),
            "eapol-m1-rescue"
        );
    }
    ...
    fn host_eapol_admission_rejects_foreign_destinations() {
        let sta = [0x88, 0xa2, 0x9e, 0x66, 0x59, 0x10];
        let sta_local_alias = [0x8a, 0xa2, 0x9e, 0x66, 0x59, 0x10];
        let ap = [0xf0, 0x72, 0xea, 0x4c, 0xc7, 0xa5];
        ...
        packet[..6].copy_from_slice(&sta_local_alias);
        assert!(host_eapol_packet_dst_allowed(&packet, sta));
    ...
    fn host_eapol_key_reply_mic_covers_m2_payload() {
        ...
        padded[len..len + 4].copy_from_slice(&[0xaa, 0xbb, 0xcc, 0xdd]);
        assert!(verify_eapol_key_mic(&padded[..len + 4], &kck).expect("MIC ignores RX padding"));
    ...
    fn gtk_kde_parser_extracts_index_and_key_material() {
        ...
        assert!(!eapol_key_data_contains_ie(&gtk, &WPA2_PSK_CCMP_RSN_IE));
        let mut m3_key_data = [0u8; 64];
        m3_key_data[..WPA2_PSK_CCMP_RSN_IE.len()].copy_from_slice(&WPA2_PSK_CCMP_RSN_IE);
        m3_key_data[WPA2_PSK_CCMP_RSN_IE.len()..WPA2_PSK_CCMP_RSN_IE.len() + gtk.len()]
            .copy_from_slice(&gtk);
        assert!(eapol_key_data_contains_ie(
            &m3_key_data[..WPA2_PSK_CCMP_RSN_IE.len() + gtk.len()],
            &WPA2_PSK_CCMP_RSN_IE
        ));
        assert!(eapol_key_data_contains_compatible_rsn_ie(
            &m3_key_data[..WPA2_PSK_CCMP_RSN_IE.len() + gtk.len()]
        ));
    }

    #[test]
    fn rsn_ie_validation_accepts_router_capability_variants() {
        assert!(rsn_ie_is_wpa2_psk_ccmp_compatible(&WPA2_PSK_CCMP_RSN_IE));

        let mut rsn_with_capabilities = [0u8; 26];
        rsn_with_capabilities[..WPA2_PSK_CCMP_RSN_IE.len()].copy_from_slice(&WPA2_PSK_CCMP_RSN_IE);
        rsn_with_capabilities[1] = 0x18;
        rsn_with_capabilities[22] = 0x0c;
        rsn_with_capabilities[23] = 0x00;
        rsn_with_capabilities[24] = 0x00;
        rsn_with_capabilities[25] = 0x00;
        assert!(rsn_ie_is_wpa2_psk_ccmp_compatible(&rsn_with_capabilities));

        let mut sae_only = WPA2_PSK_CCMP_RSN_IE;
        sae_only[19] = 0x08;
        assert!(!rsn_ie_is_wpa2_psk_ccmp_compatible(&sae_only));
    }

    #[test]
    fn host_eapol_m3_key_len_accepts_common_wpa2_router_variants() {
        assert!(host_eapol_m3_key_len_allowed(0));
        assert!(host_eapol_m3_key_len_allowed(WPA_TK_LEN as u16));
        assert!(!host_eapol_m3_key_len_allowed(32));
    }
    ...
    fn wsec_key_payload_matches_brcmfmac_ccmp_shape() {
        ...
        let zero_rsc = [0u8; 6];
        write_wsec_key_payload(&mut pairwise_payload, 0, &key, &ea, Some(&zero_rsc), false)
        ...
        assert_eq!(
            super::get_u32_le(&pairwise_payload, super::WSEC_KEY_IV_INITIALIZED_OFFSET),
            Some(1)
        );
        assert_eq!(
            super::get_u16_le(&pairwise_payload, super::WSEC_KEY_RXIV_LO_OFFSET),
            Some(0)
        );
        assert_eq!(
            super::get_u32_le(&pairwise_payload, super::WSEC_KEY_RXIV_HI_OFFSET),
            Some(0)
        );
    }
    ...
    #[test]
    fn host_eapol_proof_classifies_group_key_message() {
        let packet = host_eapol_key_packet(
            EAPOL_KEY_INFO_ACK
                | EAPOL_KEY_INFO_MIC
                | EAPOL_KEY_INFO_SECURE
        ...
        assert_eq!(proof.message, "group-key");
        assert_eq!(proof.next_action, "verify-mic-install-gtk");
        assert_eq!(proof.key_version, 2);
        assert!(!proof.pairwise);
        assert!(proof.ack);
        assert!(proof.mic);
        ...
        assert_eq!(
            EAPOL_KEY_INFO_GROUP_M2,
            EAPOL_KEY_INFO_MIC | EAPOL_KEY_INFO_SECURE | 2
        );
    }
    ...
    fn deferred_join_link_event_waits_for_join_completion_state() {
        ...
        assert_eq!(event_link_state_update(link_success, true), Some(true));
    ...
    fn linux_first_control_plane_iovar_order_precedes_clm() {
        ...
        assert_eq!(Ioctl::Up as u32, 2);
        assert_eq!(Ioctl::SetInfra as u32, 20);
    ...
    fn secure_join_uses_linux_wpa2_auth_masks_in_order() {
        ...
        assert!(linux_wpa2_join_sets_rsn_capability_policy());
        assert_eq!(WME_BSS_DISABLE_RSN_DEFAULT, 1);
    }
