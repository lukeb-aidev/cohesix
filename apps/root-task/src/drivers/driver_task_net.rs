...
const CYW43_HOST_EAPOL_WSEC_KEY_STALE_REPLY_RETRIES: usize = 3;
...
const CYW43_HOST_EAPOL_POST_ACTIVITY_POLLS: usize = CYW43_HOST_EAPOL_POST_ASSOC_POLLS;
...
#[cfg(test)]
static CYW43_HOST_EAPOL_TEST_M2_SUBMIT_FAILS: AtomicU32 = AtomicU32::new(0);
#[cfg(test)]
...
#[cfg(test)]
static CYW43_HOST_EAPOL_TEST_M4_SUBMIT_BEFORE_GTK: AtomicU32 = AtomicU32::new(0);
#[cfg(test)]
static CYW43_HOST_EAPOL_TEST_M4_SUBMIT_BEFORE_WSEC: AtomicU32 = AtomicU32::new(0);
#[cfg(test)]
static CYW43_HOST_EAPOL_TEST_M4_SUBMIT_PTK_COUNT: AtomicU32 = AtomicU32::new(0);
#[cfg(test)]
static CYW43_HOST_EAPOL_TEST_M4_SUBMIT_GTK_COUNT: AtomicU32 = AtomicU32::new(0);
#[cfg(test)]
static CYW43_HOST_EAPOL_TEST_M4_SUBMIT_WSEC_COUNT: AtomicU32 = AtomicU32::new(0);
...
fn cyw43_rx_idle_trace(bytes: &[u8]) -> Option<Cyw43RxIdleTrace> {
    ...
    if version == CYW43_RX_IDLE_TRACE_VERSION_V6 && bytes.len() < CYW43_RX_IDLE_TRACE_V6_BYTES {
        return None;
    }
    if version == CYW43_RX_IDLE_TRACE_VERSION && bytes.len() < CYW43_RX_IDLE_TRACE_V7_BYTES {
        return None;
    }
...
fn record_cyw43_firmware_supplicant_event(contract: DriverTaskContract, event: Cyw43EventFrame) {
    if contract != CYW43_WIFI_DRIVER_TASK_CONTRACT
        ...
        || event.event_type != CYW43_EVENT_PSK_SUP
    {
        return;
    }
    ...
    H: Hardware<Error = HalError>,
{
    ...
    *CYW43_RUNTIME_MAC.lock() = mac;
    ...
    if credentials.has_psk() {
        ...
        let firmware_supplicant = cyw43_enable_wpa2_firmware_supplicant(contract)?;
        if firmware_supplicant == Cyw43FirmwareSupplicantProbe::Enabled {
            match cyw43_submit_wsec_pmk(contract, credentials) {
                Ok(()) => {
                ...
                {
                    clear_cyw43_runtime_command_fault_status();
                    emit_cyw43_wsec_pmk_reject_trace(
                        contract,
                        "fallback-host-eapol",
                        completion.result,
                    );
                    cyw43_disable_wpa2_firmware_supplicant_best_effort(contract);
                }
                Err(err) => return Err(err.into_net_error()),
            }
        }
...
impl Cyw43FirmwareSupplicantProbe {
    const fn label(self) -> &'static str {
        match self {
            Self::Enabled => "sup_wpa-enabled-firmware-eapol",
...
#[cfg(feature = "kernel")]
fn cyw43_enable_wpa2_firmware_supplicant(
    contract: DriverTaskContract,
    ...
    if cyw43_probe_wpa2_firmware_supplicant_iovar(
        ...
        return Ok(Cyw43FirmwareSupplicantProbe::Enabled);
    }
    emit_cyw43_firmware_supplicant_trace(
        contract,
        "primary-plain",
...
#[cfg(feature = "kernel")]
fn cyw43_disable_wpa2_firmware_supplicant_best_effort(contract: DriverTaskContract) {
    let primary_data = 0u32.to_le_bytes();
    let _ = cyw43_probe_wpa2_firmware_supplicant_iovar(
        contract,
        "sup_wpa",
        &primary_data,
...
#[cfg(feature = "kernel")]
fn cyw43_submit_wsec_pmk_material(
    contract: DriverTaskContract,
    key: &[u8],
    flags: u16,
    stage: &'static str,
    ...
    cyw43_write_wsec_pmk_payload(&mut payload, key, flags)
        .map_err(Cyw43CommandSubmitError::Runtime)?;
    emit_cyw43_wsec_pmk_trace(contract, stage, key.len(), flags, &payload);
...
fn cyw43_submit_wsec_pmk(
    ...
    let pmk = cyw43_host_eapol::derive_wpa2_psk_pmk(ssid, psk).map_err(|_| {
        Cyw43CommandSubmitError::Runtime(DriverTaskNetError::RuntimeInit("cyw43-wsec-pmk-derive"))
    })?;
    let completion =
        match cyw43_submit_wsec_pmk_material(contract, &pmk, 0, "cyw43-control-wsec-pmk") {
            Ok(completion) => completion,
            Err(Cyw43CommandSubmitError::Completion(completion))
                if completion.code == DriverTaskCompletionCode::Fault.as_u16()
                    && completion.detail == CYW43_CONTROL_EXCHANGE_FAULT_DETAIL
                    && completion.result == CYW43_BCME_BADARG_STATUS
                    && credentials.psk_len != 64 =>
            {
                emit_cyw43_wsec_pmk_reject_trace(contract, "retry-passphrase", completion.result);
                let fallback = cyw43_submit_wsec_pmk_material(
                    contract,
                    psk,
                    CYW43_WSEC_PASSPHRASE_FLAG,
                    "cyw43-control-wsec-pmk-passphrase",
                )?;
                if fallback.code == DriverTaskCompletionCode::FrameReady.as_u16() {
                    crate::hal::driver_task::emit_driver_task_resource_init_status(
                        contract,
                        DriverTaskHotPath::Cyw43Wifi,
                        "cyw43-control-wsec-pmk-passphrase",
                ...
                return Err(Cyw43CommandSubmitError::Completion(fallback));
            }
...
fn emit_cyw43_wsec_pmk_trace(
    ...
    let _ = write!(
        line,
        "CYW43_DRIVER_TASK_WSEC_PMK contract={} stage={} key_len={} flags=0x{:04x} payload_len={} digest=0x{:08x}",
        contract.name,
        stage,
        key_len,
        flags,
...
#[cfg(feature = "kernel")]
fn emit_cyw43_wsec_pmk_reject_trace(
    contract: DriverTaskContract,
    action: &'static str,
    result: u32,
) {
    ...
    let _ = write!(
        line,
        "CYW43_DRIVER_TASK_WSEC_PMK contract={} status=rejected action={} result=0x{:08x}",
        contract.name, action, result
    );
    crate::bootstrap::log::force_uart_line_raw(line.as_str());
}
...
fn emit_cyw43_join_policy_trace(
    ...
    pmk: &'static str,
    host_eapol_rx: &'static str,
    connect_policy: &'static str,
    order: &'static str,
) {
    use core::fmt::Write;
    ...
    let _ = write!(
        line,
        "CYW43_DRIVER_TASK_JOIN_POLICY contract={} security={} ssid_len={} psk_kind={} completion_rule={} join_path={} firmware_supplicant={} pmk={} wpaie={} host_eapol_rx={} mpc=0 offloads=disabled connect_policy={} security_iovars=set-var order={}",
        contract.name,
        security,
...
#[cfg(feature = "kernel")]
fn cyw43_submit_bcdc_u32_optional_submit_fault(
    contract: DriverTaskContract,
    cmd: u32,
    value: u32,
    stage: &'static str,
    ...
    #[cfg(test)]
    if CYW43_HOST_EAPOL_TEST_IO_STUB.load(Ordering::Acquire) != 0 {
        let _ = (contract, cmd, value, stage);
        return Ok(());
    }
    let mut payload = [0u8; 4];
    payload.copy_from_slice(&value.to_le_bytes());
    ...
    match cyw43_submit_control_exchange_unmapped_with_options(
        ...
        Err(Cyw43CommandSubmitError::Completion(completion))
            if cyw43_prejoin_scan_timing_submit_fault_is_fail_soft(stage, completion) =>
        {
            clear_cyw43_runtime_command_fault_status();
            crate::hal::driver_task::emit_driver_task_resource_init_status(
                contract,
...
#[cfg(feature = "kernel")]
fn cyw43_prejoin_scan_timing_submit_fault_is_fail_soft(
    stage: &'static str,
    completion: DriverTaskCompletionRecord,
) -> bool {
    if !matches!(
        stage,
        "cyw43-control-prejoin-scan-channel-time" | "cyw43-control-prejoin-scan-unassoc-time"
    ) || completion.code != DriverTaskCompletionCode::Fault.as_u16()
        || completion.detail != CYW43_CONTROL_FRAME_DETAIL
    {
        return false;
    }
    let status = sdio_transfer_failure_status(completion.result);
    let failure_stage = (completion.result >> 24) & 0xff;
    matches!(failure_stage, 3 | 4)
        && status & (SDHCI_INT_ERROR | SDHCI_INT_DATA_TIMEOUT | SDHCI_INT_DATA_CRC) != 0
}
...
fn cyw43_apply_linux_prejoin_association_policy(
    ...
    cyw43_submit_bcdc_u32_optional_submit_fault(
        contract,
...
#[cfg(feature = "kernel")]
fn emit_cyw43_host_eapol_wsec_key_retry(
    contract: DriverTaskContract,
    stage: &'static str,
    fault: Cyw43RuntimeCommandFaultStatus,
) {
...
fn service_cyw43_host_eapol_join_submit_window(
    ...
    if cyw43_host_eapol_needs_post_activity_window(latest) {
        let post_activity_now_ms = crate::hal::timebase().now_ms();
        emit_cyw43_host_eapol_join_submit_window(
            contract,
            "post-activity-begin",
            CYW43_HOST_EAPOL_POST_ACTIVITY_POLLS,
            false,
            latest.progress.as_ref(),
        );
        let post_activity = service_cyw43_host_eapol_slice_with_outcome(
            credentials,
            CYW43_HOST_EAPOL_POST_ACTIVITY_POLLS,
            post_activity_now_ms,
        );
        emit_cyw43_host_eapol_join_submit_window(
            contract,
            "post-activity-end",
...
#[cfg(feature = "kernel")]
fn cyw43_host_eapol_needs_post_activity_window(outcome: Cyw43HostEapolSliceOutcome) -> bool {
    ...
    progress.eapol_rx != 0
        || progress.associated
        || progress.link_up
        || cyw43_host_eapol_valid_bssid_without_carrier(&progress)
}
...
fn service_cyw43_host_eapol_slice_with_outcome(
    ...
    for iteration in 0..poll_limit {
        ...
        if cyw43_host_eapol_join_timeout_expired(session, current_now_ms) {
            ...
            if let Some(reason) =
                cyw43_host_eapol_assoc_rescue_reason(probe, session, current_now_ms)
            {
                let rescue_poll = session.progress.polls as usize;
                let rescue_attempt = session.assoc_probe_attempts;
                if cyw43_try_host_eapol_assoc_rescue(
                    contract,
                    credentials,
                    session,
                    rescue_poll,
...
fn record_cyw43_host_eapol_poll_completion(
    ...
    if !result.observed_frame && !result.activity {
...
fn process_cyw43_host_eapol_control_completion(
        ...
        || completion.code == DriverTaskCompletionCode::Progress.as_u16()
    {
        ...
        result.activity |= cyw43_poll_completion_made_service_progress(completion, trace);
    }
    result
}
                ...
                match action {
                    ...
                    HostEapolAction::SendM2 { len } => {
                        ...
                        let Some(m2_completion) =
                            ...
                            emit_cyw43_host_eapol_message(contract, "m2", "tx-deferred", poll, len);
...
fn cyw43_service_host_eapol_pre_assoc(
    ...
    if cyw43_host_eapol_set_ssid_rescue_due(session) {
        ...
        cyw43_try_host_eapol_assoc_rescue(
            contract,
            credentials,
            session,
            poll,
    ...
    if let Some(reason) = cyw43_host_eapol_assoc_rescue_reason(Some(probe), session, now_ms) {
        cyw43_try_host_eapol_assoc_rescue(contract, credentials, session, poll, attempt, reason);
    }
    true
}
...
#[cfg(feature = "kernel")]
fn cyw43_host_eapol_assoc_rescue_reason(
    probe: Option<Cyw43AssocProbeResult>,
    ...
    if session.progress.associated
        ...
        || !cyw43_host_eapol_assoc_rescue_due_any(
            session.progress.polls,
            session.pre_assoc_idle_ms(now_ms),
        )
    {
        return None;
    }
    match probe {
        Some(Cyw43AssocProbeResult::NotAssociated) => Some("firmware-not-associated-limit"),
        Some(Cyw43AssocProbeResult::BssidObserved)
            if cyw43_host_eapol_valid_bssid_without_carrier(&session.progress) =>
        {
            Some("valid-bssid-no-carrier")
        }
        _ => None,
    }
}
...
#[cfg(feature = "kernel")]
fn cyw43_host_eapol_valid_bssid_without_carrier(progress: &Cyw43HostEapolProgress) -> bool {
    !progress.associated && !progress.link_up && progress.assoc_probe_status == Some("valid-bssid")
}
...
fn cyw43_probe_host_eapol_bssid_before_required(
    ...
    session.assoc_probe_attempts = session.assoc_probe_attempts.saturating_add(1);
    let attempt = session.assoc_probe_attempts;
    Some(cyw43_probe_host_eapol_assoc_state(
        contract,
        station_mac,
        session,
        poll,
...
fn cyw43_try_host_eapol_assoc_rescue(
    ...
    cyw43_try_host_eapol_assoc_rescue_with_reason(
        contract,
        credentials,
        session,
        poll,
...
#[cfg(feature = "kernel")]
fn cyw43_try_host_eapol_assoc_rescue_with_reason(
    contract: DriverTaskContract,
    credentials: WifiCredentials,
    session: &mut Cyw43HostEapolSession,
    poll: usize,
    ...
    match cyw43_submit_linux_bsscfg_join_with_stage(
        ...
        Err(Cyw43CommandSubmitError::Runtime(_)) => {
            emit_cyw43_host_eapol_assoc_rescue(
                contract,
                poll,
                attempt,
...
#[cfg(feature = "kernel")]
fn cyw43_host_eapol_required_reason(progress: &Cyw43HostEapolProgress) -> &'static str {
    ...
    } else if cyw43_host_eapol_valid_bssid_without_carrier(progress) {
...
fn cyw43_poll_control_exchange_reply(
    ...
    while cyw43_poll_deadline_open(&mut deadline) {
        if let Some((completion, reply)) =
            take_cyw43_pending_control_reply_completion(contract, cmd, id)
        {
            let cached_poll = poll.saturating_sub(1);
            emit_cyw43_control_split_reply_trace(
                contract,
                stage,
                "service-cached-matched-reply",
                cached_poll,
                0,
                completion.sequence,
                Some(reply),
        ...
        if cyw43_control_poll_progress_is_advisory(stage, control_iovar, completion) {
            cyw43_record_control_window_advisory_progress(contract, stage, completion);
            continue;
        }
        if completion.code != DriverTaskCompletionCode::FrameReady.as_u16() {
            record_cyw43_control_split_failure(
...
fn submit_cyw43_host_eapol_payload_bounded_completion(
    ...
    if CYW43_HOST_EAPOL_TEST_IO_STUB.load(Ordering::Acquire) != 0 {
        let _ = contract;
        if cyw43_host_eapol_test_stage_is_m2(stage)
            && CYW43_HOST_EAPOL_TEST_M2_SUBMIT_FAILS.load(Ordering::Acquire) != 0
        {
            CYW43_TX_DROPPED.fetch_add(1, Ordering::AcqRel);
            return None;
        }
        if cyw43_host_eapol_test_stage_is_m4(stage) {
            let ptk_count = CYW43_HOST_EAPOL_TEST_PTK_INSTALLED.load(Ordering::Acquire);
            let gtk_count = CYW43_HOST_EAPOL_TEST_GTK_INSTALLED.load(Ordering::Acquire);
            let wsec_count = CYW43_HOST_EAPOL_TEST_WSEC_REASSERTED.load(Ordering::Acquire);
            CYW43_HOST_EAPOL_TEST_M4_SUBMIT_PTK_COUNT.store(ptk_count, Ordering::Release);
            CYW43_HOST_EAPOL_TEST_M4_SUBMIT_GTK_COUNT.store(gtk_count, Ordering::Release);
            CYW43_HOST_EAPOL_TEST_M4_SUBMIT_WSEC_COUNT.store(wsec_count, Ordering::Release);
            if ptk_count == 0 {
...
fn receive_cyw43_driver_task_frame(contract: DriverTaskContract) -> Option<DriverTaskNetRxToken> {
    for _ in 0..CYW43_PENDING_RX_QUEUE_CAP {
        if let Some((flags, token)) = take_cyw43_pending_rx_token() {
            ...
            drain_cyw43_post_secure_pending_eapol(contract, "pending-scavenge");
            return Some(token);
        }
        if let Some((flags, token)) = resume_cyw43_active_prompt_poll_for_data_path(contract) {
            ...
            submit_cyw43_arp_assist_if_needed(contract, &token.buffer[..token.len]);
            drain_cyw43_post_secure_pending_eapol(contract, "resume-scavenge");
            return Some(token);
        }
        ...
        submit_cyw43_arp_assist_if_needed(contract, &token.buffer[..token.len]);
        drain_cyw43_post_secure_pending_eapol(contract, "poll-scavenge");
        return Some(token);
    }
    None
}
...
#[cfg(feature = "kernel")]
fn drain_cyw43_post_secure_pending_eapol(contract: DriverTaskContract, action: &'static str) {
    if contract != CYW43_WIFI_DRIVER_TASK_CONTRACT || !cyw43_secure_carrier_ready() {
        return;
    }
    let original_len = CYW43_PENDING_RX_QUEUE.lock().len();
    for _ in 0..original_len {
        let Some(pending) = CYW43_PENDING_RX_QUEUE.lock().pop_front() else {
            break;
        };
        if consume_cyw43_post_secure_eapol_frame(contract, pending.flags, &pending.token, action) {
            continue;
        }
            ...
            if frame_channel == DRIVER_RUNTIME_CYW43_FRAME_FLAG_CHANNEL_DATA {
                if cyw43_pre_secure_host_eapol_frame(flags, &token) {
                    emit_cyw43_data_path_trace(
                        contract,
                        "rx-host-eapol",
                        "tx-retry-control-fallback-consume",
                        0,
                        &token.buffer[..token.len],
                        None,
                        flags,
                ...
                let _ = store_cyw43_pending_control_reply(flags, token, 0);
                return true;
            } else if frame_channel == DRIVER_RUNTIME_CYW43_FRAME_FLAG_CHANNEL_EVENT {
                return cyw43_capture_event_frame_from_token(
                    contract,
                    "tx-retry-control-fallback",
                    flags,
                    &token,
...
mod tests {
    ...
    fn reset_cyw43_status_flags() {
        ...
        CYW43_HOST_EAPOL_TEST_M2_SUBMIT_FAILS.store(0, Ordering::Release);
        ...
        CYW43_HOST_EAPOL_TEST_M4_SUBMIT_PTK_COUNT.store(0, Ordering::Release);
        CYW43_HOST_EAPOL_TEST_M4_SUBMIT_GTK_COUNT.store(0, Ordering::Release);
        CYW43_HOST_EAPOL_TEST_M4_SUBMIT_WSEC_COUNT.store(0, Ordering::Release);
    ...
    fn cyw43_firmware_supplicant_probe_payloads_match_old_good_shape() {
        assert_eq!(
            Cyw43FirmwareSupplicantProbe::Enabled.label(),
            "sup_wpa-enabled-firmware-eapol"
        );
        assert_eq!(
    ...
    #[test]
    fn host_eapol_wsec_key_treats_poll_progress_as_advisory() {
    ...
    #[cfg(feature = "kernel")]
    #[test]
    fn host_eapol_m2_tx_submit_failure_is_advisory() {
        let _guard = CYW43_STATUS_TEST_LOCK
            .lock()
            .expect("cyw43 replay tests must serialize");
        reset_cyw43_status_flags();
        ...
        *CYW43_RUNTIME_MAC.lock() = EthernetAddress(station);
        ...
        CYW43_HOST_EAPOL_TEST_M2_SUBMIT_FAILS.store(1, Ordering::Release);
        let mut m1 = [0u8; MAX_FRAME_LEN];
        let m1_len =
            cyw43_host_eapol::write_test_m1_frame(&mut m1, &station, &ap).expect("test m1 frame");
        let first_result = process_cyw43_host_eapol_data_completion(
            CYW43_WIFI_DRIVER_TASK_CONTRACT,
            EthernetAddress(station),
            &mut session,
            8193,
        ...
        .expect("M2 submit failure must not tear down host-EAPOL state");

        assert!(first_result.observed_frame);
        assert!(first_result.activity);
        assert!(!first_result.secure);
        assert_eq!(session.progress.eapol_error, None);
        ...
        CYW43_HOST_EAPOL_TEST_M2_SUBMIT_FAILS.store(0, Ordering::Release);
        let second_result = process_cyw43_host_eapol_data_completion(
            CYW43_WIFI_DRIVER_TASK_CONTRACT,
            EthernetAddress(station),
            &mut session,
            8194,
        ...
        .expect("next M1 retransmit should still produce M2");

        assert!(second_result.observed_frame);
    ...
    #[cfg(feature = "kernel")]
    #[test]
    fn host_eapol_m4_tx_drain_timeout_is_advisory_after_key_install() {
        let _guard = CYW43_STATUS_TEST_LOCK
            .lock()
            .expect("cyw43 replay tests must serialize");
        reset_cyw43_status_flags();
        ...
        *CYW43_RUNTIME_MAC.lock() = EthernetAddress(station);
        ...
        .expect("M4 drain timeout after key install must not block secure release");
    ...
    #[cfg(feature = "kernel")]
    #[test]
    fn host_eapol_m4_submit_failure_after_key_install_stays_fatal() {
        let _guard = CYW43_STATUS_TEST_LOCK
            .lock()
            .expect("cyw43 replay tests must serialize");
        reset_cyw43_status_flags();
        ...
        *CYW43_RUNTIME_MAC.lock() = EthernetAddress(station);
        ...
        CYW43_HOST_EAPOL_TEST_M4_SUBMIT_FAILS.store(1, Ordering::Release);
        let mut m3 = [0u8; MAX_FRAME_LEN];
        let m3_len = cyw43_host_eapol::write_test_m3_frame(&mut m3, &station, &session.eapol)
            .expect("test m3 frame");
        ...
        .expect_err("M4 submit failure after key install must stay fatal");

        assert_eq!(err, DriverTaskNetError::RuntimeInit("host-eapol-m4-tx"));
        assert_eq!(session.progress.eapol_error, Some("host-eapol-m4-tx"));
        ...
        assert_eq!(
            CYW43_HOST_EAPOL_TEST_M4_SUBMIT_BEFORE_GTK.load(Ordering::Acquire),
            0
        );
        assert_eq!(
            CYW43_HOST_EAPOL_TEST_M4_SUBMIT_BEFORE_WSEC.load(Ordering::Acquire),
            0
        );
        assert_eq!(
            CYW43_HOST_EAPOL_TEST_SCB_AUTHORIZED.load(Ordering::Acquire),
    ...
    fn host_eapol_start_cadence_is_bounded() {
        ...
        assert_eq!(
            CYW43_HOST_EAPOL_POST_ACTIVITY_POLLS,
            CYW43_HOST_EAPOL_POST_ASSOC_POLLS
        );
    ...
    #[cfg(feature = "kernel")]
    #[test]
    fn host_eapol_post_activity_window_tracks_incomplete_handshake() {
        let _guard = CYW43_STATUS_TEST_LOCK.lock().expect("status test lock");
        reset_cyw43_status_flags();

        assert!(!cyw43_host_eapol_needs_post_activity_window(
            Cyw43HostEapolSliceOutcome::default()
        ));
    ...
    #[cfg(feature = "kernel")]
    #[test]
    fn host_eapol_valid_bssid_without_carrier_requests_one_rejoin_rescue() {
        let _guard = CYW43_STATUS_TEST_LOCK.lock().expect("status test lock");
        reset_cyw43_status_flags();
        CYW43_PRIMARY_BSSCFG_JOIN_READY.store(1, Ordering::Release);
        ...
        session.progress.polls = CYW43_HOST_EAPOL_ASSOC_RESCUE_POLL;
        session.progress.empty_polls = CYW43_HOST_EAPOL_ASSOC_RESCUE_POLL;
        session.progress.record_assoc_probe("valid-bssid", 0);

        assert!(cyw43_host_eapol_valid_bssid_without_carrier(
            &session.progress
        ));
        assert_eq!(
            cyw43_host_eapol_assoc_rescue_reason(
                Some(Cyw43AssocProbeResult::BssidObserved),
                &session,
                0
            ),
    ...
    fn cyw43_data_tx_shape_pads_short_frames_to_data_tx_floor() {
        ...
        assert_eq!(
            arp_total_len,
            CYW43_SDPCM_DATA_TX_OVERHEAD_BYTES + arp_reply.len()
        );
        ...
        assert_eq!(
            tcp_total_len,
            CYW43_SDPCM_DATA_TX_OVERHEAD_BYTES + tcp_frame.len()
        );
        assert_eq!(
            cyw43_data_tx_request_len(tcp_total_len),
            align4(tcp_total_len)
        );
    ...
    fn host_eapol_firstread_decodes_rx_idle_trace() {
        let mut frame = [0u8; CYW43_RX_IDLE_TRACE_V7_BYTES];
    ...
    #[cfg(feature = "kernel")]
    #[test]
    fn prejoin_scan_timing_submit_fault_is_fail_soft_only_for_scan_tuning() {
        ...
        let descriptor_fault = DriverTaskCompletionRecord {
            detail: CYW43_SDIO_DESCRIPTOR_TRANSFER_FAILED_DETAIL,
            ..transfer_failed
        };
        let non_fault = DriverTaskCompletionRecord {
            code: DriverTaskCompletionCode::Progress.as_u16(),
            ..transfer_failed
        };

        assert!(cyw43_prejoin_scan_timing_submit_fault_is_fail_soft(
            "cyw43-control-prejoin-scan-channel-time",
            transfer_failed
        ));
        assert!(cyw43_prejoin_scan_timing_submit_fault_is_fail_soft(
        ...
        assert!(!cyw43_prejoin_scan_timing_submit_fault_is_fail_soft(
            "cyw43-control-prejoin-event-mask",
            transfer_failed
        ));
        assert!(!cyw43_prejoin_scan_timing_submit_fault_is_fail_soft(
            "cyw43-control-prejoin-scan-channel-time",
    ...
    fn host_eapol_wsec_key_uses_oldgood_control_reply_window() {
        ...
        assert_eq!(CYW43_HOST_EAPOL_WSEC_KEY_STALE_REPLY_RETRIES, 3);
        ...
        assert_eq!(
            cyw43_host_eapol_tx_drain_window("m4-after-wsec"),
            (
                CYW43_HOST_EAPOL_TX_DRAIN_TIMEOUT_MS,
                CYW43_HOST_EAPOL_TX_DRAIN_POLLS
            )
    ...
    #[cfg(feature = "kernel")]
    #[test]
    fn host_eapol_wsec_key_retries_after_stale_nonmatching_reply() {
        test_clear_cyw43_runtime_replay_status();
        ...
        let fault = cyw43_host_eapol_wsec_key_retry_fault("cyw43-host-eapol-ptk")
            .expect("stale nonmatching PTK wsec_key fault should be retried once");
    ...
    fn cyw43_runtime_idle_trace_updates_hardware_netstats() {
        ...
        let mut trace = [0u8; CYW43_RX_IDLE_TRACE_V7_BYTES];
    ...
    fn cyw43_prepoll_pre_secure_eapol_routes_to_host_eapol_not_pending_rx() {
        ...
        *CYW43_HOST_EAPOL_SESSION.lock() =
        ...
        let guard = CYW43_HOST_EAPOL_SESSION.lock();
        let session = guard.as_ref().expect("session remains active after M2");
        assert_eq!(session.progress.eapol_rx, 1);
    ...
    #[cfg(feature = "kernel")]
    #[test]
    fn cyw43_prepoll_pre_secure_event_routes_to_host_eapol_not_pending_rx() {
        let _lock = CYW43_STATUS_TEST_LOCK
            .lock()
            .expect("cyw43 status test lock");
        reset_cyw43_status_flags();
        ...
        *CYW43_HOST_EAPOL_SESSION.lock() =
        ...
        assert_eq!(CYW43_ASSOCIATED.load(Ordering::Acquire), 1);
        let guard = CYW43_HOST_EAPOL_SESSION.lock();
        let session = guard.as_ref().expect("session remains active after event");
        assert_eq!(session.progress.event_rx, 1);
        assert_eq!(session.progress.data_rx, 0);
        assert_eq!(session.progress.association_event, Some("assoc"));
    ...
    #[cfg(feature = "kernel")]
    #[test]
    fn cyw43_pending_rx_delivery_prefers_high_priority_frame() {
        let _lock = CYW43_STATUS_TEST_LOCK
            .lock()
            .expect("cyw43 status test lock");
        reset_cyw43_status_flags();
        ...
        let (_, token) =
            take_cyw43_pending_rx_token().expect("high-priority pending frame is available");
        token.consume(|bytes| assert_eq!(bytes, &tcp));
        reset_cyw43_status_flags();
    }
