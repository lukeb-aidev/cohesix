...
use cas_tool::{
    build_bundle, load_delta_base, load_signing_key, load_template_config, load_upload_manifest,
    write_bundle,
};
...
struct PackArgs {
    ...
    template: PathBuf,
    /// Confirm chunk_bytes from the selected template; differing values are rejected.
    #[arg(long)]
...
fn pack_bundle(args: PackArgs) -> Result<()> {
    ...
    let template = load_template_config(&args.template)
        .with_context(|| format!("load cas template {}", args.template.display()))?;
    if let Some(chunk_bytes) = args.chunk_bytes {
        if chunk_bytes != template.chunk_bytes {
            anyhow::bail!(
                "--chunk-bytes {} does not match selected template chunk_bytes {}",
                chunk_bytes,
                template.chunk_bytes
            );
        }
    }
...
fn upload_bundle(args: UploadArgs) -> Result<()> {
    let (manifest, manifest_bytes, chunks_dir) = load_upload_manifest(&args.bundle)?;
