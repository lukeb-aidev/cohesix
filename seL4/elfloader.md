...
(EFI only) Elfloader is entered at _gnuefi_start entry point.
(EFI only) Elfloader relocates itself
Elfloader _start is called. This is in arch-arm/<arch_bitness>/crt0.S.
...
The elfloader sets up the initial page tables for the kernel (see init_hyp_boot_vspace or init_boot_vspace).
...
### Root Task Demo
See a Rust Root Task example here:
(https://github.com/seL4/rust-root-task-demo)