...
- Current USB proof refinement: the high-BAR Pi 4 command proof publishes `DCBAAP` then `CRCR` before event-ring registers on the fresh platform-reset lane, starts with `USBCMD.RUN`, rings doorbell 0, then proves completion only by polling the event ring for the U-Boot 5 s command-event budget while PCIe INTx/MSI/MSI-X delivery remains masked. With no current-boot port-status event it submits No Op before any slot allocation. With current-boot port-status events it preserves those PSC events until after the command doorbell, seeds the Linux-captured command-event state (`USBCMD.RUN|INTE`, `DNCTRL=2`, `IMOD=0xa0`, `IMAN.IE`), submits bounded Enable Slot as the first real command, then skips and HAL-flushes PSC acknowledgements inside the U-Boot-shaped command wait before accepting only the matching completion. On success it immediately disables the slot before live root-port sampling is reopened. A timeout is reported as `command_probe=enable-slot-timeout` / `USB_BLOCKER=cmd-event-ring-timeout` unless the RUN store preserved stale controller command bits, in which case the normalizer reports `USB_BLOCKER=usbcmd-run-preserved-reset-bit`; neither blocker authorizes Linux-captured root-port enumeration. Deferred capture accepts completed command proof labels including `no-op-ok`, `enable-slot-ok`, and cleanup-failed variants.
...
- Current USB proof refinement: the prompt-safe command proof performs bounded event-ring CPU-sync polls after doorbell 0; the Linux-shaped PSC lane also performs one bounded doorbell replay and one fresh ring recovery before returning to the shell. The HAL posted-write flush for doorbell 0 and later endpoint doorbells uses BCM2711 EXT_CFG readback only and logs `bar_drain=skipped reason=pi4-run-doorbell-xhci-read-toxic`; failed endpoint-doorbell drains now surface as `PostedWriteFlushFailed` instead of later transfer timeouts. Address Device uses U-Boot's full-speed EP0 ordering (`64` first, `8` fallback), and SET_CONFIGURATION is sent only after the active configuration endpoint contexts are programmed with xHCI Configure Endpoint. PCIe INTx/MSI/MSI-X delivery remains masked, and IRQ27 remains the seL4 virtual timer, never USB progress.
...
- Pi 4 local-seat Wi-Fi boots keep the serial prompt last. Root-task runs the bounded USB pre-net probe, defers early kernel Wi-Fi net-console init with `action=root-console-wait-for-wifi`, resumes CYW43455 before the prompt with `[net-console] deferred resume reason=pre-root-console action=start-wifi`, and publishes `cohesix>` only after Wi-Fi association/addressing succeeds, terminally fails, or reaches the bounded pre-root timeout. A trace that stops at `wifi-net-console-pending-before-root-console` remains blocked and the proof loop reports `WIFI_BLOCKER=boot-waiting-for-wifi`.
