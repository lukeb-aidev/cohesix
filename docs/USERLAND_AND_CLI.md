...
- PMUCONTROL recovery uses one Linux-shaped Function 1 CMD52 word lane: diagnostics label its four ordered byte reads/writes `probe-pmucontrol-cmd52-word-primary` with `retry=none`, and never imply a CMD53 fallback. A post-release HT timeout that occurs before Function 2 is enabled is reported at Gate 7 without synthesizing the `7a join-submit` sub-gate; its next action is a clean full-generation recovery that preserves the conservative upload lane through ARM release and promotes the operational lane exactly once before the pure HT request.
