...
- Current USB proof refinement: the high-BAR Pi 4 command proof programs Linux-shaped command-event state (`USBCMD.RUN|INTE`, `IMOD=0xa0`, `IMAN.IE`) while keeping PCIe INTx/MSI delivery masked, then proves completion only by polling the event ring. The no-candidate path submits a prompt-safe No Op before any slot allocation; the current-boot port-event path submits Enable Slot through the same bounded/re-doorbell/recovery lane. A timeout is reported as `command_probe=no-op-unproven` or `command_probe=enable-slot-timeout` / `USB_BLOCKER=cmd-event-ring-timeout` unless the RUN store preserved stale controller command bits, in which case the normalizer reports `USB_BLOCKER=usbcmd-run-preserved-reset-bit`; neither blocker authorizes Linux-captured root-port enumeration. Deferred capture accepts completed poll-only command proof labels including `no-op-ok`, `enable-slot-ok`, and cleanup-failed variants; historical Linux-event labels are diagnostic only.
