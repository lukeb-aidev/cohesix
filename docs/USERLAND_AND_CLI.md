...
- HDMI status now separates USB keyboard detection, command-input readiness, and first-byte evidence. Gate 8 enumeration still proves keyboard discovery, and Gate 10 still proves first-byte delivery, but first-byte proof is diagnostic evidence rather than the action that unlocks the console.
...
- On Pi 4 U-Boot profiles, authenticated reboot best-effort marks one-shot BCM2711 PM RSTS fast-boot state before arming reset, using the existing Cohesix high marker. Root-task orders and drains the marker write before arming reset, but live Pi 4 evidence shows PM RSTS high-marker retention is not reliable enough to be the only reboot handoff. The generated `boot.cmd` forces serial input before loading Cohesix policy or checking reboot state, prefers the high marker when present, and otherwise accepts the hardware-observed software-reset bit only when a saved Cohesix policy was loaded from `cohesix.env`; accepted sources boot saved policy or manifest defaults without showing the U-Boot wizard. Cold boots without a marker or software-reset saved-policy source still enter the splash/menu path, where the default first action continues with saved policy or manifest defaults.
- If no accepted reboot source is visible to U-Boot, the generated script emits one bounded `fast boot marker absent` line with the observed RSTS/high/reset marker values and whether saved policy was loaded before entering the normal cold-boot menu. This diagnostic does not bypass the menu and does not create persistent boot policy.
...
- `netstats` emits a policy line: `mode=<off|static|dhcp> policy=<wired|wifi|auto> active=<iface> standby=<iface|none> addr_src=<source> ip=<ipv4> gateway=<ipv4> dhcp=<phase>`, and `netstatus` includes `tcp_ready=<yes|no>` so DHCP-bound evidence is not confused with listener or authenticated-shell proof.
