...
- Local-seat HDMI echoes USB keyboard input at the same parser-ingress point as the serial root console. After root-console handoff, the event pump flushes serial echo before runtime Wi-Fi work, drains USB keyboard input before and after runtime work, settles across bounded idle HID polls, and yields only not-ready `wifi-host-eapol-pending` / `wifi-host-eapol-required` runtime work to active serial or USB keyboard input. Ready network polling still runs on keyboard turns, while deferred EAPOL remains limited to one runtime Wi-Fi poll per event turn so it cannot hide typed characters on HDMI. HID polling drains a burst of interrupt-IN reports in one pass so press/release/next-key sequences are requeued before the next event-loop turn. HDMI boot/progress lines are rate-limited to a 5-10 s visible cadence while Wi-Fi progress remains suppressed during the USB boot activity window.
