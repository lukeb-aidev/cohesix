...
- Current USB proof refinement: the high-BAR Pi 4 no-candidate path now submits a Linux-shaped prompt-safe No Op command before any slot allocation. It keeps PCIe INTx/MSI delivery masked, programs the Linux-shaped command-event state (`USBCMD.RUN|INTE`, `IMOD=0xa0`, `IMAN.IE`), and proves completion only by polling the event ring. A timeout is reported as `command_probe=no-op-unproven` / `USB_BLOCKER=cmd-event-ring-timeout` unless the RUN store preserved stale controller command bits, in which case the normalizer reports `USB_BLOCKER=usbcmd-run-preserved-reset-bit`; neither blocker authorizes Linux-captured root-port enumeration. Deferred capture accepts completed poll-only command proof labels including `no-op-ok`, `enable-slot-ok`, and cleanup-failed variants; historical Linux-event labels are diagnostic only.
