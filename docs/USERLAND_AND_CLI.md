...
- Local-seat HDMI echoes USB keyboard input at the same parser-ingress point as the serial root console once the linked display runtime is proven attached. On the physical Pi 4 owner-state path, the HDMI framebuffer descriptor is published before driver-task bootstrap, the `hdmi-text` linked runtime receives a bounded nonblocking engine-init retry after descriptor load, and the early `Starting HDMI` frame is a bounded nonblocking render proof that cannot hold the UART prompt hostage. Before the first normal `cohesix>` prompt is published, root-task runs the deferred Wi-Fi replay path and one bounded local-seat prompt-settle attempt for HDMI/USB visibility; failures are logged as compact fail-closed blockers, not as prompt suppression. HDMI no longer waits for USB keyboard attach or an unproved local-seat dependency; serial remains the authority console if HDMI engine init or framebuffer proof is unavailable. If linked HDMI does not reply, the HAL-mapped firmware framebuffer may be used as a diagnostic-only mirror with `acceptance=red`; visible text from that path is not driver-task owner-state proof. USB keyboard runtime attach gets one bounded prompt-settle attempt before `cohesix>` and remains bounded after the prompt: local-seat first replays the deferred PCIe root descriptor, then submits one bounded nonblocking USB engine-init call; ongoing keyboard polls remain nonblocking and, on no reply, leave `[local-seat] runtime keyboard poll suspended reason=driver-task-no-reply action=serial-shell` instead of holding serial behind repeated local-seat calls. Root-console startup emits raw UART `[mark] root-console.start.*` breadcrumbs only when the normal prompt is about to be published; deferred Wi-Fi replay and verbose driver diagnostics stay on the serial UART before that prompt so missing `/log/queen.log` or 9P access cannot hide the debug stream. After root-console handoff, serial and USB keyboard input remain concurrent physical console sources feeding the same parser when USB proof succeeds: the event pump flushes serial echo before runtime Wi-Fi work, drains USB keyboard input before and after runtime work once keyboard polling has been enabled, and yields both not-ready `wifi-host-eapol-pending` / `wifi-host-eapol-required` runtime work and ready NIC data polling to active serial input, active USB keyboard input, or pending serial output. Deferred EAPOL remains limited to one runtime Wi-Fi poll per event turn so it cannot hide typed characters on HDMI, and ready DHCP/UDP/TCP work is capped by fixed per-poll quanta so a live Wi-Fi connection cannot drain an arbitrary backlog in one compatibility turn. HID boot keyboards are programmed with idle duration `0` so the xHCI event ring is reserved for change reports instead of periodic empty completions while Wi-Fi is active; HID polling still drains a bounded burst of interrupt-IN reports in one pass so press/release/next-key sequences are requeued before the next event-loop turn. HDMI boot/progress lines are rate-limited to a 5-10 s visible cadence after display proof.
