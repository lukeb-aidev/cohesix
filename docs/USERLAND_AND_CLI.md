...
- Pi 4 local-seat Wi-Fi boots keep the serial prompt after the bounded Wi-Fi pre-root proof, but they do not wait for an unbounded WPA/EAPOL exchange. Root-task runs the bounded USB pre-net probe, defers early kernel Wi-Fi net-console init with `action=root-console-wait-for-wifi`, resumes CYW43455 before the prompt with `[net-console] deferred resume reason=pre-root-console action=start-wifi`, and publishes `cohesix>` after Wi-Fi association/addressing succeeds, terminally fails, reaches `wifi-host-eapol-pending`, or reaches the bounded pre-root timeout. `wifi-host-eapol-pending` keeps DHCP/data blocked while enabling prompt-side `wifi diag` and `nettest`; pre-root polling still uses bounded host-EAPOL bursts, but post-root pending EAPOL polling yields to active physical input while ready network polling still runs. A trace that stops at `wifi-net-console-pending-before-root-console` remains blocked and the proof loop reports `WIFI_BLOCKER=boot-waiting-for-wifi`.
