...
- `usb <help|status|dump-state|diag|enable-kbd|probe-kbd>` – Pi 4 USB local-seat diagnostics on the serial/local diagnostics console only, including when HDMI has degraded to headless mode. `usb status` and `usb probe-kbd` surface a compact `golden_path ...` route summary before the existing `verdict=... focus=...` line. `usb probe-kbd` now emits a four-line `golden_path preflight ...` / `irq_contract preflight ...` block before xHCI MMIO starts: the chosen route/origin/handoff/seed/halt-guard, the first expected diagnostic edge plus immediate follow-up step, the planned happy-path shape (`ctor=... pre=... legacy=... run=... publish=... post_ready=...`), and the active poll-only IRQ contract. The same commands now also emit `usb: contract ...`, `usb: diag_contract ...`, and `usb: diag_values ...` lines summarising the active step, decisive blocker, freshness, and the latest xHCI diagnostic tuple. Stop-seeded controller-ready paths may report `event_candidate_mask=...` plus `command_probe=enable-slot-ok` for a candidate-backed slot proof or `command_probe=no-op-ok` for a zero-event command-ring No Op proof; the No Op path republishes the late command TRB with `xhci-cmd-ring-submit`, snapshots `cmd-ring-after-enqueue-*`, snapshots `cmd-event-ring-before-*`, emits software-only `cmd-gate-pre-doorbell-plan-*` tuples with `USBCMD.INTE=0` / `IMAN.IE=0` planned evidence, rings doorbell 0 under `cmd-doorbell-write` / `cmd-doorbell-write-done`, skips the toxic immediate doorbell readback on the `platform-reset-complete` lane, emits software-only `cmd-gate-post-doorbell-plan-*` tuples, and synchronizes the event-ring TRB before poll reads, then emits `cmd-ring-timeout-*`, `cmd-event-ring-timeout-*`, and `cmd-gate-timeout-plan-*` if the command completion is still absent so `event_syncs` counts real event-ring CPU-sync polls. These gate-plan labels intentionally avoid live xHCI register reads on the critical No Op proof path. The stop-seeded `platform-reset-complete` lane now treats both command probing and `PORTSC` sampling as diagnostic-only after controller-ready, records deferred root-port state, and returns to the shell instead of taking the proven IRQ27-toxic live root-port read. Other stop-seeded lanes still require a stronger ownership step before root-port enumeration. `irq27`/`IRQ 27` is the seL4 virtual-timer PPI on Pi 4 and is never reported as a USB/xHCI interrupt source. That leaves one compact transcript block showing whether runtime is about to take the full U-Boot-style fresh-init path or a later trusted fallback even if the board halts immediately afterward.
- Current USB proof refinement: a prompt-safe No Op timeout now keeps polling the same command TRB after the first prompt-safe event-ring sync instead of queuing a second No Op or enabling Linux-shaped event generation. A timeout is reported as `command_probe=no-op-unproven` and does not authorize Linux-captured root-port enumeration; deferred capture requires `enable-slot-ok` or `no-op-ok`, with `no-op-linux-event-ok` retained only as a historical diagnostic label.
- `wifi <help|dump-state|probe-ht|diag|load-fw|retry>` – Pi 4 WiFi bring-up diagnostics on the serial/local diagnostics console only. `wifi dump-state` now emits a compact `golden_path ...` route summary before the existing `verdict=... focus=...` line so operators can see the strict HT/IOR2 production gate, the current step, and the next expected control-plane edge. It now also emits a single `wifi: contract ...` line that pairs the current control-plane step with the expected edge, the observed low-level state, and the blocker class, plus firmware/HT contract lines (`wifi: firmware_contract ...`, `wifi: firmware_ht_req ...`, `wifi: firmware_ht_state ...`), SDHCI evidence lines (`wifi: sdhci_contract ...`, `wifi: sdhci_live ...`, `wifi: sdhci_preserved ...`), and raw control-plane evidence (`wifi: cccr ...`, `wifi: sdio_shadow ...`, `wifi: preserved_failure ...`). Together they expose current vs preserved SDHCI transaction evidence (`cmd`, `arg`, `present-state`, `int-status`), firmware reset-vector and upload verification state, ALP/HT request values, observed `CHIPCLKCSR`, F1/F2 state, CCCR/F1 register state, backplane window bytes, and the strongest cached failure without overrunning the fixed shell line budget. A bounded `wifi diag` / `probe-ht` pass may add `diagnostic-force-ht-*` lines after the exact strict `0x50` HT timeout shape, but that probe is diagnostic-only and does not authorize production Function 2. These commands do not extend the shared TCP/`cohsh` grammar; TCP clients still reject raw `wifi ...` lines as parse errors.
