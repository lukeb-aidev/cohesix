...
- Before the isolated USB keyboard path can capture input, HDMI shows the one-shot `USB console starting...` line instead of asking for a key. The pre-input `System starting; press any key to start the USB console` line is one-shot only after keyboard enumeration/HID polling is ready and before first-byte proof; pre-ready arming and settle keys are consumed rather than parsed as commands. Once command-ready is logged, the prompt remains visible through soft post-first-byte settle churn, and later USB recovery/no-reply pressure uses the busy/deferred warning instead of a recovering banner or a second ready banner.
