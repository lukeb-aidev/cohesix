...
- Local-seat HDMI echoes USB keyboard input at the same parser-ingress point as the serial root console once the linked display runtime is proven attached. On the physical Pi 4 owner-state path, the HDMI framebuffer descriptor is published before driver-task bootstrap, the `hdmi-text` linked runtime receives a bounded nonblocking engine-init retry after descriptor load, and the early `Starting HDMI` frame is a bounded nonblocking render proof that cannot hold the UART prompt hostage. The first normal `cohesix>` prompt is published before deferred Wi-Fi replay and before USB prompt-settle attach; those paths then run as bounded, UART-visible work and report compact fail-closed blockers on failure. HDMI no longer waits for USB keyboard attach or an unproved local-seat dependency; serial remains the authority console if HDMI engine init or framebuffer proof is unavailable. If linked HDMI does not reply, the HAL-mapped firmware framebuffer may be used as a diagnostic-only mirror with `acceptance=red`; visible text from that path is not driver-task owner-state proof. USB keyboard runtime attach is prompt-side: local-seat first replays the deferred PCIe root descriptor, then submits one bounded nonblocking USB engine-init call; ongoing keyboard polls remain nonblocking and, on no reply, leave `[local-seat] runtime keyboard poll suspended reason=driver-task-no-reply action=serial-shell` instead of holding serial behind repeated local-seat calls. Root-console startup emits raw UART `[mark] root-console.start.*` breadcrumbs when the prompt is published; deferred Wi-Fi replay and verbose driver diagnostics remain on the serial UART after that prompt so missing `/log/queen.log` or 9P access cannot hide the debug stream. After root-console handoff, serial and USB keyboard input remain concurrent physical console sources feeding the same parser when USB proof succeeds: the event pump flushes serial echo before runtime Wi-Fi work, drains USB keyboard input before and after runtime work once keyboard polling has been enabled, and yields both not-ready `wifi-host-eapol-pending` / `wifi-host-eapol-required` runtime work and ready NIC data polling to active serial input, active USB keyboard input, or pending serial output. Deferred EAPOL remains limited to one runtime Wi-Fi poll per event turn so it cannot hide typed characters on HDMI, and ready DHCP/UDP/TCP work is capped by fixed per-poll quanta so a live Wi-Fi connection cannot drain an arbitrary backlog in one compatibility turn. HID boot keyboards are programmed with idle duration `0` so the xHCI event ring is reserved for change reports instead of periodic empty completions while Wi-Fi is active; HID polling still drains a bounded burst of interrupt-IN reports in one pass so press/release/next-key sequences are requeued before the next event-loop turn. HDMI boot/progress lines are rate-limited to a 5-10 s visible cadence after display proof.
...
- Pi 4 local-seat Wi-Fi boots publish the normal serial prompt before the deferred driver attempt. Root-task runs the bounded USB pre-net probe, defers early kernel Wi-Fi net-console init with `action=root-console-wait-for-wifi`, records `[net-console] deferred resume skipped reason=driver-task-net-runtime-unproved action=root-prompt-first` whenever physical-Pi driver-task proof prevents the earliest pre-root resume, then prints `cohesix>` before running bounded SDIO bus-owner and CYW43 Wi-Fi descriptor replay. The TCP console attaches only if both linked runtimes return proof; otherwise the boot leaves a compact error and keeps serial authoritative. Prompt-side `wifi diag` and `nettest` report that boot-attempt state without depending on hidden services, while DHCP/data stay blocked until SDIO plus network runtime proof succeeds. A trace that stops at `wifi-net-console-pending-before-root-console` remains blocked and the proof loop reports `WIFI_BLOCKER=boot-waiting-for-wifi`.
