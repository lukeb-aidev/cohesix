...
- PMUCONTROL recovery uses one Linux-shaped Function 1 CMD52 word lane: diagnostics label its four ordered byte reads/writes `probe-pmucontrol-cmd52-word-primary` with `retry=none`, and never imply a CMD53 fallback. A post-release HT timeout that occurs before Function 2 is enabled is reported at Gate 7 without synthesizing the `7a join-submit` sub-gate; its next action is `verify-card-reset-and-reenumeration-before-generation-reload`. That recovery must show the SDIO owner clearing `IENx`, aborting Function 2, asserting CCCR `ABORT.RES`, retaining the old poisoned epoch while emitting fresh CMD0/CMD5/CMD3/CMD7 plus Function 1/backplane transport progress, and only then accepting the typed generation commit that installs a clean new epoch before a second firmware upload. The conservative upload lane remains continuous through ARM release and promotes exactly once before the pure HT request.
...
- `policy.sha256`: `a5f50ef44c7c8b2bf9713d0916a381b952df6c3a4e084c4afd1116aaceb44427`
