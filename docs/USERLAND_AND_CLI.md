...
- On Pi 4 U-Boot profiles, authenticated reboot marks and reads back a one-shot BCM2711 PM RSTS fast-boot flag before arming reset. The generated `boot.cmd` loads saved `cohesix.env` policy, clears that flag, and boots those settings without showing the U-Boot wizard; absent saved policy, it boots manifest defaults. The staged Pi 4 script also defaults to unattended saved/manifest boot, with the wizard reserved for explicit `coh_force_menu=1` recovery/configuration.
