...
- Current USB proof refinement: the high-BAR Pi 4 no-candidate path now submits a cold-boot-first `Enable Slot` command on the prompt-safe poll-only lane. It keeps `USBCMD.INTE=0` and interrupter `IMAN.IE=0`, with PCIe INTx/MSI delivery masked, and proves completion only by polling the event ring. A timeout is reported as `command_probe=enable-slot-unproven` / `USB_BLOCKER=cmd-event-ring-timeout` unless the RUN store preserved stale controller command bits, in which case the normalizer reports `USB_BLOCKER=usbcmd-run-preserved-reset-bit`; neither blocker authorizes Linux-captured root-port enumeration. Deferred capture accepts completed poll-only command proof labels including `enable-slot-ok` and cleanup-failed variants because a later Disable Slot failure does not erase the prior Enable Slot completion; historical Linux-event labels are diagnostic only.
- Current USB proof refinement: the Pi 4 USB proof is cold-boot-only. `scripts/pi4_trace_normalize.py` emits `USB_BOOTLOADER_HANDOFF_SEEN=yes` when USB logs contain stop-seed, bootloader-owned, bootloader-authorized, seeded cold-start, preserve-state, `run-uboot`, or `uboot-first` evidence; `scripts/pi4_gate_proof.sh` rejects that by default. A passing USB proof must therefore show `USB_BOOTLOADER_HANDOFF_SEEN=no` and reach the high-BAR path through live HAL PCIe/VL805 ownership, not U-Boot xHCI handoff.
- Current USB proof refinement: on the Pi 4 prompt-safe poll-only lane, the command proof now performs 64 bounded event-ring CPU-sync polls and emits one doorbell-0 replay at poll 4 as `cmd-prompt-safe-poll-retry` if no completion has appeared. The HAL posted-write flush for doorbell 0 uses BCM2711 EXT_CFG readback only and logs `bar_drain=skipped reason=prompt-safe-no-xhci-read`; PCIe INTx/MSI delivery remains masked, and IRQ27 remains the seL4 virtual timer, never USB progress.
