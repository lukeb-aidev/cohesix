...
- Current USB proof refinement: the high-BAR Pi 4 no-candidate path now submits a cold-boot-first `Enable Slot` command on a bounded Linux-shaped event-generation lane: `USBCMD.INTE`, interrupter `IMAN.IE`, and Linux-captured `IMOD=0xa0` are programmed only for event generation, while PCIe INTx/MSI delivery stays masked and completion is still proven by polling the event ring. A timeout is reported as `command_probe=enable-slot-linux-event-unproven` / `USB_BLOCKER=cmd-event-ring-timeout` unless the RUN store preserved stale controller command bits, in which case the normalizer reports `USB_BLOCKER=usbcmd-run-preserved-reset-bit`; neither blocker authorizes Linux-captured root-port enumeration. Deferred capture accepts completed command proof labels including `enable-slot-ok`, `enable-slot-linux-event-ok`, and cleanup-failed variants because a later Disable Slot failure does not erase the prior Enable Slot completion.
...
- Current USB proof refinement: on that prompt-safe Linux-shaped event-generation lane, the command proof now performs 64 bounded event-ring CPU-sync polls and emits one doorbell-0 replay at poll 4 as `cmd-prompt-safe-linux-event-retry` if no completion has appeared. This is still a polling proof with PCIe INTx/MSI masked; IRQ27 remains the seL4 virtual timer and is never classified as USB progress.
