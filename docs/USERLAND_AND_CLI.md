...
- **Local diagnostics seat** on the Pi 4 U-Boot profile family (`pi4-uboot-aarch64` with legacy alias `uefi-aarch64`) using USB keyboard + HDMI text mirror, feeding the same root-console parser and command semantics as PL011.
...
- On `pi4-uboot-aarch64` (legacy alias `uefi-aarch64`), local-seat keyboard input is routed into the same parser and HDMI mirrors the same output lines with bounded truncation and bounded in-place scroll. Kernel debug-syscall text remains serial/UART only.
...
- `usb <help|status|enable-kbd|probe-kbd>` – Pi 4 USB local-seat diagnostics on the serial/local diagnostics console only. `usb status` and `usb probe-kbd` surface a compact `golden_path ...` route summary before the existing `verdict=... focus=...` line. `usb probe-kbd` now emits a four-line `golden_path preflight ...` / `irq_contract preflight ...` block before xHCI MMIO starts: the chosen route/origin/handoff/seed/halt-guard, the first expected diagnostic edge plus immediate follow-up step, the planned happy-path shape (`ctor=... pre=... legacy=... run=... publish=... post_ready=...`), and the active poll-only IRQ contract. The same commands now also emit `usb: contract ...`, `usb: diag_contract ...`, and `usb: diag_values ...` lines summarising the active step, decisive blocker, freshness, and the latest xHCI diagnostic tuple. That leaves one compact transcript block showing whether runtime is about to take the full U-Boot-style fresh-init path or a later trusted fallback even if the board halts immediately afterward.
- `wifi <help|dump-state|probe-ht|load-fw|retry>` – Pi 4 WiFi bring-up diagnostics on the serial/local diagnostics console only. `wifi dump-state` now emits a compact `golden_path ...` route summary before the existing `verdict=... focus=...` line so operators can see whether the active path is still on the strict startup-link route or has already fallen back to bounded no-HT, plus the current step and next expected control-plane edge. It now also emits a single `wifi: contract ...` line that pairs the current control-plane step with the expected edge, the observed low-level state, and the blocker class, plus SDHCI evidence lines (`wifi: sdhci_contract ...`, `wifi: sdhci_live ...`, `wifi: sdhci_preserved ...`) and raw control-plane evidence (`wifi: cccr ...`, `wifi: sdio_shadow ...`, `wifi: preserved_failure ...`). Together they expose current vs preserved SDHCI transaction evidence (`cmd`, `arg`, `present-state`, `int-status`), CCCR/F1 register state, backplane window bytes, and the strongest cached failure without overrunning the fixed shell line budget. That makes one shell capture sufficient to tell whether bring-up is stuck on HT, Function 2 readiness, Function 1 sideband, or a specific failing SDIO transaction. These commands do not extend the shared TCP/`cohsh` grammar; TCP clients still reject raw `wifi ...` lines as parse errors.
