...
- `usb <help|status|enable-kbd|probe-kbd>` – Pi 4 USB local-seat diagnostics on the serial/local diagnostics console only. `usb status` and `usb probe-kbd` surface a compact `golden_path ...` route summary before the existing `verdict=... focus=...` line. `usb probe-kbd` now emits a `golden_path preflight ...` pair before xHCI MMIO starts, including the chosen origin/handoff/seed/halt-guard, the first expected stop-revalidation edge, and the immediate follow-up step, so a hard halt still leaves a route-level breadcrumb in the transcript.
