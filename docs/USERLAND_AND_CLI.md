...
- TCP console dispatch preserves isolated runtime boundaries while copying Linux's immediate-progress rule: after each accepted network-origin command in a bounded receive burst, root runs a bounded TCP response flush before dispatching the next queued command. Data-ready Genet and Wi-Fi isolated runtime NICs may run up to three bounded receive/dispatch rounds in one event-pump turn; pre-association Wi-Fi and host-EAPOL replay keep their existing Wi-Fi-specific budgets and physical-console yield behavior.
