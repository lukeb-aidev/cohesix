...
- HDMI status now separates USB keyboard detection, command-input readiness, and first-byte evidence. Gate 8 enumeration still proves keyboard discovery, Gate 9 proves first-report/idle HID readiness, and command-ready is the Gate 10 parser-admission latch. First-byte proof is diagnostic evidence rather than the action that unlocks the console.
- Gate 10 is command-ready parser admission, not first-byte proof and not a full printable-key proof. Pi 4 local-seat HID diagnostics emit one-shot first-non-empty-report, first-printable-byte, and first-unmapped-usage lines so USB keyboard tests can distinguish interrupt-IN delivery from printable key decode. Runtime keyboard bytes are admitted to the parser only when they arrive in a typed `DRIVER_RUNTIME_USB_KEYBOARD_FRAME_FLAG_INPUT` frame whose length and result agree; untyped, stale, or cross-role `FrameReady` completions are rejected before parser ingress.
- Physical Pi 4 HDMI is a high-impact progress surface with bounded console/scrollback frame attempts, not a root-owned framebuffer fallback. On the owner-state path, the HDMI framebuffer descriptor is published before driver-task bootstrap, the `hdmi-text` isolated runtime receives a bounded nonblocking engine-init retry after descriptor load, and the early Cohesix HDMI frame clears the whole mapped framebuffer before drawing inside the safe text rectangle. Normal command-output and keyboard-feedback frame submits now keep the isolated display retryable after a no-reply and emit sampled raw-UART plus `/log/queen.log` `HDMI_FRAME_SUBMIT ...` summaries with reason, byte length, completion status, and cross-sink `console_seq`, `telemetry_sinks`, and `prompt_refresh` fields; detailed `HDMI_FRAME_RING ...`, `HDMI_FRAME_QUEUE ...`, `HDMI_FRAME_COUNTERS ...`, and `HDMI_FRAME_PROGRESS ...` rows are retained for pre-prompt, fatal, and sparse sustained no-reply samples, and all diagnostic rows emitted for one HDMI frame submission share the same `console_seq`. One frame timeout no longer marks the isolated display permanently failed. When driver-task pointer-free proof is present, root submits bounded isolated runtime USB local-seat service before publishing `Cohesix console ready`, records the selected Wi-Fi replay, runs the bounded replay before publishing the interactive prompt, and then publishes `Cohesix console ready` after the replay returns ready, pending, or fail-closed; if that proof is missing, it publishes the prompt with fail-closed blockers and does not start hidden post-prompt Wi-Fi replay. HDMI no longer waits for USB keyboard attach or an unproved local-seat dependency; serial remains the complete authority console if HDMI engine init or framebuffer proof is unavailable. If isolated HDMI does not reply, root does not write the HAL-mapped firmware framebuffer. USB keyboard runtime attach is pre-root only after pointer-free proof and otherwise prompt-side bounded: if the isolated USB runtime already has an active command, local-seat attach defers with one `prompt-settle attach deferred reason=usb-runtime-active` summary before retrying after the normal quiet window; otherwise local-seat first replays the deferred PCIe root descriptor, then submits one bounded USB engine-init call. Ongoing keyboard polls remain nonblocking and, on no reply, use bounded backoff while keeping owner-state recovery armed instead of holding serial behind repeated local-seat calls. Root-console startup emits raw UART `[mark] root-console.start.*` breadcrumbs when the startup banner is published; deferred Wi-Fi descriptor replay may delay `Cohesix console ready` until ready, pending, or fail-closed, while later EAPOL/DHCP settle and diagnostics remain on the serial UART so missing `/log/queen.log` or 9P access cannot hide the debug stream. After root-console handoff, serial remains a physical console parser source and USB keyboard input joins that parser once root console is ready and the linked USB keyboard endpoint has been admitted; the first real typed key is command input, not an arming token. The event pump flushes serial echo before runtime Wi-Fi work, drains USB keyboard input before and after runtime work once keyboard polling has been enabled, yields not-ready `wifi-host-eapol-pending` / `wifi-host-eapol-required` runtime work to active serial input, active USB keyboard input, or pending serial output, gives Wi-Fi DHCP-pending turns priority under physical-console pressure, and keeps DHCP-bound Genet/CYW43 data polling on a bounded service turn so ARP/TCP setup is not starved. Deferred EAPOL remains limited to one runtime Wi-Fi poll per event turn, and ready DHCP/UDP/TCP work is capped by fixed per-poll quanta so a live Wi-Fi connection cannot drain an arbitrary backlog in one compatibility turn. HID boot keyboards are programmed with idle duration `0` to avoid periodic idle-report backlog, while HID polling still drains a bounded burst of interrupt-IN reports in one pass so press/release/next-key sequences are requeued before the next event-loop turn. Root logs `[local-seat] usb keyboard command-ready action=enable-command-input ...` after the root/TCP console readiness gate, linked USB keyboard admission, and absence of active recovery/no-reply/hard queue blockers; HDMI does not print a separate readiness challenge or transient USB-busy warning. If USB polling or HDMI output is pressure-gated, serial and `/log/queen.log` record bounded `command-deferred` diagnostics while HDMI holds the prompt until command input is safe. Serial logs low-level USB first-byte proof as evidence while forwarding the first accepted typed frame to the command parser. HDMI boot/progress lines are rate-limited to a 5-10 s visible cadence after display proof, identical driver progress fault lines are shown once per state change while serial/log evidence keeps bounded repeat/progress detail, normal glyph/scroll writes are clipped to the safe area, and network-origin TCP console output is mirrored to HDMI with logarithmic sampling as REST/hive-gateway concurrency rises while serial and local-seat command output remain complete.
- USB owner-state proof is separate from local-seat input readiness. After PCIe owner-state and deferred descriptor replay, linked-runtime xHCI/controller progress may publish `DRIVER_TASK_OWNER_STATE ... hot_path=usb-keyboard owner_state=driver-owned descriptor_version=4 descriptor_seal=valid bus_link_seal=valid`; USB command-ready requires linked-runtime keyboard endpoint admission and no active hard recovery/no-reply/queue blocker. Full local-seat proof additionally requires first-report/idle evidence, `USB_LOCAL_SEAT_STATE=ready`, no post-ready busy warning, and clean first-byte/burst evidence when the test includes typed-key proof.
- Before the isolated USB keyboard path can capture input, HDMI shows the one-shot `USB console starting...` line. There is no arming-key flow: when linked runtime keyboard admission is ready, the deferred `cohesix>` prompt becomes the readiness signal, and the first typed key is parsed normally.
- Serial and local-seat blank or whitespace-only submissions are physical-console no-ops. Non-empty repeated parse failures are coalesced so terminal CR/LF floods or a stuck key cannot saturate the UART/HDMI transcript or block a following `ping`, `reboot`, `usb`, `wifi`, or `smp activity` command.
- TCP console dispatch preserves isolated runtime boundaries while copying Linux's immediate-progress rule: after each accepted network-origin command in a bounded receive burst, root runs a bounded TCP response flush before dispatching the next queued command. Data-ready Genet keeps its existing three bounded receive/dispatch rounds in one event-pump turn; data-ready CYW43 Wi-Fi may run up to ten bounded rounds, a Wi-Fi-only deeper post-dispatch flush window that first services the linked runtime RX pre-poll, drains any resulting RX into smoltcp with a charged interface poll, and then flushes TCP, plus a small idle RX-maintenance window when DHCP-bound Wi-Fi has no buffered network-origin command line. Pre-association Wi-Fi and host-EAPOL replay keep their existing Wi-Fi-specific budgets and physical-console yield behavior.
...
- The `wifi: firmware_contract ... nvram=...` field reports the Linux-shaped normalized NVRAM upload length, not the raw captured text file length. For the captured Raspberry Pi OS Pi 4B CYW43455 bundle, raw NVRAM is 2074 bytes, the uploaded normalized payload is 1744 bytes, and the separate Broadcom tail token brings the downloaded NVRAM region to 1748 bytes.
...
- On Pi 4 U-Boot profiles, authenticated reboot best-effort marks BCM2711 PM RSTS state before arming reset, using the existing Cohesix high marker as reboot evidence only. The generated `boot.cmd` forces serial input, reloads Cohesix policy, reads and clears the marker when visible, emits one bounded `boot marker diagnostics` line with RSTS/high/reset/saved-policy state, and then enters the interactive Cohesix menu by default. Saved policy is still the default first menu action, but it is selected by the operator instead of being auto-booted from the software-reset bit.
- The marker diagnostic does not bypass the menu and does not create persistent boot policy.
...
- REST Live Hive samples `hive-gateway` `/v1/meta/status` at the normal status-poll interval and renders broker wait/retry pressure as fixed-budget pollen around the queen. Gateway pressure changes marker intensity, radius, and orbit speed only; it does not increase the particle budget, lower render quality, change frame caps, or change poll cadence.
...
- `telemetry push <src_file> --device <id>` – request an OS-named segment under `/queen/telemetry/<device_id>/seg/` and append bounded telemetry records using `cohsh-telemetry-push/v1` envelopes (UTF-8, allowlisted extensions only; chunked to `max_record_bytes=4096` and `telemetry_ingest.max_bytes_per_segment`). Current root-task builds return the new segment as `seg_id=<id>` on the successful `ctl` acknowledgement; `cohsh` uses that fast path before falling back to `latest`.
...
The script builds `root-task` with the serial and TCP console features, embeds a boot-minimized copy in the staged elfloader, retains the unchanged target ELF as `rootserver` for diagnostics and external QEMU loading, compiles NineDoor and workers, copies host tools (`cohsh`, `gpu-bridge-host`, `host-sidecar-bridge`) into `out/cohesix/host-tools/`, and assembles the sub-4-MiB CPIO payload without a root-task entry. The payload inventory verifies that every worker and manifest-declared isolated runtime keeps the exact bytes produced for the seL4 target.【F:scripts/cohesix-build-run.sh†L815-L1008】
...
- `netstats` reports deterministic target fields: `profile_backend=<label> active_driver=<label> enabled=<bool> running=<bool> udp=<ip:port> tcp=<ip:port> last=<result>`.
- QEMU behavior is unchanged (`127.0.0.1:{31338,31339}` hostfwd workflows remain valid).
- Pi 4 `pi4-uboot-aarch64` uses the active control-plane address (wired GENETv5 or Wi-Fi CYW43455). It reports `profile_backend=bcmgenet-v5` for the compiler-visible Pi 4 network profile and `active_driver=bcmgenet-v5` or `active_driver=cyw43` for the selected runtime driver.
- `netstats` emits a policy line: `mode=<off|static|dhcp> policy=<wired|wifi|auto> active=<iface> standby=<iface|none> addr_src=<source> ip=<ipv4> gateway=<ipv4> dhcp=<phase>`, and `netstatus` includes `tcp_ready=<yes|no>` so DHCP-bound evidence is not confused with listener or authenticated-shell proof. On CYW43 Wi-Fi, `addr_src` / `dhcp` can report `wifi-rx-overflow` / `rx-overflow`, `wifi-rx-starvation` / `rx-starvation`, or `wifi-tx-credit-anomaly` / `tx-credit-anomaly` when runtime data-path evidence contradicts a clean DHCP/TCP-ready state.
- `netstats` separates TCP establishment from remote shell proof:
  ...
  receive budget.
- `netstats` emits TCP response-flush and CYW43 RX-maintenance counters:
  ...
  batches that still had TCP work at the flush cap. On active CYW43 Wi-Fi the
  same line includes `cyw43_tail_polls=<count> cyw43_tail_hits=<count>
  cyw43_tail_idle=<count> cyw43_tail_budget_errors=<count>` so pressure tests
  can tell productive post-flush and idle RX tail polling from idle or
  budget-limited polling.
- `netstats` emits local-seat HDMI pressure counters for remote console output:
  ...
  records lines sent to TCP but skipped for HDMI by network-output policy or
  display pressure, and the same line reports bounded HDMI pending/no-reply
  counters when a local seat is attached. When `netstats` is invoked from the
  serial or local-seat console, its full `netstats:`, `netstatus:`, and
  `nettest:` diagnostic output uses the normal local-seat mirror path so HDMI
  shows more than the final `OK NETSTATS` acknowledgement.
- `netstats` emits driver TX and ARP edge counters:
  `tx_submit=<count> tx_complete=<count> tx_free=<count> tx_in_flight=<count> tx_double_submit=<count> tx_zero_len_attempt=<count> arp_rx=<count> arp_tx=<count>`. On CYW43, `tx_submit` records SDPCM data frames accepted by the runtime and `tx_complete` records only credit-backed completion proof for the submitted sequence; `tx_submit > tx_complete` is a Wi-Fi data-path blocker, not a clean in-flight steady state.
- `netstats` emits a Wi-Fi/EAPOL line: `wifi_assoc=<0|1> wifi_link=<0|1> eapol_rx=<count> eapol_start=<count> eapol_secure=<0|1> wifi_rxq_cur=<count> wifi_rxq_hwm=<count> wifi_rxq_drops=<count> wifi_runtime_rxq_cur=<count> wifi_runtime_rxq_hwm=<count> wifi_runtime_rxq_ovf=<0|1> wifi_runtime_rxq_max_drain=<count> wifi_runtime_rxq_drain_hit=<0|1>`. A second Wi-Fi trace line reports `wifi_trace_faults=<count> wifi_trace_tx_retries=<count> wifi_arp_tha_zeroed=<count>`. A third Wi-Fi post-DHCP RX line reports `wifi_post_dhcp_rx any=<count> unicast=<count> arp=<count> ipv4=<count> icmp=<count> tcp=<count> last_len=<bytes> last_ethertype=0x<hex>`. Routine post-DHCP foreign broadcast DHCP and non-local ARP traffic, including link-local metadata probes, is suppressed from `CYW43_DRIVER_TASK_DATA_PATH` logs; faults, drops, retries, host-EAPOL, local DHCP control, ARP target-hardware rewrites, ARP directed at the assigned Wi-Fi address, and IPv4 frames directed at the assigned Wi-Fi address remain visible.
