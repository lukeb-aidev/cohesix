...
- `log` streams `/log/queen.log`; `log dump <file.txt> [--force]` is a host-only `cohsh` convenience that reads the same VM path through the active transport and writes retained payload lines to a local text file. It is not a new root-console verb and does not change `.coh` script grammar.
