...
- The `wifi: firmware_contract ... nvram=...` field reports the Linux-shaped normalized NVRAM upload length, not the raw captured text file length. For the pinned upstream Linux Pi 4B CYW43455 bundle, raw NVRAM is 1883 bytes, the uploaded normalized payload is 1708 bytes, and the separate Broadcom tail token brings the downloaded NVRAM region to 1712 bytes.
...
- `netstats` emits post-dispatch TCP response-flush counters:
  ...
  can tell productive post-flush RX tail polling from idle or budget-limited
  polling.
