...
- Observe forwarded packets (helpful on macOS `lo0`): `sudo tcpdump -i lo0 -n -vv '(tcp port 31337 or udp port 31338 or tcp port 31339)'`.
- For smoke testing over the default slirp hostfwd path: `echo -n "ping" | nc -u -w1 127.0.0.1 31338` and `printf "hi" | nc -v 127.0.0.1 31339`.
