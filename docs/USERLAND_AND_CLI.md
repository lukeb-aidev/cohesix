...
- USB owner-state proof is separate from local-seat input readiness. After PCIe owner-state and deferred descriptor replay, linked-runtime xHCI/controller progress may publish `DRIVER_TASK_OWNER_STATE ... hot_path=usb-keyboard owner_state=driver-owned`; USB command-ready additionally requires linked-runtime keyboard endpoint admission, first-report/idle proof, and no active hard recovery/no-reply/queue blocker. First-byte proof remains diagnostic evidence, not command admission.
