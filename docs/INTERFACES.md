...
- The Pi 4 local-seat runtime treats a firmware DT `xhci` node marked `status = "disabled"` as stale handoff rather than immediate loss of keyboard support: it translates BCM2711 `0x7e...` SoC-bus `reg` values into CPU physical addresses for diagnostics, ignores stale disabled-node hints as active xHCI runtime sources, pins the trusted handed-off xHCI BAR first, avoids deriving a VL805 ECAM/config aperture from that BAR, emits concise candidate-decision breadcrumbs (`kind/cfg/cov/pwin/pin/fh/fs/vh`) plus a targeted firmware-handoff trust-gate line (`cmd_safe/token/irq/trusted/reason`) before skipping an xHCI MMIO source, and treats the standard U-Boot `usb stop` handoff as trusted only when the bootloader exports the safe high-BAR contract. If that contract is rejected, or if runtime needs a VL805 mailbox reset and only sees an unconfirmed result outside the posted-fallback / soft-continue weak-handoff set, root-task emits the corresponding `reject-untrusted-high-bar` or `mailbox-reset-unconfirmed action=skip-candidate` breadcrumb and leaves local-seat degraded instead of touching xHCI MMIO on an unsafe snapshot path.
- On the active trusted Pi 4 xHCI path, local-seat replays the bounded runtime VL805 mailbox reset-notify only when runtime PCI config replay is available. Linux resets VL805 before it enumerates and reprograms BAR0/COMMAND; the current Cohesix Pi 4 safe-mode path keeps VL805 ECAM reads/writes disabled, so root-task skips the mailbox reset with `action=skip-mailbox-notify reason=pci-config-replay-unavailable` rather than invalidating the U-Boot-exported BAR authority before the first xHCI capability read. Runtime probes stay poll-only and do not bind raw PCIe child-INTx or bridge sinks before controller entry because Linux routes VL805 through MSI. The trusted high BAR remains the runtime hint; capability layout is read from the mapped controller unless a legacy DT snapshot is present. With the trusted high-BAR contract, prompt-safe selection first takes `handoff=none` / `origin=live-runtime-default` so Cohesix performs an xHCI-level reset/start against the bootloader-owned PCI state and builds fresh DCBAA, scratchpads, command ring, and event ring. `action=select-stop-seed-runtime mode=cold-start-from-snapshot` means bootloader post-stop authority has enabled the bounded resetless fallback; if the live default attempt fails and a stop-state seed is available, runtime then tries `handoff=none` / `origin=resetless-stop-seed` without preserving firmware-owned ring state. A confirmed runtime mailbox reset may still retry the fully unseeded `mode=cold-start-from-snapshot` HCRST fallback; posted-fallback and soft-continue remain on the no-touch seeded `mode=cold-start-from-snapshot` diagnostic lane. The runtime now also emits a compact seed/branch checkpoint before halt revalidation, so a missing skip marker after a seeded attempt identifies seed propagation rather than a later `RUN` edge fault. Any remaining `xhci.diag stage=0x0253 tag=crcr-read-begin` breadcrumb is therefore part of the runtime-owned path, not a preserved stop-state shortcut.
- Current Pi 4 xHCI prompt-safe ordering is stricter than the older text above when a safe halted stop-state exists: after the skipped unseeded live-default candidate, local-seat now attempts `handoff=none` / `origin=resetless-stop-seed` even for posted/soft reset evidence before falling back to the non-enumerating seeded `cold-start-from-snapshot` diagnostic lane. It still stays poll-only, still rejects IRQ27/INTx as xHCI delivery, and never uses raw CPU DMA addresses when the `0x4_00000000` PCIe DMA alias is enabled.
- The preserved-controller deferred-ownership breadcrumbs remain available inside `usb-oxide` for generic regression work, but the active Pi 4 local-seat runtime no longer depends on them. Posted-fallback mailbox resets can still keep the trusted high-BAR candidate alive on Pi 4 as diagnostic evidence, but only mailbox ACK or bootloader post-stop reset authority selects the runtime-owned stop-seed lane; if the trusted high-BAR contract is absent, runtime still fails gracefully back to the UART console.
- The Pi 4 CYW43455 firmware-load path keeps the startup SDIO link active through chip discovery, RAM sizing, AI core disable/reset, and the initial Function 1 firmware upload. After upload, the active control-plane contract is now a bounded two-stage path: root-task still attempts strict HT-clock readiness first and still requires real Function 2 readiness before using the control plane, but if the final stronger `wait-ht-clock` retry leaves `HT_AVAIL_REQ` latched, `ALP_AVAIL` high, `FORCE_HT` asserted, and complete HT-assist shadow state, it emits `action=continue mode=bounded-no-ht`, enters the slower no-HT transport profile, and lets `setup-firmware-channel` / `wait-firmware-ready` prove mailbox viability instead of failing immediately at the PMU boundary. The `wait-ht-clock` ALP-prime substep still preserves the forced HT / retry request bits instead of dropping back to a bare ALP-only request, logs `alp-request=0x..`, emits decoded `status=request-readback|ht-rerequest-readback|timeout-soft|timeout-hard|bounded-no-ht-early-exit ... ht_req=... alp_req=... force_ht=... clkreq_off=... alp=... ht=... wake=... sleep=... cardcap=...` breadcrumbs, gives the final stronger required retry a larger bounded wait budget (`action=retry-stronger-request ... wait_loops=...`) with chunked `progress=wait-ht-clock polls=... csr=... refresh_index=...` checkpoints, and short-circuits that stronger retry with `action=bounded-no-ht-early-exit ... shortcut=...` once the bounded no-HT contract has remained stable for one soft-wait progress chunk. On that bounded path, `wait-firmware-ready` keeps the mailbox-ready poll on the startup SDIO clock, retries that mailbox read through the incrementing Function 1 path when the SDIO-core windowed read is still fragile, and after one bounded recovery pass may provisionally accept repeated fragile mailbox-read failures so control-plane init can test the live transport instead of stopping at the mailbox poll. The first bounded no-HT control-plane write now stays on the startup SDIO clock until one ioctl completes successfully; only after that proof edge does the driver promote back to the normal control-plane clock. HAL still retains the guarded startup-link fallback and reply-rearm ladder, but Function 2 enable now primes the Linux-sized 512-byte FBR block size before the IORX wait, post-firmware-ready probes still preserve already-latched F2 enables, strict reply recovery now force-clears and re-enables F2 when the card stays stuck enabled-but-not-ready, and no-HT Function 2 writes remain on the same 512-byte cadence while reply reads stay on smaller conservative bursts. The bounded no-HT setup/rearm path now keeps the CYW43455 chip-specific `watermark=0x60`, `SBSDIO_DEVICE_CTL|=F2WM_ENAB`, and `MESBUSYCTRL` writes, while still emitting `action=linux-sdio-minimal ... skip=function-int-mask+ienx` so the fragile interrupt-side writes remain gated off until the reply path proves stable. The serial path reports that split through `write_chunk_limit=...`, `reply_chunk_limit=...`, `sdio function2 ready-snapshot ... diagnosis=...`, and `action=preserve-enable-latch ... reason=linux-aligned-no-clear`, so the next failure can be attributed to F2 latch state, packetization, or reply visibility instead of a generic promoted-rearm timeout. If the speculative higher-clock reply probe still stalls after that first proof write, the driver drops back to the startup link, preserves the live card selection on that same startup link (`action=sdio-host-live-resume-*`), forces the host back to the true startup-link profile (`400000 Hz`, `1bit`), and rearms Function 2 there without pretending the card was freshly inserted. Only if that live-card startup-link resume fails does HAL fall back to a fresh `CMD5` reattach (`action=sdio-host-reattach-*`), after which it waits once for the original outstanding ioctl reply on the slower link and only resends that first ioctl if the recovered reply path still times out. Before a stalled or failed promoted reply path is declared dead, the driver now forces a failure-only low-clock SDIO-core probe and reports `probe=low-clock` values for `hostintmask`, `function-int-mask`, `tohost-mailbox`, and `int-status`, so the next serial capture says exactly which reply-side register path is still unreadable at the safe startup clock. Only after control-plane init succeeds does it clear the bounded no-HT transport profile and return to the normal data-clock target without the guarded reply-probe path.
- Current Pi 4 CYW43455 high-speed behavior follows the Linux capture: firmware upload and normal control-plane work request a 50 MHz SDIO target on the 4-bit high-speed lane, while the no-HT fallback keeps its conservative `1562500 Hz -> 800000 Hz -> startup` ladder. `wait-firmware-ready` no longer lowers a proven promoted link to 400 kHz for mailbox polling, and pre-HT control-plane staging preserves the current promoted lane instead of forcing a startup-clock detour.
- The initial `setup-firmware-channel` pass and the later rearm block both use the same bounded low-clock write window. On the strict/full path that block still covers mailbox version / `HOSTINTMASK` / watermark / `MESBUSYCTRL`; on the bounded no-HT path it still emits `action=linux-sdio-minimal`, but now only skips the interrupt-side `function-int-mask` and `IENx` writes after the mailbox version / `HOSTINTMASK` / CYW43455 watermark / `DEVCTL(F2WM_ENAB)` / `MESBUSYCTRL` programming. If the control-plane link is already above the startup clock, HAL temporarily lowers those critical SDIO-core and Function 1 register writes back to the startup clock, emits `firmware-channel-write-clock action=lower ...`, performs the setup/rearm block, and then emits `action=restore ...` when the faster control-plane clock is restored. This keeps the critical mailbox/device-control writes on the safer transfer shape without pretending failed writes committed.
- On the bootloader-authorized stop-state-only xHCI branch, runtime still uses the trusted CAP layout plus stop-state diagnostics, but it no longer preserves firmware-owned ring state or the abandoned stop-state deferred `CRCR` shortcut. The first trusted high-BAR prompt-safe attempt remains the runtime-owned `mode=none` / `origin=live-runtime-default` lane. If runtime falls through to the stop-seed fallback, that stop-state-only snapshot suppresses the early halt revalidation touch, selects `mode=none` / `origin=resetless-stop-seed`, skips the first live `USBCMD.HCRST` / `CONFIG` touch (`xhci.diag stage=0x0234` / `0x0245`), and rebuilds runtime-owned ring state locally from zero-seeded `DCBAAP`/`CRCR`/`ERST*`/`ERDP` state. On weaker seeded `cold-start-from-snapshot` fallback evidence, runtime stays no-touch and non-enumerating. Any remaining `crcr-read-begin` breadcrumb is therefore part of the runtime-owned path rather than a preserved stop-state handoff. The broader deferred-ownership ladder remains an internal `usb-oxide` recovery tool, not part of the active Pi 4 handoff contract.
- As-built correction: current prompt-safe Pi 4 high-BAR probing uses the Linux-captured VL805 capability tuple when no firmware capability snapshot is present, avoiding the live high-BAR `CAPLENGTH` probe that halted the board. When U-Boot exports trusted bootloader stop-state authority, the resetless stop-seed runtime lane is now the first usable high-BAR attempt; older wording that says the unseeded live-default lane remains first is superseded.
...
- Once the UART root shell is live, Pi 4 backend keyboard polling stays manual until an explicit serial/local-seat debug command enables it. `usb status` reports whether the runtime local-seat backend is attached and whether keyboard polling is still deferred, then replays a compact `verdict=... focus=...` line plus the labelled cached xHCI stage values when a diagnostic edge exists; `usb enable-kbd` arms deferred runtime probing, and `usb probe-kbd` immediately runs one on-demand runtime xHCI keyboard probe pass after arming polling. The same serial-local debug surface exposes `wifi dump-state`, `wifi probe-ht`, `wifi load-fw`, and `wifi retry` for Pi 4 CYW43455 bring-up without extending the shared network console grammar, and `wifi dump-state` now ends with a compact `verdict=... focus=...` line that classifies the current last-mile blocker from the cached transport snapshot.
