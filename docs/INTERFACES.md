...
- The Pi 4 USB deferred-capture contract requires command-ring proof. `command_probe=no-op-deferred` is no longer sufficient; the active high-BAR runtime submits prompt-safe Linux-shaped Enable Slot as the first command-ring proof and immediately follows a successful slot allocation with Disable Slot cleanup. Live root-port sampling unlocks only after that command completes. `command_probe=no-op-ok`, `command_probe=enable-slot-ok`, and the enable-slot cleanup-failed variant prove command/event-ring consumption; No Op proof labels remain legacy diagnostic evidence only. Historical `enable-slot-linux-event-ok` / `no-op-linux-event-ok` labels are diagnostic only and do not satisfy the cold-boot USB gate. A concrete `tag=cmd-timeout` now normalizes as `USB_BLOCKER=cmd-timeout`, and concrete `no-op-unproven`, `enable-slot-unproven`, `no-op-linux-event-unproven`, or legacy `enable-slot-linux-event-unproven` proof summaries normalize as `USB_BLOCKER=cmd-event-ring-timeout`; these supersede the earlier timer-only `cmd-poll-pending` state.
- Current M26b USB correction: the Pi 4 platform-reset lane uses the prompt-safe Linux-shaped event-generation path (`USBCMD.RUN|INTE`, `IMOD=0xa0`, `IMAN.IE`) while PCI INTx/MSI/GIC delivery remains masked. It preserves the leading port-status events that Linux shows before the first Enable Slot completion, submits Enable Slot through the command-proof path instead of the plain xHCI command helper, and lets the command wait skip those non-command events before accepting the command completion. Each consumed non-command event republishes `ERDP.EHB` with the HAL posted-write flush before polling the next slot, matching the controller-visible event-ring progression Linux relies on. Any missing command completion remains `cmd-event-ring-timeout`, and IRQ27 remains the seL4 timer only.
- If a Linux-shaped prompt-safe command proof reaches the command timeout, `usb-oxide` performs one bounded fresh command/event-ring recovery before returning to the shell. The recovery allocates new HAL DMA-backed command and event rings, republishes `ERSTSZ`/`ERSTBA`/`ERDP`/`CRCR` with the Pi 4 PCIe DMA alias, drains the posted CRCR write through the HAL EXT_CFG flush hook, and retries Enable Slot once while PCI INTx/MSI delivery remains masked. A second timeout remains `cmd-event-ring-timeout`, now proving the blocker survived fresh ring publication rather than stale command/event-ring state.
...
- Current M26b USB correction: the live HAL-proven `platform-reset-complete` lane now publishes the event-ring table before handing the dequeue pointer to VL805: `ERSTSZ` / `ERSTBA` are written first and `ERDP` is deferred until after that table is visible. Command timeouts keep live command-gate snapshots deferred because the latest board traces proved post-timeout live xHCI reads can halt through the seL4 timer path; this covers `CRCR`, `USBCMD` / `USBSTS`, `DCBAAP`, interrupter registers, and `PORTSC`. The command RUN edge now also clears stale controller command bits (`HCRST`, `LHCRST`, `CSS`, and `CRS`) from trusted `USBCMD` seeds before publishing `USBCMD.RUN`; a stale `USBCMD=0x00000003` trace now normalizes as `USB_BLOCKER=usbcmd-run-preserved-reset-bit`.
- As-built refinement: on the active cold-boot `platform-reset-complete` USB path, local-seat drains leading `PORT_STATUS_CHANGE` events before command proof, acknowledges each one with `ERDP.EHB`, and drains those low/high dword writes through the Pi 4 HAL posted-write hook. The subsequent port-event proof submits Enable Slot through the Linux-shaped prompt-safe lane; the empty-event proof uses No Op.
