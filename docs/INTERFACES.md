...
- Current June 12 Wi-Fi correction: `/Users/lukasbower/pi4-serial-20260612-064922.log` supersedes the June 11 `sdio-engine-init-no-reply` frontier. The selected Wi-Fi interface completes linked descriptor replay for both `cyw43455` and `sdio-host`, SDIO engine init returns ready with detail `0x5500`, and the CYW43 SDIO prereq reports ready; the active blocker is now CYW43 engine init no-reply with the CYW43 progress marker at `engine-init-runtime-entry`. Root publishes fixed-ring command records with a sequence-last contract, cleans staged descriptor/payload bytes plus command/completion records before publishing the real sequence, and invalidates completion/progress records before consuming linked-runtime evidence. Linked runtimes read that shared ring before endpoint polling so bounded one-way turns do not require an endpoint wake or reply cap before any SDIO, firmware, DHCP, `nettest`, or `netstats` claim can be accepted. The CYW43 engine-init interface now exposes `cyw43-engine-init-branch`, `cyw43-state-reset-begin`, `cyw43-state-reset-done`, `cyw43-forbidden-sdio-mmio`, `cyw43-bus-link-check-begin`, `cyw43-shared-control-check-begin`, `cyw43-shared-control-missing`, and `cyw43-shared-control-ready`, so prompt-side `wifi diag`, `NET_DRIVER_TASK_REPLAY_STATUS`, `DRIVER_TASK_RESOURCE_INIT`, and proof normalization distinguish a state-slot, forbidden-resource, SDIO-owner-link, shared-control, or completion-publication blocker instead of reporting stale HAL power/reset. CYW43 transport details remain fail-closed: partial `0x5401..0x5407` completions are progress with `result=0`, and only `0x5408` reports ready with `result=1`. The linked CYW43 control ABI separates fire-and-forget SDPCM control frames from matched `CONTROL_EXCHANGE` turns: station setup must receive expected CDC command/ioctl-id replies for the Linux-first preflight (`bus:txglomalign=8`, optional `ulp_sdioctrl`, `bus:rxglom=1`, `cur_etheraddr`, `BRCMF_C_GET_REVINFO`, `mpc=0`) before `WLC_UP`, and read iovars such as `cur_etheraddr` return a bounded CDC response frame before the root-side host-EAPOL and DHCP/data gates can advance.
