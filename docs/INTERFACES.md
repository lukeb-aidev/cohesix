...
- As-built refinement: ARMCR4 clear-reset and post-reset single-byte probes now use the windowed CMD53-read path with current-window and rewindowed CMD53 byte fallbacks (`mode=cmd53-windowed-read32-cmd53-byte-current-window fallback=cmd53-byte-rewindow`) instead of the fragile CMD52-current-window shortcut. After ARMCR4 release, `armcr4-release-proof` still tries to live-read `AI_IOCTRL` and `AI_RESETCTRL`; a concrete `CPUHALT` or reset-asserted readback fails before HT with `cyw43-armcr4-release-not-live`, while a fragile SDIO readback timeout is logged as `action=continue-to-ht reason=linux-release-path` so the next hardware run proves or blocks on the required DEVON/HT-clock gate instead of masking it as an ARMCR4 readback edge.
...
- Current Pi 4 xHCI prompt-safe ordering is stricter when a safe halted stop-state exists: local-seat attempts `handoff=none` / `origin=reset-owned-stop-seed` first for both the default U-Boot-stop-seed path and the mailbox/bootloader-authorized paths, treats that attempt as a controller-entry skip when it has no runtime-ring ownership, then falls back to the unseeded live-default lane only for trusted diagnostic strategies that explicitly allow a fallback. That fallback is separate from the stopped seed and must pass the mailbox-reset notify gate before the `platform-reset-complete` replay-proven lane starts. It keeps xHCI completion handling poll-driven, rejects seL4 IRQ 27 as an xHCI source because it is the kernel virtual-timer PPI, and never uses raw CPU DMA addresses when the `0x4_00000000` PCIe DMA alias is enabled. On high-BAR pre-controller ownership writes, local-seat binds only the PCIe host and child INTx sink IDs (`180` and `175`); IRQ27 remains kernel-owned and diagnostic-only, and HAL disables VL805 MSI/INTx delivery for this lane. When a seeded `platform-reset-complete` path has live HAL BAR/COMMAND proof and reaches controller-ready, usb-oxide replays `CONFIG.MaxSlotsEn`, local-seat still defers root-port `PORTSC` sampling because the latest board trace proved that first post-`RUN` port read is toxic on the stop-seeded path. That lane now mirrors Linux's event-generation bits (`USBCMD.INTE`, `IMAN.IE`) while still polling the event ring and keeping PCI interrupt delivery masked, then writes the current event dequeue pointer with split `ERDP.EHB` low/high stores (`0x031b` / `0x031c`) so inherited Event Handler Busy state cannot suppress completions. It then drains event-ring `PORT_STATUS_CHANGE` candidates into `event_candidate_mask`, logs the drained/probe mask flow, runs `enable_slot`/`disable_slot` only when candidates exist, and runs a command-ring No Op proof when `event_candidate_mask=0`. The No Op path publishes the late command TRB with an explicit `xhci-cmd-ring-submit` DMA share before ringing doorbell 0, snapshots the first four event-ring TRBs before the doorbell and again on timeout, emits `cmd-doorbell-write` / `cmd-doorbell-write-done` / `cmd-doorbell-post-barrier`, and synchronizes the current event-ring TRB for CPU reads during command polling. A remaining `cmd-poll-only-timeout` with nonzero `event_syncs` is therefore a doorbell or controller command-consumption edge rather than stale command-ring contents, a stale CPU event-ring cache line, disabled xHC event generation, or uncleared EHB. It does not call root-port reset/enumeration helpers. Bootloader-owned stop-seed and preserve-state lanes also skip `PORTSC`.
- As-built refinement: on the active stop-seeded `platform-reset-complete` USB path, the poll-only EHB cleanup writes `IMAN.IP` as a write-one-to-clear value, then enables xHC event generation with `IMAN.IE` while PCI interrupt delivery remains masked. Any drained `PORT_STATUS_CHANGE` candidate is still OR-ed into the command-probe mask before deciding between `enable_slot` and No Op.
...
- As-built refinement: the post-ARMCR4-release `post-release-proof` breadcrumbs include a compact tuple for the firmware image range, normalized NVRAM range, NVRAM tail magic, reset-vector address/value, CPUHALT release value, upload/current/preferred SDIO clocks, and cached `CHIPCLKCSR` / `WAKEUPCTRL` / `SLEEPCSR` / `CARDCAP` labels. That tuple is cached diagnostic context; `armcr4-release-proof` remains the live release check when readable, but fragile readback timeouts are advisory on the Linux release path so the mandatory following gate is the live DEVON/HT-clock proof.
- As-built correction: the Pi 4 CYW43455 production post-download `wait-ht-clock` edge now emits `post-download-ht-sideband-prime`, programs wake/CMD14 sideband, and requires live `SLEEPCSR.KSO|DEVON` before the production HT request. Cached post-release DEVON may be preserved as diagnostic pre-HT evidence only; it no longer lets production skip the live clear/set ladder when the current register is only KSO. KSO-only or cached-DEVON states do not authorize Function 2. Root-task still does not add `FORCE_HT` before proof; it waits for real `HT_AVAIL` before production Function 2 traffic. Explicit diagnostics may still run bounded `diagnostic-force-ht-*` characterization after the exact stable `0x50` timeout shape, but that probe remains `production_continue=no`.
