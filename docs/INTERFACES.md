...
- Current Pi 4 xHCI prompt-safe ordering is stricter when a safe halted stop-state exists: local-seat attempts `handoff=none` / `origin=reset-owned-stop-seed` first for both the default U-Boot-stop-seed path and the mailbox/bootloader-authorized paths, treats that attempt as a controller-entry skip when it has no runtime-ring ownership, then falls back to the unseeded live-default lane only for trusted diagnostic strategies that explicitly allow a fallback. That fallback is separate from the stopped seed and must pass the mailbox-reset notify gate before the `platform-reset-complete` replay-proven lane starts. It keeps xHCI event progression poll-only, rejects seL4 IRQ 27 as an xHCI source because it is the kernel virtual-timer PPI, and never uses raw CPU DMA addresses when the `0x4_00000000` PCIe DMA alias is enabled. On high-BAR pre-controller ownership writes, local-seat binds only the PCIe host and child INTx sink IDs (`180` and `175`); IRQ27 remains kernel-owned and diagnostic-only. When a seeded `platform-reset-complete` path has live HAL BAR/COMMAND proof and reaches controller-ready, local-seat still defers root-port `PORTSC` sampling because the latest board trace proved that first post-`RUN` port read is toxic on the stop-seeded path; it drains event-ring `PORT_STATUS_CHANGE` candidates into `event_candidate_mask`, runs `enable_slot`/`disable_slot` only when candidates exist, and runs a command-ring No Op proof when `event_candidate_mask=0`. The No Op path publishes the late command TRB with an explicit `xhci-cmd-ring-submit` DMA share before ringing doorbell 0, emits `cmd-doorbell-write` / `cmd-doorbell-write-done`, and synchronizes the current event-ring TRB for CPU reads during command polling. A remaining `cmd-poll-only-timeout` with nonzero `live_reads` is therefore a doorbell or controller command-consumption edge rather than stale command-ring contents or a stale CPU event-ring cache line. It does not call root-port reset/enumeration helpers. Bootloader-owned stop-seed and preserve-state lanes also skip `PORTSC`.
...
- As-built refinement: before ARMCR4 release, root-task emits `firmware stage=nvram-normalize raw=... normalized=... total_with_tail=... state=...` and performs a bounded proof of the fully staged firmware image, normalized NVRAM head/tail, Broadcom tail token, and reset-vector write. On ALP-only uploads, unavailable image/NVRAM/tail readback after all bounded retry profiles is advisory and logs `continue-before-armcr4-release`, while any proven byte mismatch is a terminal firmware-readback blocker. Image reads use the Linux-shaped 2048-byte Function 1 block transfer before the 512-byte byte-mode retry profiles. On pre-upload HT-proven uploads, the same bounded proof remains diagnostic unless a byte mismatch is proven. Function 2 remains blocked until readable `CHIPCLKCSR.HT_AVAIL` and CCCR `IOR2` readiness are observed; the Pi 4-specific forced tuple is diagnostic only and does not authorize production Function 2 traffic.
