...
- Current M26b correction: incrementing-address CMD53 is again the Linux-shaped primary word-write path for AI-core pre-reset writes. The latest 2026-05-10 trace showed the CMD52-first workaround failing on SOCRAM before the word path could prove progress, so ARMCR4/SOCRAM pre-reset `AI_IOCTRL` now logs `cmd53-word-windowed-prereset fallback=cmd52-byte-current-window`; a fixed-address `0x91681004` word write and the older `cmd52-byte-transfer-window-prereset fallback=cmd53-word-windowed` label are stale evidence.
...
- Current M26b correction: SOCRAM disable no longer uses `prereset-zero-ioctrl` on the upstream write-first path. It now writes `AI_IOCTRL.FGC|CLK` at `stage=prereset-fgc-clock`, starts with CMD53 word-windowed Function 1 backplane access, keeps CMD52 current-window as recovery only, clears stale firmware proof/contract evidence and live release counters at the start of each firmware-load attempt, and reports a pre-reset SOCRAM failure as `socram-prereset-fgc-cmd53-r5-rejected`.
