...
- The Pi 4 local-seat runtime treats a firmware DT `xhci` node marked `status = "disabled"` as stale handoff rather than immediate loss of keyboard support: it translates BCM2711 `0x7e...` SoC-bus `reg` values into CPU physical addresses for diagnostics, ignores stale disabled-node hints as active xHCI runtime sources, pre-seeds the captured VL805 high BAR first, avoids deriving a VL805 ECAM/config aperture from that BAR, and emits concise candidate-decision breadcrumbs (`kind/cfg/cov/pwin/pin/fh/fs/vh`) plus targeted trust-gate lines before skipping an xHCI MMIO source. The standard path treats U-Boot `usb stop` only as menu/input cleanup; an opt-in high-BAR handoff contract is trusted only when the bootloader exports the safe high-BAR contract and stop-state registers.
- On the active Pi 4 xHCI path, local-seat replays the bounded runtime VL805 mailbox reset-notify only when runtime PCI config replay is available. Linux resets VL805 before it enumerates and reprograms BAR0/COMMAND; the current Cohesix Pi 4 safe-mode path keeps VL805 ECAM reads/writes disabled, so root-task normally starts from the captured high BAR and performs poll-only runtime ownership without binding raw PCIe child-INTx or bridge sinks before controller entry. Capability layout is read from the mapped controller unless an opt-in/legacy DT snapshot is present. Prompt-safe selection now permits the unseeded live-default `handoff=none` path first, publishing Cohesix-owned DCBAA, scratchpads, command ring, and event ring before RUN without preserving firmware-owned ring state. Opt-in stop-state evidence remains available as a bounded fallback/diagnostic lane. The runtime now also emits a compact seed/branch checkpoint before halt revalidation, so a missing skip marker after a seeded attempt identifies seed propagation rather than a later `RUN` edge fault. Any remaining `xhci.diag stage=0x0253 tag=crcr-read-begin` breadcrumb is therefore part of the runtime-owned path, not a preserved stop-state shortcut.
- Current Pi 4 xHCI prompt-safe ordering is stricter than the older text above when a safe halted stop-state exists: local-seat now attempts the unseeded live-default `handoff=none` path first, then attempts `handoff=none` / `origin=reset-owned-stop-seed` only when mailbox ACK or bootloader post-stop authority owns the stop seed, before falling back to the non-enumerating seeded `cold-start-from-snapshot` diagnostic lane. It still stays poll-only, still rejects IRQ27/INTx as xHCI delivery, and never uses raw CPU DMA addresses when the `0x4_00000000` PCIe DMA alias is enabled.
- The preserved-controller deferred-ownership breadcrumbs remain available inside `usb-oxide` for generic regression work, but the active Pi 4 local-seat runtime no longer depends on them. Posted-fallback mailbox resets can still keep the high-BAR candidate alive on Pi 4 as diagnostic evidence, but only mailbox ACK or opt-in bootloader post-stop reset authority selects the runtime-owned stop-seed lane; when the opt-in high-BAR contract is absent, runtime still probes the captured high BAR directly and falls back to the UART console only if that Cohesix-owned probe fails.
...
- As-built correction: current prompt-safe Pi 4 high-BAR probing makes the Cohesix-owned live-default lane usable again by pre-seeding the Linux-captured VL805 BAR0 and permitting `handoff=none` in prompt-safe mode. When U-Boot exports trusted bootloader stop-state authority, the reset-owned stop-seed runtime lane remains a fallback rather than replacing the first live-default attempt.
