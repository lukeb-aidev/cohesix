...
- After the serial root prompt is live, routine USB prompt-slice progress, hub-port snapshots, and transient enumeration breadcrumbs are retained in `/log/queen.log` and explicit `usb` diagnostics instead of being streamed through the interactive serial or HDMI surfaces. The user-facing USB readiness alert is emitted once the isolated HID path proves an accepted first byte: `usb keyboard ready: first_report=yes first_byte=yes input=local-seat ...`. Controller-ready, keyboard-enumerated, and first-report-only states remain diagnostic evidence, not a keyboard-ready operator alert.
