...
- Root-task also emits a combined `xhci-handoff-contract` breadcrumb after DT parse, a companion `xhci-handoff-dtb source=... prestop=.../... poststop=.../...` breadcrumb, and an `xhci-handoff-rings dcbaap=... crcr=... erstba0=... erdp0=... erstsz0=...` breadcrumb so a partial or rejected handoff can be diagnosed from root-task logs alone even if early U-Boot serial output is unavailable and the bootloader-owned ring state can be compared directly against the first runtime ownership writes. On the trusted mailbox-reset snapshot path, runtime now stages `ERDP0` from that same handoff snapshot and delays its first live low/high `ERDP` publish until after `ERSTSZ0` / `ERSTBA0`, with dedicated `0x02b5`-`0x02bf` breadcrumbs, so the next boot can isolate the first toxic event-ring ownership write instead of folding `ERDP` into the earlier event-ring table programming.
