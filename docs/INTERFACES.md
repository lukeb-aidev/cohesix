...
- As-built refinement: before ARMCR4 release, root-task now emits `firmware stage=nvram-normalize raw=... normalized=... total_with_tail=... state=...` and performs a bounded proof of the fully staged firmware image, normalized NVRAM head/tail, Broadcom tail token, and reset-vector write. On ALP-only uploads, read-unavailable image/NVRAM/tail proof is a terminal firmware-readback blocker before ARMCR4 release; image reads use the Linux-shaped 2048-byte Function 1 block transfer instead of the generic 256-byte byte-mode backplane read. On pre-upload HT-proven uploads, the same bounded proof remains diagnostic unless a byte mismatch is proven. Function 2 remains blocked until readable `CHIPCLKCSR.HT_AVAIL` and CCCR `IOR2` readiness are observed; the Pi 4-specific forced tuple is diagnostic only and does not authorize production Function 2 traffic.
