...
- As-built correction: the Pi 4 CYW43455 production post-download `wait-ht-clock` edge now emits `post-download-ht-sideband-prime`, programs wake/CMD14 sideband, and must not emit `post-download-sleepcsr-clear-set` when live `SLEEPCSR=0x03` is already ready for Function 2. A live `SLEEPCSR=0x01` KSO-only result remains wake evidence only, not Function 2 readiness, and cached DEVON from a diagnostic proof no longer permits the production HT request. Root-task still does not add `FORCE_HT` before proof; it replays Linux's firmware-callback SD-only `CHIPCLKCSR=0` fence only after live KSO+DEVON is present, then issues `HT_AVAIL_REQ` and waits for real `HT_AVAIL` before production Function 2 traffic. Explicit diagnostics may still run forced-tuple characterization paths.
