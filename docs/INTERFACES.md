...
- Current Pi 4 xHCI prompt-safe ordering is stricter when a safe halted stop-state exists: local-seat attempts `handoff=none` / `origin=reset-owned-stop-seed` first for both the default U-Boot-stop-seed path and the mailbox/bootloader-authorized paths, treats that attempt as a controller-entry skip when it has no runtime-ring ownership, then falls back to the unseeded live-default lane only for trusted diagnostic strategies that explicitly allow a fallback. That fallback is separate from the stopped seed and must pass the mailbox-reset notify gate before the `platform-reset-complete` replay-proven lane starts. It keeps xHCI completion handling poll-driven, rejects seL4 IRQ 27 as an xHCI source because it is the kernel virtual-timer PPI, and never uses raw CPU DMA addresses when the `0x4_00000000` PCIe DMA alias is enabled. On high-BAR pre-controller ownership writes, local-seat binds only the PCIe host and child INTx sink IDs (`180` and `175`); IRQ27 remains kernel-owned and diagnostic-only, and HAL disables VL805 MSI/INTx delivery for this lane. When a high-BAR `platform-reset-complete` path has live HAL BAR/COMMAND proof and reaches controller-ready, usb-oxide replays `CONFIG.MaxSlotsEn`, local-seat still defers root-port `PORTSC` sampling because the latest board trace proved that first post-`RUN` port read is timer-preempted toxic on both stop-seeded and fresh high-BAR lanes. That lane keeps `USBCMD.INTE=0` and `IMAN.IE=0`, polls the event ring, and keeps PCI interrupt delivery masked, then writes the current event dequeue pointer with split `ERDP.EHB` low/high stores (`0x031b` / `0x031c`) so inherited Event Handler Busy state cannot suppress completions. It then drains event-ring `PORT_STATUS_CHANGE` candidates into `event_candidate_mask`, logs the drained/probe mask flow, runs `enable_slot`/`disable_slot` only when candidates exist, and runs a command-ring No Op proof when `event_candidate_mask=0`. The No Op path publishes the late command TRB with an explicit `xhci-cmd-ring-submit` DMA share before ringing doorbell 0, snapshots the first four event-ring TRBs before the doorbell and again on timeout, emits `cmd-doorbell-write` / `cmd-doorbell-write-done` / `cmd-doorbell-post-barrier`, and synchronizes the current event-ring TRB for CPU reads during command polling. A remaining `cmd-poll-only-timeout` with nonzero `event_syncs` is therefore a doorbell or controller command-consumption edge rather than stale command-ring contents, a stale CPU event-ring cache line, uncleared EHB, or an accidental dependency on delivered interrupts. After a successful command proof, the prompt-safe lane may enumerate the Linux-captured root port 1 as already enabled at high speed through a reset-skipped constructor, still without touching `PORTSC`. Bootloader-owned stop-seed and preserve-state lanes also skip `PORTSC`.
- The Pi 4 gate proof normalizer treats an IRQ27 line after command doorbell publication as timer evidence only. If no command timeout or live command-ring snapshot has appeared yet, the stable gate summary is `USB_BLOCKER=cmd-poll-pending`; once `cmd-poll-only-timeout` or a more precise live timeout detail appears, that later blocker supersedes the pending state.
- The Pi 4 USB deferred-capture contract requires command-ring proof. `command_probe=no-op-deferred` is no longer sufficient; the active runtime keeps the first No Op command in flight and continues poll-only event-ring synchronization instead of queuing a second No Op with Linux-shaped event generation. `command_probe=no-op-linux-event-ok` is retained only as a historical diagnostic label. A concrete `tag=cmd-timeout` now normalizes as `USB_BLOCKER=cmd-timeout`, which supersedes the earlier timer-only `cmd-poll-pending` state.
- As-built refinement: on the active stop-seeded `platform-reset-complete` USB path, the poll-only EHB cleanup writes `IMAN.IP` as a write-one-to-clear value and leaves `IMAN.IE` clear while PCI interrupt delivery remains masked. Any drained `PORT_STATUS_CHANGE` candidate is still OR-ed into the command-probe mask before deciding between `enable_slot` and No Op.
