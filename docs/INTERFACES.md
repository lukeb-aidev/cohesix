...
- Current June 12 Wi-Fi correction: `/Users/lukasbower/pi4-serial-20260612-212306.log` supersedes the June 12 20:03 transport-init frontier. The selected Wi-Fi interface completes linked descriptor replay for both `cyw43455` and `sdio-host`, SDIO engine init returns ready with detail `0x5500`, the CYW43 SDIO prereq reports ready, CYW43 engine init returns ready, and the linked transport turn reaches firmware upload. Root publishes fixed-ring command records with a sequence-last contract, cleans staged descriptor/payload bytes plus command/completion records before publishing the real sequence, and invalidates completion/progress records before consuming linked-runtime evidence. Linked runtimes read that shared ring before endpoint polling so bounded one-way turns do not require an endpoint wake or reply cap. The active Wi-Fi frontier is now the Gate 6 firmware stream: the trace reaches `CYW43_DRIVER_TASK_STREAM_PROGRESS ... uploaded=557056 total_len=609309`, then ends during retained-stage owner recovery after a Function 1 CMD53 firmware chunk reports `cyw43-firmware-retry-exhausted` (`0x5329`) with R5 `0x0800` and `CYW43_SDIO_PAYLOAD_CMP ... status=match`. Firmware release, Function 2 readiness, DHCP, `nettest`, and remote `cohsh` are still unproven. CYW43 transport details remain fail-closed: partial `0x5401..0x5407` completions are progress even when their result is `0`, and only `0x5408` reports ready with `result=1`. The linked CYW43 control ABI separates fire-and-forget SDPCM control frames from matched `CONTROL_EXCHANGE` turns: station setup must receive expected CDC command/ioctl-id replies for the Linux-first preflight (`bus:txglomalign=8`, optional `ulp_sdioctrl`, `bus:rxglom=1`, `cur_etheraddr`, `BRCMF_C_GET_REVINFO`, `mpc=0`) before `WLC_UP`, and read iovars such as `cur_etheraddr` return a bounded CDC response frame before the root-side host-EAPOL and DHCP/data gates can advance.
