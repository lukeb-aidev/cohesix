...
- As-built correction: the Pi 4 CYW43455 post-download `wait-ht-clock` edge mirrors the captured Linux `CLK_SDONLY -> CLK_AVAIL` order. Root-task clears `CHIPCLKCSR` to SD-only, holds a bounded off-window, then issues `HT_AVAIL_REQ` without pre-programming SR wake/cardcap/KSO sideband. KSO and DEVON remain diagnostic sideband evidence and are refreshed after Function 2 readiness, not substitutes for the readable `CHIPCLKCSR.HT_AVAIL` proof. Console HT probes that run after a failed boot keep the more informative cached boot snapshot when the live diagnostic path has already lost card context.
...
- As-built refinement: before ARMCR4 release, root-task now emits `firmware stage=nvram-normalize raw=... normalized=... total_with_tail=... state=...` and performs a bounded proof of the normalized NVRAM head, normalized NVRAM tail, Broadcom tail token, and reset-vector write. Read-unavailable outcomes are diagnostic because Function 1 may still be fragile, but any proven byte mismatch fails before ARMCR4 release. Function 2 remains blocked until `CHIPCLKCSR.HT_AVAIL` is observed.
- Current Pi 4 CYW43455 high-speed behavior follows the Linux capture: firmware upload and normal control-plane work request a 50 MHz SDIO target on the 4-bit high-speed lane, with the BCM2711 SDHCI mailbox-reported `100000000 Hz` rate corrected to the effective Arasan `250000000 Hz` parent so the legal divider produces the observed `41666666 Hz` transfer clock. No-HT remains a firmware-upload fallback only; Function 2 requires real CCCR `IORX` readiness and a readable `CHIPCLKCSR.HT_AVAIL` proof before the Linux `FORCE_HT` edge. `wait-firmware-ready` no longer lowers a proven promoted link to 400 kHz for mailbox polling, and pre-HT control-plane staging preserves the current promoted lane instead of forcing a startup-clock detour.
