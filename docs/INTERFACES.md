...
- `/log/queen.log` is bounded, not unbounded: the live root-task ring retains up to 2048 lines. Console `cat`, `tail`, and `log` export that retained window in bounded 64-line batches and emit `END` only after the final retained line. Host dump workflows in `cohsh` and SwarmUI are projections over the same path and do not introduce a new VM verb or evidence channel.
