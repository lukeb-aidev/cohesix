...
- The Pi 4 USB admission proof still uses BCM2711 EXT_CFG selector/COMMAND readback to prove VL805 BAR/COMMAND ownership before xHCI entry, but the linked USB runtime now drains xHCI operational-register writes inside its own declared MMIO aperture. `USBCMD.RUN`, doorbells, `CONFIG`, `DNCTRL`, and 64-bit base-register high dwords are followed by bounded same-runtime xHCI readback instead of a nested USB-to-PCIe child command. The previous HAL EXT_CFG-only drain remains historical context for old traces; new traces should identify the exact xHCI readback frontier through `usb-*written` / `usb-*flushed` progress markers if that edge is toxic.
...
- As-built refinement: on the active cold-boot `platform-reset-complete` USB path, local-seat preserves leading `PORT_STATUS_CHANGE` events until after command proof publishes Enable Slot and rings doorbell 0, then acknowledges each consumed event with `ERDP.EHB` and drains those low/high dword writes through the linked USB runtime's same-runtime xHCI readback. No Op remains diagnostic-only and cannot unlock root-port sampling.
