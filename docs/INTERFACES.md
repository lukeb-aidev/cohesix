...
- For `pi4-uboot-aarch64` (and migration alias `uefi-aarch64`), profile backend and addressing are authored through `hw.network.*`: `hw.network.backend=bcmgenet-v5`, `hw.network.mode=(off|static|dhcp)`, `hw.network.interface=(wired|wifi|auto)`, manifest static IPv4 when `mode=static`, and bounded DHCP retry/timeout fields when `mode=dhcp`. Runtime diagnostics split `profile_backend=bcmgenet-v5` from the selected `active_driver=bcmgenet-v5|cyw43`.
...
- Historical root-HAL Wi-Fi traces may contain mixed CMD52/CMD53 core-control
  fallback labels. That implementation is not an active physical Pi 4 Wi-Fi
  service: target stateful debug operations fail with
  `pi4-wifi-driver-task-runtime-required`, and the manifest-declared CYW43 and
  SDIO runtimes are the only production owners.
- The active isolated-runtime ARMCR4 interface uses a dedicated strict AI
  lane: select the backplane window, then issue only an incrementing four-byte
  Function 1 CMD53 transfer. IOCTRL uses a write/read transaction flush without
  equality. RESETCTRL uses the bounded Linux clear/settle/read loop, with later
  mailbox/devready proof authoritative when immediate reset readback is
  unavailable. This lane cannot invoke firmware-upload recovery, mutate card
  width or host clock, use CMD52 or fixed-address CMD53 fallback, or clear the
  first primitive fault. Completion results identify the exact ARMCR4
  write/flush edge and optional readback.
- The isolated SDIO owner exposes Raspberry Pi Linux's `mmc-bcm2835` Wi-Fi
  host model, pinned to Raspberry Pi kernel commit
  `89050b1059997d38d55462b323b099a6436dc10d`, to that interface.
  Byte/halfword registers use aligned 32-bit reads and immediate RMW writes;
  only TRANSFER_MODE is shadowed so COMMAND commits their paired 32-bit word.
  Ordinary writes receive the controller's timer-backed minimum two-SD-clock
  delay (6 us at 400 kHz, 1 us at 50 MHz), while FIFO words use the raw
  no-delay path. `RESET_ALL` is issued with current timing and then invalidates
  active/requested clock caches plus the transfer-mode shadow, so subsequent
  writes use the 400 kHz floor until `set_ios`. Initial card
  selection is one retained lifecycle with a 1 ms post-CMD0 settle,
  command-local three-retry bound, one CMD5 OCR discovery query, and up to 100
  CMD5-ready polls spaced 10 ms apart. Backplane attach never re-enumerates an
  already selected card after a strict ChipCommon fault.
- The isolated SDIO owner contains every failed descriptor with a fresh
  bounded deadline before returning the ring to CYW43. Completion is withheld
  until `CMD_INHIBIT` and `DATA_INHIBIT` are clear. Only an entry-inhibit
  failure, which proves the command was not issued, permits one identical
  Function 1 CMD53 replay; later-stage Function 1 failures and all Function 2
  failures are never replayed. Failed containment masks CARD_INT and poisons
  the owner until an exact generation reset succeeds. CARD_COMMAND and
  HOST_CONFIG use the same rule, and partial LOW/MID/HIGH backplane-window
  programming invalidates the cached aperture.
- Current CYW43 control/reconnect interface: ordinary matched BCDC controls use
  retained `CONTROL_EXCHANGE`. The descriptor identity includes generation,
  command/id/flags, payload offset/length, and a full payload digest. Each
  runtime turn advances one action, TX is exact-once, and an interleaved
  event/data frame returns `FrameReady` detail `0x5802` while retaining that
  identity for a later byte-identical resume. PTK/GTK `wsec_key` uses the same
  retained exchange with pre-TX drain and a carrier/key-epoch commit fence.
  No-reply exhaustion or any completion that does not prove the runtime cursor
  terminal latches pair restart before root releases ownership. Incremental key
  state retains the exact descriptor/id/bytes until that fence is consumed; it
  is never reissued. The
  root `NetStack` owns the single
  association retry supervisor and uses a capped 1/2/4/8/16/30-second schedule
  forever; no second root or host-EAPOL join owner may race it. Successful
  `SET_SSID` is the association result, LINK-up is carrier telemetry rather
  than an association substitute, and firmware WPA additionally requires
  `PSK_SUP` status 6. Each attempt has one non-sliding 24.576-second ceiling.
  At every outer and inner CYW43 pre-poll boundary, and after later
  host-EAPOL/event service, NetStack synchronizes the connection generation
  and ends the turn on change before draining preserved frames or entering
  smoltcp, DHCP, or TCP work, including TCP-only flush turns.
  Post-secure station-filter repair exposes `repair-pending` for matched
  optional firmware rejection and `repair-deferred` for proven-terminal
  failure. If repair loses terminal cursor proof, it exposes
  `pair-restart-required`, submits no later filter command, and closes the
  root-visible carrier/security/data gates before returning to the event pump.
- Current CYW43/SDIO recovery interface: an SDIO child operation that remains
  issued-unknown beyond the completion grace publishes sticky runtime phase
  `446` (`cyw43-sdio-pair-restart-required`). HAL then fences and restarts the
  two manifest-declared runtimes as one unit, replays SDIO before CYW43, and
  leaves normal service behind a context-replay gate until firmware, station
  controls, and network policy have been restored. No old-pair completion is
  applied after phase 446, and root's retained SDHCI mapping is recovery-only
  authority rather than a steady physical-driver path. Both receive-ready
  waits, both sealed-descriptor replays, both engine replays, and handoff share
  one aggregate 3-second deadline, consulted inside every replay send/yield
  attempt as well as between phases. A fresh pending restart supersedes
  required/active context replay, and replay entry rejects pending or active
  restart state. Recovery MMIO/ring reuse requires successful
  suspension of both TCBs; if either suspend fails, root fences submissions and
  performs no recovery MMIO against the possibly-live owner.
...
- Historical June 14 Wi-Fi correction: `/Users/lukasbower/pi4-serial-20260614-132747.log` superseded `/Users/lukasbower/pi4-serial-20260614-075442.log` for that boot frontier. That image reached firmware upload and release, then failed before join at `bus:txglomalign=8`. The split `CONTROL_FRAME`/`CONTROL_POLL` remediation introduced for that frontier is now superseded for ordinary controls, joins, and PTK/GTK `wsec_key`; all three use the current exact-once retained `CONTROL_EXCHANGE` contract above. The historical trace fields remain valid evidence labels, but they do not define the current control owner.
- Historical M26b root-HAL traces also record CMD52-first ARMCR4/SOCRAM/D11
  experiments and adjacent-wrapper addresses. They remain comparison evidence
  only. The active isolated runtime targets ARMCR4 wrapper `0x18102000` and
  cannot select those fallback transfer shapes.
...
- After the serial root prompt is live, routine USB prompt-slice progress, hub-port snapshots, and transient enumeration breadcrumbs are retained in `/log/queen.log` and explicit `usb` diagnostics instead of being streamed through the interactive serial or HDMI surfaces. While the isolated USB keyboard path is still below keyboard enumeration/HID polling readiness, HDMI shows `USB console starting...` and does not ask the operator to press a key. There is no arming-key prompt: unattended HID first-report/idle proof can open the command-ready latch once root/TCP console readiness, linked USB keyboard admission, and the absence of active hard recovery/no-reply/queue blockers are all true. First-byte proof remains diagnostic evidence only, and the first real typed byte after command-ready enters the parser normally. Once the command-ready latch opens, transient keyboard poll cooldown, single no-reply samples, or HDMI output pressure do not close parser ingress; only hard USB recovery, stale endpoint queues, or sustained no-reply streaks can degrade local-seat input.
- USB owner-state diagnostics close the isolated controller ownership boundary, not the user-input readiness boundary. `DRIVER_TASK_OWNER_STATE ... hot_path=usb-keyboard owner_state=driver-owned descriptor_version=4 descriptor_seal=valid bus_link_seal=valid` may appear after PCIe owner-state and xHCI/controller progress; command-ready requires linked-runtime keyboard endpoint admission and no active hard recovery/no-reply/queue blocker. Full local-seat acceptance additionally requires first-report/idle proof, `USB_LOCAL_SEAT_STATE=ready`, no post-ready busy signal, and clean first-byte/burst evidence when the test includes typed-key proof.
...
- The Pi 4 CYW43455 firmware-load path uses one Linux-normal SDIO lane after card selection and Function 1 enable. The runtime programs card width before the host, enables card-side CCCR high-speed timing when supported, selects 50 MHz only with `SPEED.EHS` and otherwise caps the host at 25 MHz, and always keeps the `mmc-bcm2835` host's `HOST_CONTROL.HISPD` bit clear. It then keeps that card/host configuration unchanged through `brcmf_sdio_buscoreprep()`-shaped ALP setup, probe attach, AI-core passive state, firmware/NVRAM upload, reset-vector publication, and ARMCR4 activation. Function 2 remains disabled during upload. After activation, `RELEASE` performs no SDHCI or card-lane mutation: it writes `CHIPCLKCSR=0`, issues one pure `HT_AVAIL_REQ`, requires real `ALP_AVAIL|HT_AVAIL`, and only then applies `FORCE_HT` for Function 2 propagation. A stable `CHIPCLKCSR=0x50` remains a Gate-7 failure and never authorizes Function 2.
- Firmware and normalized NVRAM remain bounded by the generated 8192-byte owner window. Function 1 bulk uses block-mode CMD53 and ordinary remainders/window edges use the same lane's byte-mode CMD53 shape; these are transfer shapes, not fallback profiles. Retained-stage replay may resume only on the identical negotiated lane. No one-bit upload mode, lower-clock ladder, forced byte replay, CMD52 data-window rescue, root-owned physical path, or same-generation lane mutation is selectable.
...
- The Pi 4 CYW43455 NVRAM upload mirrors Linux `brcmf_fw_nvram_strip()` before the Broadcom tail token is published. The raw Raspberry Pi 4B Linux NVRAM text remains the build input, but root-task uploads only compact NUL-separated `key=value` entries with comments, empty lines, carriage returns, invalid keys, and `RAW1` records removed, and it adds `boardrev=0xff` only when the file lacks a board revision. For the captured Raspberry Pi OS Pi 4B bundle this changes the payload from the raw 2074-byte text file to a 1744-byte upload at `0x0021792c`, then writes the separate `0xfe4b01b4` token at the top-of-RAM tail; that matches the Linux-shaped total NVRAM download length of 1748 bytes.
...
- Current Pi 4 CYW43455 timing follows Linux MMC ordering: startup clock is used only for enumeration, then width and timing are negotiated before backplane attach. Card-side CCCR `SPEED.EHS` authorizes the 50 MHz target while the captured `mmc-bcm2835` host deliberately keeps `HOST_CONTROL.HISPD` clear; a card without SHS uses the 25 MHz default-speed ceiling. The selected lane is continuous through AI-control CMD52, multiblock firmware CMD53, the final byte/window remainder, and ARM release. Diagnostics report observed host/card/clock and `linux-normal-block` or `linux-byte-remainder`; they do not report retired fallback ladders.
- The final firmware owner chunk follows Linux `brcmf_sdiod_skbuff_write`: the logical 3101-byte Pi 4 tail is rounded to a 3104-byte transport write with exactly three zero-checked pad bytes. `arg0` carries logical length, `payload_len` carries transport length, and `CYW43_DRIVER_TASK_STREAM_TAIL_PAD` reports `logical_len`, `transport_len`, `pad`, and `mode=byte-remainder`. Runtime accounting advances by logical bytes and rejects nonzero, nonfinal, or non-four-byte padding.
...
- `nettest` and `netstats` remain backend-agnostic console verbs (no grammar changes). `netstats` includes deterministic target fields:
  `profile_backend=<label> active_driver=<label> enabled=<bool> running=<bool> udp=<ip:port> tcp=<ip:port> last=<result>`.
...
- Wi-Fi Gate 10 proof requires explicit command evidence in the capture:
  successful `nettest` or authenticated `cohsh` plus final `netstats` showing
  DHCP-bound Wi-Fi, secure EAPOL, and non-zero TX/RX counters. DHCP-bound
  startup evidence is Gate 9 only; `netstatus ... tcp_ready=no` or missing
  command evidence remains `tcp-proof-missing` / `cohsh=requires-nettest`.
...
- `netstats` reports driver TX and ARP edge counters on a dedicated line:
  `tx_submit=<count> tx_complete=<count> tx_free=<count> tx_in_flight=<count> tx_double_submit=<count> tx_zero_len_attempt=<count> arp_rx=<count> arp_tx=<count>`. For CYW43, `tx_complete` is credit-backed SDPCM completion evidence for the submitted sequence; `tx_submit > tx_complete` is reported as a Wi-Fi TX credit anomaly rather than treated as clean readiness.
