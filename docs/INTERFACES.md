...
- As-built refinement: on the weaker stop-state-only xHCI snapshot branch, runtime still strips all bootloader-owned ring state and still enters `usb-oxide` through `cold-start-from-stop-state-snapshot`, but that path now skips only the live `HCRST` store while keeping the live `CONFIG` write plus fresh ring/event-ring publication. The stop-state snapshot therefore remains a trusted CAP/stop-state hint, not a fully reset-equivalent ownership shortcut.
