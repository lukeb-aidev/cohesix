...
- The Pi 4 Wi-Fi HAL emits `firmware core-ctrl mode=cmd53-windowed-read32-cmd52-current-window-write8 fallback=cmd52-byte-rewindow split-window` before AI-core control, and if the primary mixed Function 1 backplane path times out it now recovers writes with a forced rewindowed CMD52 pass while reads still first try byte-sized CMD52 recovery against the current backplane window before falling back to a rewindowed CMD52 pass. Small CMD53 byte-mode transfers now program the host as a single block with block-count enable asserted instead of leaving the host block count path unset, incrementing CMD53 transfers advance the Function 1 address per chunk, the serial log emits bounded `sdio xfer chunk fn=1 ... base=... chunk=... off=...` breadcrumbs for the early/new transfer shapes plus `backplane window program ... low=... mid=... high=...` on the flagged Function 1 control path, and same-window Function 1 accesses now emit `backplane window reuse ...` instead of replaying an unchanged SBADDRLOW/MID/HIGH CMD52 triplet. AI core-control writes keep the general `AI_IOCTRL` path on the direct unflagged current-window CMD52 address, SOCRAM `assert-reset` now emits `firmware core-ctrl reset-write mode=cmd53-word-windowed fallback=cmd52-byte-current-window-rewindow ...`, and SOCRAM `clear-reset` emits `firmware core-ctrl reset-write mode=cmd52-byte-current-window fallback=cmd52-byte-rewindow ...` so the previously-working reset assert stays on the word path while the fragile reset-release edge uses the upstream byte path. Once a core is already held in reset the SOCRAM `in-reset-configure` path now skips a redundant `AI_IOCTRL` rewrite when the asserted-reset hold value matches the pre-reset `FGC|CLK` value, emitting `in-reset-configure-skip ... reason=redundant-after-assert` instead of forcing another post-reset write. When that SOCRAM skip path is taken immediately after a fresh reset assert, the immediate `AI_RESETCTRL` confirmation read is also deferred and logged as `in-reset-ready-read-deferred ... reason=redundant-after-assert` so Function 1 is not probed again while the core is still fragile. The SOCRAM `core_reset` path now also emits `core-reset ... stage=skip-disable reason=held-reset-from-prior-disable` before the reset-clear sequence, and the first SOCRAM `clear-reset` readback is now likewise deferred once, emitting `clear-reset-read-deferred ... reason=socram-fragile-first-read`, so the next remaining failure moves to the first later post-reset access instead of the repeated `core_disable` entry read or the immediate reset-clear confirmation read. Non-redundant in-reset `AI_IOCTRL` writes still emit `firmware core-ctrl in-reset-write mode=cmd53-word-windowed-in-reset ...` and first use the flagged 32-bit backplane write path before falling back to current-window and rewindowed CMD52 recovery. The generic `firmware core-ctrl access ...` breadcrumb still prints both `bus=...` and `trace_bus=...` so logs show the exact unflagged byte address used for live CMD52 writes alongside the flagged address used for window tracing. SDHCI command or data-path failures on that path emit `sdhci recover stage=... mask=cmd+data` plus paired `sdio shadow ...` breadcrumbs so logs show both the forced host recovery and the last cached backplane-window / chipclk / wake / sleep / cardcap state without additional failing probe traffic. Firmware load now also primes `firmware stage=pre-reset-ht-assist` before the ARMCR4/SOCRAM reset-heavy path whenever cached chip-clock state lacks an HT request, so remaining failures after the backplane-window fix point at the reset edge itself rather than missing HT assist state. The AI core-control sequence now matches the upstream Broadcom AI reset order: write `FGC|CLK` before asserting reset, defer the first `RESETCTRL` readback until after the in-reset configure settle, configure the core while held in reset, then clear reset and leave `CLK` enabled. ARMCR4 also carries the upstream `CPUHALT` bit through disable and final release so the firmware CPU is parked until the post-load reset completes.
- The first SOCRAM reset-release path now adds `clear-reset-prewrite-delay ... reason=socram-fragile-first-write` and, on failure, one `clear-reset-write-retry ... reason=socram-fragile-first-write` recovery attempt before the deferred `clear-reset-read-deferred ...` readback path, so the next remaining failure can move beyond the immediate reset-release edge.
- The Pi 4 local-seat runtime treats a firmware DT `xhci` node marked `status = "disabled"` as stale handoff rather than immediate loss of keyboard support: it translates BCM2711 `0x7e...` SoC-bus `reg` values into CPU physical addresses for diagnostics, ignores stale disabled-node hints as active xHCI runtime sources, pins the trusted handed-off xHCI BAR before the higher VL805 ECAM page so the lower handoff aperture is not lost to monotonic device retype ordering, keeps VL805 ECAM preseed on a map-only path because live ECAM reads still trigger fatal irq 27 entries on Pi 4, avoids broader live runtime VL805 config-space discovery while that path remains unsafe on Pi 4, emits concise candidate-decision breadcrumbs (`kind/cfg/cov/pwin/pin/fh/fs/vh`) plus a targeted firmware-handoff trust-gate line (`safe/token/irq/trusted/reason`) before skipping an xHCI MMIO source, treats the standard U-Boot `usb stop` handoff as a cold-start opportunity for a safe high BAR only when the bootloader exports the safe post-stop contract, and treats VL805 reset as bootloader-owned on that path instead of issuing a runtime mailbox reset request. The low `0xfe980000` / `0x7e980000` BCM2711 USB aliases are not VL805 runtime sources on Pi 4; they identify the SoC `brcm,bcm2835-usb` block and now remain diagnostic breadcrumbs only. Runtime uses the seL4 bcm2711 `device-untypes@600000000` window for the U-Boot-discovered VL805 BAR when present, logs whether the handed-off BAR still has device coverage without touching ECAM (`xhci handoff window mmio=... coverage=... pinned=...`), prefers the bootloader-exported capability snapshot over a live first-touch xHCI capability read on that trusted handoff BAR, passes the validated snapshot into `usb-oxide`, requests GIC SPI 143 directly via `IRQControl Get`, and emits explicit `xhci irq sink request ...`, `xhci irq sink reusing existing handler ... after=get-revoke-first`, or `xhci irq sink ambiguous existing handlers ... after=get-revoke-first` breadcrumbs before the first trusted operational write. It also makes a best-effort legacy IRQ 27 shadow bind attempt strictly as a contract check and emits `xhci irq shadow request ...`, `xhci irq shadow armed ...`, or `xhci irq shadow unavailable ... reason=irq-shadow-owned|irq-shadow-get-handler`; if that shadow line is already owned or cannot be claimed, runtime surfaces `xhci handoff rejected reason=irq-shadow-contract action=stop-runtime-probe` and does not continue into low-alias probing. The trusted-high path remains firmware-quiesced, so the runtime driver skips redundant read-based early quiesce and stop/reset work, preserves the firmware-owned `CONFIG` register (`fw-handoff-skip-config-write`), and only then proceeds into `DCBAAP` / `CRCR` ownership setup. The later runtime polling-mode path likewise emits `skip-imod-write` and `skip-iman-write` instead of touching those registers while the controller remains firmware-quiesced, so the next final breadcrumb identifies the first remaining unsafe MMIO write outside the interrupter scrub itself. To cover read-side stalls, it emits named pre-read tags before the critical MMIO readbacks (`config-readback-begin`, `dcbaap-readback-begin`, `crcr-readback-begin`, `runtime-ring-readback-begin`, `usbsts-run-readback-begin`, `usbcmd-run-readback-begin`, `controller-ready-poll-begin`) plus the matching result/terminal tags (`config-readback`, `dcbaap-readback`, `crcr-readback`, `runtime-ring-readback`, `usbsts-run-readback`, `usbcmd-run-readback`, `controller-ready`, `controller-ready-timeout`). It still explicitly logs the bootloader-exported command word when safe-mode runtime skips a live VL805 PCI-command replay, and otherwise degrades to serial-only input with explicit `[local-seat] ... unavailable` breadcrumbs.
