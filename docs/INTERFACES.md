...
- Current June 11 Wi-Fi correction: `/Users/lukasbower/pi4-serial-20260611-220842.log` supersedes the earlier 21:30 `sdio-engine-init-fault` wording for the active interface frontier. The selected Wi-Fi interface completes linked descriptor replay for both `cyw43455` and `sdio-host`, then still fails Gate 2 as `sdio-engine-init-no-reply` with the `sdio-host` progress marker at `engine-init-hw-begin`. Root publishes fixed-ring command records with a sequence-last contract, cleans staged descriptor/payload bytes plus command/completion records before publishing the real sequence, and invalidates completion/progress records before consuming linked-runtime evidence. Linked runtimes read that shared ring before endpoint polling so bounded one-way turns do not require an endpoint wake or reply cap before any SDIO, firmware, DHCP, `nettest`, or `netstats` claim can be accepted. The SDIO runtime-init and engine-init turns keep the active request across a finite bounded no-reply resume window, so root does not overwrite a still-running linked SDIO adoption/reset turn while the shell remains responsive. The SDIO engine-init interface returns exact completion details: `0x5500` is `ready`, while `0x5501`, `0x5502`, `0x5503`, `0x5510`, `0x5511`, `0x5512`, and `0x5513` report `adopt-power-missing`, `adopt-clock-failed`, `adopt-inhibit-failed`, `reset-all-failed`, `reset-cmd-data-failed`, `clock-failed`, and `inhibit-failed`. The progress interface also exposes `sdio-state-reset-begin`, `sdio-state-reset-done`, and `sdio-hw-entry` before first SDHCI MMIO access, so prompt-side `wifi diag`, `SDIO_DRIVER_TASK_REPLAY_STATUS`, and `DRIVER_TASK_RESOURCE_INIT` can distinguish pre-MMIO state reset, first SDHCI access, power/present, clock, inhibit, reset, and later CYW43 card-command blockers. CYW43 transport details remain fail-closed: partial `0x5401..0x5407` completions are progress with `result=0`, and only `0x5408` reports ready with `result=1`. The linked CYW43 control ABI separates fire-and-forget SDPCM control frames from matched `CONTROL_EXCHANGE` turns: station setup must receive the expected CDC command/ioctl-id reply, and read iovars such as `cur_etheraddr` return a bounded CDC response frame before the root-side host-EAPOL and DHCP/data gates can advance.
