...
- On the active Pi 4 xHCI path, local-seat replays the bounded runtime VL805 mailbox reset-notify before the Linux-captured high-BAR `handoff=none` live-default lane can touch reset state. Linux resets VL805 before it enumerates and reprograms BAR0/COMMAND; Cohesix keeps generic VL805 ECAM reads/writes disabled and now uses the HAL-owned BCM2711 EXT_CFG_INDEX/DATA replay path as the default live ownership proof. That live path binds the seL4-visible PCIe host and child INTx-A GIC IDs (`180` and `175`), validates VL805 vendor/device/class, the Linux PCI bus BAR `0xc0000004/0x00000000` translated through the outbound window, COMMAND memory/bus-master/interrupt-disable state, and poll-only MSI disable before treating the high BAR as live config authority. Static Linux capture remains BAR/capability layout evidence only; after the capture-only path froze at `DCBAAP_LOW`, it no longer authorizes fresh xHCI ownership stores by itself. Controller-ready enumeration does not use Linux virtual IRQ 27 or the Linux MSI virq 30 as seL4 IRQ numbers. Opt-in U-Boot handoff can still export halted high-BAR ownership evidence for diagnostics, but a stopped seed is not authority to bypass fresh publication proof.
- The unseeded live-default `handoff=none` path remains the next bounded diagnostic candidate when the probe ladder selects it. On the Linux-captured high BAR its preflight now reports `route=trusted-high-bar-primary`, `ctor=capture-quiesce`, `halt_guard=skip-live-halt-read`, `pre=mailbox-reset-required`, `legacy=skip-legacy`, `next=mailbox-reset-notify`, and `followup=mailbox-reset-then-platform-init`. Once `mailbox-acked` plus live BCM2711 EXT_CFG BAR/COMMAND proof exists, local-seat promotes to `handoff=platform-reset-complete` with `pre=skip-pre-reset`, `run=run-uboot`, and `publish=rings-pre-run`; the xHCI layer publishes fresh runtime ownership and keeps enumeration poll-driven. When mailbox ACK is present without live COMMAND proof, local-seat still promotes to the platform-reset policy but the local publication gate blocks controller entry and returns to the prompt before touching xHCI ownership registers; a stop-state seed does not bypass that gate.
...
- As-built correction: the Pi 4 CYW43455 post-download `wait-ht-clock` edge now primes bounded Broadcom wake/CMD14 sideband and uses the captured Linux `SLEEPCSR=0 -> DEVON -> SLEEPCSR.KSO -> KSO|DEVON` sequence before the `CLK_SDONLY -> CLK_AVAIL` clock request. Root-task still does not add `FORCE_HT` before proof; it clears the first chip-clock request to SD-only, issues `HT_AVAIL_REQ`, and waits for real `HT_AVAIL` before production Function 2 traffic. If that first pass reaches the Pi 4 `0x50` timeout shape even with sideband evidence logged, production boot fails before Function 2 instead of forcing no-HT traffic. Console HT probes that run after a failed boot may inspect forced-tuple behavior explicitly and keep the more informative cached boot snapshot.
...
- As-built refinement: before ARMCR4 release, root-task now emits `firmware stage=nvram-normalize raw=... normalized=... total_with_tail=... state=...` and performs a bounded proof of the normalized NVRAM head, normalized NVRAM tail, Broadcom tail token, and reset-vector write. Read-unavailable outcomes are diagnostic because Function 1 may still be fragile, but any proven byte mismatch fails before ARMCR4 release. Function 2 remains blocked until readable `CHIPCLKCSR.HT_AVAIL` and CCCR `IOR2` readiness are observed; the Pi 4-specific forced tuple is diagnostic only and does not authorize production Function 2 traffic.
- Current Pi 4 CYW43455 high-speed behavior follows the Linux capture: firmware upload and normal control-plane work request a 50 MHz SDIO target on the 4-bit high-speed lane, with the BCM2711 SDHCI mailbox-reported `100000000 Hz` rate corrected to the effective Arasan `250000000 Hz` parent so the legal divider produces the observed `41666666 Hz` transfer clock. Strict transport requires real `CHIPCLKCSR.HT_AVAIL` and CCCR `IORX` readiness; any Pi 4 no-HT probe is an explicit diagnostic lane, not normal boot. `wait-firmware-ready` no longer lowers a proven promoted link to 400 kHz for mailbox polling, and pre-HT control-plane staging preserves the current promoted lane instead of forcing a startup-clock detour.
- Current M26b correction: the production Pi 4 Wi-Fi boot path no longer accepts a KSO-only or forced ALP/KSO tuple as Function 2 readiness. Post-download HT probing is bounded, primes wake/CMD14 sideband without using `FORCE_HT`, performs the Linux-style `SLEEPCSR=0` DEVON poll followed by `SLEEPCSR.KSO` until `0x03` when possible, treats `SLEEPCSR=0x01` as wake-only, requires real `CHIPCLKCSR.HT_AVAIL` plus Function 2 readiness before strict Function 2 traffic, and leaves no-HT Function 2 probing to explicit debug diagnostics rather than normal boot.
- Current M26b correction: the Pi 4 Wi-Fi probe-attach KSO phase mirrors upstream `brcmf_sdio_kso_init()`: KSO may be set or observed before firmware upload, but DEVON is not a pre-upload requirement. A KSO-only `SLEEPCSR=0x01` at `stage=linux-probe-attach-kso` is wake evidence, while the production blocker remains the later post-release `CHIPCLKCSR.HT_AVAIL` proof before Function 2.
- Current M26b correction: the default Pi 4 USB path treats the Linux VL805 capture as BAR/capability layout evidence only. Fresh `platform-reset-complete` publication now requires live PCI config COMMAND proof through the HAL-owned BCM2711 EXT_CFG lane after the VL805 mailbox reset ACK; capture-only high-BAR pinning and a captured `COMMAND=0x0546` shadow are not enough to touch DCBAAP. The default path still does not use Linux virq 27 or Linux MSI virq 30 as seL4 xHCI IRQs.
