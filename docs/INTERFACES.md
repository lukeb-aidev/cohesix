...
- On that same experimental no-HT Wi-Fi probe path, `wait-firmware-ready` now also tries the first host-mailbox read as a single incrementing CMD53 transfer instead of the older bytewise direct-access fallback, and if that first mailbox read still dies in the SDHCI data/timeout path it emits `experimental-assume-ready` and keeps probing so the next remaining failure boundary moves beyond the first mailbox read itself. After firmware-ready, the same path now emits `post-firmware-ready-function2-recheck action=repoll`, immediately reasserts/repolls Function 2, and then deliberately drops the post-firmware control plane back to the startup link (`hold_control_plane_transport(startup-link)`) at `400 kHz` / `1-bit` until the first real control response arrives. That same path now also caps non-incrementing Function 2 FIFO transfers at 64-byte experimental chunks (`sdio xfer chunk-limit ... reason=experimental-no-ht-f2-fifo`) instead of issuing an initial 511-byte single-shot CMD53, so the first control-plane CLM/ioctl write no longer starts on the most fragile byte-mode transfer shape. The first outbound Function 2 control-plane write is now also a bounded probe: root-task emits `[cyw43] control tx probe ... chunk_limit=... no_ht=...` before the transfer, and if that first write still dies on an SDHCI IO-path boundary it emits `experimental-assume-control-write`, refreshes the SDIO command path, snapshots `IOEX` / `IORX` / `IENX`, `HOSTINTMASK`, `TOHOSTMAILBOXDATA`, `SDIO_INT_STATUS`, `RFRAMEBCLO/BCHI`, `MESBUSYCTRL`, and the cached transport shadow, and falls back from the incrementing CMD53 snapshot reads to the older direct byte reads when those register snapshots themselves hit the same SDIO edge. Once the first matched control response arrives, root-task emits `control transport promote ...` and restores `4-bit` plus the bounded data-clock target before the rest of control-plane bring-up continues, so the next remaining failure boundary moves beyond the first outbound control frame rather than stopping at `sdhci-transfer-finish`.
