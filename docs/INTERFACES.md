...
- As-built refinement: ARMCR4 clear-reset and post-reset single-byte probes now use the windowed CMD53-read path with current-window and rewindowed CMD53 byte fallbacks (`mode=cmd53-windowed-read32-cmd53-byte-current-window fallback=cmd53-byte-rewindow`) instead of the fragile CMD52-current-window shortcut. After ARMCR4 release, `armcr4-release-proof` must live-read `AI_IOCTRL` and `AI_RESETCTRL`, prove `CPUHALT` clear and reset deasserted, and fail before the HT request with `cyw43-armcr4-release-readback-unavailable` or `cyw43-armcr4-release-not-live` if that proof is absent.
...
- Current Pi 4 xHCI prompt-safe ordering is stricter when a safe halted stop-state exists: local-seat attempts `handoff=none` / `origin=reset-owned-stop-seed` first for both the default U-Boot-stop-seed path and the mailbox/bootloader-authorized paths, treats that attempt as a controller-entry skip when it has no runtime-ring ownership, then falls back to the unseeded live-default lane only for trusted diagnostic strategies that explicitly allow a fallback. That fallback is separate from the stopped seed and must pass the mailbox-reset notify gate before the `platform-reset-complete` replay-proven lane starts. It keeps xHCI event progression poll-only, rejects seL4 IRQ 27 as an xHCI source because it is the kernel virtual-timer PPI, and never uses raw CPU DMA addresses when the `0x4_00000000` PCIe DMA alias is enabled. On high-BAR pre-controller ownership writes, local-seat binds only the PCIe host and child INTx sink IDs (`180` and `175`); IRQ27 remains kernel-owned and diagnostic-only. When a seeded `platform-reset-complete` path has live HAL BAR/COMMAND proof and reaches controller-ready, local-seat still defers root-port `PORTSC` sampling because the latest board trace proved that first post-`RUN` port read is toxic on the stop-seeded path; before draining events it writes the current event dequeue pointer with `ERDP.EHB` set (`0x031b` / `0x031c`) so inherited Event Handler Busy state cannot suppress poll-only completions. It then drains event-ring `PORT_STATUS_CHANGE` candidates into `event_candidate_mask`, runs `enable_slot`/`disable_slot` only when candidates exist, and runs a command-ring No Op proof when `event_candidate_mask=0`. The No Op path publishes the late command TRB with an explicit `xhci-cmd-ring-submit` DMA share before ringing doorbell 0, emits `cmd-doorbell-write` / `cmd-doorbell-write-done`, and synchronizes the current event-ring TRB for CPU reads during command polling. A remaining `cmd-poll-only-timeout` with nonzero `live_reads` is therefore a doorbell or controller command-consumption edge rather than stale command-ring contents, a stale CPU event-ring cache line, or uncleared EHB. It does not call root-port reset/enumeration helpers. Bootloader-owned stop-seed and preserve-state lanes also skip `PORTSC`.
- As-built refinement: on the active stop-seeded `platform-reset-complete` USB path, the poll-only EHB cleanup also writes `IMAN.IP` as a write-one-to-clear value while leaving `IMAN.IE` disabled, then preserves any drained `PORT_STATUS_CHANGE` candidate by OR-ing it into the command-probe mask before deciding between `enable_slot` and No Op.
...
- The Pi 4 CYW43455 firmware-load path keeps Function 1 sideband active through chip discovery, RAM sizing, and AI core disable/reset. During backplane prep it mirrors the Linux `brcmf_sdio_buscoreprep()` clock shape and performs the `SDIOPULLUP` (`0x1000F`) clear as a single-shot best-effort write after the 4-bit lane is active and Function 1 sideband is readable; a failed clear is recovered and logged as non-terminal. The Linux `brcmf_sdio_probe_attach()` lane leaves Function 2 disabled, so Cohesix no longer attempts the pre-firmware F2 enable that latched `IOEX=0x06` without `IOR2`; its KSO step mirrors upstream `brcmf_sdio_kso_init()` and does not require `DEVON` before firmware upload. Before upload, root-task switches the card and host into the Linux high-speed lane by programming CCCR `SPEED.EHS` and setting SDHCI `HOST_CONTROL.HISPD`, then mirrors Linux's pre-download `alp_only` clock edge by requesting `CHIPCLKCSR.ALP_AVAIL_REQ`; `ALP_AVAIL` is sufficient for firmware/NVRAM upload, while a readable `HT_AVAIL` bit is only stronger diagnostic evidence at this phase. If `HT_AVAIL` is not visible, Cohesix still keeps the Linux-shaped high-speed 4-bit upload first: firmware writes use 32 KiB Function 1 block transfers at the 50 MHz requested lane, then step down through 25 MHz and 12.5 MHz before the final 1.5625 MHz byte-mode fallback. On that ALP-only upload path, bounded readback is attempted before ARMCR4 release and verifies the fully staged firmware image/NVRAM/tail after NVRAM upload rather than reading the image before NVRAM. The image proof uses Linux-shaped 2048-byte Function 1 block reads, then 512-byte byte-mode retry profiles starting with 1.5625 MHz 4-bit before the 400 kHz 4-bit and 1-bit fallbacks; proven byte mismatches remain fatal, and read-unavailable proof after all retry profiles fails before ARMCR4 release. If a successful retry left the SDIO card on the conservative 1-bit lane, root-task restores 4-bit before the reset-vector write and emits `firmware stage=firmware-release-width action=restore-4bit ... reason=linux-pre-armcr4-ht-proof`. Immediately after ARMCR4 release, Cohesix emits a live `armcr4-release-proof` before the cached `post-release-proof` tuple. After upload, production `wait-ht-clock` first primes `WAKEUPCTRL.HTWAIT` and CCCR `CARDCAP=CMD14_SUPPORT|CMD14_EXT`, preserves live `SLEEPCSR.KSO|DEVON` without the clear/set ladder when it is already device-on, and otherwise accepts a live KSO acknowledgement or cached post-release DEVON tuple as authority only to issue the fresh firmware-callback `CHIPCLKCSR=0` fence and `CHIPCLKCSR.HT_AVAIL_REQ`. Function 2 still requires real `CHIPCLKCSR.HT_AVAIL` plus live readiness before any strict traffic; KSO-only and cached-DEVON states are not Function 2 readiness.
- The first firmware-image proof remains the 2048-byte Linux-shaped Function 1 block read. If that read times out, `firmware_verify_profile` reports the fallback `chunk=512 byte_mode=y` shape and the retry verification reads the staged image, NVRAM, and tail through 512-byte CMD53 byte-mode chunks at 1.5625 MHz 4-bit, then 400 kHz 4-bit, then 400 kHz 1-bit. On those byte-mode retry profiles, a CMD53 read timeout is recovered and retried through the existing Function 1 CMD52 small-backplane path for the same proof chunk before the readback is classified unavailable. A retry that still cannot read back fails before ARMCR4 release; any byte mismatch remains the explicit firmware-readback blocker.
- As-built refinement: the post-ARMCR4-release `post-release-proof` breadcrumbs include a compact tuple for the firmware image range, normalized NVRAM range, NVRAM tail magic, reset-vector address/value, CPUHALT release value, upload/current/preferred SDIO clocks, and cached `CHIPCLKCSR` / `WAKEUPCTRL` / `SLEEPCSR` / `CARDCAP` labels. That tuple is cached diagnostic context; the live release gate is the preceding `armcr4-release-proof` readback, which proves CPUHALT/reset state before the later firmware-callback SD-only clock fence.
- As-built correction: the Pi 4 CYW43455 production post-download `wait-ht-clock` edge now emits `post-download-ht-sideband-prime`, programs wake/CMD14 sideband, and must not emit `post-download-sleepcsr-clear-set` when live `SLEEPCSR=0x01` already acknowledges KSO or live `SLEEPCSR=0x03` is already ready for Function 2. Cached post-release DEVON may be preserved as pre-HT evidence only. KSO-only or cached-DEVON states let production issue the SD-only fence and `HT_AVAIL_REQ`, but they do not authorize Function 2. Root-task still does not add `FORCE_HT` before proof; it waits for real `HT_AVAIL` before production Function 2 traffic. Explicit diagnostics may still run bounded `diagnostic-force-ht-*` characterization after the exact stable `0x50` timeout shape, but that probe remains `production_continue=no`.
...
- On the bootloader-authorized stop-state-only xHCI branch, runtime still uses the trusted CAP layout plus stop-state diagnostics, but it no longer preserves firmware-owned ring state or the abandoned stop-state deferred `CRCR` shortcut. The first captured high-BAR prompt-safe attempt is now `mode=none` / `origin=reset-owned-stop-seed` when a stopped seed exists. That stop-state-only snapshot suppresses controller entry unless runtime-ring ownership is proven; in the common no-ring-seed case local-seat returns from the attempt before xHCI constructor/pre-reset scrub writes, live `USBCMD.HCRST`, `IMAN`, fresh runtime ownership publishes, or `RUN`. On weaker seeded `cold-start-from-snapshot` fallback evidence, runtime stays no-touch and non-enumerating. Any remaining `crcr-read-begin` breadcrumb is therefore part of an enumerating runtime-owned fallback path rather than a preserved stop-state handoff. The broader deferred-ownership ladder remains an internal `usb-oxide` recovery tool, not part of the active Pi 4 handoff contract. On the live replay-proven `platform-reset-complete` lane, command doorbell writes are posted and labelled, but the immediate live doorbell readback is skipped because the latest board trace made that read toxic before the command completion could be observed.
- As-built correction: current prompt-safe Pi 4 high-BAR probing makes the reset-owned stop-seed lane usable as a poll-safe diagnostic path from the Linux-captured VL805 BAR0 layout once the local high-BAR window is pinned and the staged stopped seed is present. It uses the captured capability layout instead of an unsafe early live cap read and permits `handoff=none` in prompt-safe mode. When U-Boot exports trusted bootloader stop-state authority through the opt-in handoff path, the same reset-owned stop-seed lane remains the first attempt, but without runtime-ring ownership it returns immediately as `policy-return`/`fallback-next`; the unseeded live-default lane remains the next diagnostic fallback until stronger ownership proof exists. That Linux-captured high-BAR live-default fallback now expects `0x0204 tag=fw-handoff-skip-pre-quiesce` immediately after `0x0117` only until the mailbox reset gate is acknowledged; the live replay-proven path enters `platform-reset-complete` with `pre=skip-pre-reset`, `run=run-uboot`, and `publish=rings-pre-run`, while a mailbox ACK without BAR/COMMAND proof selects the same platform policy but is stopped by the local publication gate before controller entry.
- The trusted Pi 4 xHCI `RUN` edge has bounded serial diagnostics. `xhci.diag stage=0x0217` records the halt-revalidation decision (`handoff`, seed-flag bitmask, and whether live rereads will be skipped), `0x0218` records entry into the skip branch, `0x0213` / `0x0214` bracket the first live `USBSTS` halt-revalidation read, `0x0215` / `0x0216` bracket the first live `USBCMD` halt-revalidation read, `0x0110` marks controller constructor/init completion, `0x02e5` means the `USBCMD.RUN` store returned, `0x02db` records the first `USBCMD` readback after that store, `0x02dc` records the first `USBSTS` sample after `RUN`, `0x02dd` emits bounded HCH-wait progress snapshots, `0x02df` records observable `USBSTS` transitions during that wait, and `0x02de` captures the final `USBCMD`/`USBSTS` pair before a `0x0272` timeout. `0x02da` is reserved for preserve-state ERSTSZ-publish skip diagnostics. On the trusted preserve-state stop-snapshot path, runtime seeds `RUN` from the bootloader-exported `USBCMD` snapshot and emits `0x02ee` before the store instead of performing a fresh live `USBCMD` read; live read-modify-write seeds remain visible through `0x02ec` / `0x02ed`, and fully blind paths emit `0x02ef`. On `platform-reset-complete` with a stop-state seed, runtime now avoids the immediate post-RUN live `USBCMD`/`USBSTS` readback, takes a bounded blind settle first, avoids an implicit Drop-time live stop on fallback/no-keyboard return, and treats `controller-init-complete -> root-port read-begin` as the proven toxic edge. The replacement diagnostic is `root-port sample skipped reason=platform-reset-stop-seed-portsc-toxic`, `0x031b` / `0x031c` ERDP.EHB acknowledgement, then a bounded event-ring candidate drain; nonzero `event_candidate_mask` then permits only a command-ring proof (`enable_slot` plus best-effort `disable_slot`) before any future safe-port-state work, while `event_candidate_mask=0` enqueues a command-ring No Op TRB, republishes the command ring with `xhci-cmd-ring-submit`, and reports `command_probe=no-op-ok` if the event ring returns its completion. That ladder makes the remaining USB failure attributable to seed propagation, the skip/live halt-revalidation split, halt-revalidation reads, a toxic live seed read, the `RUN` store itself, first post-`RUN` reads, command-ring DMA visibility, doorbell/event-ring proof after EHB acknowledgement, deferred root-port sampling/enumeration, or a controller that accepts `RUN` but never drops `HCH`.
