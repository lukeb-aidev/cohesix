...
- On the weaker stop-state-only xHCI snapshot branch, runtime still uses the trusted CAP layout plus stop-state diagnostics, but it no longer treats that weak snapshot as reset-equivalent and it no longer preserves firmware-owned ring state. A stop-state-only snapshot now returns to the ordinary cold-start reset / `CONFIG` / live ownership-publication path, so the old stop-state deferred `CRCR` ownership read is gone with that branch; any remaining `crcr-read-begin` breadcrumb is now part of the normal post-reset live seed path instead of the abandoned stop-state shortcut. The broader deferred-ownership ladder remains an internal `usb-oxide` recovery tool, not part of the active Pi 4 handoff contract.
