...
- The Pi 4 local-seat runtime treats a firmware DT `xhci` node marked `status = "disabled"` as stale handoff rather than immediate loss of keyboard support: it translates BCM2711 `0x7e...` SoC-bus `reg` values into CPU physical addresses for diagnostics, ignores stale disabled-node hints as active xHCI runtime sources, defers the captured VL805 high BAR during boot unless firmware, HAL, or the default U-Boot post-stop contract provides an explicit high-BAR source, avoids deriving a VL805 ECAM/config aperture from that BAR, and emits concise candidate-decision breadcrumbs (`kind/cfg/cov/pwin/pin/fh/fs/vh`) plus targeted trust-gate lines before skipping an xHCI MMIO source.
- On the active Pi 4 xHCI path, local-seat replays the bounded runtime VL805 mailbox reset-notify only when runtime PCI config replay is available. Linux resets VL805 before it enumerates and reprograms BAR0/COMMAND; the current Cohesix Pi 4 safe-mode path keeps VL805 ECAM reads/writes disabled, so boot-time root-task only uses the captured high BAR after an explicit firmware/HAL/U-Boot source names it and still performs poll-only runtime ownership without binding raw PCIe child-INTx or bridge sinks before controller entry. The default source is the post-`usb stop` U-Boot high-BAR contract when a menu/input USB session was active; HAL/seL4 device-untyped coverage remains the no-handoff source. Capability layout uses the known-good Linux VL805 capture for the high BAR unless a legacy DT snapshot is present. Prompt-safe selection permits the unseeded live-default `handoff=none` path after a verified high-BAR source exists, publishing Cohesix-owned DCBAA, scratchpads, command ring, and event ring before RUN without preserving firmware-owned ring state. Stop-state evidence remains available as a bounded fallback/diagnostic lane. The runtime now also emits a compact seed/branch checkpoint before halt revalidation, so a missing skip marker after a seeded attempt identifies seed propagation rather than a later `RUN` edge fault. Any remaining `xhci.diag stage=0x0253 tag=crcr-read-begin` breadcrumb is therefore part of the runtime-owned path, not a preserved stop-state shortcut.
...
- As-built correction: current prompt-safe Pi 4 high-BAR probing only makes the Cohesix-owned live-default lane usable after an explicit firmware/HAL/seL4 device-untyped source names the Linux-captured VL805 BAR0. In that case it uses the captured capability layout instead of an unsafe early live cap read and permits `handoff=none` in prompt-safe mode. When U-Boot exports trusted bootloader stop-state authority, the reset-owned stop-seed runtime lane remains a fallback rather than replacing the first live-default attempt.
