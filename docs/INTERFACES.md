...
- Current M26b correction: the ARMCR4 `AI_RESETCTRL` reset-assert edge is no longer allowed to start with the special CMD52 transfer-window write at Function 1 address `0x0b800`. It now starts with the canonical Linux-shaped CMD53 word write and logs `mode=cmd53-word-windowed fallback=cmd52-byte-current-window`; any fallback CMD52 uses the ordinary Function 1 current-window byte address `0x03800` only after the word path fails.
...
- The Pi 4 USB deferred-capture contract requires command-ring proof. `command_probe=no-op-deferred` is no longer sufficient; the active high-BAR no-candidate runtime now submits the cold-boot-first `Enable Slot` command as the first command-ring entry, matching the Linux-observed first real xHCI command instead of queuing a No Op ahead of it. The non-PCIe diagnostic lane keeps the older poll-only No Op proof. `command_probe=enable-slot-ok` and its cleanup-failed variant prove command/event-ring consumption because cleanup happens after the Enable Slot completion. Historical `enable-slot-linux-event-ok` / `no-op-linux-event-ok` labels are diagnostic only and do not satisfy the cold-boot USB gate. A concrete `tag=cmd-timeout` now normalizes as `USB_BLOCKER=cmd-timeout`, and concrete `enable-slot-unproven`, `no-op-linux-event-unproven`, or legacy `enable-slot-linux-event-unproven` proof summaries normalize as `USB_BLOCKER=cmd-event-ring-timeout`; these supersede the earlier timer-only `cmd-poll-pending` state.
- Current M26b USB correction: when the Pi 4 platform-reset lane has no port-event candidates, local-seat uses the prompt-safe Linux-shaped event-generation path (`USBCMD.RUN|INTE`, `IMOD=0xa0`, `IMAN.IE`) while PCI INTx/MSI/GIC delivery remains masked, then submits `Enable Slot` first. The previous No Op discriminator could leave an unconsumed command in front of the Linux-captured cold-boot command; it is no longer part of the active high-BAR proof. A successful `enable-slot-ok` moves the proof frontier to safe port state; any missing command completion remains `cmd-event-ring-timeout`, and IRQ27 remains the seL4 timer only.
- If that direct high-BAR `Enable Slot` proof reaches the prompt-safe command timeout, `usb-oxide` performs one bounded fresh command/event-ring recovery before returning to the shell. The recovery allocates new HAL DMA-backed command and event rings, republishes `ERSTSZ`/`ERSTBA`/`ERDP`/`CRCR` with the Pi 4 PCIe DMA alias, drains the posted CRCR write through the HAL EXT_CFG flush hook, and retries the same command once while PCI INTx/MSI delivery remains masked. A second timeout remains `cmd-event-ring-timeout`, now proving the blocker survived fresh ring publication rather than stale command/event-ring state.
