...
- The Pi 4 local-seat runtime treats a firmware DT `xhci` node marked `status = "disabled"` as stale handoff rather than immediate loss of keyboard support: it translates BCM2711 `0x7e...` SoC-bus `reg` values into CPU physical addresses for diagnostics, ignores stale disabled-node hints as active xHCI runtime sources, pins the trusted handed-off xHCI BAR before the higher VL805 ECAM page so the lower handoff aperture is not lost to monotonic device retype ordering, keeps VL805 ECAM preseed on a map-only path because live ECAM reads still trigger fatal irq 27 entries on Pi 4, avoids broader live runtime VL805 config-space discovery while that path remains unsafe on Pi 4, emits concise candidate-decision breadcrumbs (`kind/cfg/cov/pwin/pin/fh/fs/vh`) plus a targeted firmware-handoff trust-gate line (`cmd_safe/token/irq/trusted/reason`) before skipping an xHCI MMIO source, treats the standard U-Boot `usb stop` handoff as a cold-start opportunity for a safe high BAR only when the bootloader exports the safe post-stop contract, and treats VL805 reset as bootloader-owned on that path instead of issuing a runtime mailbox reset request. If the bootloader contract is rejected, runtime no longer uses the presence of the pinned config window as permission to probe the high BAR anyway; it now emits `xhci runtime blocked action=reject-untrusted-high-bar ...` and leaves local-seat degraded instead of touching xHCI MMIO after an untrusted handoff. The low `0xfe980000` / `0x7e980000` BCM2711 USB aliases are not VL805 runtime sources on Pi 4; they identify the SoC `brcm,bcm2835-usb` block and now remain diagnostic breadcrumbs only. Runtime uses the seL4 bcm2711 `device-untypes@600000000` window for the U-Boot-discovered VL805 BAR when present, logs whether the handed-off BAR still has device coverage without touching ECAM (`xhci handoff window mmio=... coverage=... pinned=...`), prefers the bootloader-exported capability snapshot over a live first-touch xHCI capability read on that trusted handoff BAR, passes the validated snapshot into `usb-oxide`, and keeps the trusted runtime polling-driven while arming only a bounded PCIe INTx sink on GIC SPI 143 instead of requesting primary seL4 xHCI IRQ handlers. Root-task now emits `xhci irq policy mmio=... mode=poll-only+pcie-intx-sink reason=fw-handoff-cold-start` before controller bring-up, so the next remaining failure is attributable to the controller transition itself rather than broad IRQ ownership reconstruction. Root-task also now logs the DT-exported handoff detail that explains a partial contract: `xhci-handoff-halted=...`, `xhci-handoff-safe=...`, `xhci-stop-state usbcmd=... usbsts=... iman0=...`, `xhci-handoff-dtb-ext source=... prestop=ready/irq/halted/safe poststop=ready/irq/halted/safe`, and `xhci-handoff-reject source=... reason=...`. The trusted-high path still skips only the redundant pre-quiesce work inherited from firmware, and it now stays read-only across both live halt revalidation and `HCRST`: the latest Pi 4 traces showed that the first pre-reset `USBCMD` scrub write, the first live halt-revalidation read, and the first `HCRST` write can all still terminate the board on the trusted handoff BAR. Runtime therefore emits the skip breadcrumbs for the pre-reset `USBCMD` / `USBSTS` / `IMOD` / `IMAN` scrub on that path, trusts the bootloader-exported halted/safe contract instead of issuing any live `USBCMD` / `USBSTS` revalidation reads, skips `HCRST`, and adopts the handed-off halted controller state before `CONFIG` / `DCBAAP` / `CRCR` programming. On that resetless adoption path it preserves the firmware-owned controller configuration where `usb-oxide` already has explicit skip branches (`CONFIG`, `IMOD`, `IMAN`, readbacks), still zeros `DNCTRL` before the first command can observe stale device notifications, preserves the controller-reserved bits U-Boot preserves when programming `CRCR` / `ERSTSZ` / `ERSTBA`, primes the initial `ERDP` without `EHB` set, and now keeps the first xHCI ownership pointers inside the low identity-mapped BCM2711 PCIe DMA window instead of adding a synthetic `0x4_0000_0000` bus alias before using the normal polling-mode run transition on the adopted controller. Those first 64-bit ownership writes now emit split low/high breadcrumbs (`dcbaap-write-low`, `dcbaap-write-high`, `crcr-write-low`, `crcr-write-high`, `erstba-write-low`, `erstba-write-high`, `erdp-write-low`, `erdp-write-high`) and use strict low-then-high programming order so a remaining post-handoff fault identifies the exact half-write that failed. Live aarch64 xHCI MMIO accesses now also use U-Boot-style `dmb` ordering around register reads and writes, so the trusted handoff path matches the bootloader’s device-access discipline more closely before the first ownership transition. The reset path itself is still named in the diag stream (`reset-write`, `reset-complete`, `reset-hcrst-timeout`, `reset-cnr-timeout`), but the trusted handoff path now emits `fw-handoff-skip-reset` instead of touching `HCRST`, so a remaining failure moves to the first later ownership write rather than the reset edge itself. To cover read-side stalls, it emits named pre-read tags before the critical MMIO readbacks (`config-readback-begin`, `dcbaap-readback-begin`, `crcr-readback-begin`, `runtime-ring-readback-begin`, `usbsts-run-readback-begin`, `usbcmd-run-readback-begin`, `controller-ready-poll-begin`) plus the matching result/terminal tags (`config-readback`, `dcbaap-readback`, `crcr-readback`, `runtime-ring-readback`, `usbsts-run-readback`, `usbcmd-run-readback`, `controller-ready`, `controller-ready-timeout`). If the first connected-port mask is still empty after controller bring-up, local-seat now emits per-port `xhci root-port stage=detect-zero|detect-slow-zero|detect-slow-hit ... portsc=...` breadcrumbs and performs one bounded 100 ms slow recheck before declaring that no root-port device is present. It now also names the command-path breadcrumbs that were already present in the probe stream and adds explicit timeout-only state snapshots: `cmd-submit`, `cmd-ring-enqueue`, `cmd-completion`, `cmd-ccs-expected-ptr`, `cmd-ccs-mismatch`, `cmd-fail`, `cmd-fail-state`, `cmd-wait-other-event`, `cmd-timeout`, `cmd-timeout-state`, and `cmd-timeout-last-event`. It still explicitly logs the bootloader-exported command word when safe-mode runtime skips a live VL805 PCI-command replay, and otherwise degrades to serial-only input with explicit `[local-seat] ... unavailable` breadcrumbs.
...
- The Pi 4 CYW43455 firmware-load path now keeps the startup SDIO link active through chip discovery, RAM sizing, and AI core disable/reset, then uses aligned CMD53 block-mode firmware writes for Function 1 while AI core-control registers prefer Linux-style flagged 4-byte Function 1 backplane accesses and log explicit current-window and rewindow retries when those control-path reads or writes fail. During AI core disable, the immediate post-`assert-reset` readback is now deferred until after the in-reset configure settle so SOCRAM reset does not fail purely because Function 1 briefly stops answering direct reads during the reset edge.
