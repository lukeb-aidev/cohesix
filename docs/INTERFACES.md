...
- The Pi 4 USB deferred-capture contract requires command-ring proof. `command_probe=no-op-deferred` is no longer sufficient; the active high-BAR no-candidate runtime submits a prompt-safe Linux-shaped No Op command as the first command-ring proof, while the current-boot port-event path submits Enable Slot through the same Linux-shaped bounded/re-doorbell/recovery lane. Live root-port sampling unlocks only after one of those commands completes. `command_probe=no-op-ok`, `command_probe=enable-slot-ok`, and the enable-slot cleanup-failed variant prove command/event-ring consumption. Historical `enable-slot-linux-event-ok` / `no-op-linux-event-ok` labels are diagnostic only and do not satisfy the cold-boot USB gate. A concrete `tag=cmd-timeout` now normalizes as `USB_BLOCKER=cmd-timeout`, and concrete `no-op-unproven`, `enable-slot-unproven`, `no-op-linux-event-unproven`, or legacy `enable-slot-linux-event-unproven` proof summaries normalize as `USB_BLOCKER=cmd-event-ring-timeout`; these supersede the earlier timer-only `cmd-poll-pending` state.
- Current M26b USB correction: the Pi 4 platform-reset lane uses the prompt-safe Linux-shaped event-generation path (`USBCMD.RUN|INTE`, `IMOD=0xa0`, `IMAN.IE`) while PCI INTx/MSI/GIC delivery remains masked. With no port-event candidates it submits No Op; with current-boot port-event candidates it submits Enable Slot through the same command-proof path instead of the plain xHCI command helper. Any missing command completion remains `cmd-event-ring-timeout`, and IRQ27 remains the seL4 timer only.
- If a Linux-shaped prompt-safe command proof reaches the command timeout, `usb-oxide` performs one bounded fresh command/event-ring recovery before returning to the shell. The recovery allocates new HAL DMA-backed command and event rings, republishes `ERSTSZ`/`ERSTBA`/`ERDP`/`CRCR` with the Pi 4 PCIe DMA alias, drains the posted CRCR write through the HAL EXT_CFG flush hook, and retries the same command once while PCI INTx/MSI delivery remains masked. The active no-candidate proof retries No Op; the active current-boot port-event proof retries Enable Slot. A second timeout remains `cmd-event-ring-timeout`, now proving the blocker survived fresh ring publication rather than stale command/event-ring state.
...
- As-built refinement: on the active cold-boot `platform-reset-complete` USB path, the EHB cleanup writes `IMAN.IP` as a write-one-to-clear value and leaves delivery masked at PCI/GIC. Any drained `PORT_STATUS_CHANGE` candidate is still OR-ed into the command-probe mask before selecting the Linux-shaped prompt-safe Enable Slot proof; the no-candidate path uses the same lane with No Op.
