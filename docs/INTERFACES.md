...
- As-built correction: the Pi 4 CYW43455 production post-download `wait-ht-clock` edge preserves the live post-release sideband snapshot and must not emit `post-download-ht-sideband-prime` or `post-download-sleepcsr-clear-set`. A live `SLEEPCSR=0x01` KSO-only result remains wake evidence only, not Function 2 readiness. Root-task still does not add `FORCE_HT` before proof; it replays Linux's SD-only `CHIPCLKCSR=0` fence, issues `HT_AVAIL_REQ`, and waits for real `HT_AVAIL` before production Function 2 traffic. Explicit diagnostics may still run the sideband characterization path.
...
- Current M26b correction: the production Pi 4 Wi-Fi boot path no longer accepts a KSO-only, cached-DEVON, or forced ALP/KSO tuple as Function 2 readiness. Post-download HT probing is bounded, avoids wake/CMD14/SLEEPCSR writes on the production HT request, permits KSO-only only as wake evidence while entering the Linux clock-off/`HT_AVAIL_REQ` sequence, requires real `CHIPCLKCSR.HT_AVAIL` plus Function 2 readiness before strict Function 2 traffic, and leaves sideband primers or no-HT Function 2 probing to explicit debug diagnostics rather than normal boot.
