...
- The unseeded live-default `handoff=none` path remains the next bounded diagnostic candidate when the probe ladder selects it. On the Linux-captured high BAR its preflight now reports `route=trusted-high-bar-primary`, `ctor=capture-quiesce`, `halt_guard=skip-live-halt-read`, `pre=mailbox-reset-required`, `legacy=skip-legacy`, `next=mailbox-reset-notify`, and `followup=mailbox-reset-then-platform-init`. Once `mailbox-acked` evidence exists, a ready capture-backed replay contract (`cfg_window=linux-capture-witness`, `cfg_source=linux-capture-replay`, `cfg_writes=capture-backed`, `command_source=linux-capture-replay`) promotes the lane to `handoff=resetless-reinit`, skips xHCI HCRST/config, and publishes fresh runtime rings under the same poll-only IRQ contract. If capture replay cannot pin the high BAR, bind the PCIe/INTx sinks, or prove the captured command shadow, local-seat keeps `handoff=platform-reset-complete` / `policy=platform-reset-complete` and returns before `usb-oxide` controller construction. If the mailbox notify is unconfirmed, local-seat skips the lane with `stage=pre-xhci-reset ... action=skip-platform-init`.
