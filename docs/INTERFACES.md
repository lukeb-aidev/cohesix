...
- As-built refinement: ARMCR4 clear-reset and post-reset single-byte probes now skip the fragile CMD53 data-phase path entirely and begin on the current-window CMD52 read path (`mode=cmd52-byte-current-window retry=cmd52-byte-rewindow`), so mandatory reset-release proof and repeated `armcr4-core-up` retries no longer spend time bouncing between two failing CMD53 read shapes before the later HT decision. The earlier ARMCR4 `postreset-clock-en-readback` / `postreset-reset-readback` probes now also treat a direct `sdio-cmd52-read` failure as the same bounded fragile post-reset read boundary as the older SDHCI timeout class, so those edges defer/retry instead of terminating bring-up immediately on the first strict CMD52 miss.
- As-built correction: Pi 4 Wi-Fi now raises the SDIO request to the Linux high-speed control-plane lane before the first ARMCR4/SOCRAM reset-control sequence, not only after firmware/NVRAM staging. The first ARMCR4 clear-reset proof therefore no longer runs at the `399361 Hz` startup divider when Function 1 and high-speed CCCR capability are already available.
...
- On the active Pi 4 xHCI path, local-seat replays the bounded runtime VL805 mailbox reset-notify before the Linux-captured high-BAR `handoff=none` live-default lane can touch reset state. Linux resets VL805 before it enumerates and reprograms BAR0/COMMAND; Cohesix keeps generic VL805 ECAM reads/writes disabled and keeps the BCM2711 EXT_CFG_INDEX/DATA window disabled by default because the latest seL4 board trace trapped immediately after entering that lane. The opt-in live path masks and clears the BCM2711 PCIe host INTx/MSI source registers, binds the seL4-visible PCIe host and child INTx-A GIC IDs (`180` and `175`), validates PCIe link state, VL805 vendor/device/class, the Linux PCI bus BAR `0xc0000004/0x00000000` translated through the outbound window, and COMMAND before treating the high BAR as live config authority. With default live EXT_CFG disabled, the Linux capture becomes a capture-backed replay witness only after local-seat pins/promotes the high BAR, records the captured `COMMAND=0x0546` shadow, installs the bounded PCIe sinks, and receives mailbox reset confirmation. The default capture-backed fresh-ring path no longer maps the BCM2711 PCIe status/source page or masks host INTx/MSI sources before ownership writes because prior board traces made that host-page touch suspect; it instead logs `xhci irq pre-reset quiesce begin ... host_mmio=no`, skips host MMIO masking, explicitly ACKs the already-bound GIC IDs `180` and `175`, and keeps controller-ready enumeration poll-driven. That ACK-only gate now covers every high-BAR `platform-reset-complete` publication path, including variants that still carry a stopped seed; the seed is not authority to bypass fresh publication proof. That witness may then authorize the fresh-ring `platform-reset-complete` lane, where `usb-oxide` skips the second HCRST after mailbox reset and performs CONFIG/DCBAAP/ring publication locally. Controller-ready enumeration does not use Linux virtual IRQ 27 or the Linux MSI virq 30 as seL4 IRQ numbers. Opt-in U-Boot handoff can still export halted high-BAR ownership evidence for diagnostics. Capability layout uses the known-good Linux VL805 capture for the high BAR unless a legacy DT snapshot is present. Prompt-safe selection probes the non-authoritative stopped seed exported by the staged script first, but that lane is only a poll-safe policy probe unless runtime-ring ownership is also proven. The reset-owned stop-seed attempt reports `next=policy-return` with `followup=fallback-next` when a live fallback remains, suppresses xHCI constructor entry, and emits no expected xHCI diagnostic stage.
- The unseeded live-default `handoff=none` path remains the next bounded diagnostic candidate when the probe ladder selects it. On the Linux-captured high BAR its preflight now reports `route=trusted-high-bar-primary`, `ctor=capture-quiesce`, `halt_guard=skip-live-halt-read`, `pre=mailbox-reset-required`, `legacy=skip-legacy`, `next=mailbox-reset-notify`, and `followup=mailbox-reset-then-platform-init`. The opt-in BCM2711 replay path masks and clears host PCIe interrupt sources before binding the HAL PCIe/INTx sinks and before xHCI ownership writes; the default capture-backed path binds only the HAL sinks before ownership writes. Once `mailbox-acked` plus capture-backed or live BAR/COMMAND replay evidence exists, local-seat promotes to `handoff=platform-reset-complete` as a fresh runtime-ring lane with `pre=skip-pre-reset`, `run=run-uboot`, and `publish=rings-pre-run`. When the mailbox ACK is present without capture-backed or live COMMAND proof, local-seat still promotes to the platform-reset policy but the local publication gate blocks controller entry and returns to the prompt before touching xHCI ownership registers; a stop-state seed does not bypass that gate.
- Current Pi 4 xHCI prompt-safe ordering is stricter when a safe halted stop-state exists: local-seat attempts `handoff=none` / `origin=reset-owned-stop-seed` first for both the default U-Boot-stop-seed path and the mailbox/bootloader-authorized paths, treats that attempt as a controller-entry skip when it has no runtime-ring ownership, then falls back to the unseeded live-default lane only for trusted diagnostic strategies that explicitly allow a fallback. That fallback is separate from the stopped seed and must pass the mailbox-reset notify gate before the `platform-reset-complete` replay-proven lane starts. It keeps xHCI event progression poll-only, rejects seL4 IRQ 27 as an xHCI source because it is the kernel virtual-timer PPI, and never uses raw CPU DMA addresses when the `0x4_00000000` PCIe DMA alias is enabled. On high-BAR pre-controller ownership writes, local-seat binds only the PCIe host and child INTx sink IDs (`180` and `175`); IRQ27 remains kernel-owned and diagnostic-only.
- The preserved-controller deferred-ownership breadcrumbs remain available inside `usb-oxide` for generic regression work, but the active Pi 4 local-seat runtime no longer depends on them. Posted-fallback mailbox resets can still keep the high-BAR candidate alive on Pi 4 as diagnostic evidence, but the default runtime high-BAR lane is Cohesix-owned cold start using the Linux capture as layout evidence after the local window is pinned. Bootloader post-stop reset authority remains opt-in fallback evidence rather than the default source.
- The Pi 4 CYW43455 firmware-load path keeps Function 1 sideband active through chip discovery, RAM sizing, and AI core disable/reset. During backplane prep it mirrors the Linux `brcmf_sdio_buscoreprep()` clock shape and performs the `SDIOPULLUP` (`0x1000F`) clear as a single-shot best-effort write after the 4-bit lane is active and Function 1 sideband is readable; a failed clear is recovered and logged as non-terminal. The Linux `brcmf_sdio_probe_attach()` lane leaves Function 2 disabled, so Cohesix no longer attempts the pre-firmware F2 enable that latched `IOEX=0x06` without `IOR2`. Before upload, root-task switches the card and host into the Linux high-speed lane by programming CCCR `SPEED.EHS` and setting SDHCI `HOST_CONTROL.HISPD`, then mirrors Linux's pre-download `alp_only` clock edge by requesting `CHIPCLKCSR.ALP_AVAIL_REQ`; `ALP_AVAIL` is sufficient for firmware/NVRAM upload, while a readable `HT_AVAIL` bit is only stronger diagnostic evidence at this phase. If `HT_AVAIL` is not visible, Cohesix logs `firmware stage=write-firmware-bulk policy=conservative-alp-only` and uses byte-mode firmware writes with the 1.5625 MHz / 800 kHz / restore-clock ladder instead of the fast 25 MHz ladder. Firmware upload still follows the non-debug Linux order: write image, write NVRAM/tail, then release ARMCR4; the upstream `brcmf_sdio_verifymemory()` RAM compare is a Linux `DEBUG` build path, so Cohesix logs `firmware_verify policy=skip-readback ... reason=linux-nondebug-debug-only-readback` instead of forcing a pre-NVRAM Function 1 readback. Immediately after ARMCR4 release, Cohesix emits `post-release-proof` breadcrumbs that summarize the firmware upload range, normalized NVRAM range, NVRAM tail magic, reset-vector address/value, CPUHALT release, upload/current/preferred clocks, and cached CHIPCLKCSR/WAKEUPCTRL/SLEEPCSR/CARDCAP labels without adding another backplane read. After upload, root-task follows the Pi 4 non-SR post-download clock edge: clear to SD-only only for the first pass, prime wake/CMD14/KSO sideband (`WAKEUPCTRL.HTWAIT`, `CARDCAP=CMD14_SUPPORT|CMD14_EXT`, KSO), record DEVON only as diagnostic evidence, request `CHIPCLKCSR.HT_AVAIL_REQ`, and poll with Linux-scale bounded `active-ht-*` breadcrumbs. A first-pass `0x50` (`HT_AVAIL_REQ|ALP_AVAIL`) remains a timeout shape, not an HT proof; after that shape, the bounded retry may write `0x12` (`HT_AVAIL_REQ|FORCE_HT`) without replaying the SD-only fence. If the retry reaches `HT_REQ|FORCE_HT|ALP_AVAIL` with wake/CMD14/KSO sideband complete, Cohesix enters the explicit Pi 4 no-HT Function 2 probe, preserves that forced HT request during the poll, and may continue once on an `IOEX.F2` latch even when CCCR `IOR2` remains clear. Function 2 enable then primes the Linux-sized 512-byte FBR block size, and after F2 readiness or the bounded no-HT latch probe Cohesix refreshes the same idempotent SR sideband at `stage=post-function2-sr-init`. HAL still retains the guarded startup-link fallback and reply-rearm ladder, but no-HT Function 2 writes remain on the same 512-byte cadence while reply reads stay on smaller conservative bursts. The serial path reports that split through `write_chunk_limit=...`, `reply_chunk_limit=...`, `sdio function2 ready-snapshot ... diagnosis=...`, and `action=experimental-continue-without-ready`, so the next failure can be attributed to ALP upload readiness, KSO sideband proof, HT/forced-tuple state, F2 ready state, SR sideband state, packetization, or reply visibility instead of a generic promoted-rearm timeout.
- As-built refinement: the post-ARMCR4-release `post-release-proof` breadcrumbs now include a compact tuple for the firmware image range, normalized NVRAM range, NVRAM tail magic, reset-vector address/value, CPUHALT release value, upload/current/preferred SDIO clocks, and cached `CHIPCLKCSR` / `WAKEUPCTRL` / `SLEEPCSR` / `CARDCAP` labels. The tuple is derived from already-computed upload layout and cached sideband state, so it proves or falsifies the release preconditions without adding a fragile post-release Function 1 read.
- As-built correction: the Pi 4 CYW43455 post-download `wait-ht-clock` edge mirrors the captured Linux `CLK_SDONLY -> CLK_AVAIL` order only on the first pass while preserving the proven Cohesix wake sideband. Root-task primes `WAKEUPCTRL.HTWAIT` / CMD14 cardcap / KSO as wake evidence, treats DEVON as diagnostic before HT, then issues `HT_AVAIL_REQ`. If that first pass reaches the Pi 4 `0x50` timeout shape, the bounded retry issues `HT_AVAIL_REQ|FORCE_HT` without another SD-only bounce; a `0x52` tuple may continue into the explicit Pi 4 no-HT Function 2 probe, where strict `IOR2` remains preferred but one short latch-only probe can test direct F2 FIFO traffic. Console HT probes that run after a failed boot use the same wake sideband prime and keep the more informative cached boot snapshot.
...
- As-built refinement: before ARMCR4 release, root-task now emits `firmware stage=nvram-normalize raw=... normalized=... total_with_tail=... state=...` and performs a bounded proof of the normalized NVRAM head, normalized NVRAM tail, Broadcom tail token, and reset-vector write. Read-unavailable outcomes are diagnostic because Function 1 may still be fragile, but any proven byte mismatch fails before ARMCR4 release. Function 2 remains blocked until either readable `CHIPCLKCSR.HT_AVAIL` is observed or the Pi 4-specific `HT_REQ|FORCE_HT|ALP_AVAIL` plus wake/CMD14/KSO tuple is present; the latter enters only the bounded no-HT latch probe when CCCR `IOR2` does not appear.
- Current Pi 4 CYW43455 high-speed behavior follows the Linux capture: firmware upload and normal control-plane work request a 50 MHz SDIO target on the 4-bit high-speed lane, with the BCM2711 SDHCI mailbox-reported `100000000 Hz` rate corrected to the effective Arasan `250000000 Hz` parent so the legal divider produces the observed `41666666 Hz` transfer clock. The bounded pre-Function-2 `FORCE_HT` retry is a clock-start attempt, not proof; strict transport still requires real CCCR `IORX` readiness, while the Pi 4 no-HT probe may continue once from a visible `IOEX.F2` latch to test firmware/control-plane traffic directly. `wait-firmware-ready` no longer lowers a proven promoted link to 400 kHz for mailbox polling, and pre-HT control-plane staging preserves the current promoted lane instead of forcing a startup-clock detour.
- The initial `setup-firmware-channel` pass and the later rearm block both use the same bounded low-clock write window. On both strict/full and bounded no-HT paths, the default block covers mailbox version / `HOSTINTMASK` / CYW43455 watermark / `DEVCTL(F2WM_ENAB)` / `MESBUSYCTRL` and deliberately defers the interrupt-side `function-int-mask`, `CCCR.IENx`, and SDHCI `CARD_INT` signal arming. If the control-plane link is already above the startup clock, HAL temporarily lowers those critical SDIO-core and Function 1 register writes back to the startup clock, emits `firmware-channel-write-clock action=lower ...`, performs the setup/rearm block, and then emits `action=restore ...` when the faster control-plane clock is restored. This keeps the critical mailbox/device-control writes on the safer transfer shape without pretending failed writes committed.
- On the bootloader-authorized stop-state-only xHCI branch, runtime still uses the trusted CAP layout plus stop-state diagnostics, but it no longer preserves firmware-owned ring state or the abandoned stop-state deferred `CRCR` shortcut. The first captured high-BAR prompt-safe attempt is now `mode=none` / `origin=reset-owned-stop-seed` when a stopped seed exists. That stop-state-only snapshot suppresses controller entry unless runtime-ring ownership is proven; in the common no-ring-seed case local-seat returns from the attempt before xHCI constructor/pre-reset scrub writes, live `USBCMD.HCRST`, `IMAN`, fresh runtime ownership publishes, or `RUN`. On weaker seeded `cold-start-from-snapshot` fallback evidence, runtime stays no-touch and non-enumerating. Any remaining `crcr-read-begin` breadcrumb is therefore part of an enumerating runtime-owned fallback path rather than a preserved stop-state handoff. The broader deferred-ownership ladder remains an internal `usb-oxide` recovery tool, not part of the active Pi 4 handoff contract.
- As-built correction: current prompt-safe Pi 4 high-BAR probing makes the reset-owned stop-seed lane usable as a poll-safe diagnostic path from the Linux-captured VL805 BAR0 layout once the local high-BAR window is pinned and the staged stopped seed is present. It uses the captured capability layout instead of an unsafe early live cap read and permits `handoff=none` in prompt-safe mode. When U-Boot exports trusted bootloader stop-state authority through the opt-in handoff path, the same reset-owned stop-seed lane remains the first attempt, but without runtime-ring ownership it returns immediately as `policy-return`/`fallback-next`; the unseeded live-default lane remains the next diagnostic fallback until stronger ownership proof exists. That Linux-captured high-BAR live-default fallback now expects `0x0204 tag=fw-handoff-skip-pre-quiesce` immediately after `0x0117` only until the mailbox reset gate is acknowledged; the capture-backed or live replay-proven path enters `platform-reset-complete` with `pre=skip-pre-reset`, `run=run-uboot`, and `publish=rings-pre-run`, while a mailbox ACK without BAR/COMMAND proof selects the same platform policy but is stopped by the local publication gate before controller entry.
- The trusted Pi 4 xHCI `RUN` edge now has its own bounded serial diagnostics. `xhci.diag stage=0x0217` records the halt-revalidation decision (`handoff`, seed-flag bitmask, and whether live rereads will be skipped), `0x0218` records entry into the skip branch, `0x0213` / `0x0214` bracket the first live `USBSTS` halt-revalidation read, `0x0215` / `0x0216` bracket the first live `USBCMD` halt-revalidation read, `0x02da` means the `USBCMD.RUN` store returned, `0x02db` records the first `USBCMD` readback after that store, `0x02dc` records the first `USBSTS` sample after `RUN`, `0x02dd` emits bounded HCH-wait progress snapshots, `0x02df` records observable `USBSTS` transitions during that wait, and `0x02de` captures the final `USBCMD`/`USBSTS` pair before a `0x0272` timeout. On the trusted preserve-state stop-snapshot path, runtime now seeds `RUN` from the bootloader-exported `USBCMD` snapshot and emits `0x02ee` before the store instead of performing a fresh live `USBCMD` read; live read-modify-write seeds remain visible through `0x02ec` / `0x02ed`, and fully blind paths emit `0x02ef`. That ladder now makes the remaining USB failure attributable to one of seven phases: seed propagation, the skip/live halt-revalidation split, the first live halt-revalidation `USBSTS` read, the follow-up `USBCMD` read, a toxic live seed read, the `RUN` store itself, the first post-`RUN` reads, or a controller that accepts `RUN` but never drops `HCH`.
