...
- Current June 9 Wi-Fi correction: `/Users/lukasbower/pi4-serial-20260609-093312.log` supersedes the older SDIO-card-select wording for the active interface frontier. The selected Wi-Fi interface now reaches linked-runtime intake proof for both `sdio-host` and `cyw43455`, but CYW43455 descriptor replay still no-replies before `command-observed`; the projected first failed gate remains `hal-power-reset`, while the direct blocker is linked-runtime command delivery at gate 0. Root publishes fixed-ring command records with sequence last, and linked runtimes consume bounded one-way turns from the shared ring plus endpoint poll before any SDIO, firmware, DHCP, `nettest`, or `netstats` claim can be accepted. The linked CYW43 control ABI now separates fire-and-forget SDPCM control frames from matched `CONTROL_EXCHANGE` turns: station setup must receive the expected CDC command/ioctl-id reply, and read iovars such as `cur_etheraddr` return a bounded CDC response frame before the root-side host-EAPOL and DHCP/data gates can advance.
