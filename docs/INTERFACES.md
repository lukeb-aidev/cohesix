...
- The Pi 4 local-seat runtime treats a firmware DT `xhci` node marked `status = "disabled"` as stale handoff rather than immediate loss of keyboard support: it translates BCM2711 `0x7e...` SoC-bus `reg` values into CPU physical addresses for diagnostics, ignores stale disabled-node hints as active xHCI runtime sources, pins the trusted handed-off xHCI BAR before the higher VL805 ECAM page so the lower handoff aperture is not lost to monotonic device retype ordering, keeps VL805 ECAM preseed on a map-only path because live ECAM reads still trigger fatal irq 27 entries on Pi 4, avoids broader live runtime VL805 config-space discovery while that path remains unsafe on Pi 4, emits concise candidate-decision breadcrumbs (`kind/cfg/cov/pwin/pin/fh/fs/vh`) plus a targeted firmware-handoff trust-gate line (`cmd_safe/token/irq/trusted/reason`) before skipping an xHCI MMIO source, and treats the standard U-Boot `usb stop` handoff as a cold-start opportunity for a safe high BAR only when the bootloader exports the safe post-stop contract. If the bootloader contract is rejected, runtime no longer uses the presence of the pinned config window as permission to probe the high BAR anyway; it now emits `xhci runtime blocked action=reject-untrusted-high-bar ...` and leaves local-seat degraded instead of touching xHCI MMIO after an untrusted handoff. The low `0xfe980000` / `0x7e980000` BCM2711 USB aliases are not VL805 runtime sources on Pi 4; they identify the SoC `brcm,bcm2835-usb` block and now remain diagnostic breadcrumbs only. Runtime uses the seL4 bcm2711 `device-untypes@600000000` window for the U-Boot-discovered VL805 BAR when present, logs whether the handed-off BAR still has device coverage without touching ECAM (`xhci handoff window mmio=... coverage=... pinned=...`), and still logs the DT-exported handoff detail that explains a partial contract: `xhci-handoff-halted=...`, `xhci-handoff-safe=...`, `xhci-stop-state usbcmd=... usbsts=... iman0=...`, `xhci-handoff-dtb-ext source=... prestop=ready/irq/halted/safe poststop=ready/irq/halted/safe`, and `xhci-handoff-reject source=... reason=...`. When the trusted high BAR is selected, local-seat now avoids runtime VL805 mailbox/config replay on that path because those config-space touches are still unsafe on Pi 4, keeps the bootloader-exported safe PCI command word untouched, cold-starts xHCI directly from the trusted bootloader capability snapshot, and installs only the bounded bcm2711 PCIe INTx sink while leaving xHCI ownership itself polling-driven. The active proof line is `xhci runtime trust source=fw-handoff-direct-snapshot ...`, and local-seat also emits `vl805 reset handoff=bootloader-owned stage=runtime detail=skip-runtime-mailbox-reset action=cold-start-from-snapshot` plus `xhci handoff=bootloader-owned stage=runtime detail=skip-mailbox-reset+trusted-cap-snapshot action=cold-start-from-snapshot` before entering `usb-oxide`. If no trusted snapshot exists, it falls back to a live xHCI capability probe and cold-init path. The high-BAR path keeps command completion and enumeration on the polling path, the command-path breadcrumbs (`cmd-submit`, `cmd-ring-enqueue`, `cmd-completion`, `cmd-ccs-expected-ptr`, `cmd-ccs-mismatch`, `cmd-fail`, `cmd-fail-state`, `cmd-wait-other-event`, `cmd-timeout`, `cmd-timeout-state`, `cmd-timeout-last-event`) remain the primary evidence surface if initialization still stalls, safe-mode runtime still skips live VL805 PCI-command replay while ECAM reads are unsafe, and otherwise root-task degrades to serial-only input with explicit `[local-seat] ... unavailable` breadcrumbs.
- The preserved-controller `DCBAAP` probe breadcrumbs remain available inside `usb-oxide` for regression work, but the active Pi 4 local-seat runtime no longer depends on them. On the current path, `xhci runtime trust source=fw-handoff-direct-snapshot ...`, `vl805 reset handoff=bootloader-owned stage=runtime detail=skip-runtime-mailbox-reset action=cold-start-from-snapshot`, and `xhci handoff=bootloader-owned stage=runtime detail=skip-mailbox-reset+trusted-cap-snapshot action=cold-start-from-snapshot` mean runtime kept the trusted bootloader capability snapshot while avoiding runtime VL805 mailbox/config writes and explicitly reinitialising xHCI itself. `usb-oxide` now skips the constructor-time polling-scrub writes, the init-time pre-quiesce scrub, the legacy-ownership claim, and the live halt revalidation read on that trusted cold-start path because those early ownership MMIO touches have proven unsafe there, so the earliest breadcrumbs should now be `fw-handoff-skip-usbcmd-mask-write`, `fw-handoff-skip-usbsts-clear-write`, `fw-handoff-skip-imod-write`, `fw-handoff-skip-iman-write`, `fw-handoff-skip-pre-quiesce`, `fw-handoff-skip-legacy-ownership`, and `fw-handoff-skip-stop-revalidation`. After that, the trusted cold-start path now mirrors U-Boot's post-reset ownership sequence more closely: it seeds `CONFIG`, `CRCR`, and `ERST*` from live post-reset reads (`config-read`, `crcr-read`, `runtime-ring-read`), preserves non-slot/reserved bits before writing `config-write`, `crcr-write-low`, `erstsz-write`, and `erstba-write-low`, writes `RUN` from a live `usbcmd-run-read`, and only zeroes `imod-write` / `iman-write` after `controller-ready` instead of before `RUN`. The decisive bring-up sequence is therefore `reset-write`, `reset-write-issued`, `reset-complete`, `config-read`, `config-write`, `crcr-read`, `crcr-write-low`, `erdp-write-low`, `runtime-ring-read`, `erstsz-write`, `erstba-write-low`, `usbcmd-run-read`, `usbcmd-run-write`, `controller-ready`, `imod-write`, and `iman-write` rather than the older resetless skip-reset/skip-config sequence. If logs stop before `reset-write-issued`, the reset store itself faulted; if they stop after `reset-write-issued`, the next suspect is the first post-reset seed read/handshake. On that trusted handoff path, command submission also rings xHCI doorbells without the extra posted readback that `usb-oxide` used previously, matching the working U-Boot doorbell sequence more closely before first enumeration.
