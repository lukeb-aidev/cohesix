...
- As-built correction: the Pi 4 CYW43455 post-download `wait-ht-clock` edge now primes bounded Broadcom wake/CMD14 sideband, preserves cached `SLEEPCSR=0x03` DEVON evidence when present, and otherwise uses the bounded `SLEEPCSR=0 -> DEVON -> SLEEPCSR.KSO -> KSO|DEVON` probe before requesting HT. The current path skips the SD-only chip-clock fence because the latest board trace showed `SLEEPCSR=0x01` plus a stable `CHIPCLKCSR=0x50` after that fence, then fast-fails that stable shape after 8 polls. Root-task still does not add `FORCE_HT` before proof; it issues `HT_AVAIL_REQ` directly and waits for real `HT_AVAIL` before production Function 2 traffic.
...
- Current M26b correction: the default Pi 4 USB path treats the Linux VL805 capture as BAR/capability layout evidence only. Live PCI config COMMAND proof through BCM2711 EXT_CFG is opt-in diagnostic after the latest board trace halted immediately after EXT_CFG select, so capture-only high-BAR pinning and a captured `COMMAND=0x0546` shadow are still not enough to touch DCBAAP. The default path blocks fresh publication and returns to the UART prompt until a HAL-owned BCM2711 PCIe host config operation exists. The default path still does not use Linux virq 27 or Linux MSI virq 30 as seL4 xHCI IRQs.
