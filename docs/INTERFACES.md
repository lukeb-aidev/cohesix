...
- Current Pi 4 xHCI prompt-safe ordering is stricter when a safe halted stop-state exists: local-seat attempts `handoff=none` / `origin=reset-owned-stop-seed` first for both the default U-Boot-stop-seed path and the mailbox/bootloader-authorized paths, treats that attempt as a controller-entry skip when it has no runtime-ring ownership, then falls back to the unseeded live-default lane only for trusted diagnostic strategies that explicitly allow a fallback. That fallback is separate from the stopped seed and must pass the mailbox-reset notify gate before the `platform-reset-complete` replay-proven lane starts. It keeps xHCI event progression poll-only, rejects seL4 IRQ 27 as an xHCI source because it is the kernel virtual-timer PPI, and never uses raw CPU DMA addresses when the `0x4_00000000` PCIe DMA alias is enabled. On high-BAR pre-controller ownership writes, local-seat binds only the PCIe host and child INTx sink IDs (`180` and `175`); IRQ27 remains kernel-owned and diagnostic-only. When a seeded `platform-reset-complete` path has live HAL BAR/COMMAND proof and reaches controller-ready, local-seat still defers root-port `PORTSC` sampling because the latest board trace proved that first post-`RUN` port read is toxic on the stop-seeded path; it drains event-ring `PORT_STATUS_CHANGE` candidates into `event_candidate_mask`, runs `enable_slot`/`disable_slot` only when candidates exist, and runs a command-ring No Op proof when `event_candidate_mask=0`. It does not call root-port reset/enumeration helpers. Bootloader-owned stop-seed and preserve-state lanes also skip `PORTSC`.
...
- As-built correction: the Pi 4 CYW43455 production post-download `wait-ht-clock` edge now emits `post-download-ht-sideband-prime`, programs wake/CMD14 sideband, and must not emit `post-download-sleepcsr-clear-set` when live `SLEEPCSR=0x03` is already ready for Function 2. Cached DEVON is diagnostic only: if production must write SLEEPCSR before HT proof, it writes KSO-only (`0x01`) rather than replaying cached `0x03`. A live `SLEEPCSR=0x01` KSO-only result remains wake evidence only, not Function 2 readiness, but it no longer blocks the production HT request itself. Root-task still does not add `FORCE_HT` before proof; it replays Linux's firmware-callback SD-only `CHIPCLKCSR=0` fence, issues `HT_AVAIL_REQ`, and waits for real `HT_AVAIL` before production Function 2 traffic. Explicit diagnostics may still run forced-tuple characterization paths.
...
- Current M26b correction: the production Pi 4 Wi-Fi boot path no longer accepts a KSO-only, cached-DEVON, or forced ALP/KSO tuple as Function 2 readiness. Post-download HT probing is bounded, primes wake/CMD14 sideband on the production HT request, preserves an already-ready live `SLEEPCSR.KSO|DEVON` without the clear/set ladder, treats KSO-only as enough wake evidence to issue the real HT request, requires real `CHIPCLKCSR.HT_AVAIL` plus Function 2 readiness before strict Function 2 traffic, and leaves no-HT Function 2 probing to explicit debug diagnostics rather than normal boot.
