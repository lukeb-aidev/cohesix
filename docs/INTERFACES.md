...
- On the active Pi 4 xHCI path, local-seat replays the bounded runtime VL805 mailbox reset-notify before the Linux-captured high-BAR `handoff=none` live-default lane can touch reset state. Linux resets VL805 before it enumerates and reprograms BAR0/COMMAND; Cohesix keeps generic VL805 ECAM reads/writes disabled, but the Pi 4 path now uses the HAL-mapped BCM2711 EXT_CFG_INDEX/DATA aperture to validate PCIe link state, VL805 vendor/device/class, the Linux PCI bus BAR `0xc0000004/0x00000000` translated through the outbound window to CPU MMIO `0x600000000`, and COMMAND before treating the high BAR as fresh runtime ownership. Before seL4 IRQ binding or Cohesix-owned pre-controller writes, local-seat masks and clears the BCM2711 PCIe host INTx/MSI source registers, then HAL binds the seL4-visible PCIe host and child INTx-A GIC IDs (`180` and `175`) as bounded level-triggered sinks; controller-ready enumeration remains poll-driven and does not use Linux virtual IRQ 27 or the Linux MSI virq 30 as seL4 IRQ numbers. Opt-in U-Boot handoff can still export halted high-BAR ownership evidence for diagnostics. Capability layout uses the known-good Linux VL805 capture for the high BAR unless a legacy DT snapshot is present. Prompt-safe selection probes the non-authoritative stopped seed exported by the staged script first, but that lane is only a poll-safe policy probe unless runtime-ring ownership is also proven. The reset-owned stop-seed attempt reports `next=policy-return` with `followup=fallback-next` when a live fallback remains, suppresses xHCI constructor entry, and emits no expected xHCI diagnostic stage.
- The unseeded live-default `handoff=none` path remains the enumerating candidate when the probe ladder selects it. On the Linux-captured high BAR its preflight now reports `route=trusted-high-bar-primary`, `ctor=capture-quiesce`, `halt_guard=skip-live-halt-read`, `pre=mailbox-reset-required`, `legacy=skip-legacy`, `next=mailbox-reset-notify`, and `followup=mailbox-reset-then-platform-init`. Once `mailbox-acked` evidence exists, local-seat keeps the no-config case as `handoff=platform-reset-complete` / `policy=platform-reset-complete` and returns before `usb-oxide` controller construction. When BCM2711 config replay has validated link state, VL805 identity, the translated captured PCI bus BAR, and COMMAND, the same mailbox-acked lane promotes to `handoff=resetless-reinit`, skips xHCI HCRST/config, and publishes fresh runtime rings. If the mailbox notify is unconfirmed, local-seat skips the enumerating lane with `stage=pre-xhci-reset ... action=skip-platform-init`.
