...
- On the weaker stop-state-only xHCI snapshot branch, runtime still drops all bootloader-owned ring state, but it now re-enters `usb-oxide` through `cold-start-from-stop-state-snapshot` using only the trusted CAP layout plus the stop-state fields (`USBCMD`/`USBSTS`/`IMAN0`), skips replaying the second live runtime `HCRST` store, and then returns to the normal cold-start `CONFIG` and ring publication flow. The UART xHCI stage map still exposes the stronger deferred ownership ladder with named `dcbaap-defer-*`, `erdp-defer-*`, `erstba-defer-*`, and `crcr-defer-*` tags instead of unlabeled `0x02a*` / `0x02b*` / `0x02c*` codes.
- The preserved-controller `DCBAAP` probe breadcrumbs remain available inside `usb-oxide` for regression work, but the active Pi 4 local-seat runtime no longer depends on them. On the trusted high-BAR path, root-task now first issues the bounded VL805 mailbox reset-notify and then lets `usb-oxide` take the cold-start-from-snapshot path that uses the bootloader-exported stop/ring seed snapshot to suppress the fragile pre-reset ownership reads while also treating the mailbox reset as the active reset boundary. The acknowledged VL805 reset-notify now reuses the shared long-lived mailbox request page (`reuse-shared` / `alloc-shared`), while the posted fallback path keeps its own dedicated page. Before any live post-reset xHCI ownership write, `usb-oxide` now inserts a short blind post-reset settle (`reset-post-settle-begin` / `reset-post-settle-done`), then returns to the normal `USBSTS.CNR` completion poll before republishing runtime-owned `CONFIG` / `ERDP` / `ERST*` / deferred `DCBAAP` / deferred `CRCR` / `USBCMD.RUN` state from the bootloader-exported stop/ring seed snapshot instead of resuming live post-reset seed reads. `DCBAAP` itself is now staged as a probe rather than a single opaque write: `0x0257` marks the deferred-publish branch, `0x0258` records the staged-current pointer (live when that read is still allowed, otherwise the trusted snapshot value), the target pointer, and whether a live pre-read happened at all, `0x02a0` records the low/high change mask, `0x02a1`-`0x02a4` rewrite the staged-current value, `0x02a5`/`0x02a6` publish the target low dword, `0x02a7`/`0x02a8` publish the target high dword, `0x02a9` marks the final handoff point, and only then does the deferred `dcbaap-write-low/high` pair run. `CRCR` is now staged after that deferred `DCBAAP` publish rather than becoming the first live post-reset command-ring ownership write: `0x02aa` marks the deferred `CRCR` handoff, `0x02ab` records the staged-current/target change mask, `0x02ac`-`0x02af` rewrite the staged-current seed, `0x02b0`/`0x02b1` publish the target low dword, `0x02b2`/`0x02b3` publish the target high dword, and `0x02b4` marks the final `CRCR` handoff point before `USBCMD.RUN`. The decisive trusted ladder is therefore `fw-handoff-skip-pre-quiesce`, `fw-handoff-skip-legacy-ownership`, `fw-handoff-skip-stop-revalidation`, `fw-handoff-skip-reset`, `reset-post-settle-begin`, `reset-post-settle-done`, the post-settle `USBSTS.CNR` poll, `config-write` / `config-write-pre-store` / `config-write-issued`, followed by the `ERDP` / `ERST*` ownership writes, `0x0257`, `0x0258` / `0x02a0`, `0x02a1`-`0x02a4`, `0x02a5`/`0x02a6`, `0x02a7`/`0x02a8`, `0x02a9`, the deferred `DCBAAP` write, `0x02aa`-`0x02b4`, and `usbcmd-run-write`, `controller-ready`, `imod-write`, and `iman-write`. If logs stop before `reset-post-settle-done`, the blind settle is still the last observable boundary; if they stop immediately after the shared acknowledged mailbox page breadcrumb (`reuse-shared` / `alloc-shared`), the remaining suspect is the acknowledged mailbox protocol/timeout path rather than an alias-mismatch or a second runtime request-page allocation; if they stop after `reset-post-settle-done` but before `config-write-pre-store`, the remaining suspect is now the restored post-settle `USBSTS.CNR` poll; if they stop after `config-write-pre-store`, the `CONFIG` publish remains the first live xHCI store boundary; if they stop at `0x0257` on the stronger mailbox-reset path, runtime is still reaching a live `DCBAAP` edge before the deferred publish plan advances; if they stop after `0x02a1`-`0x02a4`, then even rewriting the staged-current `DCBAAP` value is toxic; if they stop after `0x02a5`/`0x02a6` but before `0x02a7`/`0x02a8`, the low-dword transition remains the first toxic ownership change; if they stop after `0x02a9` or the deferred `dcbaap-write-low/high`, the final `DCBAAP` target publish is still the remaining boundary; if they stop after `0x02aa`-`0x02af`, the staged-current `CRCR` rewrite is still toxic; if they stop after `0x02b0`/`0x02b1` but before `0x02b2`/`0x02b3`, the `CRCR` low-dword transition is now the first toxic ownership change; if they stop after `0x02b4`, the remaining boundary has moved beyond `CRCR`; if they stop after `usbcmd-run-write` but before `controller-ready`, the next live `USBSTS` readiness poll is the remaining boundary. A halt line ending in `Kernel entry via Interrupt, irq 27` is treated only as the kernel timer PPI being the last tracked entry, not as proof that the xHCI or PCIe SPI escaped. On the softer stop-state-only fallback, the trusted CAP snapshot now keeps `usb-oxide` on the same `cold-start-from-stop-state-snapshot` path using only the stop-state fields, so runtime skips replaying a second live `HCRST` store and the remaining boundary moves to the first normal cold-start ownership publish instead of another reset write. On that trusted handoff path, command submission also rings xHCI doorbells without the extra posted readback that `usb-oxide` used previously, matching the working U-Boot doorbell sequence more closely before first enumeration.
...
- As-built refinement: the weaker Pi 4 stop-state-only xHCI snapshot branch now treats the trusted stop-state snapshot itself as sufficient to skip replaying a second live runtime `HCRST` store after local-seat strips the bootloader-owned ring seeds; that path still uses the normal cold-start config and ring publication flow.
- On that same experimental no-HT Wi-Fi probe path, a successful `empty-poll` promotion is now only a probe, not a committed transport switch: root-task logs `control transport promote probe ... commit=deferred`, keeps the no-HT probe state active, blocks repeated empty-poll promotions, and only clears the experimental transport state after a later matched control reply proves the promoted link can actually answer. While that probe remains active, the first ioctl reply wait also uses the widened no-HT budget (`wait_budget=32000`) so the repaired startup-link path can tolerate the slower `400 kHz` / `1-bit` control plane before declaring `ioctl-timeout`.
