...
- The staged Pi 4 U-Boot boot script presents a Linux-style numbered wizard whose default action continues with saved Cohesix policy when present and otherwise boots manifest defaults. Saving settings persists only the Cohesix policy fields into `cohesix.env`; it does not rewrite the manifest or the generic U-Boot environment. At boot, the script reloads `cohesix.env`, explicitly bootstraps menu/input USB with `pci enum; usb start` whenever no active input session is marked, and switches `stdin` to `usbkbd,serial` only when that session is live so the wizard prefers the attached keyboard while retaining serial fallback. The old bring-up-only USB snapshot spam and diagnostics submenu were removed once the runtime local-seat handoff breadcrumbs became the primary evidence surface. Unsafe downstream hubs still defer eager per-port `PORT_POWER` during early enumeration, delayed child `status-error` retries still avoid stray `PORT_POWER` writes, and the Apple `05ac:1006` keyboard hub keeps its targeted 250 ms post-config settle plus a 5 s hub debounce budget so downstream keyboard functions enumerate reliably without a global scripted startup delay. During controller probe, U-Boot exports both the raw xHCI BAR (`coh_xhci_mmio_raw`) and the host-translated MMIO address it actually mapped (`coh_xhci_mmio`), preserves the VL805 PCI command bits required for Cohesix handoff in `coh_xhci_pci_cmd`, and exports a matching xHCI capability snapshot (`coh_xhci_cap_length`, `coh_xhci_hci_version`, `coh_xhci_hcs1`, `coh_xhci_hcs2`, `coh_xhci_hccparams1`, `coh_xhci_dboff`, `coh_xhci_rtsoff`). On the `usb stop` path, the PCI driver now relies on the existing xHCI reset/cleanup flow, preserves the PCI command word, masks legacy/MSI/MSI-X delivery, and only exports `coh_xhci_handoff_ready=1` after that stop succeeds; a halt timeout returns a driver error instead of falling through to xHCI cleanup and rebooting the board. The `xhci-pci` driver now also emits direct serial breadcrumbs in probe/remove with the exported BARs, PCI command, and the live `coh_xhci_*` token state so driver activity can be correlated with the boot script handoff capture. Immediately before DTB handoff capture, the staged boot script now forces a real `pci enum; usb reset`, logs token state before reprobe and after the forced reprobe, emits a serial-only `xhci-reprobe-result reset=<success|failed> ready=<0|1|absent> irq=<0|1|absent> input=<0|1>` breadcrumb, snapshots the resulting pre-stop token pair, runs `usb stop`, computes a final handoff contract from either the post-stop driver-exported pair or a restored validated pre-stop pair, mirrors the translated host-physical BAR into DTB `/chosen/cohesix,xhci-mmio`, mirrors the PCI command word into `/chosen/cohesix,xhci-pci-cmd`, mirrors the final ready and interrupt-quiesced tokens into `/chosen/cohesix,xhci-handoff-ready` and `/chosen/cohesix,xhci-irq-quiesced`, mirrors the xHCI capability snapshot into `/chosen/cohesix,xhci-cap-*`, and also mirrors the contract source plus pre/post token snapshots into `/chosen/cohesix,xhci-handoff-source`, `/chosen/cohesix,xhci-handoff-prestop-*`, and `/chosen/cohesix,xhci-handoff-poststop-*`. The handoff source now records whether the final decision came from `reprobe-post-stop-paired`, `reprobe-saved-pre-stop-pair`, `reprobe-partial`, or `reprobe-absent`. It then continues to hand off `coh_net_mode`, `coh_net_interface`, `coh_static_ip`, `coh_static_prefix_len`, `coh_static_gateway`, `coh_wifi_ssid`, and `coh_wifi_psk` before the padded `bcm2711-rpi-4-b.dtb` reaches the seL4 elfloader via the U-Boot `uImage`/`bootm` path. Root-task applies only bounded overrides, rejects a leaked BCM2711 DMA-alias `chosen-pci-bar` as non-MMIO, uses a safe bootloader BAR + PCI command pair plus explicit handoff-ready and irq-quiesced tokens as a cold-start xHCI runtime source, pins the trusted high BAR before the higher VL805 ECAM page inside the shared PCIe device-untyped aperture so the handoff window remains mappable, keeps VL805 ECAM preseed on a map-only path in safe mode because live ECAM reads are still provoking fatal irq 27 entries, skips unsafe live PCI-command replay later in safe-mode runtime, and otherwise falls back to manifest defaults.
- Root-task also emits a combined `xhci-handoff-contract` breadcrumb after DT parse and a companion `xhci-handoff-dtb source=... prestop=.../... poststop=.../...` breadcrumb so a partial or restored handoff can be diagnosed from root-task logs alone even if early U-Boot serial output is unavailable.
- The Pi 4 Wi-Fi HAL emits `firmware core-ctrl mode=cmd53-windowed-read32-cmd52-current-window-write8 fallback=cmd52-byte-rewindow split-window` before AI-core control, and if the primary mixed Function 1 backplane path times out it now recovers writes with a forced rewindowed CMD52 pass while reads still first try byte-sized CMD52 recovery against the current backplane window before falling back to a rewindowed CMD52 pass. Small CMD53 byte-mode transfers now program the host as a single block with block-count enable asserted instead of leaving the host block count path unset, incrementing CMD53 transfers advance the Function 1 address per chunk, the serial log emits bounded `sdio xfer chunk fn=1 ... base=... chunk=... off=...` breadcrumbs for the early/new transfer shapes plus `backplane window program ... low=... mid=... high=...` on the flagged Function 1 control path, and same-window Function 1 accesses now emit `backplane window reuse ...` instead of replaying an unchanged SBADDRLOW/MID/HIGH CMD52 triplet. AI core-control writes keep the general `AI_IOCTRL` path on the direct unflagged current-window CMD52 address, but once a core is already held in reset the `in-reset-configure` `AI_IOCTRL` write now emits `firmware core-ctrl in-reset-write mode=cmd53-word-windowed-in-reset ...` and first uses the flagged 32-bit backplane write path before falling back to current-window and rewindowed CMD52 recovery. `AI_RESETCTRL` still emits `firmware core-ctrl reset-write mode=cmd53-word-windowed ...` and uses the same flagged 32-bit backplane path first. The generic `firmware core-ctrl access ...` breadcrumb still prints both `bus=...` and `trace_bus=...` so logs show the exact unflagged byte address used for live CMD52 writes alongside the flagged address used for window tracing. SDHCI command or data-path failures on that path emit `sdhci recover stage=... mask=cmd+data` plus paired `sdio shadow ...` breadcrumbs so logs show both the forced host recovery and the last cached backplane-window / chipclk / wake / sleep / cardcap state without additional failing probe traffic. Firmware load now also primes `firmware stage=pre-reset-ht-assist` before the ARMCR4/SOCRAM reset-heavy path whenever cached chip-clock state lacks an HT request, so remaining failures after the backplane-window fix point at the reset edge itself rather than missing HT assist state. The AI core-control sequence now matches the upstream Broadcom AI reset order: write `FGC|CLK` before asserting reset, defer the first `RESETCTRL` readback until after the in-reset configure settle, configure the core while held in reset, then clear reset and leave `CLK` enabled. ARMCR4 also carries the upstream `CPUHALT` bit through disable and final release so the firmware CPU is parked until the post-load reset completes.
- The Pi 4 local-seat runtime treats a firmware DT `xhci` node marked `status = "disabled"` as stale handoff rather than immediate loss of keyboard support: it translates BCM2711 `0x7e...` SoC-bus `reg` values into CPU physical addresses for diagnostics, ignores stale disabled-node hints as active xHCI runtime sources, pins the trusted handed-off xHCI BAR before the higher VL805 ECAM page so the lower handoff aperture is not lost to monotonic device retype ordering, keeps VL805 ECAM preseed on a map-only path because live ECAM reads still trigger fatal irq 27 entries on Pi 4, avoids broader live runtime VL805 config-space discovery while that path remains unsafe on Pi 4, emits concise candidate-decision breadcrumbs (`kind/cfg/cov/pwin/pin/fh/fs/vh`) plus a targeted firmware-handoff trust-gate line (`safe/token/irq/trusted/reason`) before skipping an xHCI MMIO source, treats the standard U-Boot `usb stop` handoff as a cold-start opportunity for a safe high BAR only when the bootloader also exports `cohesix,xhci-handoff-ready=1`, `cohesix,xhci-irq-quiesced=1`, and the matching `/chosen/cohesix,xhci-cap-*` snapshot, and treats VL805 reset as bootloader-owned on that path instead of issuing a runtime mailbox reset request. The low `0xfe980000` / `0x7e980000` BCM2711 USB aliases are not reused as VL805 xHCI runtime mirrors; on Pi 4 they identify the SoC `brcm,bcm2835-usb` block, so local-seat keeps them only as diagnostic breadcrumbs and probes the trusted high BAR directly. It uses the seL4 bcm2711 `device-untypes@600000000` window for the U-Boot-discovered VL805 BAR when present, logs whether the handed-off BAR still has device coverage without touching ECAM (`xhci handoff window mmio=... coverage=... pinned=...`), prefers the bootloader-exported capability snapshot over a live first-touch xHCI capability read on that trusted handoff BAR, passes the validated snapshot into `usb-oxide`, requests GIC SPI 143 directly via `IRQControl Get`, only attempts reuse after a `seL4_RevokeFirst` response, and emits explicit `xhci irq sink request ...`, `xhci irq sink reusing existing handler ... after=get-revoke-first`, or `xhci irq sink ambiguous existing handlers ... after=get-revoke-first` breadcrumbs before the first trusted operational write. It marks that path as firmware-quiesced so the runtime driver skips its redundant read-based early quiesce and stop/reset sequence while still issuing a write-only interrupt scrub, but the trusted scrub now skips both the early `USBCMD=0` touch and the early `USBSTS` clear, emitting `fw-handoff-skip-usbcmd-mask-write` and `fw-handoff-skip-usbsts-clear-write` before continuing with `IMOD` / `IMAN`, so logs can distinguish a fatal first operational-register write from a fatal interrupter-state scrub. It emits named `xhci.diag` tags for both the trusted-handoff scrub writes (`fw-handoff-usbcmd-mask-write`, `fw-handoff-skip-usbcmd-mask-write`, `fw-handoff-usbsts-clear-write`, `fw-handoff-skip-usbsts-clear-write`, `fw-handoff-imod-write`, `fw-handoff-iman-write`) and the critical runtime register writes (`config-write`, `dcbaap-write`, `crcr-write`, `erstsz-write`, `erstba-write`, `erdp-write`, `imod-write`, `iman-write`, `usbsts-clear-write`, `usbcmd-run-write`) so the last breadcrumb identifies the exact write that faulted. It still explicitly logs the bootloader-exported command word when safe-mode runtime skips a live VL805 PCI-command replay, prefers a verified runtime VL805 BAR when one exists, and otherwise degrades to serial-only input with explicit `[local-seat] ... unavailable` breadcrumbs.
