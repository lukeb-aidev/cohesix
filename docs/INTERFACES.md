...
- The unseeded live-default `handoff=none` path remains the next bounded diagnostic candidate when the probe ladder selects it. On the Linux-captured high BAR its preflight now reports `route=trusted-high-bar-primary`, `ctor=capture-quiesce`, `halt_guard=skip-live-halt-read`, `pre=mailbox-reset-required`, `legacy=skip-legacy`, `next=mailbox-reset-notify`, and `followup=mailbox-reset-then-platform-init`. Once `mailbox-acked` evidence exists, a ready capture-backed replay contract (`cfg_window=linux-capture-witness`, `cfg_source=linux-capture-replay`, `cfg_writes=capture-backed`, `command_source=linux-capture-replay`) promotes the lane to `handoff=platform-reset-complete`, skips xHCI HCRST/config, publishes fresh runtime rings before `RUN`, and uses the U-Boot-style `USBCMD.RUN` store under the same poll-only IRQ contract. If capture replay cannot pin the high BAR, bind the PCIe/INTx sinks, or prove the captured command shadow, local-seat keeps `handoff=platform-reset-complete` / `policy=platform-reset-complete` as a no-publish witness and returns before `usb-oxide` controller construction. If the mailbox notify is unconfirmed, local-seat skips the lane with `stage=pre-xhci-reset ... action=skip-platform-init`.
