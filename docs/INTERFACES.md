...
- On the active trusted Pi 4 xHCI path, local-seat still replays the bounded runtime VL805 mailbox reset-notify through the shared long-lived request page and keeps ownership polling-driven with only the bounded bcm2711 PCIe child-INTx and bridge sinks. The trusted CAP snapshot remains the runtime hint, and `action=fresh-init-from-cap-snapshot mode=...` means runtime is rebuilding DCBAA, scratchpads, command ring, and event ring locally instead of preserving firmware-owned ring state. A confirmed runtime mailbox reset still leads with the fully unseeded `mode=cold-start-from-snapshot`, while posted-fallback and soft-continue now lead with the trusted stop-state-seeded `mode=cold-start-from-snapshot`, retry the unseeded fresh-init path next, and only then fall back to `mode=resetless-reinit`; only the bootloader-owned stop-state handoff remains on `mode=preserve-controller-state`. The runtime now also emits a compact seed/branch checkpoint before halt revalidation, so a missing skip marker after a seeded cold-start attempt identifies seed propagation rather than a later `RUN` edge fault. Any remaining `xhci.diag stage=0x0253 tag=crcr-read-begin` breadcrumb is therefore part of the runtime-owned path, not a preserved stop-state shortcut.
...
- On the bootloader-owned stop-state-only xHCI snapshot branch, runtime still uses the trusted CAP layout plus stop-state diagnostics, but it no longer preserves firmware-owned ring state or the abandoned stop-state deferred `CRCR` shortcut. A stop-state-only snapshot suppresses the early halt revalidation touch, takes the same bounded post-mailbox-reset blind settle before runtime MMIO ownership begins, keeps the active runtime strategy on the seeded `mode=cold-start-from-snapshot` ladder, skips the first live `USBCMD.HCRST` / `CONFIG` touch (`xhci.diag stage=0x0234` / `0x0245`), and rebuilds runtime-owned ring state locally from zero-seeded `DCBAAP`/`CRCR`/`ERST*`/`ERDP` state. On the preserve-state halted stop-snapshot path, runtime now completes the staged event-ring table plus `DCBAAP`, `CRCR`, and `DNCTRL` publishes before `USBCMD.RUN`, so the controller never starts against stale firmware-owned ring state. Any remaining `crcr-read-begin` breadcrumb is therefore part of the runtime-owned path rather than a preserved stop-state handoff. The broader deferred-ownership ladder remains an internal `usb-oxide` recovery tool, not part of the active Pi 4 handoff contract.
