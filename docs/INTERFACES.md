...
- Current June 13 Wi-Fi correction: `/Users/lukasbower/pi4-serial-20260613-081613.log` supersedes the June 12 firmware-stream frontier. The selected Wi-Fi interface reaches linked firmware upload completion, NVRAM tail, firmware release ready, and owner-state replay ready, but the first matched `CONTROL_EXCHANGE` fails at `cyw43-control-txglomalign` with `op=11 flags=0x0002 payload_len=36 detail=0x530b result=0x43030000`, normalized as `WIFI_GATE=7` / `cyw43-control-rx-no-rframe`. Root publishes fixed-ring command records with a sequence-last contract, cleans staged descriptor/payload bytes plus command/completion records before publishing the real sequence, and invalidates completion/progress records before consuming linked-runtime evidence. Linked runtimes read that shared ring before endpoint polling so bounded one-way turns do not require an endpoint wake or reply cap. The linked CYW43 control ABI separates fire-and-forget SDPCM control frames from matched `CONTROL_EXCHANGE` turns: station setup must receive expected CDC command/ioctl-id replies for the Linux-first preflight (`bus:txglomalign=8`, optional `ulp_sdioctrl`, `bus:rxglom=1`, `cur_etheraddr`, `BRCMF_C_GET_REVINFO`, `mpc=0`) before `WLC_UP`, and read iovars such as `cur_etheraddr` return a bounded CDC response frame before the root-side host-EAPOL and DHCP/data gates can advance. The first three Linux-first preflight controls use the plain 12-byte SDPCM control header; the 20-byte hardware-extension header is used only after `bus:rxglom` succeeds.
