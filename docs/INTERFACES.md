...
- On the weaker stop-state-only xHCI snapshot branch, runtime still uses the trusted CAP layout plus stop-state diagnostics, but it no longer preserves firmware-owned ring state. A stop-state-only snapshot now suppresses the early halt revalidation touch plus the first runtime `USBCMD.HCRST` and `CONFIG` writes, then rebuilds runtime-owned ring state locally through the deferred publish ordering with zero-seeded `DCBAAP`/`CRCR`/`ERST*` state and without the abandoned stop-state deferred `CRCR` shortcut. Any remaining `crcr-read-begin` breadcrumb is therefore part of the fresh runtime ownership path rather than a preserved stop-state handoff. The broader deferred-ownership ladder remains an internal `usb-oxide` recovery tool, not part of the active Pi 4 handoff contract.
