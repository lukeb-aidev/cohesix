...
- The Pi 4 USB deferred-capture contract requires command-ring proof. `command_probe=no-op-deferred` is no longer sufficient; the active high-BAR no-candidate runtime first submits a prompt-safe Linux-shaped No Op discriminator, then always submits the cold-boot-first `Enable Slot` command so the proof follows the Linux-observed first real xHCI command even when the No Op discriminator times out. The non-PCIe diagnostic lane keeps the older poll-only No Op proof. `command_probe=enable-slot-ok` and its cleanup-failed variant prove command/event-ring consumption because cleanup happens after the Enable Slot completion. Historical `enable-slot-linux-event-ok` / `no-op-linux-event-ok` labels are diagnostic only and do not satisfy the cold-boot USB gate. A concrete `tag=cmd-timeout` now normalizes as `USB_BLOCKER=cmd-timeout`, and concrete `enable-slot-unproven`, `no-op-linux-event-unproven`, or legacy `enable-slot-linux-event-unproven` proof summaries normalize as `USB_BLOCKER=cmd-event-ring-timeout`; these supersede the earlier timer-only `cmd-poll-pending` state.
- Current M26b USB correction: when the Pi 4 platform-reset lane has no port-event candidates, local-seat now uses a two-step command proof through the prompt-safe Linux-shaped event-generation path (`USBCMD.RUN|INTE`, `IMOD=0xa0`, `IMAN.IE`) while PCI INTx/MSI/GIC delivery remains masked. A missing No Op completion is diagnostic and no longer terminal; a missing Enable Slot completion is the command/event-ring blocker, while a completed No Op followed by a missing Enable Slot completion narrows the failure to slot-command state. A successful `enable-slot-ok` moves the proof frontier to safe port state; any missing command completion remains `cmd-event-ring-timeout`, and IRQ27 remains the seL4 timer only.
