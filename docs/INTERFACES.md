...
- On the active trusted Pi 4 xHCI path, local-seat replays the bounded runtime VL805 mailbox reset-notify only when runtime PCI config replay is available. Linux resets VL805 before it enumerates and reprograms BAR0/COMMAND; the current Cohesix Pi 4 safe-mode path keeps VL805 ECAM reads/writes disabled, so root-task skips the mailbox reset with `action=skip-mailbox-notify reason=pci-config-replay-unavailable` rather than invalidating the U-Boot-exported BAR authority before the first xHCI capability read. Runtime probes stay poll-only and do not bind raw PCIe child-INTx or bridge sinks before controller entry because Linux routes VL805 through MSI. The trusted high BAR remains the runtime hint; capability layout is read from the mapped controller unless a legacy DT snapshot is present. With the trusted high-BAR contract, prompt-safe selection first skips the unseeded live-default candidate and then tries `handoff=none` / `origin=reset-owned-stop-seed`; that lane suppresses HCRST, skips the already-programmed xHCI `CONFIG=0x20` register, clears pending interrupter state with an `IMAN.IP` write before `DCBAAP`, and publishes Cohesix-owned DCBAA, scratchpads, command ring, and event ring before RUN without preserving firmware-owned ring state. A confirmed runtime mailbox reset may still retry the fully unseeded `mode=cold-start-from-snapshot` HCRST fallback; posted-fallback and soft-continue remain on the no-touch seeded `mode=cold-start-from-snapshot` diagnostic lane. The runtime now also emits a compact seed/branch checkpoint before halt revalidation, so a missing skip marker after a seeded attempt identifies seed propagation rather than a later `RUN` edge fault. Any remaining `xhci.diag stage=0x0253 tag=crcr-read-begin` breadcrumb is therefore part of the runtime-owned path, not a preserved stop-state shortcut.
- Current Pi 4 xHCI prompt-safe ordering is stricter than the older text above when a safe halted stop-state exists: after the skipped unseeded live-default candidate, local-seat now attempts `handoff=none` / `origin=reset-owned-stop-seed` only when mailbox ACK or bootloader post-stop authority owns the stop seed, before falling back to the non-enumerating seeded `cold-start-from-snapshot` diagnostic lane. It still stays poll-only, still rejects IRQ27/INTx as xHCI delivery, and never uses raw CPU DMA addresses when the `0x4_00000000` PCIe DMA alias is enabled.
...
- The Pi 4 CYW43455 firmware-load path keeps the startup SDIO link active through chip discovery, RAM sizing, AI core disable/reset, and the initial Function 1 firmware upload. After upload, root-task still attempts strict HT-clock readiness first and still requires real Function 2 readiness before using the control plane, but if the final stronger `wait-ht-clock` retry leaves `HT_AVAIL_REQ` latched, `ALP_AVAIL` high, `FORCE_HT` asserted, and complete HT-assist shadow state on the promoted control-plane clock, it emits `timeout-linux-alp-force-probe` and proceeds to the Linux-style Function 2 proof edge instead of failing immediately at the PMU boundary. The `wait-ht-clock` ALP-prime substep still preserves the forced HT / retry request bits instead of dropping back to a bare ALP-only request, logs `alp-request=0x..`, emits decoded `status=request-readback|ht-rerequest-readback|timeout-soft|timeout-hard ... ht_req=... alp_req=... force_ht=... clkreq_off=... alp=... ht=... wake=... sleep=... cardcap=...` breadcrumbs, and gives the final stronger required retry a bounded wait budget with chunked `progress=wait-ht-clock polls=... csr=...` checkpoints. HAL still retains the guarded startup-link fallback and reply-rearm ladder, but Function 2 enable now primes the Linux-sized 512-byte FBR block size before the IORX wait, post-firmware-ready probes still preserve already-latched F2 enables, strict reply recovery now force-clears and re-enables F2 when the card stays stuck enabled-but-not-ready, and no-HT Function 2 writes remain on the same 512-byte cadence while reply reads stay on smaller conservative bursts. The serial path reports that split through `write_chunk_limit=...`, `reply_chunk_limit=...`, `sdio function2 ready-snapshot ... diagnosis=...`, and `action=preserve-enable-latch ... reason=linux-aligned-no-clear`, so the next failure can be attributed to F2 latch state, packetization, or reply visibility instead of a generic promoted-rearm timeout.
- Current Pi 4 CYW43455 high-speed behavior follows the Linux capture: firmware upload and normal control-plane work request a 50 MHz SDIO target on the 4-bit high-speed lane, with the BCM2711 SDHCI mailbox-reported `100000000 Hz` rate corrected to the effective Arasan `200000000 Hz` parent so the 50 MHz target uses the firmware-observed divider encoding. No-HT remains a diagnostic and firmware-upload fallback only; Function 2 requires real CCCR `IORX` readiness, while missing `CHIPCLKCSR.HT_AVAIL` is no longer a hard pre-F2 blocker when the promoted strict path has the Linux ALP+FORCE_HT sideband shape. `wait-firmware-ready` no longer lowers a proven promoted link to 400 kHz for mailbox polling, and pre-HT control-plane staging preserves the current promoted lane instead of forcing a startup-clock detour.
...
- On the bootloader-authorized stop-state-only xHCI branch, runtime still uses the trusted CAP layout plus stop-state diagnostics, but it no longer preserves firmware-owned ring state or the abandoned stop-state deferred `CRCR` shortcut. The first trusted high-BAR prompt-safe attempt remains the runtime-owned `mode=none` / `origin=live-runtime-default` lane. If runtime falls through to the stop-seed fallback, that stop-state-only snapshot suppresses the early halt revalidation touch, selects `mode=none` / `origin=reset-owned-stop-seed`, skips the first live `USBCMD.HCRST` / `CONFIG` touch (`xhci.diag stage=0x0234` / `0x0245`), clears `IMAN.IP` before `DCBAAP`, and rebuilds runtime-owned ring state locally from zero-seeded `DCBAAP`/`CRCR`/`ERST*`/`ERDP` state. On weaker seeded `cold-start-from-snapshot` fallback evidence, runtime stays no-touch and non-enumerating. Any remaining `crcr-read-begin` breadcrumb is therefore part of the runtime-owned path rather than a preserved stop-state handoff. The broader deferred-ownership ladder remains an internal `usb-oxide` recovery tool, not part of the active Pi 4 handoff contract.
- As-built correction: current prompt-safe Pi 4 high-BAR probing uses the Linux-captured VL805 capability tuple when no firmware capability snapshot is present, avoiding the live high-BAR `CAPLENGTH` probe that halted the board. When U-Boot exports trusted bootloader stop-state authority, the reset-owned stop-seed runtime lane is now the first usable high-BAR attempt; older wording that says the unseeded live-default lane remains first is superseded.
