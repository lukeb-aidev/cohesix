...
- Authenticated Pi 4 `cohsh reboot` requests best-effort mark the BCM2711 PM RSTS register with a one-shot Cohesix fast-boot value, then arm the watchdog through the BCM2711 PM WDOG/RSTC restart sequence. Root-task orders and drains the marker write before arming reset, but PM RSTS readback is not treated as a policy gate on live hardware; if the high marker is visible to U-Boot, the generated script first forces serial input, consumes the marker after loading policy and before any menu/input setup, clears the marker with the PM write password, then runs the same `coh_boot_sequence` with saved policy or manifest defaults. The hardware-observed software-reset status bit is logged only as diagnostic context and does not bypass the menu without the Cohesix marker. This skips the logo/menu/input path for authenticated software reboots when marker state is present and does not create a new control-plane secret, a persistent U-Boot environment setting, or a bootloader xHCI handoff.
- If U-Boot does not observe accepted marker state, the script logs a bounded `fast boot marker absent` diagnostic containing the observed RSTS/high/reset marker values plus saved-policy state and then enters the normal cold-boot wizard. That path is recovery evidence only; it does not silently convert a cold boot into an unattended boot.
