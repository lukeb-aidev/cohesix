...
- The Pi 4 USB deferred-capture contract requires command-ring proof. `command_probe=no-op-deferred` is no longer sufficient; the active high-BAR no-candidate runtime now uses one cold-boot-first `Enable Slot` command on the bounded Linux-shaped event-generation path, while the non-PCIe diagnostic lane keeps the older No Op proof. `command_probe=enable-slot-linux-event-ok` and its cleanup-failed variant prove command/event-ring consumption because cleanup happens after the Enable Slot completion. A concrete `tag=cmd-timeout` now normalizes as `USB_BLOCKER=cmd-timeout`, and concrete `enable-slot-unproven` or `enable-slot-linux-event-unproven` proof summaries normalize as `USB_BLOCKER=cmd-event-ring-timeout`; both supersede the earlier timer-only `cmd-poll-pending` state.
- Current M26b USB correction: when the Pi 4 platform-reset lane has no port-event candidates, local-seat now submits the cold-boot-first `Enable Slot` command through the bounded Linux-shaped event-generation path rather than using No Op as the primary command-ring proof. A successful `enable-slot-linux-event-ok` moves the proof frontier to safe port state; a missing completion remains `cmd-event-ring-timeout` and still treats IRQ27 as the seL4 timer only.
