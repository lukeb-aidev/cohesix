...
- Current June 13 Wi-Fi correction: `/Users/lukasbower/pi4-serial-20260613-174015.log` supersedes the earlier firmware-stream, revinfo, and no-rframe frontiers. The selected Wi-Fi interface reaches linked firmware upload completion, NVRAM tail, firmware release ready, owner-state replay ready, matched control replies through `BRCMF_C_GET_REVINFO`, `mpc=0`, `WLC_UP`, WPA2 setup, PAE multicast admission, and `WLC_SET_SSID`; the active blocker is now `WIFI_GATE=7` / `cyw43-data-rx-firstread-empty` beneath the coarse `host-eapol-required` policy cause. Root publishes fixed-ring command records with a sequence-last contract, cleans staged descriptor/payload bytes plus command/completion records before publishing the real sequence, and invalidates completion/progress records before consuming linked-runtime evidence. Linked runtimes read that shared ring before endpoint polling so bounded one-way turns do not require an endpoint wake or reply cap. The linked CYW43 control ABI separates fire-and-forget SDPCM control frames from matched `CONTROL_EXCHANGE` turns: station setup must receive expected CDC command/ioctl-id replies for the Linux-first preflight (`bus:txglomalign=8`, optional `ulp_sdioctrl`, `bus:rxglom=1`, `cur_etheraddr`, `BRCMF_C_GET_REVINFO`, `mpc=0`) before `WLC_UP`, programs the Linux `event_msgs_ext` join-event mask before and after `WLC_UP`, and keeps DHCP/data blocked until root-side host-EAPOL proves M1/M2/M3/M4 plus PTK/GTK installation. The first three Linux-first preflight controls use the plain 12-byte SDPCM control header; the 20-byte hardware-extension header is used only after `bus:rxglom` succeeds.
