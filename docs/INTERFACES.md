...
- The Pi 4 Wi-Fi HAL emits `firmware core-ctrl mode=cmd53-word-windowed fallback=cmd53-word-current-window split-window` before AI-core control, and if the primary Linux-style flagged 4-byte Function 1 backplane path times out it now first attempts a `cmd53-word-current-window` retry without reprogramming the backplane window before falling back to a full rewindowed retry. SDHCI command failures on that path also emit `sdhci recover stage=cmd-wait|cmd-error mask=cmd+data` so logs show when host recovery was forced between fallback attempts.
- The Pi 4 local-seat runtime treats a firmware DT `xhci` node marked `status = "disabled"` as stale handoff rather than immediate loss of keyboard support: it translates BCM2711 `0x7e...` SoC-bus `reg` values into CPU physical addresses for diagnostics, ignores stale disabled-node hints as active xHCI runtime sources, keeps a map-only VL805 ECAM window pinned even in safe mode so it can replay the bootloader-exported PCI command bits before the first trusted high-BAR MMIO read, avoids broader live runtime VL805 config-space discovery while that path remains unsafe on Pi 4, emits concise candidate-decision breadcrumbs (`kind/cfg/cov/pwin/pin/fh/fs/vh`) plus a targeted firmware-handoff trust-gate line (`safe/token/irq/trusted/reason`) before skipping an xHCI MMIO source, treats the standard U-Boot `usb stop` handoff as a cold-start opportunity for a safe high BAR only when the bootloader also exports `cohesix,xhci-handoff-ready=1` and `cohesix,xhci-irq-quiesced=1`, and treats VL805 reset as bootloader-owned on that path instead of issuing a runtime mailbox reset request. The low `0xfe980000` / `0x7e980000` BCM2711 USB aliases are not reused as VL805 xHCI runtime mirrors; on Pi 4 they identify the SoC `brcm,bcm2835-usb` block, so local-seat keeps them only as diagnostic breadcrumbs and probes the trusted high BAR directly. It uses the seL4 bcm2711 `device-untypes@600000000` window for the U-Boot-discovered VL805 BAR when present, logs whether the first runtime MMIO map reused a pinned trusted handoff window, logs the `vl805 handoff pci-cmd restore before=... target=... after=... ready=...` replay result, prefers a verified runtime VL805 BAR when one exists, and otherwise degrades to serial-only input with explicit `[local-seat] ... unavailable` breadcrumbs.
