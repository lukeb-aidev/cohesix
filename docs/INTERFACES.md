...
- The staged Pi 4 U-Boot boot script presents a Linux-style numbered wizard whose default action continues with saved Cohesix policy when present and otherwise boots manifest defaults. Saving settings persists only the Cohesix policy fields into `cohesix.env`; it does not rewrite the manifest or the generic U-Boot environment. At boot, the script reloads `cohesix.env`, reuses the Pi 4 preboot USB session, and switches `stdin` to `usbkbd,serial` so the wizard prefers the attached keyboard while retaining serial fallback. It relies on the Pi 4 U-Boot defaults to rebind the active console to `usbkbd,serial` immediately after `usb start` in `CONFIG_PREBOOT`, but the old bring-up-only USB snapshot spam and diagnostics submenu were removed once the runtime local-seat handoff breadcrumbs became the primary evidence surface. Unsafe downstream hubs still defer eager per-port `PORT_POWER` during early enumeration, delayed child `status-error` retries still avoid stray `PORT_POWER` writes, and the Apple `05ac:1006` keyboard hub keeps its targeted 250 ms post-config settle plus a 5 s hub debounce budget so downstream keyboard functions enumerate reliably without a global scripted startup delay. During `usb start`, U-Boot exports both the raw xHCI BAR (`coh_xhci_mmio_raw`) and the host-translated MMIO address it actually mapped (`coh_xhci_mmio`), preserves the VL805 PCI command bits required for Cohesix handoff in `coh_xhci_pci_cmd`, and keeps legacy INTx masked because U-Boot polls xHCI rather than relying on PCI interrupts. The staged boot script mirrors the translated host-physical BAR into DTB `/chosen/cohesix,xhci-mmio`, mirrors the PCI command word into `/chosen/cohesix,xhci-pci-cmd`, logs the `raw -> phys` breadcrumb, quiesces USB with `usb stop` before `bootm`, and continues to hand off `coh_net_mode`, `coh_net_interface`, `coh_static_ip`, `coh_static_prefix_len`, `coh_static_gateway`, `coh_wifi_ssid`, and `coh_wifi_psk` before the padded `bcm2711-rpi-4-b.dtb` reaches the seL4 elfloader via the U-Boot `uImage`/`bootm` path. Root-task applies only bounded overrides, rejects a leaked BCM2711 DMA-alias `chosen-pci-bar` as non-MMIO, uses a safe bootloader BAR + PCI command pair from that quiesced handoff as a cold-start xHCI runtime source, performs its own bounded VL805 PCI BAR decode at runtime when the pinned ECAM window is available, and otherwise falls back to manifest defaults.
- The Pi 4 local-seat runtime treats a firmware DT `xhci` node marked `status = "disabled"` as stale handoff rather than immediate loss of keyboard support: it translates BCM2711 `0x7e...` SoC-bus `reg` values into CPU physical addresses for diagnostics, ignores stale disabled-node hints as active xHCI runtime sources, defers VL805 ECAM preseed when safe mode still forbids runtime config-space reads, avoids live runtime VL805 config-space reads while that path remains unsafe on Pi 4, emits concise candidate-decision breadcrumbs (`kind/cfg/cov/pwin/pin/fh/fs/vh`) plus a targeted firmware-handoff trust-gate line (`safe/cfg/tp/vv/pwin/cov`) before skipping an xHCI MMIO source, treats the standard U-Boot `usb stop` handoff as a cold-start opportunity for a safe high BAR without reopening VL805 config-space reads, emits a one-shot `vl805 reset notify=requested reason=fw-handoff-cold-start` breadcrumb on that trusted cold-start path before the first xHCI MMIO read, routes that reset request through the HAL-preseeded mailbox mapping, uses the seL4 bcm2711 `device-untypes@600000000` window for the U-Boot-discovered VL805 BAR when present, prefers a verified runtime VL805 BAR when one exists, and otherwise falls back to bounded legacy xHCI alias probing before degrading to serial-only input with explicit `[local-seat] ... unavailable` breadcrumbs.
- The Pi 4 CYW43455 firmware-load path now keeps the startup SDIO link active through chip discovery, RAM sizing, and AI core disable/reset, then uses aligned CMD53 block-mode firmware writes for Function 1 while dedicated flagged word accesses remain reserved for AI core-control registers.
