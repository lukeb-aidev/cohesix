...
- Current June 9 Wi-Fi correction: `/Users/lukasbower/pi4-serial-20260609-115541.log` supersedes the older 11:25, 10:58, and 09:33 wording for the active interface frontier. The selected Wi-Fi interface now completes linked descriptor replay for both `cyw43455` and `sdio-host`, then fails Gate 2 as `sdio-engine-init-no-reply` when the `sdio-host` linked runtime no-replies on `engine-init` with progress stuck at `engine-init-begin`. Root publishes fixed-ring command records with a sequence-last contract over an uncached HAL ring page: stage a zero-sequence record with a reset completion, issue the shared-memory store barrier, write the real sequence, then issue the barrier again. Linked runtimes read that shared ring before endpoint polling so bounded one-way turns do not require an endpoint wake or reply cap before any SDIO, firmware, DHCP, `nettest`, or `netstats` claim can be accepted. Shared command/completion/progress, bus-owner, and root-shared payload pages are uncached mappings in root and runtime VSpaces, so shared-ring coherency is volatile access plus barriers on both sides; cache clean/invalidate instructions remain for descriptor-declared device DMA buffers only. The SDIO runtime-init and engine-init turns keep the active request across a finite bounded no-reply resume window, so root does not overwrite a still-running linked SDIO adoption/reset turn while the shell remains responsive. The SDIO engine-init interface now emits first-edge progress for HAL adoption interrupt-clear, present-state read, reset clock-disable, and reset power-disable so prompt-side `wifi diag` can distinguish a stale image, first SDHCI MMIO access, power/present absence, clock failure, inhibit failure, and later CYW43 card-command blockers. CYW43 transport details are fail-closed: partial `0x5401..0x5407` completions are progress with `result=0`, and only `0x5408` reports ready with `result=1`. The linked CYW43 control ABI separates fire-and-forget SDPCM control frames from matched `CONTROL_EXCHANGE` turns: station setup must receive the expected CDC command/ioctl-id reply, and read iovars such as `cur_etheraddr` return a bounded CDC response frame before the root-side host-EAPOL and DHCP/data gates can advance.
