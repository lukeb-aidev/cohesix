...
- The isolated SDIO owner exposes the BCM2711 Linux iProc host model to that
  interface. Byte/halfword registers are accessed only through aligned 32-bit
  reads and RMW writes. BLOCK_SIZE/BLOCK_COUNT and
  TRANSFER_MODE/COMMAND remain shadowed until COMMAND, when the owner emits one
  combined 32-bit block word followed by one combined 32-bit command word.
  Low-speed writes receive a timer-backed four-SD-clock gap. Initial card
...
- Current Pi 4 CYW43455 timing follows Linux MMC ordering: startup clock is used only for enumeration, then width and timing are negotiated before backplane attach. CCCR `SPEED.EHS` and SDHCI `HOST_CONTROL.HISPD` jointly authorize the 50 MHz target; a card without SHS uses the 25 MHz default-speed ceiling. The selected lane is continuous through AI-control CMD52, firmware block/remainder CMD53, and ARM release. Diagnostics report observed host/card/clock and `linux-normal-block` or `linux-byte-remainder`; they do not report retired fallback ladders.
