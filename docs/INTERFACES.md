...
- The Pi 4 CYW43455 firmware-load path keeps Function 1 sideband active through chip discovery, RAM sizing, and AI core disable/reset. During backplane prep it recognizes the Linux `brcmf_sdio_buscoreprep()` `SDIOPULLUP` (`0x1000F`) clear, but Cohesix currently defers that CMD52 on Pi 4 because the latest board trace showed CRC/end-bit failures before Function 1 sideband was readable. Before upload, root-task switches the card and host into the Linux high-speed lane by programming CCCR `SPEED.EHS`, setting SDHCI `HOST_CONTROL.HISPD`, requesting the 50 MHz SDIO target, and requesting `CHIPCLKCSR.ALP_AVAIL` for the Linux/Broadcom `alp_only` firmware-download lane. If `ALP_AVAIL` is present without `HT_AVAIL`, the upload continues with `firmware stage=pre-write-alp-clock action=continue-alp-only ... reason=linux-brcmfmac-alp-only-firmware-download`; a visible `HT_AVAIL` bit is recorded only as stronger evidence, not as a pre-upload gate. Firmware upload now follows the non-debug Linux order: write image, write NVRAM/tail, then release ARMCR4; the upstream `brcmf_sdio_verifymemory()` RAM compare is a Linux `DEBUG` build path, so Cohesix logs `firmware_verify policy=skip-readback ... reason=linux-nondebug-debug-only-readback` instead of forcing a pre-NVRAM Function 1 readback. After upload, root-task keeps the strict Linux non-SR post-download clock gate: clear to SD-only, replay only KSO as pre-Function-2 sideband while preserving `WAKEUPCTRL`/`CARDCAP` shadows, request `CHIPCLKCSR.HT_AVAIL_REQ`, poll bounded `status=active-ht-request-readback|active-ht-timeout ... ht_req=... force_ht=n ...` breadcrumbs, and fail before Function 2 if readable `HT_AVAIL` never appears. The Linux-style `FORCE_HT` write is applied only after HT is proven, immediately before Function 2 enable, so a later `CHIPCLKCSR` value with `FORCE_HT` is Function 2 propagation evidence rather than a pre-HT retry. HAL still retains the guarded startup-link fallback and reply-rearm ladder, but Function 2 enable now primes the Linux-sized 512-byte FBR block size before the IORX wait, post-firmware-ready probes still preserve already-latched F2 enables, strict reply recovery now force-clears and re-enables F2 when the card stays stuck enabled-but-not-ready, and no-HT Function 2 writes remain on the same 512-byte cadence while reply reads stay on smaller conservative bursts. The serial path reports that split through `write_chunk_limit=...`, `reply_chunk_limit=...`, `sdio function2 ready-snapshot ... diagnosis=...`, and `action=preserve-enable-latch ... reason=linux-aligned-no-clear`, so the next failure can be attributed to sideband state, HT proof, F2 latch state, packetization, or reply visibility instead of a generic promoted-rearm timeout.
