...
- As-built refinement: the experimental no-HT first-reply repair path now gives `control-plane-post-write-rearm` a bounded Function 2 `reply-probe` dwell instead of the pure `experimental-bypass` budget. If that startup-link reply repair still exhausts its bounded zero-frame rearm budget without Function 2 ready or any `RFRAMEBC` publication, root-task now first speculatively promotes the same in-flight ioctl onto the promoted `4-bit` / data-clock link (`control transport promote reason=startup-link-reply-timeout ... mode=speculative-first-reply`) and replays the promoted firmware-channel bind there before it gives up on that reply wait; only if that speculative promoted bind later stalls does the driver fall back to the existing bounded startup-link resend path instead of idling through the rest of the widened no-HT ioctl wait.
