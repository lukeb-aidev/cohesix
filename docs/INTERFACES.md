...
- USB owner-state diagnostics close the isolated controller ownership boundary, not the user-input readiness boundary. `DRIVER_TASK_OWNER_STATE ... hot_path=usb-keyboard owner_state=driver-owned` may appear after PCIe owner-state and xHCI/controller progress; command-ready additionally requires linked-runtime keyboard endpoint admission, first-report/idle proof, and no active hard recovery/no-reply/queue blocker.
