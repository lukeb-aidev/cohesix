...
- As-built correction: the Pi 4 CYW43455 post-download `wait-ht-clock` edge mirrors the captured Linux `CLK_SDONLY -> CLK_AVAIL` order. Root-task clears `CHIPCLKCSR` to SD-only, holds a bounded off-window, then issues `HT_AVAIL_REQ`; the KSO write is only wake evidence and DEVON is readback evidence, not a writable request bit. Function 2 remains blocked unless readable `CHIPCLKCSR.HT_AVAIL` appears.
