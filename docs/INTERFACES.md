...
- As-built refinement: on the active cold-boot `platform-reset-complete` USB path, local-seat no longer drains leading `PORT_STATUS_CHANGE` events before command proof. It keeps them queued for the Linux-shaped prompt-safe Enable Slot proof and relies on the event wait to consume non-command events before the Enable Slot completion.
