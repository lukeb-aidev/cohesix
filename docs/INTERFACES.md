...
- The Pi 4 USB deferred-capture contract requires command-ring proof. `command_probe=no-op-deferred` is no longer sufficient; the active high-BAR no-candidate runtime now submits a prompt-safe Linux-shaped No Op command as the first command-ring proof, then only unlocks live root-port sampling after that command completes. Real enumeration still issues `Enable Slot` after current-boot port evidence. `command_probe=no-op-ok`, `command_probe=enable-slot-ok`, and the enable-slot cleanup-failed variant prove command/event-ring consumption. Historical `enable-slot-linux-event-ok` / `no-op-linux-event-ok` labels are diagnostic only and do not satisfy the cold-boot USB gate. A concrete `tag=cmd-timeout` now normalizes as `USB_BLOCKER=cmd-timeout`, and concrete `no-op-unproven`, `enable-slot-unproven`, `no-op-linux-event-unproven`, or legacy `enable-slot-linux-event-unproven` proof summaries normalize as `USB_BLOCKER=cmd-event-ring-timeout`; these supersede the earlier timer-only `cmd-poll-pending` state.
- Current M26b USB correction: when the Pi 4 platform-reset lane has no port-event candidates, local-seat uses the prompt-safe Linux-shaped event-generation path (`USBCMD.RUN|INTE`, `IMOD=0xa0`, `IMAN.IE`) while PCI INTx/MSI/GIC delivery remains masked, then submits No Op as the command/event-ring proof. A successful `no-op-ok` moves the proof frontier to safe live port sampling; `Enable Slot` is reserved for the subsequent current-boot enumeration path. Any missing command completion remains `cmd-event-ring-timeout`, and IRQ27 remains the seL4 timer only.
- If a Linux-shaped prompt-safe command proof reaches the command timeout, `usb-oxide` performs one bounded fresh command/event-ring recovery before returning to the shell. The recovery allocates new HAL DMA-backed command and event rings, republishes `ERSTSZ`/`ERSTBA`/`ERDP`/`CRCR` with the Pi 4 PCIe DMA alias, drains the posted CRCR write through the HAL EXT_CFG flush hook, and retries the same command once while PCI INTx/MSI delivery remains masked. The active no-candidate proof retries No Op; historical direct `Enable Slot` traces used the same recovery shape. A second timeout remains `cmd-event-ring-timeout`, now proving the blocker survived fresh ring publication rather than stale command/event-ring state.
...
- As-built refinement: on the active cold-boot `platform-reset-complete` USB path, the EHB cleanup writes `IMAN.IP` as a write-one-to-clear value and leaves delivery masked at PCI/GIC. Any drained `PORT_STATUS_CHANGE` candidate is still OR-ed into the command-probe mask before deciding between normal `enable_slot` and the no-candidate Linux-shaped event-generation No Op proof.
