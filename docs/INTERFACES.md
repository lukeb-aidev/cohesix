...
- The Pi 4 local-seat runtime treats a firmware DT `xhci` node marked `status = "disabled"` as stale handoff rather than immediate loss of keyboard support: it translates BCM2711 `0x7e...` SoC-bus `reg` values into CPU physical addresses for diagnostics, ignores stale disabled-node hints as active xHCI runtime sources, pins the trusted handed-off xHCI BAR before the higher VL805 ECAM page so the lower handoff aperture is not lost to monotonic device retype ordering, keeps VL805 ECAM preseed on a map-only path because live ECAM reads still correlate with fatal Pi 4 halts, avoids broader live runtime VL805 config-space discovery while that path remains unsafe on Pi 4, emits concise candidate-decision breadcrumbs (`kind/cfg/cov/pwin/pin/fh/fs/vh`) plus a targeted firmware-handoff trust-gate line (`cmd_safe/token/irq/trusted/reason`) before skipping an xHCI MMIO source, and treats the standard U-Boot `usb stop` handoff as a trusted post-stop snapshot only when the bootloader exports the safe high-BAR contract. If the bootloader contract is rejected, runtime no longer uses the presence of the pinned config window as permission to probe the high BAR anyway; it now emits `xhci runtime blocked action=reject-untrusted-high-bar ...` and leaves local-seat degraded instead of touching xHCI MMIO after an untrusted handoff. The low `0xfe980000` / `0x7e980000` BCM2711 USB aliases are not VL805 runtime sources on Pi 4; they identify the SoC `brcm,bcm2835-usb` block and now remain diagnostic breadcrumbs only. Runtime uses the seL4 bcm2711 `device-untypes@600000000` window for the U-Boot-discovered VL805 BAR when present, logs whether the handed-off BAR still has device coverage without touching ECAM (`xhci handoff window mmio=... coverage=... pinned=...`), and still logs the DT-exported handoff detail that explains a partial contract: `xhci-handoff-halted=...`, `xhci-handoff-safe=...`, `xhci-stop-state usbcmd=... usbsts=... iman0=...`, `xhci-handoff-dtb-ext source=... prestop=ready/irq/halted/safe poststop=ready/irq/halted/safe`, and `xhci-handoff-reject source=... reason=...`. When the trusted high BAR is selected, local-seat now first replays the bounded runtime VL805 mailbox reset-notify on that path as an acknowledged VideoCore property call through the shared long-lived request page; if the reply arrives under the other standard VideoCore alias for that same request page, runtime now accepts it as the same acknowledged call instead of treating it as stray. If VideoCore does not acknowledge within a short bounded grace window, or if the acknowledged reply still fails mailbox protocol validation after that same-page alias acceptance, it immediately replays the same reset-notify through a dedicated posted mailbox request page before the usual settle. Because this runtime path uses a spin-only wait helper rather than a calibrated microsecond timer, the synchronous grace window stays intentionally short, the acknowledged path keeps its short settle, and the posted fallback now takes a wider cold-init settle before runtime touches VL805 again. The bootloader-exported safe PCI command word stays untouched, `usb-oxide` still enters cold-start-from-snapshot mode from the trusted post-stop capability plus stop/ring seed snapshot only when that reset-notify path actually completes with an acknowledged reply, that mailbox reset remains the active reset boundary instead of a second live `HCRST` store, and only the bounded bcm2711 PCIe child-INTx and bridge sinks are installed while xHCI ownership itself stays polling-driven. If the reset-notify path degrades to the posted fallback, or if the posted fallback itself still fails, local-seat now keeps the candidate but drops the bootloader runtime ring-seed snapshot for that weaker reset path even when those ring seeds are cached. Posted fallback therefore reuses only the trusted CAP snapshot plus the stop-state fields and falls directly into the standalone cold-init path, so `usb-oxide` goes back to the normal live reset/config/ring publication flow instead of trying to preserve runtime-owned event-ring state across the weaker posted-reset boundary. The active proof line is `xhci runtime trust source=fw-handoff-mailbox-reset-snapshot ...` only when the runtime reset actually completed with the acknowledged path, and local-seat then emits `vl805 reset handoff=runtime-owned stage=runtime action=mailbox-notify`, followed by `vl805 reset handoff=runtime-owned stage=runtime detail=mailbox-notify+settle`, and then `xhci handoff=runtime-owned stage=runtime detail=mailbox-reset+trusted-cap-snapshot action=cold-start-from-snapshot` before entering `usb-oxide`. If the mailbox path instead emits `vl805 reset handoff=runtime-owned stage=runtime detail=mailbox-posted-fallback+settle`, runtime now treats that as posted-reset evidence rather than proof that the runtime ring snapshot remains safe. When reset-notify degrades to soft-continue, or to posted fallback, local-seat still drops the trusted runtime ring snapshot for that attempt, but if a trusted CAP snapshot is cached it now reuses that static CAP layout and enters `usb-oxide` with cold-start-from-snapshot constructor/init semantics using only the stop-state fields and without the runtime ring seed snapshot, so the fallback avoids the first constructor-time `USBCMD` scrub write, the first live halt-state revalidation reads, the second live `HCRST` store, the first live `CONFIG` seed read, and the remaining live post-reset seed reads while preserving the firmware `CONFIG` value; only if no trusted snapshot exists does it fall back to a live xHCI capability probe and cold-init path. The high-BAR path keeps command completion and enumeration on the polling path, the command-path breadcrumbs (`cmd-submit`, `cmd-ring-enqueue`, `cmd-completion`, `cmd-ccs-expected-ptr`, `cmd-ccs-mismatch`, `cmd-fail`, `cmd-fail-state`, `cmd-wait-other-event`, `cmd-timeout`, `cmd-timeout-state`, `cmd-timeout-last-event`) remain the primary evidence surface if initialization still stalls, safe-mode runtime still skips live VL805 PCI-command replay while ECAM reads are unsafe, and otherwise root-task degrades to serial-only input with explicit `[local-seat] ... unavailable` breadcrumbs.
- On the weaker stop-state-only xHCI snapshot branch, runtime still drops all bootloader-owned ring state and still re-enters `usb-oxide` through `cold-start-from-stop-state-snapshot` using only the trusted CAP layout plus the stop-state fields (`USBCMD`/`USBSTS`/`IMAN0`), but after the first live `ERSTBA` publish proved toxic there it now falls back to the normal cold-start reset / `CONFIG` / ring-publication sequence instead of preserving the weaker deferred ownership shortcut. That stop-state path therefore keeps the early halt-state diagnostic skip but no longer uses the deferred `ERST*` / `ERDP` / `DCBAAP` / `CRCR` ladder or the post-run stop-state-only settle/interrupter-skip shortcuts.
- The preserved-controller `DCBAAP` probe breadcrumbs remain available inside `usb-oxide` for regression work, but the active Pi 4 local-seat runtime no longer depends on them. On the trusted high-BAR path, root-task now first issues the bounded VL805 mailbox reset-notify and then lets `usb-oxide` take the cold-start-from-snapshot path that uses the bootloader-exported stop/ring seed snapshot to suppress the fragile pre-reset ownership reads while also treating the mailbox reset as the active reset boundary. The acknowledged VL805 reset-notify now reuses the shared long-lived mailbox request page (`reuse-shared` / `alloc-shared`), while the posted fallback path keeps its own dedicated page. Before any live post-reset xHCI ownership write, `usb-oxide` now inserts a short blind post-reset settle (`reset-post-settle-begin` / `reset-post-settle-done`), then returns to the normal `USBSTS.CNR` completion poll before republishing runtime-owned state. The stronger mailbox-reset/resetless branches still use the deferred `DCBAAP` / `CRCR` / `ERDP` / `ERST*` ownership ladder, while the softer stop-state-only fallback now keeps the same `cold-start-from-stop-state-snapshot` identity only for the trusted CAP/stop-state hint and then falls back to the ordinary reset / `CONFIG` / live ring-seed / ring-publication path instead of skipping `HCRST`. A halt line ending in `Kernel entry via Interrupt, irq 27` is treated only as the kernel timer PPI being the last tracked entry, not as proof that the xHCI or PCIe SPI escaped. On that trusted handoff path, command submission also rings xHCI doorbells without the extra posted readback that `usb-oxide` used previously, matching the working U-Boot doorbell sequence more closely before first enumeration.
- On the trusted Pi 4 xHCI handoff path, `usb-oxide` now shares the initial DCBAA, scratchpad, command-ring, and ERST/event-ring buffers through the root-task DMA cache-maintenance seam before the first post-handoff `DCBAAP` / `CRCR` / runtime-ring ownership writes.
- Superseding the earlier Pi 4 ring-seed reuse experiment, the active trusted runtime path now strips all bootloader runtime ring pointers before entering `usb-oxide`: `mailbox-reset+trusted-cap-snapshot`, `mailbox-posted-fallback+trusted-stop-state-snapshot`, and `static-cap-snapshot` all now rebuild runtime ring / event-ring ownership locally. The stronger mailbox-reset/resetless paths still use the deferred ownership ladder, while the weaker stop-state-only path now falls back to the ordinary reset / `CONFIG` / ring sequence instead of preserving that ladder from firmware.
- The Pi 4 CYW43455 firmware-load path now keeps the startup SDIO link active through chip discovery, RAM sizing, and AI core disable/reset, then starts Function 1 firmware upload on aligned 256-byte CMD53 backplane chunks while AI core-control registers prefer Linux-style flagged 4-byte Function 1 backplane accesses and log explicit current-window and rewindow retries when those control-path reads or writes fail. The later firmware-channel mailbox and interrupt registers now follow the same Linux-shaped bias by preferring incrementing 32-bit SDIO-core backplane transfer-window CMD53 accesses through `CYW43_SDIO_CORE_BASE + offset` before falling back to incrementing or direct Function 1 register access. If the shorter pre-write HT wait never leaves the startup 400 kHz link, firmware upload now immediately emits `firmware stage=write-firmware mode=byte-start reason=startup-clock-no-ht current_clock=400000Hz` and keeps that attempt on the byte-mode transfer shape so boot does not spend its first fragile upload window replaying block-mode CMD53 writes before the retry path can react. If the upload path still hits its first SDHCI IO failure, that boot attempt emits `firmware stage=write-firmware-bulk-retry ... action=switch-byte-mode` and keeps the rest of the upload on the byte-mode transfer shape instead of replaying the same slower block-mode write pattern. When the shorter pre-write HT wait times out, bulk upload now stays on the startup/restore SDIO clock instead of stepping into speculative higher upload clocks. After firmware upload completes, root-task now keeps ARMCR4 reset on the startup/restore SDIO clock, raises the host SDIO link to the 12.5 MHz control-plane floor immediately after that reset boundary and before ARMCR4 core-up verification, and then reuses the same floor for the strict final HT wait. That first required `CHIPCLKCSR` request now keeps `ALP_AVAIL_REQ`, `HT_AVAIL_REQ`, and `FORCE_HT` asserted together, the second bounded retry additionally asserts `FORCE_HW_CLKREQ_OFF`, and ARMCR4 post-reset readback now retries at the raised control-plane clock before root-task falls back to the bounded deferred-read assumption. If that stronger HT retry still times out, root-task now emits `experimental-continue-without-ht` and probes `setup-firmware-channel` / `wait-firmware-ready` anyway so the next remaining failure moves beyond the HT gate instead of stopping there. The later Function 2 transport refresh still preserves the writable `CHIPCLKCSR` request bits while layering `FORCE_HT`. On the strict path, Function 2 still uses the Linux-style two-phase readiness budget (`linux-f2-extended-wait` plus one recover/reassert retry), but on the experimental no-HT bypass path it now keeps only the short initial readiness probe and skips that longer dwell/retry before falling into the existing `experimental-continue-without-ready` path, so startup no longer burns the full Linux-style Function 2 budget on a path that is already allowed to proceed without `IORX`. BCM43455 still stays on a 512-byte Function 2 block size, and timeout logs still include the cached transport shadow (`current_clock`, `preferred_data_clock`, `width`, `chipclk`, `wake`, `sleep`, `cardcap`) at each exhausted phase before retry or fallback. Successful `setup-firmware-channel`, `slow-link-channel-rearm`, `control-plane-post-write-rearm`, and `control-plane-promote-rearm` stages now also emit immediate control-plane snapshots that include `HOSTINTMASK`, `TOHOSTMAILBOXDATA`, `SDIO_INT_STATUS`, `RFRAMEBCLO/BCHI`, `WATERMARK`, `DEVICE_CTL`, and `MESBUSYCTRL`, so the next boot can tell whether the mailbox/device-control programming actually stuck on the active transport. When Pi 4 local-seat HDMI is attached before networking starts, this same bring-up path now mirrors a low-frequency `[cohesix] Initializing WiFi`, `[cohesix] Initializing WiFi.`, `[cohesix] Initializing WiFi..`, `[cohesix] Initializing WiFi...` keepalive cycle on HDMI until Wi-Fi init returns instead of appending dots on every inner progress tick. During AI core disable, the immediate post-`assert-reset` readback is now deferred until after the in-reset configure settle so SOCRAM reset does not fail purely because Function 1 briefly stops answering direct reads during the reset edge.
...
- On that same experimental no-HT Wi-Fi probe path, `wait-firmware-ready` now first tries the host mailbox through the SDIO-core backplane transfer-window path, then falls back to a single incrementing CMD53 transfer instead of the older bytewise direct-access path, and if that mailbox read still dies in the SDHCI data/timeout path it emits `experimental-assume-ready` and keeps probing so the next remaining failure boundary moves beyond the first mailbox read itself. After firmware-ready, the same path now emits `post-firmware-ready-function2-recheck action=repoll`, immediately reasserts/repolls Function 2, deliberately drops the post-firmware control plane back to the startup link (`hold_control_plane_transport(startup-link)`) at `400 kHz` / `1-bit`, and then replays the firmware-channel mailbox version plus `HOSTINTMASK` programming on that same slow link as `rearm_control_plane_channel(startup-link)` / `firmware stage=slow-link-channel-rearm ...` before the first outbound ioctl can use Function 2. That same path now also caps non-incrementing Function 2 FIFO transfers at 512-byte block-sized chunks (`sdio xfer chunk-limit ... reason=experimental-no-ht-f2-fifo`), so the first large CLM/control frame is shaped as bounded F2 blocks plus a tail instead of a long run of smaller probe slices. The first outbound Function 2 control-plane write is now also a bounded probe: root-task emits `[cyw43] control tx probe ... chunk_limit=... no_ht=...` before the transfer, and if that first write still dies on an SDHCI IO-path boundary it emits `experimental-assume-control-write`, refreshes the SDIO command path, snapshots `IOEX` / `IORX` / `IENX`, `HOSTINTMASK`, `TOHOSTMAILBOXDATA`, `SDIO_INT_STATUS`, `RFRAMEBCLO/BCHI`, `MESBUSYCTRL`, and the cached transport shadow, and falls back from the SDIO-core or incrementing CMD53 snapshot reads to the older direct byte reads when those register snapshots themselves hit the same SDIO edge. If that first no-HT control-plane write succeeds, root-task now immediately emits `experimental-post-write-rearm`, reruns the Function 2 ready poll plus the firmware-channel mailbox version and `HOSTINTMASK` programming as `control-plane-post-write-rearm ...` on that same slow link before it starts waiting for the first reply, and while that same no-HT link remains active the immediate control-plane snapshots now intentionally skip the live SDIO-core `HOSTINTMASK` / `TOHOSTMAILBOXDATA` / `SDIO_INT_STATUS` readbacks so diagnostics do not perturb the fragile receive path. Those skip lines now also report the armed reply-rearm mode (`startup-link|promoted-link|none`), attempt count, and current chunk limit so a later `ioctl-timeout` stays attributable to the startup repair path, promoted repair path, or a post-repair steady-state timeout. If that repaired empty poll still produces no first response, the driver now uses that same bounded empty-poll streak as a single slow-link recovery trigger: it logs `control transport recover reason=empty-poll-rearm ...`, reruns `slow-link-channel-rearm`, returns a bounded `cyw43-control-plane-startup-link-retry`, and immediately resends that ioctl once on the repaired startup link while blocking repeated empty-poll recoveries only for that single send attempt instead of speculating into the promoted link. If the bootstrap control ioctl still ends in `ioctl-timeout` while the experimental no-HT transport is active, the driver now logs `control transport retry reason=ioctl-timeout ...`, drops back to the startup link again, reruns `slow-link-channel-rearm`, and retries that ioctl once from the stable `400 kHz` / `1-bit` transport before failing. A later `matched-reply` promotion still uses the strict promoted-link rearm budget, and a failed promoted-link rearm continues to log `control transport promote fallback ...`, returns to the startup link, reruns `slow-link-channel-rearm`, and blocks further early empty-poll recoveries until a later matched-reply path can try again. Successful promoted rearm now emits the same immediate `control-plane-promote-rearm-ready` snapshot surface as the slow-link stages. If the first high-speed reply is still invisible after a successful promoted bind, the same reply-repair path now stays armed on the promoted link; if that promoted reply-rearm still burns through its bounded budget without Function 2 ready or a frame, root-task emits `control-plane-reply action=promoted-rearm-stalled ...`, returns `cyw43-control-plane-promoted-rearm-timeout`, and the driver immediately logs `control transport promote fallback reason=reply-rearm-stall ...`, drops back to the startup link, reruns `slow-link-channel-rearm`, returns the same bounded `cyw43-control-plane-startup-link-retry`, and immediately resends there instead of spending the rest of the widened no-HT ioctl budget on the dead promoted probe. A later matched control response still logs `control transport promote reason=matched-reply ...` when promotion remains pending and now follows the same promoted-link rearm, so the next remaining failure boundary moves beyond the first outbound control frame rather than stopping at `sdhci-transfer-finish`.
- As-built refinement: the experimental no-HT Function 2 FIFO path now uses 512-byte block-sized chunks for non-incrementing Function 2 transfers, so the first large CLM/control frame is shaped as bounded F2 blocks plus a tail instead of a long run of smaller probe slices.
- As-built refinement: the weaker Pi 4 stop-state-only xHCI snapshot branch still keeps the trusted CAP layout plus the stop-state diagnostics, but it no longer treats that weak snapshot as reset-equivalent after the first live `ERSTBA` publish proved toxic. Runtime now returns to the ordinary cold-start reset / `CONFIG` / live ring-seed / ring-publication flow on that path, while the stronger mailbox-reset/resetless snapshot paths keep the deferred `ERST*` / `ERDP` / `DCBAAP` / `CRCR` ladder and their mailbox-reset-specific shortcut breadcrumbs.
- On that same experimental no-HT Wi-Fi probe path, speculative `empty-poll` promotion is no longer used during bootstrap. Promotion now commits only on `matched-reply`; early empty polls are handled by one bounded `empty-poll-rearm` recovery on the startup link, an immediate bounded resend on that repaired startup link, and, if needed, a single `ioctl-timeout-retry` from that same startup-link transport. Those startup-link rescue paths now also reseed one bootstrap SDPCM transmit credit (`seq_max = seq + 1`) without rewinding the host sequence number, so the retry does not immediately die in `sdpcm-credit-timeout` after the link has been deliberately rearmed. The empty-poll recovery block now stays armed across the immediate startup-link resend and the later timeout-level resend for that same logical ioctl, and the first reply wait still uses the widened no-HT budget (`wait_budget=32000`) so the repaired `400 kHz` / `1-bit` control plane can settle before declaring `ioctl-timeout`.
- As-built refinement: the experimental no-HT first-reply repair path now gives `control-plane-post-write-rearm` a bounded Function 2 `reply-probe` dwell instead of the pure `experimental-bypass` budget. If that very first startup-link post-write rearm itself times out in `sdio-function2-ready-timeout`, HAL now immediately emits `control-plane-post-write-rearm action=promote-after-timeout ... mode=speculative-first-reply`, raises the control-plane link back to `4-bit` / data-clock, and replays the promoted firmware-channel bind there before the driver even enters the reply wait. If a later zero-frame reply repair on the startup link still exhausts its bounded budget without Function 2 ready or any `RFRAMEBC` publication, the existing driver-side speculative promotion path (`control transport promote reason=startup-link-reply-timeout ... mode=speculative-first-reply`) still takes over; only if that speculative promoted bind later stalls does the driver fall back to the existing bounded startup-link resend path instead of idling through the rest of the widened no-HT ioctl wait.
- As-built refinement: when the no-HT experimental path remains active after firmware load, the driver now first probes the post-firmware control-plane bind on the promoted `4-bit` / data-clock transport (`step: probe_control_plane_transport(promoted-link)` plus `control transport initial probe ready ... mode=speculative-first-reply`) before it returns to the startup-link rescue flow. If that promoted bootstrap bind fails, the driver logs `control transport initial probe fallback ... reason=no-ht-bootstrap`, drops back to the `400 kHz` / `1-bit` startup link, and keeps the existing bounded resend/rearm behavior. If that promoted bootstrap bind succeeds but the first real Function 2 control payload still dies on an SDHCI transfer boundary, the driver now treats that as `promoted-write-fail`, immediately falls back to the startup link, and resends there instead of letting the promoted path continue under `experimental-assume-control-write`. The firmware-channel rearm helper also now tries a direct Function 1 byte-write fallback for mailbox version / `HOSTINTMASK` after incrementing CMD53 fails and before it uses the bounded assume-committed path.
- As-built refinement: on the weaker stop-state-only xHCI snapshot branch, runtime still strips all bootloader-owned ring state and still enters `usb-oxide` through `cold-start-from-stop-state-snapshot`, but that path now skips both the live `HCRST` store and the matching live `CONFIG` store while keeping fresh runtime ring/event-ring publication. The stop-state snapshot therefore remains a trusted CAP/stop-state hint, not a fully reset-equivalent ownership shortcut.
- As-built refinement: the conservative no-HT Wi-Fi startup-link bootstrap still carries the first outbound control payload, but any later promoted reply-rearm now lowers the fragile mailbox / `HOSTINTMASK` / watermark / `MESBUSYCTRL` programming back to the startup clock before restoring the promoted data clock. The serial log surfaces that hybrid write path as `firmware-channel-write-clock action=lower ...` and `action=restore ...`, so a remaining promoted failure can be separated cleanly from the first high-speed reply path itself.
