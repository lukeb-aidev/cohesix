...
- Authenticated Pi 4 `cohsh reboot` requests best-effort mark the BCM2711 PM RSTS register with a one-shot Cohesix fast-boot value, then arm the watchdog through the BCM2711 PM WDOG/RSTC restart sequence. Root-task orders and drains the marker write before arming reset, but live Pi 4 evidence shows PM RSTS high-marker retention is not reliable enough to be the only reboot handoff. The generated script first forces serial input, reloads `cohesix.env`, prefers the Cohesix high marker when visible, and falls back to `source=software-reset-saved-policy` only when the hardware software-reset bit is set and a saved Cohesix policy was loaded. Accepted sources clear marker state with the PM write password and run the same `coh_boot_sequence` with saved policy or manifest defaults. This skips the logo/menu/input path for software reboots with a Cohesix fast-boot source and does not create a new control-plane secret, generic persistent U-Boot environment setting, or bootloader xHCI handoff.
- If U-Boot does not observe an accepted fast-boot source, the script logs a bounded `fast boot marker absent` diagnostic containing the observed RSTS/high/reset marker values plus saved-policy state and then enters the normal cold-boot wizard. That path is recovery evidence only; it does not silently convert a cold boot into an unattended boot.
...
- USB owner-state diagnostics close the isolated controller ownership boundary, not the user-input readiness boundary. `DRIVER_TASK_OWNER_STATE ... hot_path=usb-keyboard owner_state=driver-owned` may appear after PCIe owner-state and xHCI/controller progress; command-ready still requires the later linked-runtime HID first-byte latch described above.
