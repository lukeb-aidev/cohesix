...
- when Wi-Fi bring-up fails, the serial log must now identify the failing stage with `[cyw43] step: ...` and `[pi4-wifi] ...` breadcrumbs before the final `mailbox-*`, `sdio-*`, or `cyw43-*` error detail. In particular, firmware load now emits `firmware core-ctrl mode=cmd52-flagged fallback=cmd53-word split-window` before the ARMCR4/SOCRAM AI-core sequence so bring-up logs prove the flagged single-byte Function 1 control path and the remaining CMD53 fallback that root-task is using.
