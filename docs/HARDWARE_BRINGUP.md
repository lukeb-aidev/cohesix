...
- A stale firmware DT handoff with `xhci` marked `status = "disabled"` does not strand local-seat: root-task ignores that stale node as an active runtime source, consumes `/chosen/cohesix,xhci-mmio` and `/chosen/cohesix,xhci-pci-cmd` as bootloader breadcrumbs only, rejects a leaked `0xc0000000..0xffffffff` DMA-alias value as non-MMIO, defers VL805 ECAM preseed entirely when safe mode still forbids runtime config-space reads, avoids live runtime VL805 config-space reads on Pi 4 when that path is still unsafe, emits concise candidate-decision breadcrumbs (`kind/cfg/cov/pwin/pin/fh/fs/vh`) plus a targeted firmware-handoff trust-gate line (`safe/cfg/tp/vv/pwin/cov`) before rejecting an xHCI MMIO source, and uses the seL4 bcm2711 `device-untypes@600000000` window for the U-Boot-discovered VL805 BAR before falling back to bounded legacy xHCI alias probing and emitting an explicit degraded/unavailable reason.
