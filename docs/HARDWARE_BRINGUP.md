...
- Pi 4 Wi-Fi AI core-control diagnostics must surface both sides of the staged recovery path: flagged CMD53 windowed reads and byte-sized CMD52 writes against the current window, plus a forced CMD52 rewindow fallback for either direction when the primary path fails. Serial logs must keep the exact `firmware core-ctrl fallback ... to=cmd52-byte-current-window|cmd52-byte-rewindow`, `backplane window program ...`, `sdio xfer chunk fn=1 ...`, and `sdhci recover stage=core-ctrl-cmd52-current-window|core-ctrl-cmd52-rewindow|... mask=cmd+data` breadcrumbs so remaining failures stay attributable to either the transport path or the recovery path.
- The first SOCRAM reset-release path now adds `clear-reset-prewrite-delay ... reason=socram-fragile-first-write` and, on failure, a single `clear-reset-write-retry ... reason=socram-fragile-first-write` recovery attempt before the existing deferred `clear-reset-read-deferred ...` readback path, so logs distinguish a transient reset-release edge stall from a later post-reset failure.
