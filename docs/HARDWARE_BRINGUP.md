...
- A stale firmware DT handoff with `xhci` marked `status = "disabled"` does not strand local-seat: root-task ignores that stale node as an active runtime source, prefers the U-Boot-exported `/chosen/cohesix,xhci-mmio` host-physical BAR handoff when present, consumes `/chosen/cohesix,xhci-pci-cmd` as the bootloader's PCI command-state breadcrumb, rejects a leaked `0xc0000000..0xffffffff` DMA-alias value as non-MMIO, and either brings up the USB keyboard or emits an explicit degraded/unavailable reason.
