...
- when Wi-Fi bring-up fails, the serial log must now identify the failing stage with `[cyw43] step: ...` and `[pi4-wifi] ...` breadcrumbs before the final `mailbox-*`, `sdio-*`, or `cyw43-*` error detail. In particular, firmware load now emits `firmware core-ctrl mode=cmd53-word-windowed fallback=cmd53-word-current-window split-window` before the ARMCR4/SOCRAM AI-core sequence so bring-up logs prove the primary Linux-style flagged 4-byte Function 1 backplane control path and the remaining current-window/rewindow retries that root-task is using. If the primary word read path times out, root-task now first retries the same flagged 4-byte access against the current backplane window and only then falls back to a full rewindowed retry; the serial log also emits `sdhci recover stage=cmd-wait|cmd-error mask=cmd+data` when a command timeout forces host recovery.
