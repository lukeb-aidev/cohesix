...
- On Pi 4, `wired` selects the on-board GENETv5 path, explicit `wifi` selects the HAL-backed CYW43455 SDIO path for bounded `static` or `dhcp`, and `auto` remains DHCP-only while preferring CYW43455 only when bounded credentials are present and the Wi-Fi attach/join path succeeds; otherwise it falls back to wired with explicit boot logs.
