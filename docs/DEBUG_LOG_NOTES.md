...
- A subsequent `retype.init_cnode …` line dumps the expected root CNode capability, slot index, and guard depth derived from bootinfo. This line makes it trivial to compare the raw submission against the canonical init-thread parameters.
