...
- Init CNode must be addressed with guard-depth semantics: depth = `seL4_WordBits`, index = slot, offset = 0. Using `initThreadCNodeSizeBits` as the depth with the slot encoded in the offset will fail with `Invalid destination address`.
