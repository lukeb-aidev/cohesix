...
- **Responsibilities**:
  ...
  - Mirror GPU state into the VM by exposing a 9P transport endpoint (`secure9p-transport::Tcp`) to NineDoor and brokering `/gpu/` files; no CUDA/NVML components enter the VM.
