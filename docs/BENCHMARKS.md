...
- The current offline performance pass changes code-level hot-path shape only: CYW43 data TX token admission is firmware-credit-gated and in-place, GENET RX/TX service turns are capped, and an explicit no-USB `qemu-driver-task-smoke` run reaches the QEMU root console with `configured=9`, `failed=0`, `live_tcb_count=9`, `vspace=isolated`, `ipc_abi=shared-ring-command`, `pointer_free_ipc=yes`, and `owner_state=not-proven` for the post-network smoke TCB probe. The physical Pi path now selects ring-backed driver-task clients for GENET/CYW43, initializes the real `BcmGenetDevice` and `Cyw43NetDevice` through driver-task init commands, initializes the real `Pi4LocalSeat` through a USB/local-seat driver-task init command, initializes the real UART runtime through a serial driver-task init command, removes the physical-Pi `KernelHal` `Pi4WifiState` slot, gives root only ring-backed serial/NIC/local-seat clients, defaults the Pi 4 manifest to DHCP/`auto`, and requires local-seat initialization instead of allowing silent serial-only degradation. Steady-state Pi hardware progress for serial, GENET, CYW43/SDIO, USB/PCIe, and HDMI/local-seat is therefore behind pointer-free shared-ring service turns, while emergency early serial remains the root-owned escape hatch. Proof accounting is also stricter: idle or zero-progress ring completions cannot credit hot-path ownership, owner-state descriptor registration is rejected while runtime-image specs remain non-acceptance for all seven hot paths, aggregate owner-state cannot pass without the explicit owner hot-path mask plus acceptance-eligible runtime images, and trace normalization requires exact per-hot-path `owner_state=driver-owned descriptor=present root_pointer=no` lines. This is not a performance result: it is code-level preparation for the next hardware measurement. QEMU transport-substrate and host wire-layout proof only prove isolated VSpace/ring mechanics for the smoke trampoline, complete command coverage, runtime-image transport mapping, and fail-closed Pi admission policy; they do not prove Pi 4 hardware performance. Hardware performance closure still requires fresh Pi measurements plus concrete `DRIVER_TASK_BOOT` and service markers for the active Wi-Fi or GENET path, USB keyboard, HDMI, and clean emergency serial.
