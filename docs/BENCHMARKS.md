...
- The current offline performance pass changes code-level hot-path shape only: CYW43 data TX token admission is firmware-credit-gated and in-place, GENET compatibility RX/TX service turns are capped, and an explicit no-USB `qemu-driver-task-smoke` run reaches the QEMU root console with `configured=9`, `failed=0`, `live_tcb_count=9`, `vspace=isolated`, `ipc_abi=shared-ring-command`, `pointer_free_ipc=yes`, and `owner_state=not-proven` for the post-network smoke TCB probe. The HAL also now has a fixed-layout pointer-free command catalog for every Pi 4 hardware hot-path target, per-hot-path runtime-image mapping declarations, isolated-QEMU transport mapping masks for code/stack/IPC/ring, and a physical-Pi runtime policy that disallows steady-state callback/root fallback outside early/emergency serial. The latest physical-Pi cutover goes further by selecting inert driver-task client shells for GENET/CYW43, removing the physical-Pi `KernelHal` `Pi4WifiState` slot, and avoiding root `Pi4LocalSeat` attachment, so root-owned steady-state hardware structs are no longer constructed on the Pi profile. This is not a performance result: until the isolated runtimes implement useful MMIO/DMA/ring service, the next Pi 4 boot may regress Wi-Fi, GENET, USB keyboard, and HDMI. QEMU transport-substrate and host wire-layout proof only prove isolated VSpace/ring mechanics for the smoke trampoline, complete command coverage, runtime-image transport mapping, and fail-closed Pi admission policy; they do not prove Pi 4 hardware hot-path ownership and are not Pi 4 benchmark results. Hardware performance closure still requires fresh Pi measurements plus `DRIVER_TASK_OWNER_STATE_PROOF=yes` with concrete owner-state descriptor lines for `serial-console`, `usb-keyboard`, `hdmi-text`, `genet-nic`, `cyw43-wifi`, `sdio-host`, and `pcie-root`.
