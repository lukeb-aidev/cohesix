...
- The current offline performance pass changes code-level hot-path shape only: CYW43 data TX token admission is firmware-credit-gated and in-place, GENET compatibility RX/TX service turns are capped, and an explicit no-USB `qemu-driver-task-smoke` run now reaches the QEMU root console with `configured=9`, `failed=0`, `live_tcb_count=9`, `vspace=isolated`, `ipc_abi=shared-ring-command`, and `pointer_free_ipc=yes` for the post-network smoke TCB probe. The HAL also now has a fixed-layout pointer-free command catalog for every Pi 4 hardware hot-path target and a physical-Pi runtime policy that disallows steady-state callback/root fallback outside early/emergency serial. That is QEMU transport-substrate and host wire-layout proof only: it proves isolated VSpace/ring mechanics for the smoke trampoline, complete command coverage, and fail-closed Pi admission policy, not Pi 4 hardware hot-path ownership and not a Pi 4 benchmark result. The changes are expected to reduce root-task stalls and improve load fairness once hardware paths move behind the rings, but they are not benchmark results until measured on a freshly flashed Pi 4.
