...
- The current offline performance pass changes code-level hot-path shape only: CYW43 data TX token admission is firmware-credit-gated and in-place, GENET compatibility RX/TX service turns are capped, and an explicit no-USB `qemu-driver-task-smoke` run now reaches the QEMU root console with `configured=9`, `failed=0`, `live_tcb_count=9`, `vspace=isolated`, `ipc_abi=shared-ring-command`, `pointer_free_ipc=yes`, and `owner_state=not-proven` for the post-network smoke TCB probe. The HAL also now has a fixed-layout pointer-free command catalog for every Pi 4 hardware hot-path target, per-hot-path runtime-image mapping declarations, isolated-QEMU transport mapping masks for code/stack/IPC/ring, and a physical-Pi runtime policy that disallows steady-state callback/root fallback outside early/emergency serial. Transitional shared-ring services that still carry root context are explicitly registered as `root-context-diagnostic`; the HAL forces `DRIVER_TASK_RING_FLAG_ROOT_CONTEXT_NON_ACCEPTANCE` on those commands, and standalone SDIO bus-owner records remain non-acceptance with reason `root-hal-exec`, so they remain readiness diagnostics only. QEMU transport-substrate and host wire-layout proof only prove isolated VSpace/ring mechanics for the smoke trampoline, complete command coverage, runtime-image transport mapping, and fail-closed Pi admission policy; they do not prove Pi 4 hardware hot-path ownership and are not Pi 4 benchmark results. Hardware performance closure now additionally requires `DRIVER_TASK_OWNER_STATE_PROOF=yes` with concrete owner-state descriptor lines for `serial-console`, `usb-keyboard`, `hdmi-text`, `genet-nic`, `cyw43-wifi`, `sdio-host`, and `pcie-root`, so a pointer-free ring cannot be mistaken for driver-owned state while root-owned runtime structs still back the service turn. The changes are expected to reduce root-task stalls and improve load fairness once hardware paths move behind the rings, but they are not benchmark results until measured on a freshly flashed Pi 4.
