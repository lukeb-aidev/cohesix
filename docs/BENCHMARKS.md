...
- Six generated Pi 4 runtime specs are now acceptance-capable (`root_context_required=false`, `hardware_state_migrated=true`). Standalone `sdio-host` stays generated but non-acceptance with zero MMIO while CYW43 owns the current SDHCI Wi-Fi path. Runtime budgets are sized for throughput rather than smoke coverage: GENET has a 512-page DMA arena for the 256-RX/256-TX ring model, USB has 128 DMA pages, CYW43 has 128 DMA pages plus 64 shared pages, SDIO has 64 DMA pages, HDMI has 16 DMA plus 16 shared pages, and PCIe has 16 shared pages.
- The linked runtime engines now contain real service code: serial owns bounded mini-UART init/RX/TX, HDMI renders to a mapped framebuffer, PCIe services bounded MMIO read/write/flush turns, USB owns a direct-root-port xHCI boot-keyboard path with command/event/EP0/interrupt-IN rings, GENET programs MDIO/MAC plus bounded RX/TX descriptor rings, and CYW43 performs SDIO transport bring-up, firmware/NVRAM streaming, ARMCR4 release, bounded SDPCM control/data TX, and RX polling. Standalone SDIO validates legacy command turns and fixed-layout CMD52/CMD53/POLL_IRQ descriptors as a non-acceptance ABI guard.
