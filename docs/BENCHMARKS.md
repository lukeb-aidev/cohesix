...
- The current offline performance pass changes code-level hot-path shape only: CYW43 data TX token admission is firmware-credit-gated and in-place, GENET compatibility RX/TX service turns are capped, and an explicit no-USB `qemu-driver-task-smoke` run now reaches the QEMU root console with `configured=9`, `failed=0`, and `live_tcb_count=9` for the post-network smoke TCB probe. That remains QEMU compatibility proof only: it still reports `vspace=shared-root`, `ipc_abi=callback-pointer`, and `pointer_free_ipc=no`, so it is not full dedicated-driver isolation and not a Pi 4 benchmark result. The changes are expected to reduce root-task stalls and improve load fairness, but they are not benchmark results until measured on a freshly flashed Pi 4.
