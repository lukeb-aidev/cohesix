...
- The current offline performance pass changes code-level hot-path shape only: CYW43 data TX token admission is firmware-credit-gated and in-place, GENET RX/TX service turns are capped, and an explicit no-USB `qemu-driver-task-smoke` run reaches the QEMU root console with `configured=9`, `failed=0`, `live_tcb_count=9`, `vspace=isolated`, `ipc_abi=shared-ring-command`, `pointer_free_ipc=yes`, and `owner_state=not-proven` for the post-network smoke TCB probe. The physical Pi bootstrap path now stages linked `pi4-driver-*` runtime images, maps every bounded runtime ELF `PT_LOAD` page into isolated child VSpaces, sends a bounded pointer-free `pi4-driver-abi` runtime-init descriptor with MMIO/DMA/shared physical page metadata, semantic resource ranges, bus policy, IRQ slots, USB/PCIe and CYW43/SDIO bus links, and framebuffer metadata, and uses the fixed ring instead of shared-root service TCBs for generated Pi hot paths. Runtime init is topology proof only and is marked non-acceptance. The linked runtime memory budgets have been widened for the next hardware pass rather than tuned from benchmark proof: GENET now has a 512-page DMA arena for the existing 256-RX/256-TX ring model, USB has 128 DMA pages, CYW43 has 128 DMA pages plus 64 shared pages, SDIO has 64 DMA pages, and HDMI/local-seat/PCIe shared-control budgets are no longer one-page smoke allocations. The linked serial image handles bounded mini-UART init/RX/TX and is manifest acceptance-eligible; fresh Pi proof is still required. The six remaining linked images now have resource-gated handlers: HDMI renders to a mapped framebuffer, PCIe services bounded MMIO read/write/flush turns, SDIO validates and runs bounded single-frame command/data turns, USB decodes boot keyboard reports after a resource-gated 16-page xHCI aperture, and GENET/CYW43 accept bounded TX frames. Their generated specs remain non-acceptance until Pi hardware proves the remaining xHCI, GENET DMA, CYW43 firmware/SDPCM, SDIO bus-link, and VL805 state. Proof accounting is stricter: idle or zero-progress ring completions cannot credit hot-path ownership, runtime-init descriptor commands cannot credit owner-state, owner-state descriptor registration is rejected while the matching runtime-image spec remains non-acceptance, aggregate owner-state cannot pass without the explicit owner hot-path mask plus acceptance-eligible runtime images for all seven hot paths, and trace normalization requires exact per-hot-path `owner_state=driver-owned descriptor=present root_pointer=no` lines. This is not a performance result: it is code-level preparation for the next hardware measurement. QEMU transport-substrate and host wire-layout proof only prove isolated VSpace/ring mechanics for the smoke trampoline, complete command coverage, runtime-image transport mapping, runtime-init descriptor transport, and fail-closed Pi admission policy; they do not prove Pi 4 hardware performance. Hardware performance closure still requires fresh Pi measurements plus concrete `DRIVER_TASK_BOOT` and service markers for the active Wi-Fi or GENET path, USB keyboard, HDMI, and clean emergency serial.
- The strict Pi SDIO and PCIe client paths now consume linked-runtime completions before root hardware execution for primitive SDIO command/data and PCIe port operations; that is still code-level architecture evidence until Pi hardware proves useful Wi-Fi/GENET/local-seat progress.
