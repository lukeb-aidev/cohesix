...
- Retype now targets the init root CNode with `(node_index=seL4_CapInitThreadCNode, node_depth=initThreadCNodeSizeBits, slot)` and validates capacity via `initThreadCNodeSizeBits`.
