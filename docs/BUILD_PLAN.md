...
- **seL4 driver-task substrate**
  ...
  - As-built substrate work now includes non-MCS TCB priority/scheduling/resume/notification wrappers, CNode revoke, a remote-safe IPC-buffer bind helper, AArch64 VSpace root and boot-ASID-pool assignment wrappers, explicit page/page-table map wrappers for non-root VSpaces, bounded HAL driver-task command/completion rings, Pi 4 bootstrap-created driver TCB attempts for all nine built-in driver contracts, restricted child CSpaces, command endpoints, notifications, fault endpoint slots, stacks, manifest-selected per-driver SMP affinity, and tolerant handling for boot-seeded intermediate page tables while mapping driver IPC/stack frames. QEMU virtio compatibility builds keep the same contract set but publish an inactive `qemu-virtio-pre-net-resource-guard` report before network init so failed live-task bootstrap cannot exhaust resources needed by the virtio TCP regression path; with the explicit `qemu-driver-task-smoke` feature, they create all nine declared driver-task TCBs after virtio networking is ready, allocate per-driver VSpaces, assign ASIDs, map only a one-page driver trampoline plus stack/IPC/ring frames, complete a fixed-layout ring command without callback/context pointers, and emit console-visible `DRIVER_TASK_BOOT_SMOKE phase=post-net-qemu status=summary ... vspace=isolated ipc_abi=shared-ring-command pointer_free_ipc=yes owner_state=not-proven` as QEMU transport-substrate proof. The physical Pi 4 bootstrap now also selects the isolated VSpace/ring-transport constructor instead of the old shared-root constructor. Per-driver runtime-image specs still declare the eventual code/stack/IPC/ring/MMIO/DMA/shared-buffer mapping contract for the seven Pi 4 hardware hot paths, and isolated handles report the actual transport mapping mask and linked trampoline `code_vaddr`; those declarations remain non-acceptance while `root_context_required=true` and `hardware_state_migrated=false`.
  - QEMU/host compatibility profiles may still dispatch bounded service callbacks through live driver TCBs or root compatibility paths so existing virtual-device tests keep running. Those paths now enter through a single HAL callback-compatibility gate and a single HAL root-compatibility admission gate; physical Pi 4 builds compile out the callback slot state and both gates fail closed for steady-state hardware service. If the independent ring-backed path is unavailable, the service turn fails closed and acceptance remains red. The May 20 Pi 4 proof is not closure because every driver-task bootstrap failed with `seL4_DeleteFirst` and hot paths stayed root-task compatibility. SDIO and PCIe have declared contract TCBs and affinity assignments; their current bus operations are reached from the CYW43 and USB/local-seat ownership chain until standalone bus-operation queues are split out and fresh hardware proof shows live ownership.
  - The normal physical Pi 4 service path now uses pointer-free selector ring handlers and driver-task client shells instead of constructing root-owned steady-state hardware drivers. Serial still keeps emergency blocking UART output for boot diagnostics, but normal serial poll/flush, USB keyboard polling, HDMI text mirroring, GENET, and CYW43 all fail closed through the ring path when no driver runtime answers. Physical Pi network init selects ring-backed `GenetDriverTaskDevice` / `Cyw43DriverTaskDevice` clients instead of `BcmGenetDevice` / `Cyw43NetDevice`; local-seat init no longer constructs `Pi4LocalSeat`, so root does not attach the VL805 xHCI hooks or HDMI framebuffer backend; and physical Pi Wi-Fi HAL state construction returns `pi4-wifi-driver-task-runtime-required`. This may intentionally break Pi hardware behavior until the isolated runtimes implement MMIO/DMA/ring ownership, but it removes the old root-owned steady-state driver structs from the physical path. The HAL still splits root-context diagnostic ring registration from pointer-free selector ring registration; QEMU/host compatibility can keep root-context diagnostic services, while physical Pi hardware service uses the selector path or fails closed. Shared-ring completions are tracked as `shared_ring_roles` and do not satisfy `hot_path=dedicated` or full acceptance. The proof layer also requires `DRIVER_TASK_OWNER_STATE_PROOF=yes` backed by all seven per-hot-path `DRIVER_TASK_OWNER_STATE ... descriptor=present root_pointer=no` lines before a pointer-free ring can be counted as hardware-owned driver isolation. The QEMU smoke path proves the isolated trampoline, per-driver VSpace/ASID allocation, restricted mapping set, runtime-image declaration accounting, actual transport-region mapping, and pointer-free ring transport, but that is transport-substrate proof rather than Pi hardware hot-path ownership. Any callback-pointer or root-task compatibility service turn remains QEMU/host compatibility evidence only and fails reopened Pi closure.
