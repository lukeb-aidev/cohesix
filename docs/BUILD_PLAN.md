...
- **seL4 driver-task substrate**
  ...
  - The normal physical Pi 4 service path now uses the fixed shared-ring command ABI for serial, HDMI/local-seat, GENET, CYW43, SDIO, and PCIe service turns and fails closed instead of entering callback/root fallback when a ring-backed owner is unavailable. That is still shared-root VSpace service: the current owner handler is registered by contract and runs over a bounded command/completion ring, but the live driver state and MMIO/DMA buffers are not yet inside a separate driver-local runtime image. The HAL now splits root-context diagnostic ring registration from pointer-free selector ring registration; serial, HDMI/local-seat, and network root-pointer service turns are explicitly registered as root-context diagnostics, standalone SDIO records report `root-hal-exec`, and the HAL forces `DRIVER_TASK_RING_FLAG_ROOT_CONTEXT_NON_ACCEPTANCE` on those commands. Shared-root ring completions are tracked as `shared_ring_roles` and do not satisfy `hot_path=dedicated` or full acceptance; transitional commands that still carry root context cannot be credited as owner-state turns. The proof layer also requires `DRIVER_TASK_OWNER_STATE_PROOF=yes` backed by all seven per-hot-path `DRIVER_TASK_OWNER_STATE ... descriptor=present root_pointer=no` lines before a pointer-free ring can be counted as hardware-owned driver isolation. The QEMU smoke path proves the isolated trampoline, per-driver VSpace/ASID allocation, restricted mapping set, runtime-image declaration accounting, actual transport-region mapping, and pointer-free ring transport, but that is transport-substrate proof rather than Pi hardware hot-path ownership. Any callback-pointer or root-task compatibility service turn remains QEMU/host compatibility evidence only and fails reopened Pi closure.
