...
by ordinary EventPump turns with no automatic CYW43/SDIO operation. Paced
serial and local-seat diagnostic, authentication, and reboot commands must
still dispatch or return a typed unavailable/fenced result. High-impact
`begin`, `recovery`, `backoff`, `ready`, and `exhausted` records must retain an
HDMI mirror, but the isolated display submission may occur only in a later
`Display` turn. A sixth attempt, repeated exhausted record, same-turn display
submission, swallowed command, or automatic post-exhaustion pair restart fails
the gate.
