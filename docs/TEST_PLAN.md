...
- Pi 4 runtime evidence gate:
  ...
  - Capture boot evidence showing:
    ...
    - HDMI local-seat acceptance observes typed USB keyboard bytes echoing at parser ingress and boot/progress messages refreshing at the documented 5-10 s cadence, with Stage 02 driver coverage guarding the cadence constants, serial runtime ring RX/TX turns, and Wi-Fi progress suppression during USB boot activity and after USB first-byte proof. Pi 4 manifest-default boots must use `hw.local_seat.enabled=true` and `hw.local_seat.required=true` so HDMI/USB regressions fail visibly instead of degrading to a serial-only boot.
...
- `out/manifests/root_task_resolved.json` — `sha256:5e002c1f5819592a9d7c559f7dbfec16abac16e48d3570c166c7575961ad9c94`
