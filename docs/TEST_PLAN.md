...
Reopened Milestones 26a/26b also require HAL driver-task contract coverage before hardware claims: `hal::driver_task` must validate the serial, USB/local-seat, HDMI, GENET, CYW43, SDIO host, PCIe root, RTL8139, and virtio-net contracts. Historical M26B completion evidence remains a compatibility baseline, not reopened acceptance proof. Reopened Pi 4 captures must include compact `DRIVER_TASK_*`, `SCHED_CONTRACT`, `BUDGET_OVERRUN`, observed per-driver latency, `SERIAL_ECHO`, `USB_BURST`, and `HDMI_RESPONSIVE` evidence; `scripts/pi4_trace_normalize.py --gate-summary` now exposes those as machine-checkable hardware proof fields. Dedicated-driver-task closure is stricter than contract declaration: `DRIVER_TASK_DEDICATED` must cover the required active roles, `DRIVER_TASK_COMPATIBILITY` must be `0`, `DRIVER_TASK_DEDICATED_READY=yes` must be present, `DRIVER_TASK_FAILED_COUNT=0` must be present, serial, USB/local-seat, display, network, SDIO, and PCIe role booleans must all be `yes`, and substrate/capset/fault/revoke/scheduling/per-driver-affinity/VSpace plus pointer-free IPC and owner-state proof fields must all be `yes` when `scripts/pi4_gate_proof.sh --require-driver-task-proof` is used. Owner-state proof requires one `DRIVER_TASK_OWNER_STATE ... hot_path=<exact> owner_state=driver-owned descriptor=present root_pointer=no` line for each of `serial-console`, `usb-keyboard`, `hdmi-text`, `genet-nic`, `cyw43-wifi`, `sdio-host`, and `pcie-root`; the canonical shorthand remains `DRIVER_TASK_OWNER_STATE ... descriptor=present root_pointer=no`, but aggregate `owner_state=driver-owned` text, inferred hot paths, and truthy aliases such as `owner_state=yes` must fail. Host tests must prove the fixed-layout pointer-free command/completion records remain primitive-only and bounded, including primitive aux fields for service-turn arguments, nonzero-progress/frame-ready-only hot-path credit, owner-state descriptor rejection while the matching runtime spec is non-acceptance, owner-state acceptance requiring the explicit owner hot-path mask plus acceptance-eligible runtime images, the separate root-context diagnostic versus pointer-free selector registration classes, and the common `DRIVER_TASK_RING_FLAG_ROOT_CONTEXT_NON_ACCEPTANCE` bit forced onto transitional root-context ring commands. Host tests must also prove the shared `pi4-driver-abi` runtime-init descriptor remains bounded, pointer-free, count-checked against generated mapped resources, and non-acceptance until a later hardware service turn makes real device progress. Runtime-init records must carry primitive MMIO/DMA/shared physical page metadata, fixed virtual bases, semantic resource ranges for large apertures and large buffer arenas, bus-address policy, optional IRQ descriptors, optional bus-link descriptors, and framebuffer metadata without root pointers. Host tests must also prove the Pi 4 hardware runtime profile disallows steady-state callback/root compatibility paths and compiles out callback service slot state for physical Pi 4 builds; the physical Pi profile now requires isolated child VSpaces for driver bootstrap, loads linked `pi4-driver-*` runtime image payloads from the staged driver-runtime CPIO when present, maps all bounded `PT_LOAD` pages declared by generated `code-pages`, and uses the fixed command/completion ring smoke loop instead of shared-root service TCBs. The aggregate linked runtime set still fails owner-state acceptance; serial has bounded mini-UART init/RX/TX in the linked image and an acceptance-eligible generated spec, while the six remaining images now have resource-gated command handlers but remain non-acceptance until Pi hardware proves the xHCI, HDMI framebuffer, GENET DMA/RX/TX, CYW43 firmware/SDPCM, SDIO bus-link, and PCIe/VL805 handoff state. Host coverage must include the Pi 4 hot-path command catalog plus shared-ring service handlers or client shells for serial console, USB keyboard, HDMI text, GENET RX/TX, CYW43 RX/TX, SDIO host, and PCIe root service. Host coverage must also prove the generated `root_task.driver_images` table covers all seven required Pi 4 hot paths, budgets enough runtime code pages for current multi-segment linked images, declares at least the 16-page xHCI minimum aperture, reserves the non-minimal runtime buffer budgets (`usb` 128 DMA/32 shared, `hdmi` 16/16, `genet` 512/32, `cyw43` 128/64, `sdio` 64/32, `pcie` 16 shared, `serial` 4 shared), and that the separate `pi4-driver-*` runtime image package checks for host and `aarch64-unknown-none`. A role boolean is credited only from a line proving both `live_tcb=yes` and `hot_path=dedicated`; static contract isolation, callback-pointer live-TCB service turns, shared-root ring service turns, runtime-image declarations, runtime-region mapping, linked-image smoke loops, runtime-init descriptor commands, and any ring command marked root-context or init-descriptor non-acceptance are diagnostic until the driver state boundary is owned by an isolated ring-backed task, VSpace proof is `yes`, pointer-free IPC is `yes`, and `owner_state=driver-owned` is present. Shared-root ring completions may appear in `DRIVER_TASK_SUMMARY shared_ring_roles=...`; that is readiness evidence, not acceptance. The Pi 4 manifest defaults place both `bcmgenet-v5` and `cyw43455` on core `3`; hardware captures must show `DRIVER_TASK_BOOT ... contract=bcmgenet-v5 ... affinity_core=3` and `DRIVER_TASK_BOOT ... contract=cyw43455 ... affinity_core=3` before claiming fourth-core driver placement. The default contract set remains nine and includes QEMU NICs plus Pi 4 SDIO/PCIe support; QEMU virtio compatibility boots may publish `DRIVER_TASK_BOOT status=skipped reason=qemu-virtio-pre-net-resource-guard` before network init. When the explicit `qemu-driver-task-smoke` feature is enabled, QEMU virtio boots may then prove isolated VSpace/ASID allocation, runtime-image transport-region mapping, and pointer-free ring transport for all declared contracts after virtio networking is online. That is QEMU transport-substrate evidence only and must not be counted as Pi 4 dedicated-driver-task closure or hardware hot-path ownership. Fresh Pi hardware proof is still required before claiming Wi-Fi/DHCP, GENET/DHCP, USB keyboard, HDMI, or strongest isolated-driver acceptance.

Strict Pi SDIO command/data calls and PCIe port read/write/flush helpers now consume linked-runtime completions before any root hardware execution; host coverage must keep proving those ring turns while preserving fail-closed non-acceptance until the full hardware state machines are acceptance-eligible.
...
- Pi 4 runtime evidence gate:
  ...
  - Capture boot evidence showing:
    ...
    - HDMI local-seat acceptance observes typed USB keyboard bytes echoing at parser ingress and boot/progress messages refreshing at the documented 5-10 s cadence, with Stage 02 driver coverage guarding the cadence constants, serial runtime ring RX/TX turns, and Wi-Fi progress suppression during USB boot activity and after USB first-byte proof. Pi 4 manifest-default boots must use `hw.local_seat.enabled=true`, `hw.local_seat.required=true`, and matching `usb-kbd0`/`hdmi0` `hw.devices[] required=true` declarations so HDMI/USB regressions fail visibly instead of degrading to a serial-only boot.
...
- `out/manifests/root_task_resolved.json` — `sha256:8fc340ec6531d79bc9712a5bf81ca985942023b71367799d5e622f6333bfd603`
