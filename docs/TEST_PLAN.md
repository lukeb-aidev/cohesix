...
- Pi 4 runtime evidence gate:
  ...
  - Capture boot evidence showing:
    ...
    - when a bounded, successfully imported, coherent saved Cohesix network policy exists, the U-Boot wizard defaults to `Continue with existing config`; an absent, empty, logo-only, oversized, malformed, or incoherent `cohesix.env` defaults to `Boot with manifest defaults`
    - host-side U-Boot template guards require back/discard to reload persisted policy, existing Wi-Fi credentials to be kept or replaced without serial disclosure, invalid replacement input to preserve the old working credentials, static entry to have bounded back/exit paths, and repeated navigation to use the iterative page dispatcher rather than recursive menu calls
    - host-side save/clear template guards require export, FAT write, post-write size, readback load, or private comparison failure never to report success or invoke reset; a successful save is byte-for-byte verified before reboot, and clear redraws the manifest-default state without requiring physical deletion of `cohesix.env`
