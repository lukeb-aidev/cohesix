...
The later ten-refill GENET-only experiment used exact release RAM image
`a142b05b347c9a92dcf882b3540e370bc147f3c4`, preserving the successful bounded
passive-read dispatch repair. Both fresh boots passed raw framing, cohsh,
Worker32, medium16/high32 REST and complete wire audits, but raw p95 remained
4.998/4.115 ms against 1.845 ms and high concurrent batch means were
42.829/45.264 ms. Restore the eight-refill GENET policy and exact compiler,
runtime and manifest fixtures; the existing root/console eight-refill policy
and CPU reservations remain unchanged. Keep the selected-kernel ten-record
capacity check independent of this performance decision. Retain original
thresholds and failed records in `out/bench/pi4-genet-refill10-20260909`.
The QEMU live canary passed before an obsolete Pi fixture failed; a separate
...
The Pi 4 placement candidate puts `bcmgenet-v5` and `hdmi-text` on core `2` and retains `cyw43455` plus
`sdio-host` on core `3`. Hardware captures must show
`DRIVER_TASK_BOOT ... contract=bcmgenet-v5 ... affinity_core=2` for wired mode
...
direct-GENET child uses 70 image pages and allocates no new direct data-plane
frames: it maps 32 exact external copies of GENET-owned CPU pages, for 108
frames and 165 retained root slots. The Pi speed profile adds three image pages
above the former 66-page image, and the temporary wake observer adds one further
image page and retained mapping cap. Both retain eight anchor-derived
...
- `configs/root_task_pi4_uboot_aarch64.toml` — `sha256:676fb044dd0a9822d8f8d6da53c119cb7122628d81ec32e71f7871cc18bdc62e`
- Pi `pi4_production` transient resolved binding — `sha256:5f9dafc640e83647e4dab8e453b113a5cce55762d6ee3c34f5b40de0d96aee6d`
