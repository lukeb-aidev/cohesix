...
The later GENET refill-capacity candidate preserves the successful bounded
passive-read dispatch repair in source `28d3fafd34dc81ef68f8576e3ac4b64cb6cf3818`.
That image's raw 1024-request run achieved 661.972 requests/s but 4.760 ms p95;
219 requests exceeded the original 1.845 ms gate. Root, console and GENET
SC-consumed session shares were approximately 25.0%, 21.8% and 24.0%, below
their reservations. These aggregates do not prove current refill eligibility
or a refill-merge defect. Test only the GENET owner's eight-to-ten total refill
selection in its existing 256-byte SC. Bind the Pi source manifest, generated
profile, kernel extra-refill conversion and exact runtime descriptor to ten;
accept the selected capacity and reject eleven. Preserve root/console refills,
budgets, periods, affinity, priority, WCET, timeouts and the direct 16-slice
bound. Two fresh GENET boots must compare first raw TCP and medium/high
`rest_perf_harness.py` workloads; a same-image WiFi control protects shared
composition before retaining a beneficial candidate. Keep original thresholds
and all failed evidence. Capacity admission alone is not performance proof.
This selection supersedes the earlier GENET max-eight requirement above;
production QEMU and Pi console services both retain eight refills.
...
The Pi initial-SC storage candidate requires an authenticated two-file source
overlay: the existing VL805 DTS and all three initial-SC allocation/capability
sites in kernel boot code. Reject omitted, duplicate, unexpected or tampered
paths, and retain dry-run/idempotent source preparation. Compiler tests must
reject a 128-byte Pi initial SC and a 256-byte clean-QEMU initial SC; confirm
eight Pi refills with unchanged 5,500/10,000 us admission. Exercise the runtime
continuation predicate with the actual Pi manifest SC fields, rather than
reconstructing a stale positive fixture; reject each legacy SC field separately.
Physical diagnostics must show the selected continuation is admitted before
...
`0x402 -> 0 -> 0x403` queue publication, stale generation, malformed/poisoned
records, exact single-use terminal proof, and sideband ACK during a subsequent
queue publication. Verify that HAL leaves op8 active before its terminal
retirement, that the existing request deadline bounds deferral, and that pair
scrub clears the proof before request-sequence reuse. Both payload header
checks remain required. These source/host checks do not prove target IPC or
scheduling: the next exact-image two-WiFi/two-GENET matrix must preserve first
raw TCP and follow with functional/Worker/medium/high pressure and paired
captures. A prior quarantine remains a failed cohort even if a later boot
passes. SD readback proves delivery only.
...
5,700-us response. Root-fault moves to core 2 with unchanged scheduling and
fault semantics and a derived 2,400-us response. HDMI remains core 1 with
5,200-us response; PCIe and the GPU executor remain core 2 with derived
2,700/7,700-us responses. Exact active demand must be
...
root-fault affinity/response, HDMI affinity/task core/response, PCIe and GPU
response, GENET refill count, and any exceeded core reserve. QEMU must retain
...
perform zero YieldTo and keep same-core accounting unchanged. Failed pre-drain,
Signal/Yield result, runtime, task, config, generation, connection, lifecycle,
or topology drift must fail closed.

Only exact authenticated same-core progress with successful Yield/call
accounting may open the root-local active tail. It is unslid from the first
nonzero CNTVCT completion,
...
The superseding dual-mode convergence input is exact source
`0696803f55d215721b4e750825a5c25daf2cac05`, build ID
`5d81a2134d073df9893fe13c01f212c65d86708c0d44e8e86b2ba44eec33bc99`,
image ID `c49407add0cf15cb68f4cafa23970a41610f098e08ef9c4c4e8536282b89ca58`,
image SHA-256
`34734ec86f6d0b560bee8d716c51bc3fc5e8b4b25d6826660641aece06911721`,
retained Pi resolved-manifest SHA-256
`f83cd48d38d780d491d0317d98513a1703d24663406ed90f8b38911794dde273`,
and topology SHA-256
`33e2eea4a77712025248ac021438cb73b1dfc3740f26c4552265c0cccf82d607`.
The controlled WiFi logs are
`/Users/lukasbower/pi4-serial-20260830-192134-0696803-wifi-diagnostics.log`
and
`/Users/lukasbower/pi4-serial-20260830-192701-0696803-wifi-postpressure.log`;
the controlled GENET logs are
`/Users/lukasbower/pi4-serial-20260830-192935-0696803-genet-reboot-diagnostics.log`,
`/Users/lukasbower/pi4-serial-20260830-193501-0696803-genet-postpressure.log`,
and `/Users/lukasbower/pi4-serial-20260830-193726-0696803-genet-postrest.log`.
The boot-paired captures are
`/Users/lukasbower/tcpdump-wifi-20260830-183559.pcap` and
`/Users/lukasbower/tcpdump-usb-eth-20260830-183559.pcap`.

WiFi reaches root ready at 8.1135 seconds, USB command input at 12.135 seconds,
Gate 8 at 33.550 seconds, and DORA in 204.936 ms. It passes generation-bound
`nettest`, `wifi dump-state`, `wifi diag`, three first-attempt focused `.coh`
scripts, and 64/64 one-connection no-retry requests at 7.196 requests/s,
130.015-ms median, and 159.456-ms p95 first response. Accepted-to-issue and
issue-to-terminal average 11.175 and 11.364 ms. GENET reaches root ready at
8.282 seconds, passes generation-bound `nettest` and the same scripts, and
passes 64/64 at 47.167 requests/s, 19.980-ms median, and 20.052-ms p95. Its
packet split is causal: request-to-target-ACK is 0.545-ms median/0.634-ms p95,
effectively the accepted August 0.542/0.575-ms ingress comparator, while
ACK-to-response is 19.307/19.360 ms and response spacing is locked near
20.002 ms. Both boots retain a sealed 272/272 MCS registry and clean queues,
rings, containment, quarantine, and fault state. This exact evidence proves
correctness improvements and permits the GENET hardware-ingress comparison
only; WiFi remains
at 26.6% of August throughput and 3.16 times August p95, GENET remains below
its 180.269 requests/s and 1.677-ms p95 comparator, and neither boot proves
repeatability or current-candidate acceptance.

The next candidate's deterministic gate must preserve the QEMU
productive-micro-unit discipline while proving two Pi-only successors. Direct
GENET must quiesce after an exact `Command`/`CommandBatch` publication and
release only for a newly sequenced same-identity `StageOutput`; stale or empty
control, publication ACK, peer wake, fault, containment, another backend, and
QEMU direct VirtIO must not release it. Copied WiFi may mint one opaque
transient credit only from an exact one-unit material child-publication delta;
tests must cover lifetime, generation, active/authenticated connection,
response cursor/lane, command/service counters, pending flush, the complete
`Serial -> LocalSeat? -> Dispatch` rotation, pre-Network revalidation, single
consumption, Yield revocation, and every operator, passive-admission, recovery,
containment, quarantine, reboot, and handoff fence. The post-Dispatch positive
case must permit exactly one authenticated network command to advance the
...
every recovery/operator fence. The initial transient probe must independently
prove `Idle` and `UsbServiceDebt` admission plus `Input` rejection; its positive
USB-debt case must install the complete rotor before Network re-entry. The
terminal-return shortcut remains strict. No test may model this as a new
refill, notification, retry, or device grant.

Cold-bootstrap notification tests must keep the child-to-root wake a hint only.
An already-runnable exact parent enters Driver without consuming the hint; an
absent hint performs no second parent read; a present hint followed by an
unchanged waiting parent yields; and only a present hint followed by an exact
terminal/fault parent may continue. Attached mode must not poll this cap from
the cold supervisor. Source/target checks must retain commit-before-signal in
the child, one nonblocking root poll after the selected bounded
operator/recovery/passive condition turn, a second durable parent-condition
read as the sole authority, and the generated `NaturalPostpone` profile plus
unchanged hard 64 logical and productive caps. Tests must reject a wait, spin,
software work latch, generic badge authority, userland wall-reserve authority,
retry, new notification, or attached dual consumer. They must also prove that
useful progress followed by an empty child observation yields rather than
creating a local polling cadence.

Attached-WiFi root-wake tests must prove exactly one poll and hit delta during a
non-Network phase after the first Network unit, complete
`Serial -> LocalSeat? -> Dispatch` admission, unchanged physical lifetime and
authenticated response identity, return to Network, and no terminal-return,
input, recovery, containment, quarantine, handoff, reboot, passive-admission,
or local-fault fence. The wrapper must retain only its caller activation and
must not perform a second Network operation. Missing, duplicated, wrong-badge,
wrong-phase, cross-identity, incomplete-rotor, or post-Yield evidence rejects.
An unchanged empty snapshot without a consumed wake must Yield, and the
schema-stable productive-window `idle_admitted` counter must remain zero.

Direct-GENET child-publication tests must exercise the full Pi composer from an
exact staged-batch snapshot through Serial, optional LocalSeat, Dispatch, and
one Network observation. Acceptance requires the same generation, connection,
accepted-command epoch, command/stage counters, zero `YieldTo` accounting,
`awaiting_batch_drain: true -> false`, and exactly one response-drain increment.
The resulting phase is Serial; no second command or Network leaf runs. Every
counter drift, absent snapshot, second drain, retained awaiting state, input,
response tail, recovery, containment, quarantine, handoff, reboot,
passive-admission, and local-fault case rejects.
...
when its exact already-queued adapter lane independently passes the complete
generation, connection, authentication, recovery, flush, and batch-drain
predicate, is terminal-queued and producer-closed, and contains exactly one
completed response. Open, nonterminal, zero-completion, or multiple-completion
lanes must reject this `NoPending` fast path. `NoPending` without that lane must
defer as `OutputMissing` without quarantining the pair; identity drift must
retain fail-closed containment.

The Pi-only causal telemetry gate is independently exact. Pure tests cover
saturating samples/totals, maximum and last age, zero/backwards rejection,
strict latency-bucket edges, and independent routing of command, control, and
drain seams. Target/source checks must prove the composer recorder brackets the
measured leaf with raw 54 MHz CNTVCT ticks, accumulates valid start-to-start
period separately from start-to-finish run time, excludes the first period,
retains pending/progress/lane/generation/connection/exit identity, and converts
ticks only when the explicit operator snapshot is rendered. It must call these
records composer quanta rather than kernel activations or refills. A separate
source check must retain the exact one-block `CNTVCT -> svc -> CNTVCT` Yield
hiatus, with one exclusive trigger and the pre-Yield pending mask; no test may
infer a specific kernel refill from either aggregate.
...
next-composer entry, pre-Network, or revocation cuts, with the snapshot/lifetime,
operator/recovery, pre-Network-drift, and non-material/empty reason bits
retained. The typed GENET compose counters must
distinguish composed, no-pending, not-sealed, backpressure, and identity-drift
outcomes. `composed` proves a sealed `SyncCapture` moved; `no-pending` may
proceed only when the exact already-queued non-`SyncCapture` response lane is
independently stage-ready, and otherwise must increment `output_missing`. A
counter alone must never authorize child control. When the selected isolated
service reports an aggregate compact Deferred, exactly one of
`genet_defer`'s `passive`, `command`, `compose_open`, `compose_backpressure`,
`fence`, `prior_batch`, `control_busy`, `output_missing`, or
`stage_backpressure` counters must increment, and their sum must equal the
aggregate `genet_compact deferred` count; `compose_open` maps typed
`NotSealed`. QEMU release output must omit the complete six-row Pi batch.
When the selected isolated service exposes timing, `netstats` must add exactly five bounded seam
rows: `command-publish-root-observe`, `dispatch-stage`,
`stage-control-observe`, `stage-output-drained`, and
`output-drained-root-observe`. The test oracle must treat
`stage-control-observe` as the combined child-consumption-plus-root-observation
interval because the shared control watermark carries no timestamp. Physical
Pi source checks must prove that every child publication and root dispatch,
observation, stage, control, and drain endpoint uses the same absolute
`CNTVCT_EL0` epoch scaled by generated `TIMER_CLOCK_HZ`, never root elapsed or
smoltcp time, and that GENET, CYW43, and default stack wrappers forward the
exact dispatch identity and observation to the isolated adapter. Host tests and
QEMU must preserve the caller-time fallback and unchanged row grammar.
...
25 bounded rows: nine `mcs_quantum*` rows, command-publication-to-dispatch and
root-observation-to-dispatch rows,
...
single-claim bounds and maximum-width rendering. Exercise the actual paired registration renderer with maximum-width evidence
for all 16 selected Pi non-Worker tasks and reserve all four WiFi owner rows.
The test must reject a QEMU-generated fixture, require all eight registry rows,
all five CPU lines and the closing marker, and fit both the unchanged 64-line
TCP capture and 69-line linked-serial body. Verify missing/zero generations,
odd final pairs, index bounds and exact maximum-width formatting independently. Fresh Pi evidence must show valid
...
that a receive actually blocked. Tests must prove the
pending bits do not overlap, each Yield sample increments one trigger class,
the schema-stable budget rows separate activation/attached/bootstrap-operator/
bootstrap-driver cuts and cap/clock/reserve/policy reasons, and exact
`NaturalPostpone` productive WiFi lanes close only on the hard cap or
incompatible-policy cut rather than a userland clock/reserve estimate. Invalid
evidence is never folded into latency, the row batch does not truncate at
maximum values, the recorder never samples `SchedContext_Consumed`, and it
emits no routine serial record. QEMU release compilation must exclude all new
accounting writes and preserve its existing command output.
These source, unit, QEMU, target-compile, image, and media checks are rejection
gates only. Fresh boot-separated WiFi and GENET runs of the exact candidate
must repeat first-attempt scripts, 64-request raw TCP, medium/high REST
pressure, boot-paired packet splits, operator liveness, causal `netstats`, and
pre/post-workload `smp mcs` before any Pi latency, throughput, August-parity,
or acceptance claim.
...
On the deferred physical WiFi supervisor only, exact productive semantic
phases may remain resumable across root-control replenishments when generated
root truth is active, admitted, consumed-time capable, selects
`NaturalPostpone`, and has nonzero budget/WCET values satisfying
`wcet_us < budget_us <= period_us`. Kernel exhaustion postpones execution and
later resumes the exact instruction cursor; no continuous userland CNTVCT
window or `budget_us - wcet_us` estimate grants or revokes productive work.
Tests must prove strict `Operator -> Driver -> Operator` alternation, exactly one
physical-operation lease and retired finalizer per Driver, and both unchanged
hard limits: every full Operator, Driver, and attached Network service turn
spends one of 64 logical material-work units, while productive Driver or
attached Network progress is independently capped at 64. The generated profile
and both caps are checked before every fresh full service turn.

Historical source oracles used an exact 250-us wall reserve
(`2,750 - 2,500`) for the superseded pre-topology candidate and later an exact
3,000-us wall reserve (`5,500 - 2,500`). Those values remain evidence for the
superseded userland wall-guard candidates and are not current productive
admission authority. Missing or incompatible generated policy admits exactly
one legacy logical turn before Yield; an external wait, idle/no progress,
output backpressure, handoff, terminal/fault/reboot state, or either hard cap
yields and resets the window. Counter and elapsed-time telemetry cannot extend
or shorten a productive continuation. An attached continuation additionally
requires a CYW43 Network phase, actual NetPoller activity, and exactly one
service counter advance. The ordinary path requires
...
charged root turn after the loop-top `NaturalPostpone` profile and cap recheck;
the isolated child's ordinary lower rotor cannot burst. The prior four-Driver
restart burst is not
...
Manifest `root_task.schema = "1.17"` retains exactly one fixed, root-retained
...
tests must require `root_task.schema = "1.17"` and reject schema 1.16 or older. They must admit
...
direct-GENET child uses 69 image pages and allocates no new direct data-plane
frames: it maps 32 exact external copies of GENET-owned CPU pages, for 107
frames and 164 retained root slots. The Pi speed-profile increase from 66 pages
is three executable image frames and their retained mapping caps. Both retain eight anchor-derived
...
fixed/maximum totals of `4082/9276` and `7154/17468`, respectively. These include
the three additional console image frames and mapping caps for its speed
profile. The separate Pi timer allocation is one admitted device Frame cap for
the discontiguous `0xFE003000` system-timer page plus two root-CNode slots for the IRQ 99 handler
...
`5608/14620` for QEMU and `7666/19516` for Pi.
...
The fixed-layout tests must also prove that console-network ABI v6 retains the
...
`[640,4096)` and inactive payload suffixes remain non-authoritative and must not
...
Schema 1.16 must reject the immediate 1.15 predecessor and any ACK badge other
than the ABI v6 value 64. The root must retain the fifth Write-only cap on the
existing root-to-child Notification and additionally mint only a Write-only,
badge-1 root-control wake capability into the ABI-v6 descriptor's fixed child
slot 6. The former `reserved0` field must seal that slot; zero, aliasing,
wrong-slot, wrong-badge, broader-rights, and stale-version descriptors fail
...
sequence-last record, signal the existing component-specific notification, and
only then signal slot 6. Direct-VirtIO QEMU must emit only the semantic
publication signal so its unconsumed Pi fan-in cannot remain latched. Reversing
either physical publication or notification order, omitting the Pi fan-in, or
treating its badge as durable state must fail focused source-order tests.
...
ABI v6 compatibility tests must additionally retain the ABI-v4-introduced
...
consumes it before mutation. On copied Pi WiFi only, PublishServiceEvent may
commit one already-prepared egress packet before its Command/CommandBatch event
barrier, then signal only event and root fan-in. Regress missing event barriers,
lifecycle and direct-transport exclusion, exact generation/sequence, passive
level reads and sequence retirement. Root must retain both before one ACK;
watermark-only wakes must not consume partial bundles. Pi command headers use
publication-start time while nested batch records retain original policy time;
QEMU keeps its existing representation. Fresh paired Pi evidence must report
observed pair count and raw timing before claiming any benefit. Stable empty
hints retire before a separate service cycle. Root ACK is owed only after valid copy, sent only after durable
...
- `configs/root_task_pi4_uboot_aarch64.toml` — `sha256:3cdf05d5514eacb73b8e02eccdf444d7b22b1df1198167de76c3384e05866963`
- Pi `pi4_production` transient resolved binding — `sha256:59581ab186a50009c6c75f27980ceace956c6f1d2bfaf33a7248409120d740c3`
