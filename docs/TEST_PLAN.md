...
`permanent` records must retain an
HDMI mirror in their original order. Coverage must delay display long enough to
queue a full worst-case episode, prove that the fixed FIFO does not overwrite
start/backoff/terminal milestones, and drain at most one milestone per later
`Display` turn. A sixth attempt, repeated exhausted record, same-turn display
submission, swallowed command, automatic post-exhaustion pair restart, or
...
`full` is configured fail-closed policy and `qlog` is `/log/queen.log`. It must
prove every `preflight`, `begin`, `recovery`, `backoff`, `ready`, `exhausted`,
and generic `permanent` record fits losslessly in the fixed 256-byte serial and
...
a root grant before any service quantum. Such a coalesced grant must not advance
foreground work later; a fresh root turn is required. After that first service,
at least 4,096 standalone level reassertions must produce no second service
quantum and lose no badge. The fresh continuation must then admit exactly one
foreground phase even if the level badge is coalesced again, retaining that
badge for at most one later service boundary. CARD_INT coverage must also prove
that terminal deferred service masks the host source before IRQ acknowledgement.
...
- `configs/generated/root_task_resolved.json` — `sha256:802049e660921a088d7c651396df3104ba0ab77c72fdab9c5f8accf5699009b3`
