...
Milestone 27a authority qualification retains the selected deployment manifest,
strict-intent and legacy compatibility results, gateway refusal/audit evidence,
host execution crash/restart evidence, and the distinct gateway microbenchmark
specified by [BENCHMARKS.md](BENCHMARKS.md). Host simulations cannot accept root
policy, dedupe or debug-memory behavior; exercise those paths on the selected
QEMU/Pi target. The normal five-stage gates still apply. Use
`COHSH_BASE_MANIFEST` and `COHSH_GATED_MANIFEST` for explicitly provisioned
regression sources, with `COH_AUTH_TOKEN_REF` selecting the same Queen key.
References fail closed; the runners do not fall back to bootstrap secrets.
The generated Python profile and retained public verification key must match the
...
M27b's scoped host matrix is `configs/provider_conformance.toml`, executed by
`scripts/ci/provider_conformance_run.sh --matrix configs/provider_conformance.toml
--state-dir <fresh-directory>`. Select a single `--evidence-only`,
`--executors-only`, `--observability-only`, `--packaging-only`,
`--identity-only` or `--registry-only` group for the contract being changed;
do not rerun unaffected groups. `--validate-only` emits NOT_RUN case rows and
cannot stand in for execution. The bounded runner retains exact commands,
source inventory, generated graph/profile requirements, output hashes, result
classes and elapsed time. It stops on the first failed selected case. These are
host contracts, with no implicit native/provider/Worker/target promotion.
Native discovery uses the separate `--native-providers --live-reference` lane;
...
Private LoRA release uses the focused `coh` `peft_release` integration tests,
existing recipe/workflow guards, shared action and Worker receipt contract tests,
affected compiler profile tests and Python wrapper/native-input tests. Run native
attestation input tests in the qualified environment with all required packages;
an optional-dependency skip elsewhere is not that evidence. These checks own
comparison, phase intent, uncertainty, fresh recovery authority, confinement and
forgery contracts. Actual import/training, native evaluation, serving promotion,
regression refusal and failed-canary verified rollback require fresh admitted
execution on the selected pinned CUDA host, with exact WorkerLora receipts and
independently enrolled native evidence. Host journal fault tests cannot substitute
...
M28's protocol-control foundation uses `cargo test --locked -p coh-rtc --test
agent_protocol_controls`, `cargo test --locked -p hive-gateway --test
agent_protocol_controls`, and `cargo test --locked -p coh --lib doctor::` with
`scripts/check-generated.sh`. These are source/host checks for the eight
effective flag combinations, malformed profile refusal, startup override
refusal and existing REST availability. They cannot close MCP/A2A listener or
...
M28d selects `cargo test --locked -p coh-rtc --test agent_protocol_controls`,
`cargo test --locked -p hive-gateway --test agent_protocol_controls --test
mcp_protocol --test mcp_workflows`, the focused `cohsh` and SwarmUI manual
checks, and `scripts/check-generated.sh`. The compiler checks all eight flag
combinations; the endpoint checks real HTTP/stdout framing, auth, Origin,
revision, malformed and oversized input, scoped discovery and original
identity mapping. They are host contracts, not live CUDA or PEFT outcomes.
The selected native NeMo 1.9.0 `mcp_client` with MCP SDK 1.29.1 probes the
Streamable HTTP revision and custom-header mode against the running gateway.
For milestone acceptance, `m28d-mcp-live` binds exact source, KVM image,
selected private PEFT profile, native Orin artifacts, ordinary client package
and configuration, original job identities and shared-verifier results. The
named client must complete CUDA and PEFT comparison/deployment with refused
and uncertain/recovered cases; a passing native client probe is insufficient.

M28e selects `cargo test --locked -p coh-rtc`, `cargo test --locked -p
hive-gateway --test agent_protocol_controls --test a2a_protocol --test
a2a_jobs`, the focused A2A task-state unit test and `scripts/check-generated.sh`.
These checks prove selected routes, scoped card, malformed/authenticated
traffic, bounded state mapping and disabled surfaces. `m28e-a2a-live` must
then bind a named `a2a-sdk==0.3.26` peer, original CUDA/PEFT job and native
result to exact source, QEMU image, Jetson provider and verifier evidence.
Recover across a gateway restart under `ticket.id`, request cancellation and
retain uncertain outcomes without a new effect. The native NeMo 1.9.0
`a2a_client` must read the card and use task lookup/cancel; its timeout is
not a provider terminal. This is selected component qualification, not Pi or
Release B acceptance.

The `m28d-mcp-live` and `m28e-a2a-live` cases each require a named
...
`cohesix-python-profile/v2` contracts. A successful install or mock Worker
...
| `diagnostic.guard-worker` | NON-CLAIMING diagnostic | `non-claiming` | conditional / qemu | `cargo test -p root-task --no-default-features --test worker_fault_lifecycle -- --test-threads=1` |
...
- **Native mode (required for desktop integration completion):** the real app and Rust bridge use an authenticated gateway and accepted QEMU image. This is separate from Playwright fixture claims and from target/release qualification.
...
- Shell assertions use stable roles, labels and replay contracts. Layout IDs may evolve with the workbench; existing backend transcript and authority semantics remain canonical.
...
- **Blocking for the native desktop claim:** identity-bound source and installed-candidate native walkthroughs; no injected bridge state. See [SwarmUI native acceptance](SWARMUI.md#native-acceptance-walkthrough).

**Focused native desktop gate:** `scripts/ci/swarmui_native_e2e.sh` launches an
exact app/tool pair and records bounded command-boundary metadata. The operator
uses the visible native UI for credential refusal, gateway connection, canonical
namespace/boot reads, Live Hive pressure and Worker evidence, reviewed host-tool
execution, disconnect/reconnect, verified reference and failed-canary playback,
offline refusal and clean shutdown. The final verifier binds exact app/tool,
registry, source and accepted QEMU image hashes plus original screenshots.
A second lane uses the installed candidate; Linux AArch64 additionally runs on
the reference Jetson desktop. Retained CUDA/LoRA verification is historical and
does not substitute for new GPU or physical Pi execution.

`stage_swarmui.py` supplies exact offline browser companions for the existing
`SWARMUI_RELEASE_DIR` gate. Source and candidate run the same presentation and
performance assertions. `swarmui_showcase_capture.sh` accepts only native PASS
records and unchanged screenshot hashes. Its deterministic frame walkthrough
cannot establish live readiness or new target evidence.
...
- `configs/root_task.toml` — `sha256:ff94f6ec1dae7610c5d8894b1b10ba6b4fd9e44deef8bc6016fee2257437f737`
- `configs/generated/root_task_resolved.json` — `sha256:0fd40c602c76806c30de6963765e4d4b4057716da238c1d6fc14d8fb20858f11`
- `configs/root_task_pi4_uboot_aarch64.toml` — `sha256:bcf78ce4de8a6bae995838114711788d9da2986ddc377269630a2cd0434c7968`
- Pi `pi4_production` transient resolved binding — `sha256:a9a7c22eb54145c951c9788163bce68203b635cead1a9333a198f5f20b93988c`
...
## Milestone 27 operator contract validation

The scoped deterministic commands are:

```sh
cargo test -p cohesix-attestation
cargo test -p coh --test operator --test attest --test evidence_pack --test evidence_timeline --test evidence_case
cargo test -p coh --test operator_latency -- --nocapture
cargo test -p cohsh-core
cargo test -p cohsh --test trace --test trace_capture
cargo test -p tests --test audit_ledgers
.venv/bin/python -m pytest -q tools/cohesix-py/tests/test_operator.py tools/cohesix-py/tests/test_evidence_receipts.py tools/cohesix-py/tests/test_examples_ci_siem.py
```

The shared `tests/fixtures/operator` corpus is an offline parser fixture:
canonical Rust case outputs, Python source-link validation, secret canaries,
and PASS/FAIL/UNAVAILABLE attestation records. Independently signed TPM-wire
fixtures validate the cryptographic contract with an explicit offline class;
they are not positive signed target fixtures. TCP/REST loopback tests exercise real host transports and transport-free
replay, preserve existing trace-v1 bytes, reject tamper/oversize/truncation and
writes, and verify that authentication secrets are absent. They do not qualify
the four-core QEMU SMP+MCS target or Pi hardware. Run the complete applicable
staged target plan with fresh provenance before making those claims.
...
### Milestone 18 / 27b native field-bus closure

`m18-live-modbus-dnp3-sidecars` and `m27b-industry-sidecar-contracts` use the
scoped `modbus`/`dnp3` selections in `configs/provider_conformance.toml` for pure
codec, map, WAL, signed-grant, snapshot-expiry and SDK contracts. Run the wrapper
from an environment containing pytest; do not substitute a zero-test exit for
these cases. The focused compiler map test and native service-renderer tests
cover selection and device permissions.

The separate [native reference command](FIELD_BUS.md#native-reference-command)
owns PyModbus TCP/RTU PTY servers and OpenDNP3 at its pinned source commit. It
retains actual requests, remote responses, exact client/source/reference hashes,
WALs, refusal/control cases and an artifact inventory on each claimed native host.
For an archived source tree, `--source-manifest` supplies a complete commit/files
SHA-256 inventory. Every source file and in-tree symlink is checked; a temporary
Git index supplies the compiler's path enumeration without inventing a commit.
The index is removed after generation. Canonical generated files are restored
before protocol execution. Temporary reference signing seeds are removed at exit.

The reference lane proves host interoperability. It explicitly denies physical
UART, field-device, executable WorkerBus, Root admission and production-use-case
proof. Root ticket/snapshot integration uses the existing authenticated target
lane; no host fixture may replace that evidence or weaken an unavailable state.
