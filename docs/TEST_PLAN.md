...
- QEMU compatibility gate:
  ...
  - Optional QEMU driver-task smoke uses `cargo check -p root-task --target aarch64-unknown-none --no-default-features --features release-qemu,qemu-driver-task-smoke` plus a deliberate QEMU boot such as `scripts/cohesix-build-run.sh --cargo-target aarch64-unknown-none --root-task-features kernel,serial-console,net-console,net-backend-virtio,cache-maintenance,usb,qemu-driver-task-smoke --raw-qemu --tcp-port 31347`. After virtio networking is ready, those logs may show one `DRIVER_TASK_BOOT_SMOKE phase=post-net-qemu contract=<name> role=<role> status=created ... started=yes` line for each of the nine declared contracts. That proves a QEMU live-TCB/cap/affinity smoke path only; it is not Pi 4 driver-task proof and must still leave full dedicated-driver-task acceptance fail-closed until hardware roles, VSpace isolation, and hot-path ownership are proved.
...
- `out/manifests/root_task_resolved.json` — `sha256:bdd02b671876b5f9fc0c96c7750c46cc20988cdd3c8841be6b1c585ca65ae3da`
