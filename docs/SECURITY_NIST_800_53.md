...
- In-VM boundary: seL4 kernel (upstream), root-task event pump, target `NineDoorBridge` namespace adapter, Queen authority, root-owned Worker role/session models, synthetic namespaces, and console transport (serial + TCP console). Current profiles do not launch general Worker child TCBs or an in-VM 9P-over-TCP server.
...
| AC-5 | Implemented | Queen and Worker-session duties are separated at the application policy layer by roles, tickets, and namespaces; this is not a claim of separate Worker TCBs. | docs/ROLES_AND_SCHEDULING.md; docs/INTERFACES.md; tests/security/nist_evidence_smoke.sh |
...
| IA-2 | Implemented | Worker-role session attach requires a role and ticket whose claims are verified; attach does not launch a target Worker task. | docs/INTERFACES.md; docs/USERLAND_AND_CLI.md; tests/security/nist_evidence_smoke.sh |
