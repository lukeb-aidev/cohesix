...
| Reopened 26b DPC status | Aligned-historical / superseded | At this audit cut, reciprocal notification/IRQ topology, the bounded DPC ring, and isolated-runtime service were implemented while repeated current-image Wi-Fi functional proof remained open. Exact image `7a10b8fd6acc` later supplied the operator-approved closure matrix recorded in the current Build Plan. | `docs/BUILD_PLAN.md`; `docs/DRIVERS.md`; Pi manifest and driver-runtime source |
