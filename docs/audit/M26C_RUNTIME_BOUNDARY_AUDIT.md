...
| --- | --- | --- |
| Root-task/NineDoor VM boundary | Separate VM root-task adapter and host NineDoor implementation remain distinct; Stage 05 QEMU due-diligence and regression evidence are closed. | PASS-QEMU |
...
| Driver runtime boundary | Isolated runtime and HAL proof surfaces exist; compiler-owned DMA profile truth now resolves Pi-family profiles to `bounded-no-iommu`. | Pi runtime/DMA proof still open |
...
Pi 4 and isolated runtime boundary evidence still requires live target/runtime
proof and remains outside QEMU closure:
