...
- **Commit Hooks**: Install `pre-commit` and enable the Codex prompt linter:  
  ...
  pre-commit install
  ```
- **Agent Definitions**: `docs/community/AGENTS.md` contains JSON schema for agent roles and prompts. Review and extend to add new Codex agents or tasks.

## Agents & Task Schema

- **Agents.md**: Defines each automated agent (e.g., `docs/community/AGENTS.md`). Contains fields:
...
- Codex commands (`cohcli codex run <agent_id> [--file <path>]`) will look up `AGENTS.md` and fill in prompts automatically.
