...
try:
    tomli = importlib.import_module("tomllib")  # Python 3.11+
...
def generate_outputs(deps: list[dict], outdir: Path) -> None:
    ...
    for dep in deps:
        ...
        meta_data = json.loads(
            urllib.request.urlopen(f"https://crates.io/api/v1/crates/{name}")
            .read()
            .decode()
        )
...
def main():
    ...
    parser.add_argument(
        "--demo", action="store_true", help="generate small demo output"
    )
