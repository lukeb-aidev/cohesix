...
fn schema_1_10_is_rejected_after_operator_serial_contract_change() {
    ...
    let manifest = fs::read_to_string(repo_path("configs/root_task.toml"))
        ...
        .replacen("schema = \"1.21\"", "schema = \"1.10\"", 1);
    ...
    let message = format!("{error:#}");
    assert!(
        message.contains("unsupported root_task.schema 1.10 (expected 1.21)"),
        "unexpected rejection: {message}"
    );
}
...
fn schema_1_11_is_rejected_after_publication_ack_contract_change() {
    ...
    let manifest = fs::read_to_string(repo_path("configs/root_task.toml"))
        .expect("read default manifest")
        .replacen("schema = \"1.21\"", "schema = \"1.11\"", 1);
    fs::write(&manifest_path, manifest).expect("write legacy-schema manifest");
    ...
    let message = format!("{error:#}");
    assert!(
        message.contains("unsupported root_task.schema 1.11 (expected 1.21)"),
        "unexpected rejection: {message}"
    );
}
...
fn schema_1_12_is_rejected_after_send_batch_contract_change() {
    ...
    let manifest = fs::read_to_string(repo_path("configs/root_task.toml"))
        .expect("read default manifest")
        .replacen("schema = \"1.21\"", "schema = \"1.12\"", 1);
    fs::write(&manifest_path, manifest).expect("write legacy-schema manifest");
    ...
    let message = format!("{error:#}");
    assert!(
        message.contains("unsupported root_task.schema 1.12 (expected 1.21)"),
        "unexpected rejection: {message}"
    );
}
...
fn schema_1_13_is_rejected_after_natural_postpone_contract_change() {
    ...
    let manifest = fs::read_to_string(repo_path("configs/root_task.toml"))
        .expect("read default manifest")
        .replacen("schema = \"1.21\"", "schema = \"1.13\"", 1);
    fs::write(&manifest_path, manifest).expect("write legacy-schema manifest");
    ...
    let message = format!("{error:#}");
    assert!(
        message.contains("unsupported root_task.schema 1.13 (expected 1.21)"),
        "unexpected rejection: {message}"
    );
}
...
fn schema_1_14_is_rejected_after_worker_execution_contract_change() {
    ...
    let manifest = fs::read_to_string(repo_path("configs/root_task.toml"))
        .expect("read default manifest")
        .replacen("schema = \"1.21\"", "schema = \"1.14\"", 1);
    fs::write(&manifest_path, manifest).expect("write legacy-schema manifest");
    ...
    let message = format!("{error:#}");
    assert!(
        message.contains("unsupported root_task.schema 1.14 (expected 1.21)"),
        "unexpected rejection: {message}"
    );
}
...
fn schema_1_16_is_rejected_before_bounded_passive_timeout_policy() {
    ...
    let manifest = fs::read_to_string(repo_path("configs/root_task.toml"))
        .expect("read default manifest")
        .replacen("schema = \"1.21\"", "schema = \"1.16\"", 1);
    ...
    assert!(format!("{error:#}").contains("unsupported root_task.schema 1.16 (expected 1.21)"));
}
...
fn schema_1_17_is_rejected_before_explicit_worker_bootstrap_policy() {
    ...
    let manifest = fs::read_to_string(repo_path("configs/root_task.toml"))
        .expect("read default manifest")
        .replacen("schema = \"1.21\"", "schema = \"1.17\"", 1);
    fs::write(&manifest_path, manifest).expect("write prior-schema manifest");
    let options = options_for(
        manifest_path,
    ...
    assert!(format!("{error:#}").contains("unsupported root_task.schema 1.17 (expected 1.21)"));
}
...
fn checked_in_profiles_compile_without_radio_sidecar_output() {
    ...
    for (index, profile) in PROFILE_MANIFESTS.iter().enumerate() {
        ...
        assert_eq!(resolved["root_task"]["schema"], "1.21", "{profile}");
        assert_eq!(
