...
fn manifest_with_legacy_schema(schema: &str) -> String {
    ...
    assert_eq!(manifest["root_task"]["schema"].as_str(), Some("1.29"));
...
fn schema_1_10_is_rejected_after_operator_serial_contract_change() {
    ...
    let message = format!("{error:#}");
    assert!(
        message.contains("unsupported root_task.schema 1.10 (expected 1.29)"),
        "unexpected rejection: {message}"
    );
}
...
fn schema_1_11_is_rejected_after_publication_ack_contract_change() {
    ...
    let message = format!("{error:#}");
    assert!(
        message.contains("unsupported root_task.schema 1.11 (expected 1.29)"),
        "unexpected rejection: {message}"
    );
}
...
fn schema_1_12_is_rejected_after_send_batch_contract_change() {
    ...
    let message = format!("{error:#}");
    assert!(
        message.contains("unsupported root_task.schema 1.12 (expected 1.29)"),
        "unexpected rejection: {message}"
    );
}
...
fn schema_1_13_is_rejected_after_natural_postpone_contract_change() {
    ...
    let message = format!("{error:#}");
    assert!(
        message.contains("unsupported root_task.schema 1.13 (expected 1.29)"),
        "unexpected rejection: {message}"
    );
}
...
fn schema_1_14_is_rejected_after_worker_execution_contract_change() {
    ...
    let message = format!("{error:#}");
    assert!(
        message.contains("unsupported root_task.schema 1.14 (expected 1.29)"),
        "unexpected rejection: {message}"
    );
}
...
fn schema_1_16_is_rejected_before_bounded_passive_timeout_policy() {
    ...
    assert!(format!("{error:#}").contains("unsupported root_task.schema 1.16 (expected 1.29)"));
}
...
fn schema_1_17_is_rejected_before_explicit_worker_bootstrap_policy() {
    ...
    assert!(format!("{error:#}").contains("unsupported root_task.schema 1.17 (expected 1.29)"));
}
...
fn checked_in_profiles_compile_without_radio_sidecar_output() {
    ...
    for (index, profile) in PROFILE_MANIFESTS.iter().enumerate() {
        ...
        assert_eq!(resolved["root_task"]["schema"], "1.29", "{profile}");
        assert_eq!(
