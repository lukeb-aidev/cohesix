...
pub struct DocFragments {
    ...
    pub gpu_breadcrumbs_md: String,
    pub observability_interfaces_md: String,
    pub observability_security_md: String,
    pub ticket_quotas_md: String,
    pub trace_policy_md: String,
    pub cas_interfaces_md: String,
    pub cas_security_md: String,
    pub cohesix_py_defaults_md: String,
    pub coh_doctor_checks_md: String,
}
...
impl DocFragments {
    pub fn from_manifest(
        manifest: &Manifest,
        manifest_hash: &str,
        py_defaults: &crate::codegen::cohesix_py::CohesixPyDefaults,
    ) -> Self {
        ...
        let mut gpu_breadcrumbs_md = String::new();
        writeln!(
            gpu_breadcrumbs_md,
        ...
        .ok();
        let gpu_breadcrumbs_hash = hash_bytes(gpu_breadcrumbs_md.as_bytes());
        writeln!(
            gpu_breadcrumbs_md,
            "\n_Generated by coh-rtc (sha256: `{gpu_breadcrumbs_hash}`)._"
        )
        .ok();
            ...
            && !proc_pressure_enabled
        {
            ...
            if proc_ingest.watch {
                writeln!(
                    observability_interfaces_md,
                    "- `/proc/ingest/watch` (append-only, max_entries={}, line_bytes={}, min_interval_ms={}): `watch ts_ms=<u64> p50_ms=<u32> p95_ms=<u32> queued=<u32> backpressure=<u64> dropped=<u64> ui_reads=<u64> ui_denies=<u64>`.",
        ...
        writeln!(
            ticket_quotas_md,
            "- `ticket_limits.max_scope_rate_per_s`: `{}` (0 = unlimited)",
            manifest.ticket_limits.max_scope_rate_per_s
        )
        .ok();
        writeln!(
            ticket_quotas_md,
            "- `ticket_limits.bandwidth_bytes`: `{}` (0 = unlimited)",
            manifest.ticket_limits.bandwidth_bytes
        )
        .ok();
        writeln!(
            ticket_quotas_md,
            "- `ticket_limits.cursor_resumes`: `{}` (0 = unlimited)",
            manifest.ticket_limits.cursor_resumes
        )
        .ok();
        writeln!(
            ticket_quotas_md,
            "- `ticket_limits.cursor_advances`: `{}` (0 = unlimited)",
            manifest.ticket_limits.cursor_advances
        )
        .ok();
        let ticket_quotas_hash = hash_bytes(ticket_quotas_md.as_bytes());
        writeln!(
            ticket_quotas_md,
            "\n_Generated by coh-rtc (sha256: `{ticket_quotas_hash}`)._"
        )
        .ok();

        let mut trace_policy_md = String::new();
        writeln!(trace_policy_md, "### Trace replay limits (generated)").ok();
        writeln!(
            trace_policy_md,
            "- `trace.format.version`: `{}`",
            cohsh_core::trace::TRACE_VERSION
        )
        .ok();
        ...
        writeln!(
            trace_policy_md,
            "- `trace.max_ack_bytes`: `{}`",
            cohsh_core::command::MAX_LINE_LEN
        )
        .ok();
        let trace_policy_hash = hash_bytes(trace_policy_md.as_bytes());
        writeln!(
            trace_policy_md,
            "\n_Generated by coh-rtc (sha256: `{trace_policy_hash}`)._"
        )
        .ok();
        ...
        writeln!(
            cohesix_py_defaults_md,
            "- `console.max_line_len`: `{}`",
            cohsh_core::MAX_LINE_LEN
        )
        .ok();
        writeln!(
            cohesix_py_defaults_md,
            "- `console.max_path_len`: `{}`",
            cohsh_core::MAX_PATH_LEN
        )
        .ok();
        writeln!(
            cohesix_py_defaults_md,
            "- `console.max_json_len`: `{}`",
            cohsh_core::MAX_JSON_LEN
        )
        .ok();
        writeln!(
            cohesix_py_defaults_md,
            "- `console.max_echo_len`: `{}`",
            cohsh_core::MAX_ECHO_LEN
        )
        .ok();
        writeln!(
            cohesix_py_defaults_md,
        ...
        .ok();
        let cohesix_py_hash = hash_bytes(cohesix_py_defaults_md.as_bytes());
        writeln!(
            cohesix_py_defaults_md,
            "\n_Generated by coh-rtc (sha256: `{cohesix_py_hash}`)._"
        )
        .ok();

        let mut coh_doctor_checks_md = String::new();
        writeln!(coh_doctor_checks_md, "### coh doctor checks (generated)").ok();
        writeln!(
        ...
        writeln!(
            coh_doctor_checks_md,
            "- `check=ticket` uses `ticket.max_len={}` and TCP policy (queen tickets optional, worker tickets required).",
            cohsh_core::MAX_TICKET_LEN
        )
        .ok();
        writeln!(
            coh_doctor_checks_md,
        ...
        .ok();
        let coh_doctor_hash = hash_bytes(coh_doctor_checks_md.as_bytes());
        writeln!(
            coh_doctor_checks_md,
            "\n_Generated by coh-rtc (sha256: `{coh_doctor_hash}`)._"
        )
        .ok();
...
pub fn emit_gpu_breadcrumbs_snippet(docs: &DocFragments, path: &Path) -> Result<()> {
    let mut contents = String::new();
    writeln!(contents, "<!-- Author: Lukas Bower -->")?;
    writeln!(
        contents,
...
pub fn emit_ticket_quotas_snippet(docs: &DocFragments, path: &Path) -> Result<()> {
    let mut contents = String::new();
    writeln!(contents, "<!-- Author: Lukas Bower -->")?;
    writeln!(
        contents,
...
pub fn emit_trace_policy_snippet(docs: &DocFragments, path: &Path) -> Result<()> {
    let mut contents = String::new();
    writeln!(contents, "<!-- Author: Lukas Bower -->")?;
    writeln!(
        contents,
...
pub fn emit_cohesix_py_defaults_snippet(docs: &DocFragments, path: &Path) -> Result<()> {
    let mut contents = String::new();
    writeln!(contents, "<!-- Author: Lukas Bower -->")?;
    writeln!(
        contents,
    ...
    fs::write(path, contents).with_context(|| {
        format!(
            "failed to write cohesix python defaults snippet {}",
            path.display()
        )
    })?;
    Ok(())
...
pub fn emit_coh_doctor_snippet(docs: &DocFragments, path: &Path) -> Result<()> {
    let mut contents = String::new();
    writeln!(contents, "<!-- Author: Lukas Bower -->")?;
    writeln!(
        contents,
