...
// Purpose: Extend the stable host-integration graph with bounded provider contracts while retaining target evidence obligations.
// Author: Lukas Bower
...
pub struct HostIntegrationOutput {
    ...
    /// Language-neutral projection of the provider extension, bound to this graph.
    pub provider_registry: PathBuf,
    /// Python binding generated from the same provider extension.
    pub provider_python: PathBuf,
    /// Rust provider binding consumed by shared host authority validators.
    pub provider_rust: PathBuf,
}
...
pub fn compile_graph(
    ...
    provider_bindings: (&Path, &Path),
) -> Result<HostIntegrationOutput> {
    let (provider_python_path, provider_rust_path) = provider_bindings;
    ...
    let provider_registry = output_path.with_file_name("provider_registry.json");
    let provider_python = provider_python_path.to_path_buf();
    let projection = serde_json::json!({
        "schema": "cohesix-provider-registry/v1",
        "graph_schema": GRAPH_SCHEMA,
        "graph_sha256": sha256(&graph_bytes),
        "source_sha256": graph.meta.source_sha256,
        "resolved_manifest_sha256": graph.meta.resolved_manifest_sha256,
        "contract": graph.providers,
        "playbooks": graph.playbooks,
        "use_cases": graph.use_cases,
    });
    let mut projection_bytes = serde_json::to_vec_pretty(&projection)?;
    projection_bytes.push(b'\n');
    write_bytes(&provider_registry, &projection_bytes)?;
    let use_cases = serde_json::json!({
        "schema":"cohesix-use-case-evidence/v1", "graph_sha256":sha256(&graph_bytes),
        "source_sha256":graph.meta.source_sha256, "use_cases":graph.use_cases,
        "playbooks":graph.playbooks, "production_use_case_accepted":false,
        "required_proof":["native_provider","causal_terminal","exact_package","target_worker_if_selected","recovery","external_dependencies"],
    });
    let mut use_case_bytes = serde_json::to_vec_pretty(&use_cases)?;
    use_case_bytes.push(b'\n');
    write_bytes(
        &output_path.with_file_name("use_case_evidence.json"),
        &use_case_bytes,
    )?;

    // JSON is a string literal, never executable Python derived from source data.
    let json_literal = serde_json::to_string(&String::from_utf8(projection_bytes.clone())?)?;
    let python = format!(
        "# Generated by coh-rtc. DO NOT EDIT BY HAND.\n\
         # Author: Lukas Bower\n\
         # Purpose: Project the compiler-owned host provider contract without asserting execution.\n\
         # Copyright 2026 Lukas Bower\n\
         import json as _json\n\nREGISTRY = _json.loads({json_literal})\n"
    );
    write_bytes(&provider_python, python.as_bytes())?;
    let mut rust = String::from("// Generated by coh-rtc. DO NOT EDIT BY HAND.\n// Author: Lukas Bower\n// Purpose: Share compiler-owned provider actions and compatibility fields across Rust host tools.\n// Copyright 2026 Lukas Bower\n\n");
    rust.push_str(&format!(
        "#[cfg(feature = \"std\")]\npub const REGISTRY_JSON: &str = {json_literal};\n"
    ));
    rust.push_str("#[rustfmt::skip]\npub const PROVIDER_V1_FIELDS: &[(&str, &[&str])] = &[\n");
    for family in &graph.providers.families {
        for action in &family.actions {
            if let Some(fields) = &action.legacy_argument_fields {
                rust.push_str(&format!("    ({:?}, &{:?}),\n", action.id, fields));
            }
        }
    }
    rust.push_str("];\n");
    write_bytes(provider_rust_path, rust.as_bytes())?;
    Ok(HostIntegrationOutput {
        graph: output_path.to_path_buf(),
        doc_snippet: doc_snippet_path.to_path_buf(),
        provider_registry,
        provider_python,
        provider_rust: provider_rust_path.to_path_buf(),
    })
}
...
fn validate_and_sort(
    ...
    let dependency_ids: BTreeSet<_> = source
        ...
        .map(|value| value.id.clone())
        .collect();
    source.providers.validate(&dependency_ids, build_plan)?;
    let snapshots = &manifest["ecosystem"]["host"]["snapshots"];
    if snapshots["enable"].as_bool() == Some(true) {
        let registered: BTreeSet<_> = source
            .providers
            .families
            .iter()
        ...
        for publisher in snapshots["publishers"]
            .as_array()
            .ok_or_else(|| anyhow!("missing host snapshot publishers"))?
        {
            for provider in publisher["providers"]
                .as_array()
                .ok_or_else(|| anyhow!("missing host snapshot provider selection"))?
            {
                let id = provider
                    .as_str()
                    .ok_or_else(|| anyhow!("invalid host snapshot provider id"))?;
                if !registered.contains(id) {
                    bail!("host snapshot publisher selects unregistered provider {id}");
                }
            }
        }
    }
    ...
    let workflow_actions = source
        .providers
        .families
        .iter()
        .flat_map(|p| p.actions.iter().map(|a| a.id.clone()))
        .collect();
    let workflow_packages = source
        .providers
        .deployment_profiles
        .iter()
        .map(|p| p.id.clone())
    ...
    for playbook in &mut source.playbooks {
        playbook
            .workflow
            .validate(&workflow_actions, &workflow_packages)?;
...
fn render_doc(graph: &Graph) -> String {
    ...
    for provider in &graph.providers.families {
        let availability = match provider.availability {
            crate::provider_registry::Availability::NotEnabled => "not_enabled",
            crate::provider_registry::Availability::NotImplemented => "not_implemented",
            crate::provider_registry::Availability::Candidate => "candidate",
        };
        output.push_str(&format!(
            "| `{}` | `{}` | `{}` | {} |\n",
            provider.id,
            ...
            provider
                .actions
                .iter()
                .map(|action| format!("`{}`", action.id))
                .collect::<Vec<_>>()
                .join(", ")
        ));
    }
    output.push_str("\n| Surface | Stable integration | Owner | Mode | Read visibility |\n| --- | --- | --- | --- | --- |\n");
    for surface in &graph.providers.integration_surfaces {
        output.push_str(&format!(
            "| `{}` | `{}` | `{}` | `{}` | `{}` |\n",
            surface.id,
            surface.integration_id,
            surface.owner,
            surface.observed_mode,
            surface.read_visibility
        ));
    }
    output
}
...
mod tests {
    ...
    fn compile_modified_source(rewrite: impl FnOnce(String) -> String) -> anyhow::Error {
        ...
        compile_graph(
            ...
            (
                &temp.path().join("provider_generated.py"),
                &temp.path().join("provider_generated.rs"),
            ),
        )
