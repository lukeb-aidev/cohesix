...
def validate_provider_v1(action: str, target: str | None, args: Mapping[str, object]) -> None:
    """Check generated provider fields and unambiguous operands before submission."""
    allowed = DEFAULTS["provider_v1_fields"].get(action)
    ...
    if target is not None:
        ...
        for part in components:
            authority_id(part)
    path_fields = {"out_dir", "out", "adapter_dir", "from", "export_root", "export", "registry_root", "registry"}
    for name, value in args.items():
        if name in {"ttl_s", "mem_mb", "budget_ttl_s", "budget_ops", "priority", "streams"}:
            ...
            authority_id(value)
    for name, alias in [("job_id", "job"), ("model_id", "model"), ("out_dir", "out"),
                        ("adapter_dir", "from"), ("export_root", "export"), ("registry_root", "registry")]:
    ...
    if provider == "peft":
        ...
        required = {"peft.export": [("job_id", "job")],
                    "peft.import": [("job_id", "job"), ("model_id", "model"), ("adapter_dir", "from")],
                    "peft.activate": [("model_id", "model")], "peft.rollback": []}[action]
        if any(name not in args and alias not in args for name, alias in required):
            raise CohesixError("EPERM missing provider field")
        return
    field, index = {"systemd": ("unit", 1), "docker": ("container", 1),
                    "k8s": ("node", 2), "gpu": ("gpu_id", 1)}[provider]
    if components:
        if (components[0] != provider or len(components) not in {index + 1, index + 2}
                or (provider == "k8s" and components[1] != "node")):
            raise CohesixError("EPERM invalid provider target")
        endpoints = {"lease"} if provider == "gpu" else {suffix, suffix.replace(".", "-")}
...
def authority_id(value: str) -> str:
    ...
    if not isinstance(value, str) or not re.fullmatch(r"[A-Za-z0-9._-]{1,128}", value) or value.startswith("-") or ".." in value:
...
class QueenIntent:
    ...
    def encode(self, policy: Optional[Mapping[str, Any]] = None) -> bytes:
        ...
        if not policy["strict_queen_intents"]:
            raise CohesixError("EPERM strict Queen intents disabled by selected profile")
        epoch = self.writer_epoch
        if (policy["writer_epoch_required"] or epoch is not None) and epoch != policy["writer_epoch"]:
        ...
        try:
            payload = {
                ...
                "cmd": json.dumps(dict(self.cmd), sort_keys=True, separators=(",", ":"), allow_nan=False),
            }
