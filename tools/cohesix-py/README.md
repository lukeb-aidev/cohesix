...
- `RestBackend`: hive-gateway REST projection for multiplexed clients, including request-auth headers.
...
- `RestBackend` sends request-auth headers when `request_auth_token` is set or when one of:
  `HIVE_GATEWAY_REQUEST_AUTH_TOKEN`, `COHSH_REST_AUTH_TOKEN`, or `COH_REST_AUTH_TOKEN` is present.
