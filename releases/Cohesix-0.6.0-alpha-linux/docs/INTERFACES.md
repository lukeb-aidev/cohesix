...
- Live Hive rendering pauses while the user is actively scrolling or when the canvas is offscreen/hidden; buffered telemetry is flushed once the canvas becomes visible again to keep page interactions smooth without losing freshness.
...
- Any new command or file path must be documented here and referenced from `ROLES_AND_SCHEDULING.md` before implementation.
