// Author: Lukas Bower
// Purpose: PixiJS renderer for Live Hive visuals and adaptive quality settings.
...
export class HiveRenderer {
  constructor(container, tokens, style, onClusterToggle, onAgentSelect) {
    ...
    this.baseResolution = Math.min(window.devicePixelRatio || 1, 1.5);
    ...
    this.app = new PIXI.Application({
      backgroundAlpha: 0,
      antialias: this.baseResolution < 1.25,
      autoDensity: true,
    ...
    this.app.view.addEventListener("webglcontextrestored", () => {
  ...
  setQuality(mode) {
    ...
    const base = this.baseResolution || window.devicePixelRatio || 1;
    const scale = mode === "degraded" ? 0.6 : 1;
    const next = Math.max(0.75, base * scale);
