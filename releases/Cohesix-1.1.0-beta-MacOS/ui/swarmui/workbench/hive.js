// Author: Lukas Bower
// Purpose: Coordinate visible Live Hive snapshots, bounded polling and independent proof axes.
// Copyright 2026 Lukas Bower
import { createHiveController } from "../hive/index.js";
import { invoke, readSession, readControlValue, setStatus } from "./state.js";
...
const applyHiveBatch = (batch) => {
  window.dispatchEvent(new CustomEvent("hive-observation", { detail: batch }));
  lastHiveBatch = batch;
...
if (hiveCanvas) {
  try {
    ...
    hiveCanvas.addEventListener("pointerdown", bumpHiveInteraction, {
      passive: true,
    });
    hiveCanvas.addEventListener("pointermove", bumpHiveInteraction, {
      passive: true,
    });
    hiveCanvas.addEventListener("wheel", bumpHiveInteraction, {
      passive: true,
    });
    hiveCanvas.addEventListener("touchstart", bumpHiveInteraction, {
      passive: true,
    });
    setHiveFallback("");
  } catch (err) {
...
const renderHiveDetail = (batch) => {
  ...
  const selectedAgent = hiveDetailAgent
    ? hiveAgents.get(hiveDetailAgent)
    : null;
  ...
  const detailLines =
    detail && Array.isArray(detail.lines) ? detail.lines.join("\n") : "";
  ...
  if (hiveDetailAgent && Array.isArray(batch.overlays)) {
    const match = batch.overlays.find(
      (overlay) => overlay.agent === hiveDetailAgent,
    );
...
export const resumeHive = startHive;
export const pauseHive = () => {
  hiveActive = false;
  hivePollGeneration += 1;
  stopHivePolling();
  hiveController?.stop();
...
window.addEventListener("workbench-route", (event) => {
  if (event.detail !== "hive") pauseHive();
});
document.addEventListener("visibilitychange", () => {
  if (document.hidden) pauseHive();
});
window.addEventListener("session-changing", pauseHive);
