...
[Adoption](docs/ADOPTION.md). Its doctor and CI outcome commands use the same
...
[First-boot network policy](docs/HARDWARE_BRINGUP.md#4-set-first-boot-network-policy).
...
and follow [Hardware Bring-up](docs/HARDWARE_BRINGUP.md) for capture and diagnostics.
...
terminal and follow [Python support](docs/PYTHON_SUPPORT.md) for `RestBackend`.
For optional real NVIDIA inventory, run `./bin/gpu-bridge-host --list` on the
Linux GPU host; [Host tools](docs/HOST_TOOLS.md) covers publishing it through the
...
See [Userland and CLI](docs/USERLAND_AND_CLI.md) for commands,
[Hardware Bring-up](docs/HARDWARE_BRINGUP.md) for Pi diagnostics, and
[Host tools](docs/HOST_TOOLS.md) for mounts, GPU bridges, tickets and evidence packs.
...
See the [release factory](docs/HOST_TOOLS.md#release-factory) for native builds and
