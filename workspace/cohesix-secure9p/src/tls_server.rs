...
use ninep::server::{FsConfig, FsServer, InMemoryFs};
...
impl NinepStream for TlsStream {
    fn try_clone(&self) -> ninep::Result<Self> {
        ...
        Ok(Self {
            inner: StreamOwned::new(server, conn),
        })
    }
}
...
fn handle_client(tcp: TcpStream, cfg: Arc<ServerConfig>) -> std::io::Result<()> {
    ...
    let fs_server = FsServer::new(FsConfig {
        root: "/".into(),
        port: 0,
        readonly: false,
    });
    fs_server
        .serve_stream(tls_stream)
        .join()
        .expect("thread join");
    Ok(())
}
