...
| GPU annex instability | Dedicated Linux CUDA microservers with health-checked Secure9P tunnels and graceful job resubmission | "Plan 9 workers stay functional because GPU tasks fail over to the annex without touching on-device drivers" |
...
| Trace validator drift | Canonical rule metadata and automated `cohtrace diff` replay on the latest snapshots | "July CI logs show 99.7% match rate across validator snapshots" |
