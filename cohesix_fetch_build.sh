...
# Standard binaries (std + busybox)
for BIN in cohesix_root cohcc cohesix_build cohesix_cap cohesix_trace init; do
  log "🚀 Building $BIN"
...
# Minimal UEFI / kernel-style binaries
for BIN in logdemo kernel sel4_entry; do
  log "🚀 Building $BIN (minimal_uefi)"
  RUSTFLAGS="-C debuginfo=2" \
    cargo build --release \
      --bin "$BIN" \
...
# Copy root ELF from cargo build output to out/bin/cohesix_root.elf
...
[ -f "$ROOT/out/bin/cohesix_root.elf" ] || { echo "❌ $ROOT/out/bin/cohesix_root.elf missing" >&2; exit 1; }

# Bulletproof ELF validation
log "🔍 Validating cohesix_root ELF memory layout..."
readelf -l "$ROOT/out/bin/cohesix_root.elf" | tee -a "$LOG_FILE" >&3
if readelf -l "$ROOT/out/bin/cohesix_root.elf" | grep -q 'LOAD' && \
   ! readelf -l "$ROOT/out/bin/cohesix_root.elf" | awk '/LOAD/ {print $3}' | grep -q -E '^0xffffff80[0-9a-fA-F]{8}$'; then
  echo "❌ cohesix_root ELF LOAD segments not aligned with expected seL4 virtual space" >&2
  exit 1
fi

ROOT_SIZE=$(stat -c%s "$ROOT/out/bin/cohesix_root.elf")
...
./init-build.sh \
  ...
  -DROOT_SERVER="$ROOT/out/bin/cohesix_root.elf"
