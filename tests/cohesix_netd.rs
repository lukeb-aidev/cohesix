// CLASSIFICATION: COMMUNITY
// Filename: cohesix_netd.rs v0.1
// Date Modified: 2025-07-22
// Author: Cohesix Codex

use cohesix::net::cohesix_netd::CohesixNetd;
use std::io::{Read, Write, ErrorKind};
use std::net::{TcpStream, TcpListener};
use std::thread;
use serial_test::serial;

fn start_tcp_server() -> std::io::Result<u16> {
    let listener = TcpListener::bind("127.0.0.1:0")?;
    let port = listener.local_addr()?.port();
    thread::spawn(move || {
        if let Ok((mut stream, _)) = listener.accept() {
            let mut buf = [0u8; 16];
            if let Ok(n) = stream.read(&mut buf) {
                let _ = stream.write_all(&buf[..n]);
            }
        }
    });
    Ok(port)
}

#[test]
#[serial]
fn tcp_9p_roundtrip() -> std::io::Result<()> {
    if std::env::var("CI").is_ok() {
        eprintln!("⚠️ Skipping test in CI environment");
        return Ok(());
    }

    let port = match start_tcp_server() {
        Ok(p) => p,
        Err(e) if e.kind() == ErrorKind::PermissionDenied => {
            eprintln!("⚠️ Skipping test due to lack of permission: {:?}", e);
            return Ok(());
        }
        Err(e) => return Err(e),
    };

    let netd = CohesixNetd { port, discovery_port: 9999 };
    let mut stream = TcpStream::connect(("127.0.0.1", port))
        .expect("failed to connect tcp server");
    stream.write_all(&[0x6f])?;
    let mut out = [0u8; 1];
    stream.read_exact(&mut out)?;
    assert_eq!(out[0], 0x6f);
    drop(netd); // silence unused variable
    Ok(())
}

#[test]
#[serial]
fn discovery_broadcast() -> std::io::Result<()> {
    if std::env::var("CI").is_ok() {
        eprintln!("⚠️ Skipping test in CI environment");
        return Ok(());
    }

    let netd = CohesixNetd { port: 6000, discovery_port: 9933 };
    thread::spawn(move || {
        match netd.listen_discovery_once() {
            Ok(msg) => assert_eq!(&msg, b"cohesix_netd_discovery"),
            Err(err) => eprintln!("listener error: {err:?}"),
        }
    });
    // give thread a moment
    std::thread::sleep(std::time::Duration::from_millis(50));
    let netd2 = CohesixNetd { port: 6001, discovery_port: 9933 };
    if let Err(e) = netd2.broadcast_presence() {
        if e.kind() == ErrorKind::PermissionDenied {
            eprintln!("⚠️ Skipping test due to lack of permission: {:?}", e);
            return Ok(());
        } else {
            return Err(e);
        }
    }
    Ok(())
}

#[test]
#[serial]
fn http_fallback_post() -> std::io::Result<()> {
    if std::env::var("CI").is_ok() {
        eprintln!("⚠️ Skipping test in CI environment");
        return Ok(());
    }

    let listener = match TcpListener::bind("127.0.0.1:0") {
        Ok(l) => l,
        Err(e) if e.kind() == ErrorKind::PermissionDenied => {
            eprintln!("⚠️ Skipping test due to lack of permission: {:?}", e);
            return Ok(());
        }
        Err(e) => return Err(e),
    };
    let port = listener.local_addr()?.port();
    thread::spawn(move || {
        if let Ok((mut stream, _)) = listener.accept() {
            let mut buf = [0u8; 128];
            if let Ok(n) = stream.read(&mut buf) {
                assert!(std::str::from_utf8(&buf[..n]).unwrap().starts_with("POST"));
            }
            let _ = stream.write_all(b"HTTP/1.1 200 OK\r\nContent-Length: 0\r\n\r\n");
        }
    });
    let netd = CohesixNetd { port: 0, discovery_port: 0 };
    let url = format!("http://127.0.0.1:{}", port);
    netd.http_fallback(&url).unwrap_or_else(|e| eprintln!("http_fallback error: {e:?}"));
    Ok(())
}

