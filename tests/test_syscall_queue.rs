...
fn queue_dequeue_dispatch_order() {
    ...
    q.enqueue(Syscall::Spawn { program: "a".into(), args: vec!["1".into()] });
    q.enqueue(Syscall::Mount { src: "/dev".into(), dest: "/mnt".into() });
    q.enqueue(Syscall::Exec { path: "/bin/run".into() });
...
fn dequeue_blocked_for_non_worker() {
    ...
    let mut q = SyscallQueue::new();
    q.enqueue(Syscall::Exec { path: "/bin/ls".into() });
