...
mod tests {
    ...
    fn agents_file_exists_and_non_empty() {
        let path = Path::new("docs/community/AGENTS.md");
        assert!(path.exists(), "AGENTS.md must exist at docs/community/AGENTS.md");
    ...
    fn no_todo_placeholders_in_agents_file() {
        let content = fs::read_to_string("docs/community/AGENTS.md").unwrap();
