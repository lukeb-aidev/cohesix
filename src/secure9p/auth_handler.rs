...
#[cfg(feature = "secure9p")]
use rustls::{Certificate, server::ServerConnection};
#[cfg(feature = "secure9p")]
...
fn parse_cn(cert: &Certificate) -> Option<String> {
    ...
    parsed.subject().iter_common_name().next().map(|cn| cn.as_str().unwrap_or("").to_string())
}
...
#[cfg(feature = "secure9p")]
pub fn extract_identity(conn: &mut ServerConnection, stream: &mut dyn std::io::Read) -> Result<String> {
    ...
    if line.starts_with("JWT ") {
        ...
        let data = decode::<Claims>(token, &DecodingKey::from_secret(b"cohesix"), &Validation::new(Algorithm::HS256))?;
...
mod tests {
    ...
    fn tls_pair() -> (ServerConnection, ClientConnection) {
        ...
        let client_cfg = ClientConfig::builder()
            .with_safe_defaults()
            .with_custom_certificate_verifier(Arc::new(rustls::client::WebPkiClientVerifier::builder(Arc::new(rustls::RootCertStore::empty())).build()))
            .with_no_client_auth();
        let server = ServerConnection::new(Arc::new(server_cfg)).unwrap();
        let client = ClientConnection::new(Arc::new(client_cfg), "localhost".try_into().unwrap()).unwrap();
        (server, client)
    }
