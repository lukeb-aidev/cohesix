...
impl NamespaceLoader {
    ...
    pub fn apply(ns: &mut Namespace) -> io::Result<()> {
        let srv_dir = crate::with_srv_root!("");
        ...
        for op in &ns.ops {
            match op {
                ...
                NsOp::Mount { srv, dst } => {
                    ...
                    let file = std::path::Path::new(&srv_dir).join(name.replace('/', "_"));
...
impl Namespace {
    ...
    pub fn expose(&self) -> io::Result<()> {
        fs::create_dir_all(crate::with_srv_root!("")).ok();
        let mut f = File::create(crate::with_srv_root!("bootns"))?;
        for op in &self.ops {
            match op {
                NsOp::Bind { src, dst, flags } => {
                    let mut flag = String::new();
    ...
    pub fn persist(&self, agent_id: &str) -> io::Result<()> {
        let dir = std::path::PathBuf::from(crate::with_srv_root!("bootns"));
        fs::create_dir_all(dir.parent().unwrap())?;
        let path = dir;
        let path = path.with_file_name(agent_id);
